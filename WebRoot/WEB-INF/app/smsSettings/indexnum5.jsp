...
<div class="divtable" style="  position: relative; width: 620px;height: 250px; left:50%;top:70px;margin-left:-310px;  position: absolute; border:1px solid #C8C8CD;background:#fff;">
    ...
    <div style="width: 72%;float: left;">
        <textarea name="txt" id="sms2RemindPriv" user_id="" style="width: 350px;height: 150px; margin-top: 20px;"  class="sendsave senduser" disabled rows="3" cols="50"></textarea>
...
<script type="text/javascript">
    ...
    $(function () {
        $('#del_user').click(function(){ //清空人员
            $('#sms2RemindPriv').attr('user_id','');
            $('#sms2RemindPriv').val('');
        });

        $.ajax({
            url: '/sms2Priv/selectSms2RemindPriv',
            dataType: 'json',
            success: function (data) {
                for (var i = 0; i < data.obj.length;i++){
                    ...
                    for (var i = 0; i < data.obj.length;i++){
                        ...
                        $('#sms2RemindPriv').attr('user_id',userids) || '',
                        $('#sms2RemindPriv').val(str);
                    }
    ...
    $(function () {
        ...
        $("#okbtn").click(function () {
            $.ajax({
                type:'post',
                url: '/sms2Priv/upselectSms2RemindPriv',
                dataType: 'json',
                data:{
                    sms2RemindPriv:$('#sms2RemindPriv').attr('user_id') || '',
                },
                success: function (data) {
                    if (data.flag) {
