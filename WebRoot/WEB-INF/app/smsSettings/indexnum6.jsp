...
<div class="divtable" style="  position: relative; width: 620px;height: 250px; left:50%;top:70px;margin-left:-310px; position: absolute; border:1px solid #C8C8CD;background:#fff;">
    ...
    <div style="width: 72%;float: left;">
        <textarea name="txt" id="outPriv" user_id="" style="width: 350px;height: 150px; margin-top: 20px;"  class="sendsave senduser" disabled rows="3" cols="50"></textarea>
        <a style="margin-left: 10px;cursor: pointer;color: #0aa3e3;" id="adduesr">添加</a>
        <a style="margin-left: 10px;cursor: pointer;color: #0aa3e3;" id="del_user">清空</a>
        <input type="checkbox" style="margin-left: 6px" name="outToSelf" id="outToSelf" />
        <span>允许发送给自己</span>
        <input type="hidden"  name="oTSHidden" id="oTSHidden" value="0">
    </div>
    <div class="btnSure" style=" left: 45%; bottom: 10px;" >
        <span style="margin-left: 32px;" id="okbtn">确定</span>
    </div>
</div>
</div>
</body>
<script type="text/javascript">
    ...
    $(function () {
        ...
        $.ajax({
            url: '/sms2Priv/selOutPriv',
            dataType: 'json',
            success: function (data) {
                if(data.object.outToSelf == '1'){
                    $("input[type='checkbox'][name='outToSelf']").prop("checked", true);
                }else if(data.object.outToSelf == '0'){
                    $("input[type='checkbox'][name='outToSelf']").prop("checked", false);
                }
    ...
    $(function () {
        ...
        $("#okbtn").click(function () {
            $.ajax({
                type:'get',
                url: '/sms2Priv/upOutPriv',
                dataType: 'json',
                data:{
                    outPriv:$('#outPriv').attr('user_id') || '',
