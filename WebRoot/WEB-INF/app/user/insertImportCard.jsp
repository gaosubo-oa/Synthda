...
<div class="query">
    <div class="header">导入动态密码卡信息</div>
    <form id="ajaxform" method="post"  action="/secureKey/ImportSecureKeyInfo" enctype="multipart/form-data">
        <table style="width: 100%;    border: 1px solid #c0c0c0;
    border-top: none;">
            <tbody>
            <%--<tr class="borderNone">--%>
            <%--<td width="30%" class="color">下载导入模板：</td>--%>
            <%--<td width="70%">--%>
            <%--<a href="/file/hr/客户表.xls" style="text-decoration: underline">客户信息导入模板下载</a>--%>
            <%--</td>--%>
            <%--</tr>--%>
            <tr class="borderNone">
                <td width="30%" class="color">选择导入文件：</td>
                <td width="70%">
            ...
            <tr class="borderNone">
                <td colspan="2" style="text-align: center">
                    <a href="javascript:;" class="btnsava">导入</a>
...
<script type="text/javascript">
    $('.btnsava').click(function () {
        ...
        var type = fileName.substr(pointIndex);
        if(type!='.smd'){
            $.layerMsg({content: '导入文件格式错误！', icon: 2});
        }else {
            $('#ajaxform').ajaxSubmit({
                dataType: 'json',
                success: function (res) {
                    if (res.flag) {
                        $.layerMsg({content: '导入成功！', icon: 1}, function () {
                            parent.$('[name="notices"]').attr('src', '/secureKey/ImportSecureKeyInfo');
