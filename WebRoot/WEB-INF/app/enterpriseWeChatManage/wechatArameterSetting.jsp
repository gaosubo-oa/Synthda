...
<script type="text/javascript">
    ...
    $(function(){
        ...
        function ajax() {
            $.ajax({
                url:'/enterpriseWeChatManage/baseEnterpriseWeChatGet',
                type:'get',
                dataType:'json',
                success:function(res){
                    if(res.flag){
                        for(var i=0;i<res.data.length;i++){
                            if(res.data[i].paraName=="WECHATQY_OAURL"){
                                inputcheck('oaUrl',res.data[i].paraValue);
                            }
                            if(res.data[i].paraName=="WECHATQY_CORPID"){
                                inputcheck('corpId',res.data[i].paraValue);
                            }
                            if(res.data[i].paraName=="WECHATQY_SECRET"){
...
//            console.log(pd1)
            $.ajax({
                url:'/enterpriseWeChatManage/baseEnterpriseWeChatSetting',
                type:'POST',
    ...
    <form class="form-horizontal">
        <fieldset>
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <input id="inputOaUrl" placeholder="www.yyy.com" value="" name="oaUrl">
                    <span class="help-block">OA的外网地址格式为: <code>www.yyy.com</code> 或者 数字ip <code>123.23.12.YY</code> 不需要http前缀！如OA地址使用了https协议443端口，请填写https协议头，如：<code>https://www.yyy.com</code>。</span>
                </div>
            </div>
        </fieldset>

        <fieldset>
            <legend><h5>基础设置 - 企业微信开发凭证</h5></legend>
            <div class="control-group">
