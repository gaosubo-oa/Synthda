...
<header class="mui-bar mui-bar-nav" id="header">
    ...
    <h1 class="mui-title" id="title1">公共文件柜</h1>
...
<div id="modal2" class="mui-modal">
    ...
    <div class="mui-content" style="height: 100%;">
            ...
            <div class="mui-input-row">
                <label>附件：</label>
                <button type="button" id="add_button">添加附件
                    <form id="uploadimgform" target="uploadiframe" action="../upload?module=file_folder" enctype="multipart/form-data" method="post">
                        <input type="file" multiple="multiple" name="file" id="uploadinputimg" class="w-icon5" style="position: absolute;top:0px;left:0px;opacity: 0;width: 80px;
...
<script>
    ...
    function successData(data){
        ...
        for(var i = 0; i < data.length; i++) {
            ...
            str += "<li id='" + id + "'   class='mui-table-view-cell catalog' onclick='" + onclick + "'><div class='mui-slider-right mui-disabled' onclick='del(this,event)' data-did='" + id + "'  data-type="+type+"><span class='mui-btn mui-btn-red'>删除</span></div><div class='mui-slider-handle'><img src='" + img_url + "' class='mui-pull-left' style='width: 45px'/><span class='mui-navigate-right' style='margin-left: 40px'>" + name + "</span></div></li>";
        }
        return str2=str;
    ...
    function del(file_move,e){
        ...
        mui.confirm('确认删除此便签？','提示',btnArray,function(f){
            if(f.index == 0){
    ...
    function file_content(self){
        ...
        mui.openWindow({
            url:'/file/publicContenth5?id='+cid,
            id:'/file/publicContenth5',
        });
    }
    ...
    function save_file(){
        ...
        mui.ajax('/file/saveContent',{
            data:{
                ...
                subject:title,
                content:textarea,
                attachmentId:attachmentId, //附件id串
                attachmentName:attachmentName,//附件名称串
                contentNo:num2
            },
            dataType:'json',//服务器返回json格式数据
            type:'post',//HTTP请求类型
