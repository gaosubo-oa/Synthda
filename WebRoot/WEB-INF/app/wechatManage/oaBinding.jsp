...
<body>
<fieldset>
    <legend><h5>企业微信用户绑定OA用户</h5></legend>
</fieldset>
<div class="content">
    <div id="downChild">
...
<script type="text/javascript" charset="utf-8">
    ...
    function loadSidebar1(target,deptId) {
        $.ajax({
            ...
            success: function (data) {
                if(!data.flag){
                    layer.msg(data.data.errmsg, {icon: 2});
                    return;
                }
                var strTop = '';
                var str = '';
                        ...
                        if (v.name) {
                            if(v.parentid!=0){
                                ...
                                str += '<li><span data-types="1"  data-numtrue="1" ' +
                                    'onclick= "imgDown_s2(' +v.id + ',this)"  style="height:35px;line-height:35px;padding-left: 30px;text-overflow: ellipsis;overflow: hidden;" deptid="' + v.id + '" class="firsttr childdept">' +
                                    '<span class=""></span>' +
                                ...
                                strTop+= '<li><span data-types="1"  data-numtrue="1" ' +
                                    'onclick= "show_users(' +v.id + ')"  style="height:35px;line-height:35px;padding-left: 8px;text-overflow: ellipsis;overflow: hidden;" deptid="' + v.id + '" class="firsttr childdept">' +
                                    '<span class=""></span>' +
                                    ...
                                    '<a href="javascript:void(0)" class="dynatree-title" title="' + v.name + '"（企业微信）>' + v.name + '（企业微信）</a>' +
                                    '</span><ul style="display:none;" class="dpetWhole0"></ul></li>';
                            }
                        }
                    });
    ...
    function show_users(deptId) {
        ...
        pageObj = $.tablePage('#pagediv',sumwidth, [
            ...
            me.init('/ewx/selUsersWithBindType',[
                {
                    name:'编辑',
                    fn:function(obj) {
                        if(obj.ifBind==0){
