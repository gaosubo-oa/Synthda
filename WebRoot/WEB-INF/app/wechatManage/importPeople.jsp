    ...
    $('#btn-oa-user-sync').click(function(){
        ...
        if(pd!=2){
            layer.open({
                ...
                btn: ['关闭'],
                yes:function(index){
                    $('#iframe', window.parent.parent.document).attr('src','/dingdingManage/getArameterSetting')
                    $('#iframe', window.parent.parent.document).parent().siblings('div').find('li').eq(0).addClass('cur').siblings().removeClass('cur')
                    layer.close(index)
