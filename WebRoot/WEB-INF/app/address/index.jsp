...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="/css/address/index.css?20210624"/>
...
<%--<script>
        ...
        if (null == result) {
            ...
            location.href = '/address/addressh5';
            //return true
        }
...
<%--    <script type="text/javascript" src="/js/jquery/jquery-migrate-3.4.0.js"></script>--%>
    ...
    <script type="text/javascript">
        ...
        function goto(SHARE_TYPE, groupId, PUBLIC_FLAG, TYPE,e) {
            ...
            e.parent().siblings().find('a').removeClass('select1')
            e.parent().siblings().removeClass('select')
            e.parent().siblings().find('span').css('display','block')
            e.parent().siblings().find('span').removeClass('select1')
            ...
            document.getElementById('search_list').value = '<fmt:message code="address.th.searchContacts" />...';
            ...
            $.ajax({
                ...
                success: function (reg) {
                    if (reg['status']) {
                        ...
                        $("#sharing").attr("value","");
                        /*第一次加载时定位分组高度*/
                         my_group_count = $('#my_group_count').val();
...
//            /*第一次加载时定位分组高度*/
           ...
           if (height - work_group_count * 47 > 0) {
            ...
            $(window).on('resize',function () {
                ...
                /*字母选择样式效果*/
                $('[name=list_hover]').on('click',function () {
            ...
            $("#my_group_show").on('click',function () {
                $("#my_group").toggle();
                $("#share").hide();
                $("#work").hide();
            })
            $("#share_show").on('click',function () {
                $("#share").toggle();
                $("#work").hide();
                $("#my_group").hide();
            })
            ...
            $("#new_add2").on('click',function () {
                $('#iframe_new').attr("src", "/address/new");
            })
            //编辑用户信息edit
            $("#edit").on('click',function () {
                var addId = $('#show_add_id').val();
                $('#iframe_edit').attr("src", "/address/edit?uid=" + addId);
            })

            $("#import_info2").on('click',function () {
                $('#iframe_import').attr("src", "/address/goImportUsers");
            })
            $("#export_info2").on('click',function () {
...
//             })
            $(".add-on").on('click',function () {
                var null1= $("#search_list").val();
                var keyword = encodeURI($("#search_list").val());
                if(null1=='<fmt:message code="address.th.searchForDefaultGroupContacts" />'|| null1=='<fmt:message code="address.th.searchContacts" />...'||null1=='<fmt:message code="address.th.searchForDefaultGroupContacts2" />'){
                    alert('<fmt:message code="address.th.pleaseEnterTheContactYouWantToSearchFor" />')
                    return false;
                }
                var key1 = $("#key1").val();
                var key2 = $("#key2").val();
                ...
                $('#talklist').attr('src',"/address/showList?psnName=" + keyword + "&userName=" + keyword+'&gettype='+gettype +'&share_type=' + share_type)
//                frames['talklist'].location = "/address/showList?psnName=" + keyword + "&userName=" + keyword ;
            })
            $("#search_list").on('focus',function () {
                var keyword = $("#search_list").val();
                if (keyword.indexOf("搜索") > -1) {
                    $("#search_list").val("");
                }
        ...
        function submit_import() {
            document.getElementById('iframe_import').contentWindow.importAddress();
        }
...
<div class="wrap" style="margin-left:0px !important;">
    <div class="head">
        <div class="head_left">
            ...
            <h1 style=""><fmt:message code="main.addressbook" /></h1>
            <%--<h1>部门/成员单位管理</h1>--%>
        </div>
    </div>
<!--新建联系人-->
<div id="new_add" class="modal hide fade" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true" data-backdrop="static"
    ...
    <div class="modal-footer" style="padding-bottom: 10px;padding-top: 10px;text-align:center;">
        <button class="btn btn-primary" onClick="submit_new()"  aria-hidden="true"  style="margin-left:220px;"><fmt:message code="global.lang.save" /></button>
        ...
        <div class="bts">
            <form id="form">
                ...
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu" style="left:-64px;">
                        ...
                        <li><a id="import_info2" href="#import_info" role="button"
                               data-toggle="modal"><fmt:message code="file.th.importData" /></a>
                        </li>
        ...
        <div class="lianxi" id="lianxi">
            <ul>
                <li class="nav firstnav">
                    ...
                    <ul id="my_group">
                       <li> <a class="default" style="height: 46px;"  href="#" style="height:36px;" onClick="goto(0,0,0,1,$(this)),search_box(this.innerText)"><fmt:message code="main.th.mo" /></a></li>
                    </ul>
                </li>
                <li class="nav secondnav">
                    <span>
                        <i class="layui-icon layui-icon-share" style="font-size: 19px;color: #286B8F"></i>
                    </span>
                    <a id="share_group" style="height:40px;" href="javascript:;" onClick="goto(1,-1,1,3,$(this)),search_box(this.innerText)"><fmt:message code="diary.th.Share" /></a>
                </li>
    ...
    <div class="center" style="margin-top: 46px;">
        <div class="middle">
            <div class="input-append" style="margin-top:12px;margin-left:10px;">
                ...
                <div class="sort" style="float: right; margin-right: 68px;display: none;">
            <img class="signReaders" title="<fmt:message code="main.th.taggedRead" />" src="/img/commonTheme/theme6/icon_duoxuan_03.png">
            <img class="selectReaders" title="<fmt:message code="notice.th.allSelect" />" src="/img/commonTheme/theme6/icon_dx_01.png">
            <div id="delete"  dataheng="" data-deletetype="inbox" data-defats="otherBox" data-box=""><img src="/img/commonTheme/theme6/icon_delete_06.png" class="im"><fmt:message code="global.lang.delete" /></div>
        </div>
    </div>
            <div id="list">
                <ul id="list_letter">
...
<script>
    $(function () {
        $("#group_manage2").on('click',function () {
