...
<head>
    ...
    <script  type="text/javascript">
                ...
                success:function(rsp){
                    if(rsp.obj.length != 0){
                        ...
                        for(var i=0;i<length;i++){
                            ...
                            var colorol;
                            if(color==0){colorol="#c4ffc6"};
                            if(color==1){colorol="#ffaeab"};
                            if(color==2){colorol="#fffab6"};
                            if(color==3){colorol="#b2ebfe"};
                            if(color==4){colorol="#f0c3ff"};
                            if(color==5){colorol="#ffd1b0"};
                            ...
                            divstr +="<div class='yuan yuan"+noteId+"' style='margin:10px 12px;height:200px;width:200px;left:8px;background-color:"+colorol+";float:left'>" +
                                ...
                                "<p class='textList contents' style='background-color:"+colorol+";margin-top: 12px;padding-left: 12px;' style='width:90%;margin:0 auto'>"+content+"</p>" +
        ...
        function redact(bb){//获取noteId  即 bian_sum
            $('.bs-example-modal-sm').modal('show');
            contents=$(bb).parent().children(".contents").text();
        ...
        function new_btns(ss){
            ...
            if(ss==0){
                if($(".new_content").val()==""){
                    ...
                    var sumbs;
                    if(colpsps=="c4ffc6"){sumbs=0}
                    if(colpsps=="ffaeab"){sumbs=1}
                    if(colpsps=="fffab6"){sumbs=2}
                    if(colpsps=="b2ebfe"){sumbs=3}
                    if(colpsps=="f0c3ff"){sumbs=4}
                    if(colpsps=="ffd1b0"){sumbs=5}
                    $('.bs-example-modal-sm').modal('hide');
                    var data={
                        content:new_conte,
                        color:sumbs
            ...
            if(ss==9){ //修改便签
                ...
                $.get('/Notes/updateByPrimaryKey',{
                    ...
                    $(".yuan"+bian_sum).find(".contents").text(conlm);
            ...
            $.ajax({
                ...
                success:function(rsp){
                    ...
                    var color=rsp.object.color;
                        ...
                        var str_hour=new_hours+":"+new_minus+":"+new_condss;
                        var new_time = str_time+'     '+str_hour;
                    divstr +="<div class='yuan yuan"+noteId+"' style='margin:10px 12px;height:200px;left:8px;width:200px;background-color:"+colorol+";float:left'>" +
                            "<div style='background-color: #000;width: 100%;height: 44px;position: absolute;top: 0;left: 0;opacity: 0.3;border-top-left-radius: 15px;border-top-right-radius: 15px;'>" +
                            "</div>" +
                            "<p style='position: relative;margin-top: 22px;left: 28px'>" +
                            "<span class='title_time'>"+new_time+"</span>" +
                            "<span  class='title_hms'style='display:none;'>"+str_hour+"</span></p>" +
                            "<p class='contents textList' style='background-color:"+colorol+";margin-top: 12px;margin-left: 12px;' style='width:90%;margin:0 auto'>"+content+"</p>" +
                            "<span class='span_bj' onclick='redact(this)' ><img src='/img/notes/gos_xxx.png' class='span_remove' ressum="+noteId+" onclick='removes(this)' ></span></div>"
                    if($(".content").html()==""){
