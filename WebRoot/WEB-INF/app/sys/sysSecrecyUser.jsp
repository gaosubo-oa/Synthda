...
<%--        </div>--%>
<%--        <div class="divP">用户管理-管理范围<span class="innerText"></span></div>--%>
<%--        <div style="float: right;margin-top: 10px;margin-right: 20px;">--%>
<%--            <button style='cursor:pointer;' class="syn_user" onclick="synUser()" >同步用户</button>--%>
<%--        </div>--%>
<%--    </div>--%>
    <div class="left" style="background: #f5f7f8; top:0;">
        <div class="collect">
...
<%--                <div id="noDept"><span class="spanUP liUp noDept"  deptid="0"><img src="/img/commonTheme/${sessionScope.InterfaceModel}/icon_demission.png" style="vertical-align: middle;width: 15px;margin-right: 10px;" alt="未分配部门人员">未分配部门人员</span></div>--%>
...
<%--            </div>--%>
<%--            <div class="boto">--%>
<%--                <div class="THREE" href="javascript:void(0)"><span>批量修改密码</span></div>--%>
<%--            </div>--%>
<%--            <div class="boto">--%>
<%--                <div class="FOUR" href="javascript:void(0)"><span><fmt:message code="userManangement.th.Nologin" /></span></div>--%>
<%--            </div>--%>
<%--            <div class="boto">--%>
<%--                <div class="SEVEN" href="javascript:void(0)"><span>允许登录</span></div>--%>
<%--            </div>--%>
<%--            &lt;%&ndash;<div class="boto">--%>
...
<%--            </div>--%>
<%--            <div class="boto">--%>
<%--                <div class="Seven"><span>返回部门</span></div>--%>
<%--            </div>--%>
        </div>
    </div>
...
<script type="text/javascript">
    ...
    function saveEXaInfo(fn) {
        layer.open({
            ...
            content: '<div style="width:800px;margin:10px auto;">' +
                ...
                '<td style="border:none; text-align: right;" width="12%">附件信息：</td>\n' +
                ...
                '<input type="file" multiple="multiple" name="file"  id="fileInp" style="border:none; display:inline-block; cursor:pointer;width: 76px;">'+
                '                                 </div>' +
                '                                   </div>' +
    ...
    $(document).on("click",".deleteOneuser",function(e) {
        var uid = $(e.target).parents(".userData").attr("uid");
        $.ajax({
            type:'get',
            ...
            success:function (res) {
                if(res.flag){
                    if(res.object==1){
                        ...
                        $.layerConfirm({title:'提示',content:'确定删除该用户吗?',icon:0,offset:"200px"},function(){
                            saveEXaInfo(function() {
                                ...
                                $.ajax({
                                    ...
                                    url:'/user/deleteUser',
                                    dataType:'json',
                                    data:{'uids':uid,"attachmentId":attachmentId.join(","),"attachmentName":attachmentName.join((",")),operationReason:operationReason},
                                    success:function(res){
                                        if(res.flag){
                    ...
                    str += '<tr class="userData '+colorNum+'" uId="' + data1[i].uid + '">' +
                        ...
                        '<td>'+function(){
                            if(data1[i].uid==''){
                                ...
                                var str = '<a href="javascript:void(0)" class="deleteOneuser" style="margin-right: 5px;">删除</a>'
                        ...
                        str += '<tr class="userData '+colorNum+'" uId="' + data1[i].uid + '">' +
                            ...
                            '<td>'+function(){
                                if(data1[i].uid==''){
                                    ...
                                    return '<a href="javascript:void(0)" style="margin-right: 5px;"><fmt:message code="global.lang.edit" /></a>'
                                }
                            }()+'</td>' +
                            '</tr>';  //<a href="javascript:;">
                        <%--<fmt:message code="userManagement.th.MenuAauthority" />--%>
            ...
            $.layerConfirm({title:msg,content:msg,icon:0,offset:"200px"},function(){
                saveEXaInfo(function() {
                    ...
                    $.ajax({
                        ...
                        url:URL,
                        dataType:'json',
                        data:{'uids':string,"attachmentId":attachmentId.join(","),"attachmentName":attachmentName.join((",")),operationReason:operationReason},
                        success:function(res){
//                        deptById(data,$('.tr_befor'));
                            if(res.flag){
                                if($('#allSelUser').val()!=undefined &&$('#allSelUser').val()!=''){
                                    ...
                                    return ""
                                                    ...
                                                    '<td>'+function(){
                                                        if(data1[i].uid==''){
                                                            ...
                                                            var str = '<a href="javascript:void(0)" style="margin-right: 5px;"><fmt:message code="global.lang.edit" /></a>'
    ...
    function userDeptOrder(ele){
        ...
        var layerIndex = layer.open({
            ...
            content: '<div class="dept_order_box" style="padding: 8px"><table>' +
                '<thead><tr><th style="width: 30%">部门</th><th style="width: 30%">排序</th><th style="width: 40%">职务</th></tr></thead>' +
    ...
    function imgDown(deptNum, me) {
        ...
        if ($(me).attr('data-numstring') == 1) {
            if (boolTwo) {
                ...
                $.loadSidebar($(me).next(), deptNum,null,)
            }
        }
        if($(me).next().html()=='') {
            if (boolTwo) {
    ...
    function imgDown_s(deptNum, me) {
        ...
        if ($(me).attr('data-numstring') == 1) {
            if (boolTwo) {
                ...
                $.loadroleUser($(me).next(), deptNum, $(me).attr('data-numtrue'),null);
            } else {
                $.loadSidebar($(me).next(), deptNum,null)
            }
        }
