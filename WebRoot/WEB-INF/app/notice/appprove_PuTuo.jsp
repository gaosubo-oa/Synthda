...
<div class="head-top">
    <ul class="clearfix">
        <li class="fl head-top-li active" data-type=""><fmt:message code="notice.th.notificationsPendingApproval" /></li>
        <li class="fl head-top-img"><img src="/img/twoth.png" alt=""></li>
        <li class="fl head-top-li" data-type="0"><fmt:message code="notice.th.ApprovedNotifications" /></li>
    </ul>
</div>
<div class="navigation" style="margin-top: 46px;">
    <img src="/img/commonTheme/${sessionScope.InterfaceModel}/daishen.png" alt="">
    <h2><fmt:message code="notice.th.notificationsPendingApproval" /></h2>
</div>
<div id="pagediv">
        ...
        var arr=[
            ...
            },
            {
                ...
                selectFun:function (data,obj) {
                    return '<div title="<fmt:message code="userManagement.th.department" />：'+check(obj.deprange)+'"><fmt:message code="userManagement.th.department" />：'+check(obj.deprange)+'</div><div title="<fmt:message code="userManagement.th.role" />：'+check(obj.rolerange)+'"><fmt:message code="userManagement.th.role" />：'+check(obj.rolerange)+'</div><div title="<fmt:message code="diary.th.body" />：'+check(obj.userrange)+'"><fmt:message code="diary.th.body" />：'+check(obj.userrange)+'</div>'
                }
            },
            {
...
border-radius: 3px;"><fmt:message code="notice.th.top" /></span>'+
        ...
        $('.navigation h2').html('<fmt:message code="notice.th.notificationsPendingApproval" />');
        ...
        objs=$.tablePage('#pagediv',sumwidth,arr,function (me) {
            me.init('/sys/getPendingNotifyList',[{name:'<fmt:message code="global.lang.edit" />'},{name:'<fmt:message code="meet.th.Approval" />'},{name:'<fmt:message code="meet.th.unratified" />'}])
        })
    }
    function yishen() {
        $('.navigation h2').html('<fmt:message code="notice.th.ApprovedNotifications" />');
...
border-radius: 3px;"><fmt:message code="notice.th.top" /></span>'+
    ...
    $(document).on('click','.editAndDelete0',function () {  //编辑
        ...
        $.popWindow('/notice/newAndEdit_PuTuo?approve=true&type=edit&notifyId='+notifyId,'<fmt:message code="notice.th.EditTheAnnouncement" />','50','30','1200px','600px')
    ...
    $(document).on('click','.editAndDelete1',function () {
        ...
        $.get('/notice/updateNotify',{notifyId:obj.notifyId,publish:1,typeId:obj.typeId,approve:1,howRenind:obj.howRenind},function (json) {
            if(json.flag){
    ...
    $(document).on('click','.editAndDelete2',function () {
        var obj=objs.arrs[$(this).attr('data-i')]
        $.get('/notice/updateNotify',{notifyId:obj.notifyId,publish:3,typeId:obj.typeId,approve:1},function (json) {
            if(json.flag){
    ...
    $('#pagediv').on('click','.windowOpen',function () {
        ...
        $.popWindow('/notice/detail?notifyId='+notifyId+'&changId=1','<fmt:message code="notice.th.queryDetail" />','20','150','1200px','500px')
    })
</script>
