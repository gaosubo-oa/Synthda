...
<head>
    ...
    <title><fmt:message code="notice.th.notice" /></title>
...
<body>
<header>
    <i><a href="/m/getMainddh5" style="color: #fff"><fmt:message code="notice.th.return" /></a></i>
    <span><fmt:message code="notice.th.notice" /></span>
    <i style="width: .4rem"></i>
</header>
<div class="hd"></div>
<div class="" style="background: #ffffff;">
    <div class="" style="background-color: #f2f2f2;text-align: center;">
        <input id="search" placeholder="<fmt:message code="workflow.th.sousuo" />" style="">
    </div>
    <div class="content">
                ...
                if(res.flag){
                    ...
                    for(var i=0;i<res.obj.length;i++){
                        ...
                        app+='<div class="nav-sub" style="box-shadow: 1px 1px 7px rgba(206, 194, 194, 0.85);border-radius: .1rem;background-color: #fff;margin: .25rem .1rem;padding: .1rem 0;">'+
                            '<a class="list" href="/notice/noticeDetailsh5?notifyId='+res.obj[i].notifyId+'" style="color: #333" notifyId="'+res.obj[i].notifyId+'">'+
                            '<div class="nav1">'+
                            ...
                            '</a>'+
                            '<p class="nav3"><a href="javascript:;" class="ac">'+function(){if(res.obj[i].typeName==''){return '<fmt:message code="notice.type.alltype" />'}else{return  res.obj[i].typeName}}()+'</a><span class="ac2">'+res.obj[i].name+'</span></p>'+
                            '</div>'
                    }
                    $('.content').html(app)
                }else{
                        ...
                        if (res.flag) {
                            for (var i = 0; i < res.obj.length; i++) {
                                ...
                                app += '<div class="nav-sub" style="box-shadow: 1px 1px 7px rgba(206, 194, 194, 0.85);border-radius: .1rem;background-color: #fff;margin: .25rem .1rem;padding: .1rem 0;">' +
                                    ...
                                    '<span class="fs14">' + res.obj[i].auditDate + '</span>' +
                                    '</div>' +
                                    '<div style="position: relative;"><div class="nav1" style="width: 70%;display:inline-block;margin-right: 1.7rem;height: 0.4rem;line-height: .4rem;">'+str1+'</div>' + function () {
                                        if (res.obj[i].attachmentName != '') {
                                            return '<img style="position: absolute;top: .38rem;right: .7rem;" src="../img/enclosure.png" alt="">'
                                        } else {
                                            return ''
                                        }
                                    }() + '</div>' +
                                    '</a>' +
                                    '<p class="nav3"><a href="javascript:;" class="ac">' + function () {
                                        if (res.obj[i].typeName == '') {
                                            return '<fmt:message code="notice.type.alltype" />'
                                        } else {
                                            return res.obj[i].typeName
                                        }
                                    }() + '</a><span class="ac2">' + res.obj[i].name + '</span></p>' +
                                    '</div>'
                            }
                            // 如果没有数据
                        } else {
