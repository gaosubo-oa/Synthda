...
<head>
    ...
    <script src="/js/main_js/cont.js?20201216.1" type="text/javascript"></script>
    <script src="/js/main_js/index20.js?20201216.1" type="text/javascript"></script>
...
<div style="padding: 8px;margin-top:45px;">
    <div class="head-top">
        <ul class="clearfix">
            <li id="6501" menu_tid="650101" class="fl head-top-li" url="\/document\/draftArticlesOfCommunication" oldmenuUrl="\/document\/draftArticlesOfCommunication" data-type=""onclick="documentActive(this)">发文拟稿<h2 style="display: none">发文拟稿</h2></li>
            <li id="6505" menu_tid="650501" class="fl head-top-li" data-type="0" url="document\/addresseeRegistrationForm" oldmenuUrl="document\/addresseeRegistrationForm" data-type=""onclick="documentActive(this)">收文登记<h2 style="display: none">收文登记</h2></li>
        </ul>
    </div>
    ...
    <div class="container">
        ...
        <div class="transaction box_style" id="daiban" style="position: relative;">
            ...
            <div class="title">
                ...
                <strong>待办事项</strong>
                <span onclick="parent.getMenuOpen(this)" tid="6509" more_type="1" url="/document/documentDispatchReceive">更多公文<h2 style="display: none">公文收发</h2></span>
                <span onclick="parent.getMenuOpen(this)" tid="1020" more_type="1" url="/workflow/work/workList">更多工作<h2 style="display: none">我的工作</h2></span>

            </div>
            <div class="content">
                <ul class="theMatters" style="height: 300px"></ul>
            </div>
        </div>
        ...
        <div class="announcement box_style" id="gonggao" style="border-bottom:0; position:relative;">
            ...
            <div class="title">
                ...
                <strong>公告通知</strong>
                <span onclick="parent.getMenuOpen(this)" tid="0116" url="/notice/InformTheView" >更多<h2 style="display: none">公告通知</h2></span>
    ...
    <div class="container">
        <%--我的公文--%>
        <div class="addressee box_style" id="gongwen" style="position: relative;border-bottom:1px solid #0588c7">
            <%--翻页按钮--%>
            ...
            <div class="title">
                ...
                <%--<strong>我的公文</strong>--%>
                <strong>最新文件</strong>
                <span onclick="parent.getMenuOpen(this)" tid="0110" more_type="1" url="ToBeReadController/toReadFile">更多<h2 style="display: none">待阅事宜</h2></span>
            <%--<span onclick="parent.getMenuOpen(this)" class="a" tid="1020" url="/document/makeADraft" oldmenuUrl="document/send/backlog">更多<h2 style="display: none">我的公文</h2></span>--%>
                <%--<span onclick="amange(1)"  url="document\/inAbeyance" oldmenuUrl="document\/recv\/backlog">待办收文</span>--%>
                <%--<span  class="active" onclick="amange(0)"  url="\/document\/makeADraft" oldmenuUrl="document\/send\/backlog">待办发文</span>--%>
            </div>
            <div class="content">
        ...
        <div class="file_Management box_style" style="border-bottom:1px solid #0588c7">
            <div class="title">
                ...
                <%--<strong>文件管理</strong>--%>
                <strong>文件归档</strong>
                <span onclick="parent.getMenuOpen(this)" tid="3001" url="/newFilePub/fileCabinetHome?0" >更多<h2 style="display: none">文件管理</h2></span>
        ...
        <%--documentMore--%>
        <%--parent.getMenuOpen(this)     /document/makeADraft/document/inAbeyance--%>

    </div>
</div>
<script>

    $.post('/newFileComment/checkPriv',{userId: $.cookie('userId')},function(json){
        if(!json.flag){
    ...
    function doChangePage(num) {
        ...
        if(pNum>=1 && pNum<=maxpNum){
            $.ajax({
                url: '/workflow/work/selectWorkDocument',
                type: 'get',
                data:{
                    page:pNum,
                    pageSize:6,
...
//                右可用 107。4 左可用165.4 右不可用 224.4 左不可用 -49.4
                    ...
                    if(data.length>0){
                        ...
                        data.forEach(function(val,index){
                            ...
                            var dataurls = ''
                            if(docId && docId!=0){
                                dataurls='/workflow/work/workform?&opFlag='+data[index].opFlag+'&flowId='+data[index].flowRun.flowId+'&flowStep='+data[index].prcsId+'&runId='+data[index].runId+'&prcsId='+data[index].flowPrcs+'&isNomalType=false&hidden=true&ie_open=yes&tableName=document&tabId='+data[index].docId+''
                            }else if(docId == 0 || docId==null || docId==undefined){
                                dataurls='/workflow/work/workform?opFlag='+data[index].opFlag+'&flowId='+data[index].flowRun.flowId+'&flowStep='+data[index].prcsId+'&runId='+data[index].runId+'&prcsId='+data[index].flowPrcs+''

                            }
                            if(data[index].flowRun == undefined||data[index].flowRun.workLevel == undefined||data[index].flowRun.workLevel == 0){
                                var workLevel = '<span style="color: green;">【普通】</span>'
                            }else if(data[index].flowRun.workLevel == 1){
                                var workLevel = '<span style="color: red;">【紧急】</span>'
                            }else if(data[index].flowRun.workLevel == 2){
                                var workLevel = '<span style="color: red;">【特急】</span>'
                            }

                            str='<li onclick="windowOpenNew(this)" data-url="'+dataurls+'">'
                            str+='   <img src="/img/main_img/managementPortal/boy.png">'
                            str+='   <div>'
                            str+='       <h1 class="n_name">'+data[index].userName+'</h1>'
                            str+='     <h2 class="n_title">'+workLevel+function () {
                                    if(data[index].flowRun.runName.indexOf(data[index].createTime.split(" ")[0])==-1){
                                        return data[index].flowRun.runName
                                    }else{
                                        return data[index].flowRun.runName.split(" ")[0]
                                    }
                                }()+'</h2>'
                            str+='   </div>'
                    ...
                    if(data.length>0){
                        data.forEach(function(val,index){
                            ...
                            if(data[index].read==0){
                                strs='<span style="margin-top:5px"><img src="/img/news.png" style="height: 20px; width:35px"></span>'
                            }else{
                                strs=''
                            }
                            ...
                            str+='</li>'
                            $('.new').append(str);
                            // $('.new div').width($(window).width()/2-200)
                        })
                    }else{
                        str += '<div class="noData" style="text-align: center;border: none;">' +
                            '       <img  src="/img/main_img/shouyekong.png" alt="">' +
...
//                判断如果数据少于6条翻页按钮显示隐藏
            ...
            if(data.length>0){
                data.forEach(function(val,index){
                    ...
                    str='<li onclick="tiaozhuan(this)" style="display: flex" data-url="/notice/detail?notifyId='+data[index].notifyId+'">'
                    str+='   <div style="width: 85%">'
                    str+='       <h3 class="n_name"><b>•</b>'+data[index].subject+'</h3>'
                    str+='   </div>'
                    str+='<span>'+format(data[index].notifyDateTime)+'</span>'
                    str+=''+strs+''
            ...
            success: function (res) {
                ...
                if(data.length>0){
                    data.forEach(function(val,index){
                        ...
                        str+='   <div style="display: flex;"><h2 style="display: none">最新文件</h2>'
                        str+='       <h3 class="titleName"><b>•</b>'+data[index].sortName+'</h3>'
                        str+='       <h3 class="mainName">'+data[index].subject+'</h3>'
                        str+='   </div>'
                        str+='   <span style="float: right;margin: 15px 15px 0 0;">'+format(data[index].sendTime)+'</span>'
                        str+='</li>'
                        $('.Management').append(str);
                        // $('.Management div').width($(window).width()/2-170)
                    })
                }else{
                    str += '<div class="noData" style="text-align: center;border: none;">' +
                        '       <img  src="/img/main_img/shouyekong.png" alt="">' +
    ...
    function documentMore(me) {
        ...
        if(url == '/document/makeADraft'){
            var tid = '650105';
        }else if(url == '/document/inAbeyance'){
            var tid = '650505';
        }
    ...
    function Doamange(num) {
        ...
        if(pNums>=1 && pNums<=maxpNums){
            $.ajax({
                url: '/document/selMyDocument',
                type: 'get',
                data:{
                    page:pNums,
                    pageSize: 6,
                ...
                success: function (res) {
                    ...
                    if(data.length>0){
                        data.forEach(function(val,index){
                            ...
                            str+='       <h2 class="n_title">'+data[index].runName+'</h2>'
                            str+='   </div>'
                            str+='   <span>'+format(data[index].createTime)+'</span>'
                            str+='</li>'
                            $('.document').append(str);
    ...
    function Doamange(num){
        //首先判断url
        var url = '/document/selMyDocument'
        //var url = $("#gongwen div.title").find('.active').attr('url');
        // 代办收文
        if(url=="/document/selMyDocument"){
            gongwenNum+=num;
            if(gongwenNum<=1){
                gongwenNum=1;
            ...
            if(gongwenNum>=1 && gongwenNum<=gongwenmaxNum){
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        ...
                        if(data.length>0){
                            data.forEach(function(val,index){
                                if(data[index].documentType==0){
                                    documentType="发文"

                                }else if(data[index].documentType==1){
                                    documentType="收文"
                                }
                                if(data[index].runName==undefined){

                                    docNo=""

                                }else if(data[index].runName){

                                    docNo=data[index].runName

                                }
                                var strs;
                                if(data[index].readFlag==false){
                                    strs='<span style="margin-top:5px"><img src="/img/news.png" style="height: 20px; width:35px"></span>'
                                }else{
                                    strs=''
                                }
                                str='<li data-s="1" onclick="tiaozhuan(this)" data-url="/workflow/work/workformPreView?flowId='+data[index].flowId+'&runId='+data[index].runId+'&tabId='+data[index].id+'&tableName='+data[index].tableName+'&isNomalType=false&hidden=true&ie_open=yes&flowStep='+data[index].step+'&prcsId='+data[index].realPrcsId+'">'
                                str+='   <img src="/img/main_img/managementPortal/boy.png">'
                                str+='   <div>'
                                str+='       <h1 class="n_name">'+data[index].createrName+'</h1>'
                                str+='       <h2 class="n_title">'+data[index].runName+'</h2>'
                                str+='   </div>'
                                str += '<span style="width: 5%;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;">' + documentType + '</span>'
                                // str+='  <span style="width: 10%;text-overflow: ellipsis;white-space:nowrap;overflow:hidden;">'+docNo+'</span>'
                                str += '<span style="text-overflow: ellipsis;white-space:nowrap;overflow:hidden;">' + data[index].endTime.trim().split(" ")[0] + '</span>'
                                str+=''+strs+''
                                str+='</li>'
                                $('.document').append(str);
                                // $('.document div').width($(window).width()/2-280)
                            })
                        }else{
                            str += '<div class="noData" style="text-align: center;border: none;">' +
                                '       <img  src="/img/main_img/shouyekong.png" alt="">' +
