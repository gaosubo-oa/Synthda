...
<body>
<div class="header1">
    <i><a href="/m/getMainddh5"><fmt:message code="notice.th.return" /></a></i>
    <span><fmt:message code="main.schedule" /></span>
    <i><a href="/calender/addScheduleh5"><fmt:message code="workflow.pluginlist.add" /></a></i>
</div>
...
<div class="nav">
    <ul class="navs">
        <li class="navc fl"><fmt:message code="attend.th.calendar" /></li>
        <li class="fr" style="border-radius: 0px 4px 4px 0px;"><fmt:message code="attend.th.list" /></li>
    </ul>
</div>
    ...
    <div id="Demo">
        <div id="CalendarMain">
            <div id="title">
                ...
                <a class="selectBtn selectMonth" onclick="CalendarHandler.CreateSelectMonth()">6<fmt:message code="global.lang.month" /></a>
                <a class="selectBtn nextMonth" href="javascript:" onclick="CalendarHandler.CalculateNextMonthDays();">></a>
                <a class="selectBtn currentDay" href="javascript:" onclick="CalendarHandler.CreateCurrentCalendar(0,0,0);"><fmt:message code="notice.th.Today" /></a>
            </div>
            <div id="context">
                <div class="week">
                    <h3>一 </h3>
                    ...
                    <h3>五 </h3>
                    <h3>六 </h3>
                    <h3><fmt:message code="global.lang.day" /> </h3>
                </div>
                <div id="center">
                    <div id="centerMain">
...
<div class="nav1 " style="display: none">
    <div>
        <div class="header herderc">
            <i><fmt:message code="notice.th.Today" /></i>
            <i><a href="/calender/addScheduleh5" style="color:#00a0e9;"><fmt:message code="workflow.pluginlist.add" /></a></i>
        </div>
    ...
    <div>
        <div class="header herderc">
            <i><fmt:message code="main.th.Tomorrow" /></i>
            <i><a href="/calendar/addScheduleh5" style="color:#00a0e9;"><fmt:message code="workflow.pluginlist.add" /></a></i>
        </div>
        <div style="margin: 0 10px;font-size: 13px;">
            <div id="zbtbody2">
    ...
    <div>
        <div class="header herderc">
            <i><fmt:message code="main.th.AfterTomorrow" /></i>
            <i><a href="/calendar/addScheduleh5" style="color:#00a0e9;"><fmt:message code="workflow.pluginlist.add" /></a></i>
        </div>
        <div style="margin: 0 10px;font-size: 13px;">
            <div id="zbtbody3">
...
<script>
    ...
    $(function(){
        ...
        if(mouth31.indexOf(mouth)>=0){
            ...
            date.setDate(31);
            var sendDate = date.getTime()+'';
            sendDate = sendDate.substring(0,5);
            $.ajax({
                type: "get",
        ...
        for(var j=0; j<7; j++) {
            ...
            if(mouth31.indexOf(mouth)>=0) {
                if(i < 31) {
                    ...
                    getSevenDays(j, dataList31);
                } else {
                    if(typeof(dataList2[i - 32].calMessage)!="undefined") {
                        getSevenDays(j, dataList2[i - 32].calMessage);
                    }
                }
            } else {
                if(i < 30) {
    ...
    function getSevenDays(i, dataList)
    {
        ...
        if(typeof(dataList)!="undefined") {
            $.each(dataList, function (index, item) {
                $('#zbtbody'+(j)).append("<a class='add' href='/calender/calendarDetails' style='position:relative;' calId='"+item.calId+"' calType='"+item.calType+"' calLevel='"+item.calLevel+"'><img style='width: 40px;height: 40px;position: absolute;top: 13px;left: -5px;' src='../../img/H5/1.10.png' alt=''><ul><li class='ove1' style='width: 95%;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;'>" + item.content + "</li><li class='ove2'><fmt:message code="sup.th.startTime" />" + item.stim + "</li><li class='ove3'><fmt:message code="meet.th.EndTime" />" + item.etim + "</li></ul></a>");
            });
        }
    }
                 ...
                 $.each(dataList1, function (index, item) {
                     if(nowday==item.calData){
                         ...
                         for(var i=0;i<item.calMessage.length;i++) {
                             $('#zbtbody').append("<a class='add' href='/calender/calendarDetails' style='position:relative;' calId='"+item.calMessage[i].calId+"' calType='"+item.calMessage[i].calType+"' calLevel='"+item.calMessage[i].calLevel+"'><img style='width: 40px;height: 40px;position: absolute;top: 13px;left: -5px;' src='../../img/H5/1.10.png' alt=''><ul><li class='ove1' style='width: 95%;overflow: hidden;text-overflow: ellipsis;white-space: nowrap;'>" + item.calMessage[i].content + "</li><li class='ove2'><fmt:message code="sup.th.startTime" />" + item.calMessage[i].stim + "</li><li class='ove3'><fmt:message code="meet.th.EndTime" />" + item.calMessage[i].etim + "</li></ul></a>");
                         };
                     }
                 });
            ...
            if (getClass.indexOf("lastItem") == -1) {
                ...
                //加载选中日期的值班信息
                    if(gettext==31){
                        ...
                        date.setDate(31);
                        var date31 = date.getTime()+'';
                        date31 = date31.substring(0,5);
                        $.ajax({
                            type: "get",
                            url: "/schedule/getscheduleByDay?userId="+userId+"&time="+getDay(0),
                            dataType: "json",
                            success: function (data) {
                                ...
                                if(data.flag&&data.obj.length>0){
                                    for(var i=0;i<data.obj.length;i++) {
                                        $('#zbtbody').append("<a class='add' href='/calender/calendarDetails' style='position:relative;' calId='"+data.obj[i].calId+"' calType='"+data.obj[i].calType+"' calLevel='"+data.obj[i].calLevel+"'><img style='width: 40px;height: 40px;position: absolute;top: 13px;left: -5px;' src='../../img/H5/1.10.png' alt=''><ul><li class='ove1'>" + data.obj[i].content + "</li><li class='ove2'><fmt:message code="sup.th.startTime" />" + data.obj[i].stim + "</li><li class='ove3'><fmt:message code="meet.th.EndTime" />" + data.obj[i].etim + "</li></ul></a>");
                                    };
                                }
                            },
                        });
                            ...
                            success: function (data) {
                                ...
                                $.each(data.obj, function (index, item) {
                                    if(sendDate == item.calData){
                                        for(var i=0;i<item.calMessage.length;i++) {
                                            $('#zbtbody').append("<a class='add' href='/calender/calendarDetails' style='position:relative;' calId='"+item.calMessage[i].calId+"' calType='"+item.calMessage[i].calType+"' calLevel='"+item.calMessage[i].calLevel+"'><img style='width: 40px;height: 40px;position: absolute;top: 13px;left: -5px;' src='../../img/H5/1.10.png' alt=''><ul><li class='ove1'>" + item.calMessage[i].content + "</li><li class='ove2'><fmt:message code="sup.th.startTime" />" + item.calMessage[i].stim + "</li><li class='ove3'><fmt:message code="meet.th.EndTime" />" + item.calMessage[i].etim + "</li></ul></a>");
                                        };
                                    }
                                });
                            },
    ...
    var CalendarHandler = {
        ...
        CreateSelectYear: function (showYearStart) {
            ...
            for (var i = showYearStart; i < showYearStart + 12; i++) {
                yearindex++;
                if (i == showYearStart) {
                    $last = $("<div><fmt:message code="calendar.th.goForward" /></div>");
                ...
                if (i == showYearStart + 11) {
                    $next = $("<div><fmt:message code="calendar.th.goBackward" /></div>");
        ...
        CreateSelectMonth: function () {
            ...
            for (var i = 1; i < 13; i++) {
                if (i == this.currentMonth) $monthItem = $("<div class=\"currentMontSd\" id=\"" + i + "\">" + i + "<fmt:message code="global.lang.month" /></div>");
                else $monthItem = $("<div id=\"" + i + "\">" + i + "<fmt:message code="global.lang.month" /></div>");
        ...
        CreateCalendar: function (y, m, d) {
            ...
            $(".selectYear").html(nowYear + "<fmt:message code="chat.th.year" />");
            $(".selectMonth").html(nowMonth + "<fmt:message code="global.lang.month" />");
