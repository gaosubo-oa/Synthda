...
<div class="head-top">
    <ul class="clearfix">
        ...
        <li class="fl head-top-img"><img src="/img/twoth.png" alt=""></li>
        <li class="fl head-top-li active" data-type="1">层级关系</li>
        <li class="fl head-top-img"><img src="/img/twoth.png" alt=""></li>
...
<div class="step" style="margin-top: 60px;">
    ...
    <div class="navigation" style="margin-left: 11%;width: 100%;margin-bottom: 30px;">
        <label style="font-size: 15px;">姓名<input type="text" placeholder="" id="userName"   style="margin-right: 5px"></label>
        <label style="margin-left: 15px;font-size: 15px;">直属上级<input type="text" placeholder=""  readonly="readonly" id="selectTopUser" style="margin-right: 5px" onclick="selectTop()"></label>
        <label style="margin-left: 15px;font-size: 15px;">直属下级<input type="text" placeholder="" readonly="readonly" id="selectBottomUser" style="margin-right: 5px" onclick="selectBottom()"></label>
    ...
    <div id="pagediv" style="margin-top: 80px;">
        <table class="hieraTable" style="table-layout:fixed">
            ...
            <tr>
                ...
                <th style="width:20%">直属上级</th>
                <th style="width:20%">直属下级</th>
                <th style="width:20%">上次操作时间</th>
...
<div class="step1" style="display: none;margin-left: 10px;margin-top: 80px;">
    <table class="newNews">
        ...
        <tr>
            <td class="td_w blue_text">直属上级：</td>
            <td>
                <textarea class="td_title1" type="text" placeholder="" id="topName" disabled style="background-color: #e0e0e0;height: 100px;border-radius: 5px;"></textarea>
                <div class="release3" id="priv_add">添加</div>
            </td>
        </tr>
        <tr>
            <td class="td_w blue_text">直属下级：</td>
            <td>
                <textarea class="td_title1" type="text" placeholder="" id="bottomName" disabled style="background-color: #e0e0e0;height: 100px;border-radius: 5px;"></textarea>
...
<script type="text/javascript">
    ...
    // 选人
    function selectTop(){
        user_id = "selectTopUser";
        $.popWindow("../common/selectUser");
    }
    function selectBottom(){
        user_id = "selectBottomUser";
        $.popWindow("../common/selectUser");
    }
    ...
    var ajaxPageLe = {
        ...
        page: function (userName,selectTopUser,selectBottomUser) {
            var me = this;
            me.data.userIdName = userName;
            me.data.userTopId = selectTopUser;
            me.data.userBottomId = selectBottomUser
            $.ajax({
                url: '/userHierarchy/selectAllUserHierarchy',
                type: 'get',
                data: me.data,
                dataType: 'json',
                success: function (reg) {
                    ...
                    for (var i = 0; i < datas.length; i++) {
                        str += '  <tr id="' + datas[i].id + '">' +
                            ...
                            '                    <td title="' + datas[i].userTopIdName +'">' + datas[i].userTopIdName + '</td>' +
                            '                    <td title="' + datas[i].userBottomIdName +'">' + datas[i].userBottomIdName + '</td>' +
    ...
    $('.list').on('click', '.del', function () {
        ...
        layer.confirm(' 确定要删除吗', {
            ...
            $.ajax({
                url: '/userHierarchy/deleteUserHierarchyById',
                type: 'post',
                dataType: 'json',
                data: {id: id},
                success: function () {
            ...
            success: function (res) {
                if(res.flag){
                    ...
                    if(object!=undefined){
                        ...
                        $("#topName").val(object[0].userTopIdName || '');
                        $("#topName").attr('user_id',object[0].userTopId || '');
                        $("#bottomName").val(object[0].userBottomIdName || '');
                        $("#bottomName").attr('user_id',object[0].userBottomId || '');
                    }
                }
            }
        })
    ...
    $('#search').on('click',function () {
        ...
        var selectTopUser = $('#selectTopUser').attr('user_id')
        var selectBottomUser = $('#selectBottomUser').attr('user_id')
        ajaxPageLe.page(userName,selectTopUser,selectBottomUser);
    })
    $(function () {
        //保存
        function save(id){
            var data={
                userId:$('#setUser').attr('user_id'),
                userTopId:$('#topName').attr('user_id'),
                userBottomId:$('#bottomName').attr('user_id'),
                id:id
            };
            $.ajax({
                type: 'post',
                url: '/userHierarchy/addOrUpdateUserHierarchy',
                dataType: 'json',
                data: data,
                success: function (res) {
                    if(res.flag){
