...
<body>
    <div style="margin-top: 20px">
        <span class="zxc" style="margin: 30px;font-size: 25px">第一轮资源预定</span>
    </div>
    <table class="layui-hide" style="margin-top: 10px" id="test1" lay-filter="test1"></table>
    <center><button type="button" class="layui-btn layui-btn-sm" id="reserved" lay-event="reserved" style="height: 40px;line-height: 30px;margin-top: 10px;font-size: 15px">点击进入第二轮资源包预定</button></center>
    <script id="barDemo" type="text/html">
        <div class="long1">
            <a id="check" lay-event="check" data-index="1" style="color: blue;cursor:pointer; text-decoration:underline">查看资源包</a>
        </div>
...
<script>
    ...
    layui.use(['form', 'table', 'element', 'layedit','eleTree'], function () {
        ...
        //     dataType: "json",
        //     success:function (res) {
        //         userPriv=res.object.role.userPriv
        //     }
        // })
        // $.ajax({
        ...
        var meetTable=table.render({
            elem: '#test1'
            , url: '/cdmResource/selectAllCdmResourceClass?pageSize='+'10'+'&useFlag='+true
            ...
            , title: '用户数据表'
            , cols: [[
                {field:'className',title:'资源分类名称',align:'center'},
                {field:'classDesc',title:'分类描述',align:'center'},
                // {title:'分类包含资源包',toolbar:'#barDemo',align:'center'},
                {field:'classState',title:'预定状态',align:'center',templet:function (b) {
                        if(b.classState=='0'){
                            return '未预定'
                        }else if(b.classState=='1'){
                            return '已预定'
                        }
                    }},
            ...
            , done: function (res) {
                for(var i=0;i<res.data.length;i++) {
                    var doneres = res.data[i].classState
                    if(doneres=='1'){
                        $(".long").hide()
                        $("#reserve").hide()
                    }
        ...
        table.on('tool(test1)', function (obj) {
            ...
            if(obj.event === 'reserve'){
                ...
                //     dataType: "json",
                //     data:{
                //         name:'CDM_RESOURCE_PRIV_NO'
                //     },
                //     success:function (res) {
                //         if(res.object.paraValue!='') {
                //
                //         }else{
                //             $.layerMsg({content: '请设置当前选择资源的会员角色！', icon:3});
                //             return false;
                //         }
                //
                //
                ...
                layer.confirm('请确认是否预订' + titles + '会议资源。', {
                    ...
                    $.ajax({
                        type: 'post',
                        url: '/cdmResource/updateCdmResourceClassByUserId',
                        dataType: 'json',
                        data: {
                ...
                window.location.href='/cdmResource/Businesscheck?classIds='+classIds
