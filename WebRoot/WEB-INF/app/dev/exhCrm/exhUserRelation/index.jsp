...
<div class="mbox">
    <div style="margin-top: 20px;position: relative;" class="head">
        <img src="/img/exhuserrelation.png" style="position: absolute;left: 16px;width: 28px; "><span
            style="margin-left: 49px;font-size: 20px;">客户业务关系管理</span>
    ...
    <div style="margin: 10px 0">
        <form class="layui-form" action="">
            <div>
                    <div class="layui-inline" style="margin-left: -14px">
                        ...
                        <div class="layui-input-inline stat">
                            <%--                        <input type="text" name="userName" id="selectDept"  autocomplete="off" class="layui-input dept">--%>
                                <input id="userId2" type="text" lay-verify="required" autocomplete="off" class="layui-input">
                        </div>
                    </div>
                    <div class="layui-inline" style="margin-left: -14px">
                    ...
                    <button class="layui-btn search" type="button" style="margin-left: 10px">查询</button>
...
<script type="application/javascript">
    ...
    layui.use(['table', 'form','layer','laydate','upload'], function () {
        ...
        var tableIns = table.render({
            elem: '#demo'
            , url: '/exhUserRelation/selectByAll?useFlag=true' //数据接口
            , page: true //开启分页
            , cols: [[ //表头
                {field: "exhUserName", title: '客户名称', align: 'center'},
                ...
                {field: "detail", title: '详细说明', align: 'center'},
        ...
        table.on('tool(test)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('确定删除该数据吗？', function (index) {
                    $.post('/exhUserRelation/deleteByExhUserId', {id: data.id}, function (res) {
                        if (res.flag) {
        ...
        //             var str
        //             for (var i = 0; i < data.length; i++) {
        //                 str+='<option value=' + data[i].exhUserId + '>' + data[i].exhUserName + '</option>'
        //             }
        //             $('select[name="userId1"]').append(str);
        //             form.render('select');
        //         }
        //     }
        // })
        ...
        $('#export').on("click",function(){
            actName = $('#actId2').val();
            exhUserName = $('#userId2').val();
            if(flags){
                window.location.href ='/exhUserRelation/selectByAll?useFlag=true&export=true&actName='+actName+'&exhUserName='+exhUserName;
            }else{
                window.location.href ='/exhUserRelation/selectByAll?useFlag=true&export=true';
            }
        });
        //查询
        var actName;
        ...
        $('.search').click(function(){
            ...
            exhUserName = $('#userId2').val();
            tableIns.reload({
                url: '/exhUserRelation/selectByAll',
                page:true,
                limit: 20,
            ...
            layer.open({
                ...
                success:function () {
                    ...
                    $('#load').on('click',function () {
                        window.location.href = encodeURI("/file/exhCrm/客户业务关系管理表.xls")
                    })
                    //上传
                    //执行实例
                    var uploadInst = upload.render({
                        elem: '#test1' //绑定元素
                        ,url: '/exhUserRelation/import' //上传接口
                        ,accept:'file'
                        ,auto:false
        ...
        function creat(type, obj) {
            if (type == '0') {
                var title = '新建'
                var url = '/exhUserRelation/insertExhUserRelation'
            } else {
                var title = '编辑'
                var url = '/exhUserRelation/updateExhUserRelation'
            }
            layer.open({
                type: 1,
                title: title,
                ...
                area: ['60%', '80%'],
                content: '<form class="layui-form" id="ajaxforms" lay-filter="formsTest" style=" padding-left: 100px;padding-top: 50px;">' +
                        '<input type="hidden" name="id"/> '+
                    '<div class="layui-form-item">\n' +
                    '    <label class="layui-form-label" style="width: 90px;padding: 5px;"><span style="color: red; font-size: 15px;">*</span>客户名称:</label>\n' +
                    '    <div class="layui-input-block">\n' +
                    ' <input type="text" id="userId" name="exhUserId" style="background-color:#ebebeb;" readonly class="layui-input">' +
                    ' </div>\n' +
                    '<div class="itemFont selectBtn" style="color:#1E9FFF;">选择</div>' +
                    ' </div>' +
                    '<div class="layui-form-item">\n' +
                    '    <label class="layui-form-label" style="width: 90px;padding: 5px;"><span style="color: red; font-size: 15px;">*</span>活动名称:</label>\n' +
                    '    <div class="layui-input-block">\n' +
                    '  <input type="text" name="actId" style="background-color:#ebebeb;" readonly class="layui-input">' +
                    ...
                    '<option selected id="relationType">请选择关系种类</option>' +
                    '      </select>\n' +
                    '    </div>\n' +
                    '  </div>' +
                    '<div class="layui-form-item">\n' +
                ...
                success: function () {
                    $('.selectBtn').on("click",function() {
                        layer.open({
                            type: 2,
                            title:'选择客户名称',
                            content:"../fcrmCustomer/selectCustomer?szyx=0",
                            area:['800px','500px'],
                            btn:['确定','取消'],
                            yes:function(index,dom) {
                                ...
                                $('input[name=exhUserId]').val(text).attr("text",val)
                                layer.close(index)
                            }
                        })
                    })
                    $('.actId').on("click",function() {
                        layer.open({
                            type: 2,
                            title:'选择活动名称',
                            content:"../fcrmCustomer/selectCustomer?szyx=1",
                            area:['800px','500px'],
                            btn:['确定','取消'],
                            yes:function(index,dom) {
                                var body = layer.getChildFrame('body', index);
                    ...
                    if(type=='1'){
                        ...
                        $("input[name='exhUserId']").val(obj.exhUserName);
                        $("input[name='actId']").attr("text",obj.exhUserId)
                        $("input[name='exhUserId']").attr("text",obj.actId)
                        form.render()
                    }
                },
                yes: function (index) {
                    ...
                    data.exhUserId=$("input[name='actId']").attr("text");
                    data.actName=$("input[name='actId']").val();
                    data.actId = $("input[name='exhUserId']").attr("text");
                    data.exhUserName=$("input[name='exhUserId']").val();

                    if ($("input[name='exhUserId']").val()=="") {
                        layer.msg('请选择客户', {icon: 2});
                    } else if($("input[name='actId']").val()==""){
                    ...
                    }else if(data.relationContent===""){
                        layer.msg('请填写内容', {icon: 2});
                    }else if(data.detail===""){
                        layer.msg('"请填写详细说明"', {icon: 2});
                    }else if (data.detail.length > 200) {
                        layer.msg('详细说明限200字', {icon: 2});
                    } else {
                        if (type == 1) {
                            data.id = obj.id
                        }
