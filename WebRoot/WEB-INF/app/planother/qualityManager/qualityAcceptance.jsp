...
<script>
	...
	var TableUIObj = new TableUI('plb_quality_acceptance');
	...
	layui.use(['form', 'laydate', 'table', 'element', 'soulTable', 'eleTree'], function () {
					...
					layer.open({
						...
						yes: function (index) {
							...
							if (checkStatus.data.length > 0) {
								...
								delete approvalData.securityRectification;
								...
								newWorkFlow(flowData.flowId, function (res) {
									...
									$.ajax({
										url: '/workflow/qualityAcceptanceManager/updateRunId',
										data: submitData,
										dataType: 'json',
		...
		function tableInit(projId) {
			...
			var option = {
				elem: '#tableObj',
				url: '/workflow/qualityAcceptanceManager/select',
				toolbar: '#toolbarHead',
				cols: [cols],
		...
		function newOrEdit(type, data) {
			...
			var url = '';
			var acceptanceId = '';
			var content ='/workflow/qualityManager/addRectification?urlType=acceptance'
			var projId = $('#leftId').attr('projId');
			//$('#leftId').attr('_type',type);
			if (type == 1) {
				title = '新增';
				url = '/workflow/qualityAcceptanceManager/insert';
			} else if (type == 2) {
				title = '质量验收';
				//url = '/workflow/qualityAcceptanceManager/updateById?acceptanceFlag=1&acceptanceId='+data.acceptanceId,
				url = '/workflow/qualityAcceptanceManager/updateById'
				securityInfoDate=data;
			}else if(type == '4'){
				title = '质量验收查看详情'
				content = '/workflow/qualityManager/addRectification?urlType=acceptance&type=4'
				securityInfoDate=data;
