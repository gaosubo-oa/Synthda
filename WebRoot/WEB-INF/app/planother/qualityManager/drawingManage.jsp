...
<div class="container" style="padding-top: 15px;box-sizing: border-box">
	...
	<div style="padding: 0px 8px;" class="clearfix">
		...
		<div class="con_right">
			...
			<div class="query_module layui-form layui-row" style="position: relative;margin-left: 10px">
				<div class="layui-col-xs2">
					<input type="text" name="regionName" placeholder="区域名称" autocomplete="off" class="layui-input">
				</div>
	...
	layui.use(['treeTable','form','laydate','upload', 'element', 'layer','eleTree'], function () {
		...
		eleTree.on("nodeClick(leftTree)", function (d) {
			...
			if (currentData.projId) {
				...
				$('.no_data').hide();
				$('.table_box').show();
				treeTableShow(currentData.projId);
			} else {
				$('.table_box').hide();
				$('.no_data').show();
			}
		...
		function treeTableShow(projectId, projectIds) {
			...
			insTb = treeTable.render({
				elem: '#demoTreeTb',
				url: '/workflow/qualityManager/getRegionByProject?projectId='+projectId+'&_='+new Date().getTime(),
				toolbar: '#toolbarDemo',
				tree: {
						...
						layer.open({
							...
							success: function () {
								...
								$("[name='ttitle1']").on("click",function (e) {
									...
									if(!el){
										el=eleTree.render({
											elem: '.ele2',
											url:'/workflow/qualityManager/childTree?projectId='+projectIdd,
								...
								eleTree.on("nodeClick(data2)",function(d) {
									...
									$("[name='ttitle1']").val(label);
									$("#pele").attr("pid",id);
								})

								$("#regionName").val(checked.regionName);
								$("[name='ttitle1']").val(checked.regionParentName);
								$("#pele").attr("pid",checked.regionParent);
							...
							yes: function (index, layero) {
								...
								if(regionName==undefined||regionName==""){
									...
									$.ajax({
										url:'/workflow/qualityManager/updateRegion',
										dataType:'json',
										type:'post',
										data:region,
										success:function(res){
					...
					if(checkData==undefined||checkData.length<1){
						...
						if(projectIdd!=undefined){
							layer.confirm('确定删除选中数据吗？', function(index){
								$.ajax({
									url:'/workflow/qualityManager/deleteRegion',
									dataType:'json',
									type:'post',
					...
					layer.open({
						...
						yes: function (index, layero) {
							...
							if(regionName==undefined||regionName==""){
								...
								$.ajax({
									url:'/workflow/qualityManager/insertRegion',
									dataType:'json',
									type:'post',
									data:region,
									success:function(res){
							...
							success: function () {
								...
								for(var i=0;i<checkData.length;i++){
									...
									if (checkData[i].url != undefined && checkData[i].url != '') {
										str = '<div class="dech" deUrl="' + checkData[i].url+ '"><img class="preview" style="width: 100px;" src="/xs?'+encodeURI(checkData[i].url)+'""><input type="hidden" class="inHidden" value="' + checkData[i].attachmentId +'">' +
												'<a href="/download?'+encodeURI(checkData[i].url)+'" NAME="' + checkData[i].attachmentName + '*" style="text-decoration:none;margin-left:5px;"><img  src="/img/attachment_icon.png"/>' + checkData[i].attachmentName + '</a><img class="deImgs" style="margin-left:5px;cursor: pointer;" src="/img/file/icon_deletecha_03.png"/>' +
												'<a style="padding-left: 5px" href="/download?'+encodeURI(checkData[i].url)+'" ><img src="/img/attachmentIcon/icon_down.png"  style="padding: 0 5px;">下载</a></div>';
									} else {
										str = '';
									}
									$('#fileAll').append(str)
								...
								//附件
								fileuploadFn('#fileupload', $('#fileAll'),1);
