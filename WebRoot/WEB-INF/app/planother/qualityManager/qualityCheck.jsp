...
<head>
	...
	<script type="text/javascript" src="/js/planother/planotherUtil.js?2222021080915081"></script>
...
<script type="text/html" id="internalBar">
	<a href="javascript:;" class="openFile addRow" style="position:relative;" lay-event="butfile">
		...
		<input type="file" multiple id={{"fileupload"+d.LAY_INDEX}} data-url="/upload?module=quality"  name="file">
	</a>
</script>
...
<script>
	...
	var TableUIObj = new TableUI('plb_quality_checklist');
	...
	layui.use(['form', 'laydate', 'table', 'element', 'soulTable', 'eleTree','treeTable'], function () {
			...
			switch (obj.event) {
				...
				case 'del':
					...
					layer.confirm('确定删除该条数据吗？', function (index) {
						$.post('/workflow/qualityManager/delchecklist', {ids: ids}, function (res) {
							layer.close(index);
							if (res.code=='0') {
				...
				case 'submit':
					...
					layer.open({
						...
						success: function () {
							$.get('/plbFlowSetting/getOwnFlowData', {plbDictNo: '73'}, function (res) {
								var flowData = []
						...
						yes: function (index) {
							...
							if (checkStatus.data.length > 0) {
								...
								newWorkFlow(flowData.flowId, function (res) {
									...
									$.ajax({
										url: '/workflow/qualityManager/addSecurityOrUpdate',
		...
		function tableInit(projId) {
			...
			var option = {
				elem: '#tableObj',
				url: '/workflow/qualityManager/getSecurityCheck',
				toolbar: '#toolbarHead',
				cols: [cols],
					...
					layer.open({
						...
						success: function () {
							var tableDemoIn=table.render({
								elem: '#tableDemoIn',
								url: '/quality2Plan/select?delFlag=0&projectId='+$('#leftId').attr('projId'),
								cols: [[
									{checkbox: true},
						...
						yes: function (index,layero) {
							...
							$.post('/qualityTerm/getByIds', {ids: ids}, function (res) {
								var datas = res.obj
		...
		table.on('tool(detailTable)', function (obj) {
			...
			if (layEvent === 'del') {
				...
				if (data.securityContentId) {
					$.get('/workflow/qualityManager/delDetailsInfo', {ids: data.securityContentId}, function (res) {
			...
			layer.open({
				...
				success: function () {
					...
					if (type == 2||type == 4) {
						...
						// 获取自动编号
						getAutoNumber({autoNumberType: 'qualityChecklist'}, function(res) {
					...
					var cols = [
						...
						{
							field: 'securityGrade', title: '重大问题', minWidth: 160, templet: function (d) {
								if(d.securityGrade===0||d.securityGrade==="0"){
									return '<span class="securityGrade" securityGrade="'+d.securityGrade+'" >是</span>'
								}else if(d.securityGrade===1||d.securityGrade==="1"){
									return '<span class="securityGrade" securityGrade="'+d.securityGrade+'">否</span>'
								}else{
			...
			layer.open({
				...
				success: function () {
					...
					insTb=treeTable.render({
						elem: '#demoTreeTb',
						url: '/workflow/qualityManager/getRegionByProject',
						where:searchObj,
						tree: {
		...
		$(document).on('click', '.chooseMaterials2', function () {
			...
			layer.open({
				...
				area: ['90%', '80%'],
				maxmin: true,
				content: '/workflow/qualityManager/toStandard?urlType=qualityCheck&type=radio',
				success: function () {
				...
				yes: function (index,layero) {
					...
					if(childData){
						...
						if(childData.securityGrade===0||childData.securityGrade==="0"){
							_str = '<span class="securityGrade" securityGrade="'+childData.securityGrade+'" >是</span>'
						}else if(childData.securityGrade===1||childData.securityGrade==="1"){
							_str = '<span class="securityGrade" securityGrade="'+childData.securityGrade+'">否</span>'
						}else{
	...
	function drawingImg(_this){
		...
		$.popWindow("/workflow/qualityManager/selectDrawing?type=manage&"+url);
	}
