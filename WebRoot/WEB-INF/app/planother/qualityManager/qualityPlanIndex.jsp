...
<script>
    ...
    layui.use(['form', 'laydate', 'table', 'element', 'eleTree', 'layer','soulTable'], function () {
        ...
        var TableUIObj = new TableUI('plb_quality_plan');
            ...
            switch (obj.event) {
                ...
                case 'edit':
                    ...
                    if($('#leftId').attr('projId')){
                        ...
                        $.post('/qualityPlan/getById', {kayId: securityPlanId}, function (res) {
                            newOrEdit(1,res.obj)
                ...
                case 'del':
                    ...
                    layer.confirm('确定删除该条数据吗？', function (index) {
                        $.post('/qualityPlan/del', {ids: securityPlanId}, function (res) {
                            if (res.code=='0') {
                ...
                case 'submit':
                    ...
                    layer.open({
                        ...
                        success: function () {
                            $.get('/plbFlowSetting/getOwnFlowData', {plbDictNo: '76'}, function (res) {
                                var flowData = []
        ...
        function newOrEdit(type, data) {
            ...
            layer.open({
                ...
                '           </div>' ,
                    ...
                    '<input type="file" multiple id="fileupload" data-url="/upload?module=qualityPlan" name="file">' +
                    '</a>' +
                ...
                success: function () {
                    ...
                    if (type == 1 || type == 4) {
                        ...
                        // 获取自动编号
                        getAutoNumber({autoNumberType: 'qualityPlan'}, function(res) {
                    ...
                    var cols = [
                        ...
                        /*{
                            field: 'mainDifficulties', title: '主要难点', minWidth: 160, templet: function (d) {
                                return '<input type="text" name="mainDifficulties" class="layui-input tips" style="height: 100%;" value="' + (d.mainDifficulties || '') + '">'
                            }
                        },
                        {
                            field: 'mainRisk', title: '主要风险', minWidth: 160, templet: function (d) {
                                return '<input type="text" name="mainRisk" class="layui-input tips" style="height: 100%;" value="' + (d.mainRisk || '') + '">'
                            }
                        },
                        {
                            field: 'riskSolutions', title: '风险解决措施', minWidth: 160, templet: function (d) {
                                return '<input type="text" name="riskSolutions" class="layui-input tips" style="height: 100%;" value="' + (d.riskSolutions || '') + '">'
                            }
                        },*/
                        {
                        ...
                        },
                        {
                            field: 'securityKnowledgeName', title: '检查详细内容', minWidth: 150, templet: function (d) {
                                return '<input securityPlanId="' + (d.securityPlanId || '') + '" securityPlanDetailsId="'+(d.securityPlanDetailsId || '')+'" securityTermId="'+(d.securityTermId || '')+'" type="text" name="securityKnowledgeName" class="layui-input chooseMaterials tips" style="height: 100%;color: #0000ff;background: #e7e7e7;cursor: pointer" readonly value="' + (d.securityKnowledgeName || '') + '">'
                            }
                        }
                    ]
        ...
        table.on('tool(detailedTable)', function (obj) {
            ...
            if (layEvent === 'del') {
                if (data.securityPlanDetailsId) {
                    $.get('/qualityPlan/del', {ids: data.securityPlanDetailsId,type:"planDetails"}, function (res) {
                        if (res.code == '0') {
        ...
        table.on('tool(objectivesTable)', function (obj) {
            ...
            if (layEvent === 'del') {
                if (data.securityPlanInfoId) {
                    $.get('/qualityPlan/del', {ids: data.securityPlanInfoId,type:"planInfo"}, function (res) {
                        if (res.code == '0') {
                            layer.msg('删除成功!', {icon: 1, time: 2000});
                            obj.del();
                            table.reload('objectivesTable', {
