...
<head>
	...
	<style>
		...
		[data-field="knowledgeName"] .layui-table-cell{
			padding: 0 5px;
		}
	</style>
</head>
...
<div class="container">
	...
	<div class="wrapper">
		<div class="wrap_left">
			<h3 style="text-align: center;line-height: 35px;">质量检查、整改分析报表</h3>
			<div class="left_form">
		...
		table.on('toolbar(docDemoTabBrief)', function (obj) {
			switch (obj.event) {
				case 'export'://导出
					...
					if($('#beginDate').val()){
						str += '&beginDate='+$('#beginDate').val()
					}
					if($('#endDate').val()){
						str += '&endDate='+ $('#endDate').val()
					}
					window.location.href='/qualityAnalysis/performanceAnalysis?export=export'+str
					break;
			}
		});
		table.on('toolbar(docDemoTabBrief2', function (obj) {
			switch (obj.event) {
				case 'export'://导出
					...
					window.location.href='/qualityAnalysis/rectificationAnalysis?export=export'+str
					break;
			}
		});
