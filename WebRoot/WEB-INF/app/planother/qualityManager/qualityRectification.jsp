		...
		<div class="wrap_right">
			<div class="query_module layui-form layui-row" style="position: relative">
				...
				<div class="layui-col-xs2" style="margin-left: 10px;">
					<input type="text" name="rectificationTimeDate"  placeholder="整改期限" autocomplete="off" class="layui-input rectificationTimeDate">
				</div>
...
<script type="text/html" id="toolbarHead">
	<div class="layui-btn-container" style="height: 30px;">
		<button class="layui-btn layui-btn-sm layui-btn-normal" lay-event="examineTest">执行整改</button>
...
<script>
	...
	var colShowObj = {
		...
		securityGrade: {field: 'securityGrade', title: '重大问题',minWidth: 120, sort: true, hide: false, templet: function (d) {
				if(d.detailsInfo.securityGrade==0){
					return '<span class="securityGrade" securityGrade="'+d.detailsInfo.securityGrade+'" >是</span>'
				}else if(d.detailsInfo.securityGrade==1){
					return '<span class="securityGrade" securityGrade="'+d.detailsInfo.securityGrade+'">否</span>'
				}else{
		...
		securityDangerMeasures: {field: 'securityDangerMeasures', title: '整改措施',minWidth: 200, sort: true, hide: false,templet: function (d) {
				return '<span>'+(d.detailsInfo.securityDangerMeasures||'')+'</span>'
			}},
		rectificationPeriod: {field: 'rectificationTimeDate', title: '整改期限',minWidth: 120, sort: true, hide: false},
		/*recificationAttachmentName: {field: 'recificationAttachmentName',minWidth: 120, title: '整改通知单',templet: function (d) {
		...
		currFlowUserName: {field: 'currFlowUserName', title: '当前处理人',minWidth: 150, sort: false, hide: false},
		rectificationFlag: {field: 'rectificationFlag', title: '整改状态',minWidth: 120,templet: function (d) {
				if(d.rectificationFlag&&d.rectificationFlag==0){
					return  '<span>未整改</span>'
				}else if(d.rectificationFlag&&d.rectificationFlag==1){
					return  '<span>已整改</span>'
				}else if(d.rectificationFlag&&d.rectificationFlag==10){
					return  '<span>未验收通过</span>'
				}else{
					return  '<span></span>'
				}
			}},
		...
		rectificationUser: {field: 'rectificationUser',minWidth: 120, title: '整改人', sort: true, hide: false},
		rectificationTime: {field: 'rectificationTime', minWidth: 120,title: '整改时间', sort: true, hide: false},
	}

	var TableUIObj = new TableUI('plb_quality_rectification');
	...
	layui.use(['form', 'laydate', 'table', 'element', 'soulTable', 'eleTree'], function () {
		...
		table.on('toolbar(tableObj)', function (obj) {
			...
			switch (obj.event) {
				...
				case 'examineTest':
					...
					// 		if (!(checkStatus.data[0].auditerStatus > 0)) {
					// 			rectificationIds += v.rectificationId + ',';
					// 		}
					// 	});
					// 	layer.confirm('确定删除该条数据吗？', function (index) {
					// 		$.post('/plbMtlSettleup/delPlbMtlSettleup', {rectificationIds: rectificationIds}, function (res) {
					// 			layer.close(index);
					// 			if (res.flag) {
				...
				case 'notice':  //整改通知单
					if(checkStatus.length == 0){
						...
						var rectificationId=dataTable[0].rectificationId;
						$.ajax({
							url:'/generateDocx/generateDocxByType?rectificationId='+rectificationId+'&type=qualityRecification',
		...
		function tableInit(projId) {
			...
			//searchObj.isRecification = true;
			//searchObj.recificationStatus ='0,1,2,3';
			...
			var option = {
				elem: '#tableObj',
				url: '/workflow/qualityRectification/select',
				toolbar: '#toolbarHead',
				cols: [cols],
		...
		function newOrEdit(type, data) {
			var title = '';
			var url = '';
			var content ='/workflow/qualityManager/addRectification'
			var rectificationId = '';
			...
			if (type == 2) {
				title = '执行整改';
				//url = "/workflow/qualityRectification/updateById?rectificationFlag=1&securityContentId="+data.securityContentId+'&rectificationId='+data.rectificationId;
				url = "/workflow/qualityRectification/updateById"
				projId = data.projId;
				securityInfoDate=data;
			}else if(type == '4'){
				title = '质量整改查看详情'
				content = '/workflow/qualityManager/addRectification?type=4'
				securityInfoDate=data;
			}
		...
		function getSearchObj() {
			...
			if($('.query_module [name="rectificationTimeDate"]').val()){
				searchObj.rectificationTimeDate=$('.query_module [name="rectificationTimeDate"]').val()
			}
