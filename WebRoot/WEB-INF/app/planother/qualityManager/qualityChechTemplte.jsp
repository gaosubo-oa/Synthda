...
<script type="text/javascript">
    ...
    layui.use(['table','layer','form','element','eleTree','laydate','upload'], function(){
                ...
                if(res.obj&&res.obj.knowledgeFlag!=undefined&&res.obj.knowledgeFlag==0){
                    if(tab(res.obj.knowledgeBTime,res.obj.knowledgeETime)){
                        ...
                        el1 = eleTree.render({
                            elem: '.ele3',
                            showLine:true,
                            url:'/workflow/qualityManager/getSecurityTemplteByType?parent=0',
                            lazy: true,
                            // checked: true,
                            load: function(data,callback) {
                                $.post('/workflow/qualityManager/getSecurityTemplteByType?parent='+data.id,function (res) {
        ...
        function childFunc(secirityId){
            if(tableType!='qualityCheck'){
                SettlementTable = layui.table.render({
                    elem: '#Settlement'
                    , page:true
                    ,url:'/workflow/qualityManager/selectTemplteDetails?templteId='+secirityId
        ...
        function newOrEdit(type, data) {
            var title = '';
            var url = '';
            var templteDetailsId = '';
            if (type == 1) {
                title = '新增';
                url = '/workflow/qualityManager/insertSecurityTemplteDetails';
            }
            // else if (type == 2) {
            //     title = '编辑';
            //     url = '//workflow/qualityManager/updateSecurityTemplteDetails';
            //     templteDetailsId = data.templteDetailsId;
            // }
            layer.open({
                type: 2,
                title: title,
                ...
                yes: function (index,layero) {
                    ...
                    for(var i = 0;i<childData.length;i++){
                        childData[i].templteId=columnTrId
                    }
                    if(childData){
                        $.ajax({
                            url:url,
            ...
            if (events == 'addTest') {
                        ...
                        $("[name='ttitle3']").on("click",function (e) {
                            ...
                            if(standTypeId==undefined||standTypeId==""){
                                ...
                                layer.open({
                                    ...
                                    maxmin: true,
                                    btn: ['提交', '取消'],
                                    content: '/workflow/qualityManager/toSecirityStandard?type=checkTemplte&knowTypeIds='+standTypeId,
                                    success: function () {
                ...
                if(datas.length<1){
                    ...
                    layer.confirm('确定要删除选中的检查项吗？', {icon: 3, title:'提示'}, function(index){
                        ...
                        $.ajax({
                            url:'/workflow/qualityManager/delSecurityTemplteDetails',
                            type: 'post',
                            dataType: 'json',
                            data:{details:ids},
                            success:function(res){
                ...
                if(datas.length!=1){
                    ...
                    layer.open({
                        ...
                        content: '<div class="layui-form">' +
                            ...
                            '<label class="layui-form-label"><span style="color: red">*</span>检查内容</label>' +
                            '<div class="layui-input-block">' +
                            '<textarea type="text" id="pele3" pid name="ttitle3" required="" style="cursor: pointer;min-height: 80px;" placeholder="请选择检查内容"  autocomplete="off" class="layui-input"></textarea>' +
                            '</div>' +
                            '</div>' +
                            '<div class="inbox" style="margin: 27px 31px 10px -2px;">' +
                            '<label class="layui-form-label"><span style="color: red">*</span>整改措施</label>' +
                            '<div class="layui-input-block">' +
                            ...
                            '</div>' +
                            '<div class="inbox" style="margin: 27px 31px 10px -2px;">' +
                            '<label class="layui-form-label">重大问题<span style="color: red">*</span></label>' +
                            '<div class="layui-input-block">' +
                        ...
                        success: function () {
                            ...
                            eleTree.on("nodeChecked(data2)",function(d) {
                                ...
                                for(var i=0;i<arr.length;i++){
                                    ...
                                    var aName = arr1[i];
                                    $.ajax({
                                        url:'/workflow/qualityManager/getKnowledgeById?knowleId='+aid,
                                        dataType:'json',
                                        type:'post',
                                        async: false,
                                        success:function(res){
 ...
 */
                        ...
                        yes: function (index, layero) {
                            ...
                            if(!securityKnowledgeName){
                                ...
                                var details = {
                                    ...
                                    securityDangerGrade:securityDangerGrade,
                                    templteId:columnTrId,
            ...
            layer.open({
                ...
                yes: function (indexx) {
                    ...
                    if($("#templteName").val()==''){ //||$("#columnCode").val()==''
                        ...
                        $.ajax({
                            type: "post",
                            url: '/workflow/qualityManager/insertSecirityTemplte',
                            dataType: "json",
        ...
        $('#editPlan').click(function () {
            layer.open({
                ...
                success: function () {
                    $.ajax({
                        url:'/workflow/qualityManager/selectSecurityTemplte?secirityId='+columnTrId,
                        dataType:'json',
                        type:'post',
                        success:function(res){
                            if(res.code===0||res.code==="0"){
                ...
                yes: function (indexx) {
                    ...
                    if($("#templteName").val()==''){ //||$("#columnCode").val()==''
                        ...
                        $.ajax({
                            type: "post",
                            url: '/workflow/qualityManager/updateSecirityTemplte',
                            dataType: "json",
                            data:plbSecurityKnowledge,
                            success:function (res) {
                                if(res.code == "0" || res.code == 0){
        ...
        $('#delPlan').click(function () {
            if(columnTrId){
                layer.confirm('是否要删除', {icon: 3, title: '删除'}, function (index) {
                    $.ajax({
                        url: '/workflow/qualityManager/delSecurityTemplteByType',
    ...
    function getRepairDate2(){
        ...
        for(var i=0;i<_data.length;i++){
            _data[i].securityGrade = _data[i].securityDangerGrade?_data[i].securityDangerGrade:''
            _data[i].securityDangerDesc = _data[i].securityDanger?_data[i].securityDanger:''
        }
        return _data;
    }
