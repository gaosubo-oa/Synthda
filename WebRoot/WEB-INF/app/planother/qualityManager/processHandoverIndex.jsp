...
<div class="container">
	...
	<div class="wrapper">
		<div class="wrap_left">
			<h2 style="text-align: center;line-height: 35px;">工序交接</h2>
			<div class="left_form">
...
<script>
	...
	layui.use(['form', 'laydate', 'table', 'element', 'eleTree', 'layer','soulTable'], function () {
		...
		var colShowObj = {
			...
			constructionUnit: {field: 'constructionUnit', title: '施工单位', minWidth: 120,sort: true, hide: false},
			processHandoverPosition: {field: 'processHandoverPosition', title: '部位',minWidth: 120, sort: true, hide: false},
			acceptanceProcedure: {field: 'acceptanceProcedure', title: '验收工序',minWidth: 120, sort: true, hide: false},
		...
		table.on('toolbar(tableDemo)', function (obj) {
			...
			switch (obj.event) {
				...
				case 'del':
					...
					var processHandoverId = ''
					checkStatus.data.forEach(function (v, i) {
						processHandoverId += v.processHandoverId + ','
					})
					layer.confirm('确定删除该条数据吗？', function (index) {
						$.post('/processHandover/del', {ids: processHandoverId}, function (res) {
							if (res.code=='0') {
				...
				case 'submit':
					...
					layer.open({
						...
						success: function () {
							$.get('/plbFlowSetting/getOwnFlowData', {plbDictNo: '85'}, function (res) {
								var flowData = []
						...
						yes: function (index) {
							...
							if (checkStatus.data.length > 0) {
								...
								newWorkFlow(flowData.flowId, function (res) {
									var submitData = {
										processHandoverId:approvalData.processHandoverId,
		...
		function newOrEdit(type, data) {
			...
			layer.open({
				...
				yes: function (index) {
					...
					var attachPhotoName = '';
					for (var i = 0; i < $('#fileContent .dech').length; i++) {
						attachPhotoId += $('#fileContent .dech').eq(i).find('input').val();
						attachPhotoName += $('#fileContent .dech').eq(i).find("a").eq(0).attr('name');
					}
					obj.attachPhotoId = attachPhotoId;
					obj.attachPhotoName = attachPhotoName;
