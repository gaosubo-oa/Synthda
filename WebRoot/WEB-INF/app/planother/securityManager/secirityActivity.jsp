...
<div class="container">
	...
	<div class="wrapper">
		<div class="wrap_left">
			<h2 style="text-align: center;line-height: 35px;">安全活动</h2>
			<div class="left_form">
...
<script>
	...
	layui.use(['form', 'laydate', 'table', 'element', 'eleTree', 'layer','soulTable'], function () {
		...
		var colShowObj = {
			...
			securityActivityName: {field: 'securityActivityName', title: '安全活动名称', minWidth: 120,sort: true, hide: false},
			securityActivityType: {field: 'securityActivityType', title: '安全活动类型',minWidth: 100, sort: true, hide: false,templet: function (d) {
					if(d.securityActivityType) {
						return '<span>'+((dictionaryObj&&dictionaryObj['SECIRITY_ACTIVITY_TYPE']&&dictionaryObj['SECIRITY_ACTIVITY_TYPE']['object'][d.securityActivityType])||'')+'</span>'
					}else {
						return ''
					}
				}},
			...
			participatePersion: {field: 'participatePersion', title: '参加人员/班组',minWidth: 120, sort: true, hide: false},
			securityActivityDate: {field: 'securityActivityDate', title: '安全活动时间',minWidth: 120, sort: true, hide: false},
			securityActivityPlace: {field: 'securityActivityPlace', title: '安全活动地点',minWidth: 120, sort: true, hide: false},
		...
		table.on('toolbar(tableDemo)', function (obj) {
			...
			switch (obj.event) {
				...
				case 'del':
					...
					var securityActivityId = ''
					checkStatus.data.forEach(function (v, i) {
						securityActivityId += v.securityActivityId + ','
					})
					layer.confirm('确定删除该条数据吗？', function (index) {
						$.post('/secirityActivity/del', {ids: securityActivityId}, function (res) {
							if (res.code=='0') {
				...
				case 'submit':
					...
					layer.open({
						...
						success: function () {
							$.get('/plbFlowSetting/getOwnFlowData', {plbDictNo: '64'}, function (res) {
								var flowData = []
						...
						yes: function (index) {
							...
							if (checkStatus.data.length > 0) {
								...
								newWorkFlow(flowData.flowId, function (res) {
									var submitData = {
										securityActivityId:approvalData.securityActivityId,
