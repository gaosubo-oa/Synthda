...
<script type="text/javascript">
	...
	$.ajax({
		url: '/workflow/secirityManager/getCheckByItemId?planItemId='+planItemId,
		type: 'get',
		dataType: 'json',
		async:false,
	...
	layui.use(['table','layer','form','element','eleTree','upload','laydate'], function() {
		...
		detailsInit2 = table.render({
			elem: '#formTable2'
			, page:true
			,url:' /workflow/secirityManager/getDetailInfoGroupTypeNotUser?checkliId='+securityInfoDate.checkliId+'&type=check'
        ...
        table.on('tool(formTable3)', function(obj){
            ...
            if(layEvent === 'check1'){//检查记录页面中的查看
                ...
                layer.open({
					...
					success: function () {
                        $.ajax({
                            url:'/workflow/secirityManager/getDetailsInfoById?detailsInfoId='+tDataa.securityContentId,
                            dataType:'json',
                            type:'get',
                            async: false,
                            success:function(res){
				...
				layer.open({
					...
					success: function () {
						var data = [];
						$.ajax({
							url:'/workflow/secirityManager/flowChart?securityContentId='+tDataa.securityContentId,
							type: 'post',
							dataType:'json',
							success:function(res){
								if(res.code===0||res.code==="0"){
		...
		table.on('tool(formTable2)', function(obj){
			...
			var layEvent = obj.event;
			console.log(tData);
			if(layEvent === 'check'){//安全记录详情页面中的查看
				layer.open({
					type: 1,
					skin: 'layui-layer-molv', //加上边框
					area: ['80%', '80%'], //宽高
					...
					success: function () {
						...
						detailsInit3 = table.render({
							...
							//,url:'/workflow/secirityManager/selectDetailInfo?checkliId='+tData.checkliId
						...
						$(document).on("click","[name='securityRegionName']",function (e) {
							...
							if(tData.projectId == null|| tData.projectId == undefined){
								...
								eleTree.render({
									elem: '.eleb4',
									url:'/workflow/secirityManager/getRegionByProject?projectId='+tData.projectId,
						...
						$('#searchCust1').click(function () {
							//debugger
							var serchObjUrl = "/workflow/secirityManager/serchDetailInfo?securityKnowledgeId="+tData.securityKnowledgeId+"&checkliId="+tData.checkliId;
