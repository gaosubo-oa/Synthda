...
<script>
	...
	var TableUIObj = new TableUI('plb_security_rectification');
	...
	layui.use(['form', 'laydate', 'table', 'element', 'soulTable', 'eleTree'], function () {
			...
			switch (obj.event) {
				...
				case 'notice':  //整改通知单
					if(checkStatus.length == 0){
						...
						// if(checkStatus.data[0].auditerStatus != 0){
							...
							$.ajax({
								url:'/generateDocx/generateDocxByType?rectificationId='+rectificationId+'&type=securityRecification',
				...
				case 'submit':
					...
					layer.open({
						...
						success: function () {
							$.get('/plbFlowSetting/getOwnFlowData', {plbDictNo: '58'}, function (res) {
								var flowData = []
						...
						yes: function (index) {
							...
							if (checkStatus.data.length > 0) {
								...
								newWorkFlow(flowData.flowId, function (res) {
									...
									$.ajax({
										url: '/workflow/rectification/updateById',
										data: submitData,
										dataType: 'json',
		...
		function newOrEdit(type, data) {
			var title = '';
			var url = '';
			var content ='/workflow/secirityManager/addRectification'
			var rectificationId = '';
			...
			// } else
				if (type == 2) {
				title = '执行整改';
				//url = "/workflow/rectification/updateById?rectificationFlag=1&securityContentId="+data.securityContentId+'&rectificationId='+data.rectificationId;
                url = "/workflow/rectification/updateById"
				projId = data.projId;
				securityInfoDate=data;
			}else if(type == '4'){
				title = '安全整改查看详情'
				content = '/workflow/secirityManager/addRectification?type=4'
				securityInfoDate=data;
			}
