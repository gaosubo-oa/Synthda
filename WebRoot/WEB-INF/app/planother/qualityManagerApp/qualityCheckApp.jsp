...
<script>
  ...
  function initSecurity(projectId,testTypeNo) {
     $.ajax({
         url:"/workflow/qualityManager/selectDetailInfoGroupByType",
             ...
             for (var i=0;i<res.data.length;i++){
                 for(var j=0;j<res.data[i].knowledgeValue.length;j++){
                     htmli+="<li style='list-style: decimal; list-style: decimal;border-bottom: 1px solid #C0C0C0;font-size: 15px;margin: 15px auto;position: relative;left: 14px;'>" +
                         "<a href='/workflow/qualityManager/securityManagerContent?securityContentId="+res.data[i].knowledgeValue[j].securityContentId+"&seciritytype="+seciritytype+"&projectId="+projectId+"&testTypeNo="+testTypeNo+"&datatype="+datatype+"'>"+res.data[i].knowledgeValue[j].securityDanger+"</a>"+
                     "</li>";
                     ...
                     htmlist+="<li>" +
                         ...
                         "<a href='/workflow/qualityManager/securityManagerContent?securityContentId="+res.data[i].knowledgeValue[j].securityContentId+"&seciritytype="+seciritytype+"&projectId="+projectId+"&testTypeNo="+testTypeNo+"&datatype="+datatype+"' style='font-size: 17px'>"+res.data[i].knowledgeValue[j].securityDanger+"("+text+")</a>"+
                         "<ul>"+photolist+"</ul>"+
                         "</div>"+
                         "</li>";
                     photolist="";
  ...
  function initRectification(projectId,testTypeNo) {
        $.ajax({
            url:'/workflow/qualityManager/getRecificaOrAcceptanceGroupByType?isRecification=true&recificationStatus=0,1,2,3',
            data:{projectId:projectId,testTypeNo:testTypeNo},
            datatype:"json",
            success:function(res){
                var htm='';
  ...
  function initAcceptance(projectId,testTypeNo) {
      $.ajax({
          url:'/workflow/qualityManager/getRecificaOrAcceptanceGroupByType?isRecification=false&recificationStatus=1,2,3',
          data:{projectId:projectId,testTypeNo:testTypeNo},
          datatype:"json",
          success:function(res){
              var htm='';
