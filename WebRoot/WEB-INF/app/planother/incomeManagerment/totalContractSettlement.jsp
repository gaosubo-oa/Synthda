        ...
        <div class="wrap_right">
            <div class="query_module layui-form layui-row" style="position: relative">
                <div class="layui-col-xs2">
                    <input type="text" name="mtlSettleupNo" placeholder="结算编号" autocomplete="off" class="layui-input">
                </div>
...
<script>
    ...
    // 表格显示顺序
    var colShowObj = {
        mtlSettleupNo: {field: 'mtlSettleupNo', title: '结算编号',minWidth:110,sort: true,hide: false},
        ...
        settlementDate: {
            field: 'settlementDate', title: '结算日期',minWidth:110,sort: true,hide: false,templet: function (d) {
                return format(d.settlementDate);
            }
        },
        settleupYear: {field: 'settleupYear', title: '结算年',minWidth:90,sort: true,hide: false},
        settleupQuarter: {field: 'settleupQuarter', title: '结算季',minWidth:90,sort: true,hide: false},
        settleupMonth: {field: 'settleupMonth', title: '结算月',minWidth:90,sort: true,hide: false},
        settlementMoney: {
            field: 'settlementMoney', title: '本次结算金额', minWidth:140,sort: true,hide: false,templet: function (d) {
                ...
                case 'add':
                    ...
                    $tr.each(function () {
                        var oldDataObj = {
                            ...
                            sumSettlementQuantitiesPrice: $(this).find('input[name="sumSettlementQuantitiesPrice"]').val(),
                            onwaySettlementQuantities1: $(this).find('input[name="onwaySettlementQuantities1"]').val(),
                            onwaySettlementQuantitiesPrice: $(this).find('input[name="onwaySettlementQuantitiesPrice"]').val(),
                ...
                case 'add':
                    ...
                    layer.open({
                        ...
                        success: function () {
                            ...
                            table.on('radio(mtlStockInTable)', function (obj) {
                        ...
                        yes: function (index) {
                            var checkStatus = table.checkStatus('mtlStockInListTable');
                            ...
                            if (checkStatus.data.length > 0) {
                                ...
                                checkStatus.data.forEach(function(item) {
                                    var newDataObj = {
                                        ...
                                        cbsId: item.wbsId, // CBS id
                                        cbsName: item.cbsName, // CBS名称
                                        stockInQuantity: item.stockInQuantity, // 入库数量
                                        taxPrice: item.stockInPrice, // 含税单价
                                        noTaxMoney: item.noTaxMoney, // 不含税总价
                                        taxMoney: item.taxMoney, // 含税总价
                                        noTaxPeice: item.noTaxPeice, // 不含税单价
                                    ...
                                    if (BigNumber(newDataObj.noTaxPeice).lt(0)) {
                                        newDataObj.noTaxPeice = 0;
                                    }
                                ...
                                $tr.each(function () {
                                    var oldDataObj = {
                                        ...
                                        mtlValuationUnit: $(this).find('.mtl_info_mtlValuationUnit').text(), // 单位
                                        ...
                                        noTaxPeice: $(this).find('.noTaxPeice').val(), // 不含税单价
                                ...
                                $('[name="settlementMoney"]', $('#baseForm')).val(settlementMoney);
        ...
        function newOrEdit(type, data) {
            ...
            layer.open({
                ...
                content: ['<div class="layer_wrap _disabled"><div class="layui-collapse">',
                    ...
                    '<label class="layui-form-label form_label">总包结算编号<span field="mtlSettleupNo" class="field_required">*</span><a title="刷新编号" class="refresh_no_btn"><i class="layui-icon layui-icon-refresh"></i></a></label>' +
                    ...
                    /* endregion */
                    /* region 总包合同结算明细 */
                    '<div class="layui-colla-item"><h2 class="layui-colla-title">总包合同结算明细</h2>' +
                ...
                success: function () {
                    ...
                    var cols=[
                        ...
                        {
                            field: 'sumSettlementQuantitiesPrice', title: '累计已结算金额',minWidth:100,
                            templet: function (d) {
                                return '<input type="text"   name="sumSettlementQuantitiesPrice" class="layui-input " style="height: 100%;" value="' + (d.sumSettlementQuantitiesPrice || '')+ '">'
                            }
                        },
                        {
                            field: 'onwaySettlementQuantities1', title: '在途结算工程量',minWidth:100,
                            templet: function (d) {
                                return '<input type="text"  name="onwaySettlementQuantities1" class="layui-input " style="height: 100%;" value="' + (d.onwaySettlementQuantities1 || '')+ '">'
                            }
                        },
                        {
                            field: 'onwaySettlementQuantitiesPrice', title: '在途结算金额',minWidth:100,
                            templet: function (d) {
                                return '<input type="text"   name="onwaySettlementQuantitiesPrice" class="layui-input " style="height: 100%;" value="' + (d.onwaySettlementQuantitiesPrice || '')+ '">'
                            }
                        },
                        {
