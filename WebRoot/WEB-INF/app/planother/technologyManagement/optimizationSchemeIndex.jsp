...
<div class="container">
	...
	<div class="wrapper">
		<div class="wrap_left">
			<h2 style="text-align: center;line-height: 35px;">优化计划</h2>
			<div class="left_form">
	...
	layui.use(['form', 'laydate', 'table', 'element', 'eleTree', 'layer','soulTable'], function () {
		...
		var colShowObj = {
			documnetNo: {field: 'documnetNo', title: '单据号', minWidth: 90,sort: true, hide: false,templet: function (d) {
					return '<span optimizationId="'+d.optimizationId+'">'+(d.documnetNo||'')+'</span>'
				}},
			projectName:{field:'projectName',title:'项目名称',minWidth: 120,sort:true,hide:false},
			optimizationName: {field: 'optimizationName', title: '优化名称', minWidth: 120,sort: true, hide: false},
			schemeDesc: {field: 'schemeDesc', title: '优化描述', minWidth: 120,sort: true, hide: false},
			// mainSchemeContent: {field: 'mainSchemeContent', title: '主要内容', minWidth: 120,sort: true, hide: false},
			schemeCategory:{field:'schemeCategory',title:'优化类别',minWidth: 120,sort:true,hide:false,templet: function (d) {
					if(d.schemeCategory) {
			...
			schemeType:{field:'schemeType',title:'优化类型',minWidth: 120,sort:true,hide:false,templet: function (d) {
					if(d.schemeType) {
		...
		var dictionaryStr = 'OPTIMIZATION_CATEGORY,OPTIMIZATION_TYPE';
		...
		table.on('toolbar(tableDemo)', function (obj) {
			...
			switch (obj.event) {
				...
				case 'edit':
					...
					if($('#leftId').attr('projId')){
						var optimizationId = $('.table_box .layui-table-body .layui-form-checked').parents('tr').find('[data-field="documnetNo"] span').attr('optimizationId')
						var loadIndex = layer.load();
						$.get('/optimizationScheme/getById', {kayId:optimizationId}, function (res) {
				...
				case 'del':
					...
					$trs.each(function(){
						optimizationId += $(this).attr('optimizationId') + ','
					})
					// checkStatus.data.forEach(function (v, i) {
					// 	optimizationId += v.optimizationId + ','
					// })
					layer.confirm('确定删除该条数据吗？', function (index) {
						$.post('/optimizationScheme/del', {ids: optimizationId}, function (res) {
							if (res.code=='0') {
						...
						yes: function (index) {
							...
							if (checkStatus.data.length > 0) {
								...
								newWorkFlow(flowData.flowId, function (res) {
									var submitData = {
										optimizationId:approvalData.optimizationId,
		...
		table.on('tool(tableDemo)', function (obj) {
			...
			var optimizationId = $(obj.tr.selector).find('[data-field="documnetNo"] span').attr('optimizationId')
			if(layEvent === 'detail'){
				var loadIndex = layer.load();
				$.get('/optimizationScheme/getById', {kayId:optimizationId}, function (res) {
					layer.close(loadIndex);
