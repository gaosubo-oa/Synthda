		...
		<div class="wrap_right">
			<div class="query_module layui-form layui-row" style="position: relative">
				...
				<div class="layui-col-xs2" style="margin-left: 15px;">
					<input type="text" name="technicalName" placeholder="技术方案名称" autocomplete="off" class="layui-input">
				</div>
				<div class="layui-col-xs2" style="margin-left: 15px;">
	...
	layui.use(['form', 'laydate', 'table', 'element', 'eleTree', 'layer','soulTable'], function () {
		...
		var colShowObj = {
			documnetNo: {field: 'documnetNo', title: '单据号', minWidth: 90,sort: true, hide: false,templet: function (d) {
					return '<span technologyId="'+d.technologyId+'">'+(d.documnetNo||'')+'</span>'
				}},
			projectName:{field:'projectName',title:'项目名称',minWidth: 120,sort:true,hide:false},
			technicalName: {field: 'technicalName', title: '技术方案名称', minWidth: 120,sort: true, hide: false},
		...
		var TableUIObj = new TableUI('plb_other_technology_scheme');
		...
		table.on('toolbar(tableDemo)', function (obj) {
			...
			switch (obj.event) {
				...
				case 'edit':
					...
					if($('#leftId').attr('projId')){
						var technologyId = $('.table_box .layui-table-body .layui-form-checked').parents('tr').find('[data-field="documnetNo"] span').attr('technologyId')
				...
				case 'del':
					...
					$trs.each(function(){
						technologyId += $(this).attr('technologyId') + ','
					})
					// checkStatus.data.forEach(function (v, i) {
					// 	technologyId += v.technologyId + ','
					// })
					layer.confirm('确定删除该条数据吗？', function (index) {
						$.post('/technologyScheme/del', {ids: technologyId}, function (res) {
							if (res.code=='0') {
		...
		table.on('tool(tableDemo)', function (obj) {
			...
			var technologyId = $(obj.tr.selector).find('[data-field="documnetNo"] span').attr('technologyId')
			if(layEvent === 'detail'){
				var loadIndex = layer.load();
				$.get('/technologyScheme/getById', {kayId:technologyId}, function (res) {
					layer.close(loadIndex);
					...
					layer.open({
						...
						success: function () {
							var tableDemoIn=table.render({
								elem: '#tableDemoIn',
								url: '/workPlanning/select?delFlag=0&projectId='+$('#leftId').attr('projId'),
								cols: [[
