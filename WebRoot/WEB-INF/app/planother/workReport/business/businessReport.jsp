...
<div class="container">
	...
	<div class="wrapper">
		<div class="wrap_left">
			<h2 style="text-align: center;line-height: 35px;">经营分析</h2>
			<div class="left_form">
		...
		<div class="wrap_right">
			<div class="query_module layui-form layui-row" style="position: relative">
				...
				<%--<div class="layui-col-xs2" style="margin-left: 15px;">
					<input type="text" name="technicalName" placeholder="经营分析名称" autocomplete="off" class="layui-input">
				</div>
				<div class="layui-col-xs2" style="margin-left: 15px;">
	...
	layui.use(['form', 'laydate', 'table', 'element', 'eleTree', 'layer','soulTable'], function () {
		...
		var colShowObj = {
			documentNo: {field: 'documentNo', title: '单据号', minWidth: 90,sort: true, hide: false,templet: function (d) {
					return '<span businessReportId="'+d.businessReportId+'">'+(d.documentNo||'')+'</span>'
				}},
		...
		table.on('toolbar(tableDemo)', function (obj) {
			...
			switch (obj.event) {
				...
				case 'edit':
					...
					if($('#leftId').attr('projId')){
						var businessReportId = $('.table_box .layui-table-body .layui-form-checked').parents('tr').find('[data-field="documentNo"] span').attr('businessReportId')
						var loadIndex = layer.load();
						$.get('/businessReport/getById', {kayId:businessReportId}, function (res) {
				...
				case 'del':
					...
					var businessReportId = ''
					var $trs = $('.table_box .layui-table-body .layui-form-checked').parents('tr').find('[data-field="documentNo"] span')
					$trs.each(function(){
						businessReportId += $(this).attr('businessReportId') + ','
					})
					// checkStatus.data.forEach(function (v, i) {
					// 	businessReportId += v.businessReportId + ','
					// })
					layer.confirm('确定删除该条数据吗？', function (index) {
						$.post('/businessReport/del', {ids: businessReportId}, function (res) {
							if (res.code=='0') {
						...
						yes: function (index) {
							...
							if (checkStatus.data.length > 0) {
								...
								newWorkFlow(flowData.flowId, function (res) {
									var submitData = {
										businessReportId:approvalData.businessReportId,
		...
		table.on('tool(tableDemo)', function (obj) {
			...
			var businessReportId = $(obj.tr.selector).find('[data-field="documentNo"] span').attr('businessReportId')
			if(layEvent === 'detail'){
				var loadIndex = layer.load();
				$.get('/businessReport/getById', {kayId:businessReportId}, function (res) {
					layer.close(loadIndex);
		...
		function newOrEdit(type, data) {
			...
			layer.open({
				...
				content: ['<div class="layui-collapse _disabled">\n' +
					...
					'                   <div class="layui-form-item">\n' +
					'                       <label class="layui-form-label form_label">经营分析说明<span field="businessMemo" class="field_required">*</span></label>\n' +
					'                       <div class="layui-input-block form_block">\n' +
					'                           <input type="text" name="businessMemo"  autocomplete="off" class="layui-input">' +
					'                       </div>\n' +
					'                   </div>' +
					'               </div>' +
					'			</div>',
					...
					/* endregion */
					'  <div class="layui-colla-item">\n' +
					'    <h2 class="layui-colla-title">下月经营立项计划</h2>\n' +
				...
				success: function () {
					...
					var cols = [
						...
						{
							field: 'manageProjName', title: '经营立项名称', minWidth: 150, templet: function (d) {
								return '<input businessReportId="' + (d.businessReportId || '') + '" businessReportIdDetailId="'+(d.businessReportIdDetailId || '')+'" optimizationKnowId="'+(d.optimizationKnowId || '')+'" placeholder="请选择" readonly type="text" name="manageProjName" class="layui-input" style="height: 100%;" value="' + (d.manageProjName || '') + '">'
							}
						},
						{
							field: 'projectDate', title: '经营立项日期', minWidth: 160, event: 'dateSelection',templet: function (d) {
								return '<input type="text" name="projectDate" class="layui-input" style="height: 100%;" value="' + (d.projectDate || '') + '">'
							}
						},
						{
							field: 'changeIncome', title: '预计收入', minWidth: 160, templet: function (d) {
								return '<input type="number" name="changeIncome" class="layui-input" style="height: 100%;" value="' + (d.changeIncome || '') + '">'
							}
						},
						{
							field: 'implementationCost', title: '实施成本', minWidth: 160, templet: function (d) {
								return '<input type="number" name="implementationCost" class="layui-input" style="height: 100%;" value="' + (d.implementationCost || '') + '">'
							}
						},
						{
							field: 'estimatedProfit', title: '预计利润', minWidth: 160, templet: function (d) {
								return '<input type="text" name="estimatedProfit" class="layui-input" style="height: 100%;" value="' + (d.estimatedProfit || '') + '">'
							}
						},
						{
							field: 'estimatedProfitMargin', title: '预计利润率', minWidth: 160, templet: function (d) {
								return '<input type="text" name="estimatedProfitMargin" class="layui-input" style="height: 100%;" value="' + (d.estimatedProfitMargin || '') + '">'
							}
						}
					]
								...
								$.ajax({
									...
									success: function (res) {
										if(res&&res.data&&res.data.length>0){
											...
											_businessReportId = res.data[0].businessReportId

											layer.msg('本月已填报，请直接编辑！', {icon: 0, time: 2000});

											var loadIndex = layer.load();
											$.get('/businessReport/getById', {kayId:_businessReportId}, function (res) {
												var data = res.obj
											...
											// 获取自动编号
											getAutoNumber({autoNumberType: 'reportBusiness'}, function(res) {
				...
				yes: function (index) {
					if(type!='4'){
						...
						if(type == '1'||_businessReportId){
							obj.businessReportId= data.businessReportId||_businessReportId;
							url = '/businessReport/updateById';
						}
						...
						obj.detailsList = planningDetailsData().scheduleData;
		...
		$(document).on('click', '[name="manageProjName"]', function () {
			var _this = this
			layer.open({
				type: 1,
				title: '选择',
		...
		table.on('tool(detailedTable)', function (obj) {
			...
			if (layEvent === 'del') {
				...
				var $tr = $('.project_detailed_information').find($tr.selector).find('input[name="projectDate"]');
		...
		function planningDetailsData() {
			...
			$trs.each(function () {
				var dataObj = {
					businessReportId: $(this).find('input[name="manageProjName"]').attr('businessReportId') || '',
					businessReportIdDetailId: $(this).find('input[name="manageProjName"]').attr('businessReportIdDetailId') || '',
					optimizationKnowId: $(this).find('input[name="manageProjName"]').attr('optimizationKnowId') || '',
					manageProjName: $(this).find('input[name="manageProjName"]').val(),
					projectDate: $(this).find('input[name="projectDate"]').val(),
					changeIncome: $(this).find('input[name="changeIncome"]').val(),
					implementationCost: $(this).find('input[name="implementationCost"]').val(),
					estimatedProfit: $(this).find('input[name="estimatedProfit"]').val(),
					estimatedProfitMargin: $(this).find('input[name="estimatedProfitMargin"]').val()
				}
