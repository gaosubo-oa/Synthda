...
<div class="container">
	...
	<div class="wrapper">
		<div class="wrap_left">
			<h2 style="text-align: center;line-height: 35px;">项目进度情况 </h2>
			<div class="left_form">
...
<script type="text/html" id="internalBar">
	<a href="javascript:;" class="openFile addRow" style="position:relative;" lay-event="butfile">
		...
		<input type="file" multiple id={{"fileupload"+d.LAY_INDEX}} data-url="/upload?module=scheduleReport"  name="file">
	</a>
</script>
	...
	layui.use(['form', 'laydate', 'table', 'element', 'eleTree', 'layer','soulTable'], function () {
		...
		var colShowObj = {
			documentNo: {field: 'documentNo', title: '单据号', minWidth: 90,sort: true, hide: false,templet: function (d) {
					return '<span scheduleReportId="'+d.scheduleReportId+'">'+(d.documentNo||'')+'</span>'
				}},
			...
			memo: {field: 'memo', title: '月度进度情况说明', minWidth: 120,sort: true, hide: false},
			sheduleChangeMemo: {field: 'sheduleChangeMemo', title: '进度修编情况说明', minWidth: 120,sort: true, hide: false},
		...
		table.on('toolbar(tableDemo)', function (obj) {
			...
			switch (obj.event) {
				...
				case 'edit':
					...
					if($('#leftId').attr('projId')){
						var scheduleReportId = $('.table_box .layui-table-body .layui-form-checked').parents('tr').find('[data-field="documentNo"] span').attr('scheduleReportId')
						var loadIndex = layer.load();
						$.get('/scheduleReport/getById', {kayId:scheduleReportId}, function (res) {
				...
				case 'del':
					...
					var scheduleReportId = ''
					var $trs = $('.table_box .layui-table-body .layui-form-checked').parents('tr').find('[data-field="documentNo"] span')
					$trs.each(function(){
						scheduleReportId += $(this).attr('scheduleReportId') + ','
					})
					// checkStatus.data.forEach(function (v, i) {
					// 	scheduleReportId += v.scheduleReportId + ','
					// })
					layer.confirm('确定删除该条数据吗？', function (index) {
						$.post('/scheduleReport/del', {ids: scheduleReportId}, function (res) {
							if (res.code=='0') {
						...
						yes: function (index) {
							...
							if (checkStatus.data.length > 0) {
								...
								newWorkFlow(flowData.flowId, function (res) {
									var submitData = {
										scheduleReportId:approvalData.scheduleReportId,
		...
		table.on('tool(tableDemo)', function (obj) {
			...
			var scheduleReportId = $(obj.tr.selector).find('[data-field="documentNo"] span').attr('scheduleReportId')
			if(layEvent === 'detail'){
				var loadIndex = layer.load();
				$.get('/scheduleReport/getById', {kayId:scheduleReportId}, function (res) {
					layer.close(loadIndex);
		...
		if(week){
			_html = '           <div class="layui-row">' +
					'               <div class="layui-col-xs3 layui-col2" style="padding: 0 5px;">' +
					'                   <div class="layui-form-item">\n' +
					'                       <label class="layui-form-label form_label">单据号<span field="documentNo" class="field_required">*</span><a title="刷新单据号" class="refresh_no_btn"><i class="layui-icon layui-icon-refresh"></i></a></label>\n' +
					'                       <div class="layui-input-block form_block">\n' +
					'                       <input type="text" name="documentNo" readonly autocomplete="off" style="background: #e7e7e7" class="layui-input">\n' +
		...
		function newOrEdit(type, data) {
			...
			layer.open({
				...
				'    <div class="layui-colla-content layui-show plan_base_info">' +_html+
				/* region 第一行 */
					'  			<div class="layui-row">' +
					'  			    <div class="layui-col-xs6" style="padding: 0 5px;">' +
					'  			        <div class="layui-form-item">\n' +
					'  			            <label class="layui-form-label form_label _one">月度进度情况说明<span field="memo" class="field_required">*</span></label>\n' +
					'  			            <div class="layui-input-block form_block">\n' +
					...
					'  			    <div class="layui-col-xs6" style="padding: 0 5px;">' +
					'  			        <div class="layui-form-item">\n' +
					'  			            <label class="layui-form-label form_label">进度修编情况说明<span field="sheduleChangeMemo" class="field_required">*</span></label>\n' +
					'  			            <div class="layui-input-block form_block">\n' +
					'  			 				<textarea type="text" name="sheduleChangeMemo" style="resize: vertical;min-height: 80px" autocomplete="off" class="layui-input"></textarea>' +
					'  			            </div>\n' +
					'  			        </div>' +
					'  			    </div>' +
					'  			</div>',
				...
				success: function () {
					...
					var cols = [
						...
						{title: '上传现场照片', align: 'center', toolbar: '#internalBar', minWidth: 200},
						{
							field: 'filePosition', title: '图片说明', minWidth: 200, templet: function (d) {
								return '<input type="text" name="filePosition" scheduleReportId="' + (d.scheduleReportId || '') + '" scheduleReportDetailsId="'+(d.scheduleReportDetailsId || '')+'" class="layui-input tips" style="height: 100%;" value="' + (d.filePosition || '') + '">'
							}
						},
						{
							field: 'memo', title: '备注', minWidth: 200, templet: function (d) {
								return '<input type="text" name="_memo" class="layui-input tips" style="height: 100%;" value="' + (d.memo || '') + '">'
							}
						}
					]
								...
								$.ajax({
									...
									success: function (res) {
										if(res&&res.data&&res.data.length>0){
											...
											_scheduleReportId = res.data[0].scheduleReportId

											layer.msg('本月已填报，请直接编辑！', {icon: 0, time: 2000});

											var loadIndex = layer.load();
											$.get('/scheduleReport/getById', {kayId:_scheduleReportId}, function (res) {
												var data = res.obj
											...
											// 获取自动编号
											getAutoNumber({autoNumberType: 'scheduleReport'}, function(res) {
							...
							$.ajax({
								...
								success: function (res) {
									if(res&&res.data&&res.data.length>0){
										...
										var _reportWeek = $('#reportWeek').val()
										$("#baseForm")[0].reset()
										$('#fileContent').empty()
										$('#reportWeek').val(_reportWeek)
										// 获取自动编号
										getAutoNumber({autoNumberType: 'scheduleReport'}, function(res) {
											$('input[name="documentNo"]', $('#baseForm')).val(res.obj);
											$('input[name="createUserName"]', $('#baseForm')).val(res.object.createUserName).attr('createUser',res.obj.createUser);
					...
					if(type!=4){
						cols.push({align: 'center', toolbar: '#barDemo', title: '操作', minWidth: 80})
					}
				...
				yes: function (index) {
					if(type!='4'){
						...
						if(type == '1'||_scheduleReportId){
							obj.scheduleReportId= data.scheduleReportId||_scheduleReportId;
							url = '/scheduleReport/updateById'
						}
						...
						obj.detailsList = inspectionContent(1).dataArr;
		...
		table.on('tool(securityTable)', function (obj) {
			...
			if (layEvent === 'del') {
				obj.del();
				if (data.scheduleReportDetailsId) {
					$.get('/scheduleReport/delDetails', {ids: data.scheduleReportDetailsId}, function (res) {
		...
		function inspectionContent(type){
			...
			$trs.each(function (index) {
				...
				var dataObj = {
					scheduleReportId: $(this).find('[name="filePosition"]').attr('scheduleReportId') || '',
					scheduleReportDetailsId: $(this).find('[name="filePosition"]').attr('scheduleReportDetailsId') || '',
					filePosition: $(this).find('[name="filePosition"]').val(),
					memo: $(this).find('[name="_memo"]').val(),
