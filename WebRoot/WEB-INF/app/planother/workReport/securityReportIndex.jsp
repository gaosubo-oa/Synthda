...
<div class="container">
	...
	<div class="wrapper">
		<div class="wrap_left">
			<h2 style="text-align: center;line-height: 35px;">安全月报 </h2>
			<div class="left_form">
...
<script>
	...
	layui.use(['form', 'laydate', 'table', 'element', 'eleTree', 'layer','soulTable'], function () {
		...
		var colShowObj = {
			...
			memo: {field: 'memo', title: '月度安全情况说明', minWidth: 120,sort: true, hide: false},
			...
			switch (obj.event) {
				...
				case 'edit':
					...
					if($('#leftId').attr('projId')){
						...
						$.get('/securityReport/getById', {kayId:securityReportId}, function (res) {
							newOrEdit(1,res.obj)
				...
				case 'del':
					...
					layer.confirm('确定删除该条数据吗？', function (index) {
						$.post('/securityReport/del', {ids: securityReportId}, function (res) {
							if (res.code=='0') {
		...
		function newOrEdit(type, data) {
			...
			layer.open({
				...
				content: ['<form class="layui-form _disabled" id="baseForm" lay-filter="formTest">' +
					...
					'  			        <div class="layui-form-item">\n' +
					'  			            <label class="layui-form-label form_label">月度安全情况说明<span field="memo" class="field_required">*</span></label>\n' +
					'  			            <div class="layui-input-block form_block">\n' +
					...
					/* endregion */
					'  <div class="layui-colla-item">\n' +
					'    <h2 class="layui-colla-title">劳务实名制情况</h2>\n' +
					...
					'                       <div class="layui-input-block form_block">\n' +
					'                       <input type="text" name="manageUserNum" autocomplete="off"  class="layui-input">\n' +
					'                       </div>\n' +
					'                   </div>' +
					'               </div>' +
					'               <div class="layui-col-xs3" style="padding: 0 5px;">' +
					...
					'                       <div class="layui-input-block form_block">\n' +
					'                           <input type="text" name="workerNum"  autocomplete="off" class="layui-input">' +
					'                       </div>\n' +
					'                   </div>' +
					'               </div>' +
					'               <div class="layui-col-xs3" style="padding: 0 5px;">' +
					...
					'                       <div class="layui-input-block form_block">\n' +
					'                           <input type="text" name="wagesAmount"  autocomplete="off" class="layui-input">' +
					'                       </div>\n' +
					'                   </div>' +
					'               </div>' +
					'               <div class="layui-col-xs3" style="padding: 0 5px;">' +
					...
					'                       <div class="layui-input-block form_block">\n' +
					'                           <input type="text" name="insuredNum" autocomplete="off" class="layui-input">' +
					'                       </div>\n' +
					'                   </div>' +
					'               </div>' +
					'           </div>' ,
					...
					laydate.render({
						...
						, done: function(value){
							if(value&&$('#createMonth').val()){
								$.ajax({
									...
									success: function (res) {
										if(res&&res.data&&res.data.length>0){
											...
											$('#fileContent').empty()
											//附件
											if (res.data[0].attachmentList && res.data[0].attachmentList.length > 0) {
												var fileArr = res.data[0].attachmentList;
								...
								/*$.get('/securityCycleAnalysis/getSecurityPlan',
										{
											year:value,
							...
							$.ajax({
								...
								success: function (res) {
									if(res&&res.data&&res.data.length>0){
										...
										if (res.data[0].attachmentList2 && res.data[0].attachmentList2.length > 0) {
											var fileArr = res.data[0].attachmentList2;
					...
					layer.open({
						...
						success: function () {
							var tableDemoIn=table.render({
								elem: '#tableDemoIn',
								url: '/securityPlan/select?delFlag=0&projectId='+$('#leftId').attr('projId'),
								cols: [[
		...
		// 			$.get('/securityReport/delDetails', {ids: data.securityReportListId}, function (res) {
		// 				if (res.flag) {
