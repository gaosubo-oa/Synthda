...
<div class="container">
	...
	<div class="wrapper">
		<div class="wrap_left">
			<h2 style="text-align: center;line-height: 35px;">质量情况</h2>
			<div class="left_form">
				...
				<div class="table_box layui-form" style="display: none">
					<div class="layui-row month">
						<div class="layui-col-xs12" style="padding: 0 5px;">
							<h3 style="line-height: 50px;">月度质量情况说明</h3>
...
<script>
	...
	layui.use(['form', 'laydate', 'table', 'element', 'eleTree', 'layer','soulTable'], function () {
		...
		if(showProj){
			...
			$.get('/qualityCycleAnalysis/select', para_ms, function (res) {
		...
		eleTree.on("nodeClick(leftTree)", function (d) {
			...
			if (currentData.projId) {
				...
				$('#h2_title').text(currentData.projName+'质量情况综合分析')
				$('.no_data').hide();
				$('.table_box').show();
		...
		function tableShow(index) {
			...
			if(showProj){
				$('#h2_title').text((objData.data&&objData.data.projectName||'')+'质量情况综合分析')
			}
			...
			}else if(index==1)//项目检查整改执行情况
			{
				...
				if(!objData.docDemoTabBriefData){
					$.ajax({
						url:'/qualityAnalysis/performanceAnalysis',
						type: 'post',
						data:params,
						dataType: 'json',
						async:false,
				...
				if(!objData.docDemoTabBriefData2){
					...
					//质量整改
					$.ajax({
						url:'/qualityAnalysis/rectificationAnalysis',
						type: 'post',
						data:params,
						dataType: 'json',
						async:false,
				...
				table.render({
					elem: '#tableDemo5',
					url: '/workflow/qualityPunishment/analySis',
					toolbar: '#navbar',
					where:params,
					cols:  [[ //表头
		...
		function securityDangerAccount(params,_index){
			...
			$.get('/qualityAnalysis/checkRecificaData',params,function(res){
