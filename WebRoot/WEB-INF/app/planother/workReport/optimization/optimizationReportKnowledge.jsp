	...
	layui.use(['form','table','layer'], function () {
			...
			switch (obj.event) {
				...
				case 'del':
					...
					layer.confirm('确定删除该条数据吗？', function (index) {
						$.post('/optimizationKnowledge/del', {ids: optimizationKnowId}, function (res) {
							if (res.code == '0') {
		...
		function tableShow() {
			...
			tableIns = table.render({
				elem: '#tableDemo',
				url: '/optimizationKnowledge/select',
				toolbar: '#toolbarDemo',
				cols: [[
					{checkbox: true},
					{field: 'documnetNo', title: '单据号', minWidth: 90, sort: true, hide: false},
					{field: 'optimizationKnowName', title: '优化名称', minWidth: 120, sort: true, hide: false}
				]],
		...
		function newOrEdit(type, data) {
			...
			if (type == '0') {
				title = '新建';
				url = '/optimizationKnowledge/insert';
			} else if (type == '1') {
				title = '编辑';
				url = '/optimizationKnowledge/updateById';
			}
			layer.open({
				type: 1,
				title: title,
				...
				'               <div class="layui-col-xs12" style="padding: 0 5px;">' +
				'                   <div class="layui-form-item">\n' +
				'                       <label class="layui-form-label form_label">优化名称<span field="optimizationKnowName" class="field_required">*</span></label>\n' +
				'                       <div class="layui-input-block form_block">\n' +
				...
				success: function () {
					...
					if (type == 1) {
						...
						// 获取自动编号
						getAutoNumber({autoNumberType: 'optKnowledge'}, function (res) {
