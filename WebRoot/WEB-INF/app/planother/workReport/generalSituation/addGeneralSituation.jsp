...
<div class="container">
	...
	<div class="wrapper">
		<div class="wrap_left">
			<h2 style="text-align: center;line-height: 35px;">项目总体情况 </h2>
			<div class="left_form">
	...
	layui.use(['form', 'laydate', 'table', 'element', 'eleTree', 'layer','soulTable'], function () {
		...
		var colShowObj = {
			documentNo: {field: 'documentNo', title: '单据号', minWidth: 90,sort: true, hide: false,templet: function (d) {
					return '<span generalSituationId="'+d.generalSituationId+'">'+(d.documentNo||'')+'</span>'
				}},
		...
		if(week){
			colShowObj = {
				...
				projectName:{field:'projectName',title:'项目名称',minWidth: 120,sort:true,hide:false},
				reportWeek: {field: 'reportWeek', title: '填报周', minWidth: 120,sort: true, hide: false,templet: function (d) {
						if(d.reportWeek){
							var arr = d.reportWeek.split('-W')
							return arr[0]+'年第'+arr[1]+'周'
						}
						return ''
					}},
				createUserName: {field: 'createUserName', title: '填报人', minWidth: 120,sort: true, hide: false},
		...
		table.on('toolbar(tableDemo)', function (obj) {
			...
			switch (obj.event) {
				...
				case 'edit':
					...
					if($('#leftId').attr('projId')){
						var generalSituationId = $('.table_box .layui-table-body .layui-form-checked').parents('tr').find('[data-field="documentNo"] span').attr('generalSituationId')
						var loadIndex = layer.load();
						$.get('/generalSituation/getById', {kayId:generalSituationId}, function (res) {
				...
				case 'del':
					...
					var generalSituationId = ''
					var $trs = $('.table_box .layui-table-body .layui-form-checked').parents('tr').find('[data-field="documentNo"] span')
					$trs.each(function(){
						generalSituationId += $(this).attr('generalSituationId') + ','
					})
					// checkStatus.data.forEach(function (v, i) {
					// 	generalSituationId += v.generalSituationId + ','
					// })
					layer.confirm('确定删除该条数据吗？', function (index) {
						$.post('/generalSituation/del', {ids: generalSituationId}, function (res) {
							if (res.code=='0') {
						...
						yes: function (index) {
							...
							if (checkStatus.data.length > 0) {
								...
								newWorkFlow(flowData.flowId, function (res) {
									var submitData = {
										generalSituationId:approvalData.generalSituationId,
									...
									$.ajax({
										url: '/generalSituation/updateById',
		...
		table.on('tool(tableDemo)', function (obj) {
			...
			var generalSituationId = $(obj.tr.selector).find('[data-field="documentNo"] span').attr('generalSituationId')
			if(layEvent === 'detail'){
				var loadIndex = layer.load();
				$.get('/generalSituation/getById', {kayId:generalSituationId}, function (res) {
					layer.close(loadIndex);
		...
		if(week){
			_html = '           <div class="layui-row">' +
					...
					'               <div class="layui-col-xs3 layui-col" style="padding: 0 5px;">' +
					'                   <div class="layui-form-item">\n' +
					'                       <label class="layui-form-label form_label">填报周<span field="reportWeek" class="field_required">*</span></label>\n' +
					'                       <div class="layui-input-block form_block">\n' +
		...
		function newOrEdit(type, data) {
			...
			layer.open({
				...
				content: ['<form class="layui-form _disabled" id="baseForm" lay-filter="formTest">' +
					...
					'  			            <div class="layui-input-block form_block">\n' +
					'  			 				<textarea type="text" name="scheduleUpdateMemo" style="resize: vertical;min-height: 80px" autocomplete="off" class="layui-input"></textarea>' +
					'  			            </div>\n' +
					'  			        </div>' +
					'  			    </div>' +
					'  			</div>',
					...
					'  			            <div class="layui-input-block form_block">\n' +
					'  			 				<textarea type="text" name="nextMonthPlan" style="resize: vertical;min-height: 80px" autocomplete="off" class="layui-input"></textarea>' +
					'  			            </div>\n' +
					'  			        </div>' +
					'  			    </div>' +
					'  			</div>',
					...
					'  			            <div class="layui-input-block form_block">\n' +
					'  			 				<textarea type="text" name="otherMemo" style="resize: vertical;min-height: 80px" autocomplete="off" class="layui-input"></textarea>' +
					'  			            </div>\n' +
					'  			        </div>' +
					'  			    </div>' +
					'  			</div>',
				...
				success: function () {
					...
					var cols = [
						...
						{field: 'generalSituationName', title: '要素', width: 120},
						{
							field: 'controlledFlag', title: '是否可控', width: 120, templet: function (d) {
								if(d.controlledFlag=='1'){
									return '<input type="checkbox"  name="controlledFlag" controlledFlag="1" lay-skin="switch" lay-filter="switchTest" lay-text="是|否">'
								}else{
									return '<input type="checkbox" checked="" name="controlledFlag" controlledFlag="0" lay-skin="switch" lay-filter="switchTest" lay-text="是|否">'
								}
							}
						},
						{
							field: 'memo', title: '说明', minWidth: 300, templet: function (d) {
								return '<input type="text" name="memo" generalSituationId="' + (d.generalSituationId || '') + '" generalSituationDetailId="'+(d.generalSituationDetailId || '')+'" class="layui-input tips" style="height: 100%;" value="' + (d.memo || '') + '">'
							}
						}
					]
					var _data = [
						{generalSituationName:'进度情况'},
						{generalSituationName:'质量情况'},
						{generalSituationName:'安全情况'},
						{generalSituationName:'策划情况'},
						{generalSituationName:'技术情况'},
						{generalSituationName:'成本情况'},
						{generalSituationName:'经营情况'},
						{generalSituationName:'收发文情况'}
					]
								...
								$.ajax({
									...
									success: function (res) {
										if(res&&res.data&&res.data.length>0){
											...
											_generalSituationId = res.data[0].generalSituationId

											layer.msg('本月已填报，请直接编辑！', {icon: 0, time: 2000});

											var loadIndex = layer.load();
											$.get('/generalSituation/getById', {kayId:_generalSituationId}, function (res) {
												var data = res.obj
												...
												if (data.attachmentList2 && data.attachmentList2.length > 0) {
													...
													$('#fileContent2').append(echoAttachment(fileArr2));
												}
												//附件
												if (data.attachmentList3 && data.attachmentList3.length > 0) {
													var fileArr3 = data.attachmentList3;
													$('#fileContent3').append(echoAttachment(fileArr3));
												}

												table.render({
													elem: '#securityTable',
													data: data&&data.detailsList||_data,
													defaultToolbar: [''],
													limit: 1000,
													cols: [cols],
													done:function (res) {
											...
											// 获取自动编号
											getAutoNumber({autoNumberType: 'reportGeneral'}, function(res) {
											...
											$('[name="scheduleUpdateMemo"]').val('')
											$('[name="nextMonthPlan"]').val('')
											$('[name="otherMemo"]').val('')
											$('#fileContent').empty()
											$('#fileContent2').empty()
											$('#fileContent3').empty()
					...
					//填报周
					$(document).on('input propertychange', '[name="reportWeek"]', function () {
						if($(this).val()){
							$.ajax({
								url: '/generalSituation/select',
								type: 'post',
								...
								success: function (res) {
									if(res&&res.data&&res.data.length>0){
										...
										layer.msg('本周已填报，请直接编辑！', {icon: 0, time: 2000});
					...
					form.on('switch(switchTest)', function (data) {
						...
						if (data.elem.checked == true) {
							$(data.elem).parents('tr').find('[name="controlledFlag"]').attr('controlledFlag','0')
							form.render()
						} else {
							$(data.elem).parents('tr').find('[name="controlledFlag"]').attr('controlledFlag','1')
							form.render()
						}
					});
				...
				yes: function (index) {
					if(type!='4'){
						...
						if(type == '1'||_generalSituationId){
							obj.generalSituationId= data.generalSituationId||_generalSituationId;
							url = '/generalSituation/updateById';
						}
						...
						obj.attachmentId3 = attachmentId3;
						...
						$trs.each(function () {
							var dataObj = {
								generalSituationId: $(this).find('[name="memo"]').attr('generalSituationId') || '',
								generalSituationDetailId: $(this).find('[name="memo"]').attr('generalSituationDetailId') || '',
								generalSituationName: $(this).find('[data-field="generalSituationName"] div').text(),
								controlledFlag: $(this).find('[name="controlledFlag"]').attr('controlledFlag')||'',
								memo: $(this).find('[name="memo"]').val()||'',
							}
							dataArr.push(dataObj);
						});

						obj.detailsList = dataArr
