...
<div class="container">
	...
	<div class="wrapper">
		<div class="wrap_left">
			<h2 style="text-align: center;line-height: 35px;">总进度计划修编</h2>
			<div class="left_form">
...
<script>
	...
	if(isNew=='OneOld'){
        ...
        urlObj.updateRunId = '/revisionFormOldTwo/updateRunId'
		urlObj.plbDictNo = '81'
        ...
        urlObj.updateRunId = '/revisionFormTwo/updateRunId'
		urlObj.plbDictNo = '82'
    ...
    if(isNew=='OneOld'||isNew=='OneNew'){
        ...
        colShowObj = {
            ...
            beforeSchedule:{field:'beforeSchedule',title:'紧前任务',minWidth: 110,templet: function (d) {
	...
	layui.use(['form', 'laydate', 'table', 'element', 'eleTree', 'layer','treeTable'], function () {
		...
		// 监听排序事件
		treeTable.on('sort(tableIns)', function (obj) {
			var param = {
			...
			switch (obj.event) {
				...
				case 'submit':
					...
					layer.open({
						...
						success: function () {
							$.get('/plbFlowSetting/getOwnFlowData', {plbDictNo: urlObj.plbDictNo}, function (res) {
								var flowData = []
						...
						yes: function (index) {
							...
							if (checkStatus.data.length > 0) {
								...
								approvalData.forEach(function (item) {
									...
									scheduleNewIds += item.scheduleNewId+','
								})

								newWorkFlow(flowData.flowId, function (res) {
									var submitData = {
										scheduleNewIds:scheduleNewIds,
		...
		treeTable.on('row(tableIns)', function (obj) {
			...
			$.post(urlObj.getdata,{scheduleId:treedata.scheduleId}, function (res) {
				...
				if(isNew=='OneOld'||isNew=='OneNew'){
					//回显项目名称
					getProjName(' [name="projectName"]',$('#leftId').attr('projId')?$('#leftId').attr('projId'):data.projectId)
