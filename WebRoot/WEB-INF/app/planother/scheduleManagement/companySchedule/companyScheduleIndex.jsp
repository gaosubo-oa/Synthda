...
<head>
	...
	<style>
		...
		[data-field="revisionTime"] .ew-tree-table-cell-content,
		[data-field="revisionUserName"] .ew-tree-table-cell-content,
...
<div class="container">
	...
	<div class="wrapper">
		<div class="wrap_left">
			<h2 style="text-align: center;line-height: 35px;">总进度计划</h2>
			<div class="left_form">
...
<script>
	...
	// 表格显示顺序
	var colShowArr = [
		{field: 'documentNo', title: '编号', minWidth: 120,align:'center',templet: function (d) {
		...
		{field: 'nodeScheduleName', title: '节点计划',minWidth: 110,align:'center'},
		...
		{field: 'revisionTime', title: '修编时间',minWidth: 90,align:'center'},
		{field: ' revisionUserName', title: '修编人',minWidth: 90,align:'center'},
			...
			switch (obj.event) {
				...
				case 'edit':
					...
					if($('#leftId').attr('projId')){
						...
						$.get('/companySchedule/getById', {kayId:scheduleId}, function (res) {
				...
				case 'del':
					...
					var scheduleId = ''
					var $trs = $('.table_box .ew-tree-table-box tbody .layui-form-checked').parents('tr').find('.documentNo')
					$trs.each(function(){
				...
				case 'insert':
					...
					if($('#leftId').attr('projId')){
						//遍历表格获取每行数据
						var $trs = $('.table_box').find('tbody tr');
						var _this = null
						// var _flay = false
						$trs.each(function () {
							if($(this).find('[data-type="checkbox"] .layui-form-checked').length==1){
								_this = this
							}
						})
						choiceNode('insert',checkStatus[0].scheduleId,_this);
					}else{
						layer.msg('请选择左侧项目！', {icon: 0, time: 1500});
						return false;
					}
					...
					layer.open({
						...
						yes: function (index,layero) {
							...
							if(childData){
								if(childData.datas){
									$.ajax({
										url:'/companySchedule/importByTemplte?dataForm=2&projectId='+$('#leftId').attr('projId'),
										dataType:'json',
										type:'post',
				...
				case 'submit':
					...
					layer.open({
						...
						success: function () {
							$.get('/plbFlowSetting/getOwnFlowData', {plbDictNo: '78'}, function (res) {
								var flowData = []
