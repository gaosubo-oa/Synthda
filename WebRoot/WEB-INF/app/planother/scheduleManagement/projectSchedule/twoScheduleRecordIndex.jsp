...
<head>
	...
	<style>
		...
		[data-field="scheduleGrade"] .ew-tree-table-cell-content
		{
			/*min-width: 60px;*/
			/*width: 90px;*/
			padding: 0 !important;
...
<div class="container">
	...
	<div class="wrapper">
		<div class="wrap_left">
			<h2 style="text-align: center;line-height: 35px;">节点进度填报</h2>
			<div class="left_form">
...
<script>
	...
	var colShowArr = [
		...
		{field: 'scheduleName', title: '任务名称', minWidth: 150,align:'center'},
		{field: 'scheduleEndDate', title: '完成时间', minWidth: 90,align:'center'},
		{field: 'scheduleGrade', title: '节点等级',minWidth: 60,align:'center',templet: function (d) {
				if(d.scheduleGrade) {
		...
		{field: 'importanceLevel', title: '重要性',minWidth: 60,align:'center',templet: function (d) {
	...
	layui.use(['form', 'laydate', 'table', 'element', 'eleTree', 'layer','treeTable'], function () {
			...
			switch (obj.event) {
				...
				case 'edit2':
					...
					$trs.parents('tr').each(function (index) {
						var dataObj = {
							//: $(this).find('[name="recordBeginDate"]').val(),
		...
		function newOrEdit(type, data) {
			...
			layer.open({
				...
				'                       <div class="layui-input-block form_block">\n' +
				'                       <input type="text" name="scheduleEndDate" id="scheduleEndDate" style="background: #e7e7e7"   autocomplete="off" class="layui-input" >\n' +
				'                       </div>\n' +
				'                   </div>' +
				'               </div>' +
				'           </div>' ,
					...
					'                       <div class="layui-input-block form_block">\n' +
					'                       <input type="text" name="memo" autocomplete="off" style="background: #e7e7e7"  class="layui-input">\n' +
					'                       </div>\n' +
					'                   </div>' +
					'               </div>' +
					...
					'                       <div class="layui-input-block form_block">\n' +
					'                       <input type="text" name="createTime2" id="createTime2" disabled autocomplete="off" class="layui-input">\n' +
					'                       </div>\n' +
					'                   </div>' +
					'               </div>' +
                    '       </div>',
				...
				success: function () {
					...
					// 获取自动编号
					$.ajax({
						url:"/planningManage/autoNumber?autoNumberType=scheduleCompany",
						success:function(res){
							$('#createTime2').val(res.object.createTime);
						}
					})
					//节点等级
