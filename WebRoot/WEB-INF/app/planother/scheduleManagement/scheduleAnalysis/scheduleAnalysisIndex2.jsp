...
<head>
	...
	<script src="/js/planother/jsplumb.min.js?202201050930"></script>
			...
			<div class="query_module layui-form" style="position: relative">
				<div class="layui-row" >
					...
					<div class="layui-col-xs" style="margin-top: 3px;text-align: center">
						...
						<span style="float: left;line-height: 35px">计划时间</span>
					</div>
					<div class="layui-col-xs" style="margin-top: 3px;text-align: center">
					...
					<div class="layui-col-xs" style="margin-top: 3px;text-align: center">
						...
						<span style="float: left;line-height: 35px">未超期未完成</span>
					</div>
				...
				<div class="layui-row" style="margin: 5px 0;">
					<div class="layui-col-xs2" style="margin-left: 15px;">
						<div class="layui-form-item">
							<label class="layui-form-label">初始计划</label>
					...
					<div class="layui-col-xs2" style="margin-left: 15px;">
						<div class="layui-form-item">
							<label class="layui-form-label">最新计划</label>
							<div class="layui-input-block">
					...
					<div class="layui-col-xs2" style="margin-left: 15px;">
						<div class="layui-form-item">
							<label class="layui-form-label">刻度</label>
							<div class="layui-input-block">
								<input type="checkbox" name="scaleType" checked lay-filter="test3" lay-skin="switch" lay-text="日刻度|周刻度">
							</div>
						</div>
					</div>
					<div class="layui-col-xs2" style="margin-left: 15px;">
						<div class="layui-form-item">
							<label class="layui-form-label">一键修编</label>
							<div class="layui-input-block">
								<input type="checkbox" name="autoRevision" lay-filter="test4" lay-skin="switch" lay-text="是|否">
							</div>
						</div>
					</div>
				</div>
...
<script>
	...
	layui.use(['form', 'laydate', 'table', 'element', 'eleTree', 'layer','soulTable'], function () {
		...
		//表格显示顺序
		var colShowObj = {
			documentNo: {field: 'documentNo', title: '单据号', minWidth: 90},
			projectName:{field:'projectName',title:'项目名称',minWidth: 120},
			finedDept: {field: 'finedDept', title: '被罚单位', minWidth: 120},
			punishmentUser:{field:'punishmentUser',title:'被罚人',minWidth: 90},
			punishmentDate: {field: 'punishmentDate', title: '罚款日期',minWidth: 120},
			punishmentMoney: {field: 'punishmentMoney', title: '罚款金额',minWidth: 120},
			punishmentReason: {field: 'punishmentReason', title: '罚款原因',minWidth: 120},
			currFlowUserName: {field: 'currFlowUserName', title: '当前处理人',minWidth: 130}
		}
		...
		var TableUIObj = new TableUI('analysisData');
		...
		function tableShow(projId) {
			...
			$.ajax({
				...
				success:function(res){
					year2Header = res.obj.headerData.year2Header
					header = res.obj.headerData.dayHeader
					threeHeader = res.obj.headerData.threeHeader


					$('#duration').find('tr').empty()
                ...
                success:function(res){
					...
					for(var i=0;i<headerData.length;i++){
						...
						for(var j=0;j<dataObj.length;j++){
							for(var k=0;k<dataObj[j]['data'].length;k++){
								...
								if(dataObj[j]['data'][k]['partyMoney']==1){
									...
									// var dataId = ''
									// if(dataId=='127-2021-11-12'||dataId=='128-2021-11-14'){
									// 	a1[k].push('<div style="position: absolute;top: '+300*addd+'px;left: '+300*addd+'px;width: auto;height: 2px;background-color:'+dataObj[j].color+';height:'+dataObj[j].crude+'px;margin: '+5*j+'px -16px;padding-right: 10px" id="item_left'+addd+'"></div>');
									// 	addd++;
									// }else {
										a1[k].push('<div style="position: absolute;height: 2px;background-color:'+dataObj[j].color+';height:'+dataObj[j].crude+'px;margin: '+5*j+'px -1px;" id="'+dataId+'"></div>');
									// }
								}else {
									if(dataObj[j]['data'][k]['imgFlag']){
										a1[k].push('<img style="width: 100%" src="/img/planother/diamond-two.png">');
									}else {
										a1[k].push('<div class="transparent_class" style="position: absolute;height: 2px;background-color:'+dataObj[j].color+';height:'+dataObj[j].crude+'px;margin: '+5*j+'px -1px;"></div>');
									}
								}
							}
						}
						for(var n=0;n<a1.length;n++){
							var str2 = ''
							var _index = 0 //判断单元格是否全都没有线
							var _flay = false //判断有没有小图标
							for(var m=0;m<a1[n].length;m++){
								str2 += a1[n][m]
								if(a1[n][m].indexOf('transparent_class')>-1){
									_index++
								}
								if(a1[n][m].indexOf('img')>-1){
									_flay = true
									break
								}
							}
							if(_index==a1[n].length){
								str2 = ''
							}
							if(_flay){
								str2 = '<img style="width: 100%" src="/img/planother/diamond-two.png">'
							}
