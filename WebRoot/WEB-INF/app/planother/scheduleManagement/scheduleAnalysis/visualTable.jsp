...
<div class="container">
	...
	<div class="wrapper">
		<div class="wrap_left">
			<h2 id="_name" style="text-align: center;line-height: 35px;">进度可视化表格</h2>
			<div class="left_form">
...
<%--				<div class="layui-col-xs1" style="margin-left: 15px;">
					<div class="layui-form-item">
						<div class="layui-input-block form_block">
							<input type="text" name="scheduleName" id="scheduleName"  placeholder="工序"  autocomplete="off" class="layui-input">
						</div>
					</div>
				</div>
...
<script>
	...
	var dataFormStrHeader = getQueryString("dataFormStrHeader");

	var dataFormStr = getQueryString('dataFormStr');
	...
	layui.use(['form', 'laydate', 'table', 'element', 'eleTree', 'layer'], function () {
		...
		eleTree.on("nodeClick(leftTree)", function (d) {
			...
			if (currentData.projId) {
				...
				params.delFlag="0"
				params.dataFormStr=dataFormStrHeader

				$.ajax({
					url:'/scheduleAnalysis/analysisDHeader',
					type: 'post',
					data: params,
					dataType: 'json',
					// async:false,
					success:function(res){
						var dataObj = res.obj
						var _searchDom = ''
						var searchBtn = '<div class="layui-col-xs1" style="margin-top: 3px;text-align: center">\n' +
								'<button type="button" class="layui-btn layui-btn-sm searchData">查询</button>' +
								'</div>'
						...
						if(dataObj){
							dataObj.forEach(function(item){
								_searchDom += '<div class="layui-col-xs1" style="margin-left: 15px;">\n' +
										'<div class="layui-form-item">\n' +
										'<div class="layui-input-block form_block">\n' +
										'<input type="text" name="'+item.filed+'" id="'+item.filed+'"  placeholder="'+item.name+'"  autocomplete="off" class="layui-input search">\n' +
										'</div>\n' +
										'</div>\n' +
										'</div>'
							})
							_searchDom += searchBtn
							$('#searchDom').html(_searchDom)
							form.render()
						}
					}
				})
		...
		function tableShow() {
			...
			params.delFlag="0"
			params.dataFormStr=dataFormStr
			$.ajax({
