...
<script type="text/html" id="barOperation">
	...
	{{#  } }}
	{{#  if(d.recificationStatus != 1&&d.recificationStatus != 3){ }}
	<a class="layui-btn layui-btn-xs" lay-event="check" >质量整改</a>
	{{#  } }}
...
<script type="text/javascript">
	...
	layui.use(['table','layer','form','element','eleTree','upload','laydate'], function() {
        ...
        form.on('select(columnName1)',function (data) {
            if(data.value=='1'){
                ...
                table.reload("Settlement",{
                    url:'/workflow/qualityManager/getDetailInfo?isRecification=true&recificationFlag=0&recificationStatus=0'
                });
            }else if(data.value=='2'){
				recificationStatus_1 = '0';
                table.reload("Settlement",{
                    url:'/workflow/qualityManager/getDetailInfo?isRecification=true&recificationFlag=1&recificationStatus=0'
                });
            }else if(data.value=='3'){
				recificationStatus_1 = '1';
                table.reload("Settlement",{
                    url:'/workflow/qualityManager/getDetailInfo?isRecification=true&recificationStatus=1'
                });
            }else if(data.value=='4'){
                recificationStatus_1 = '2';
                table.reload("Settlement",{
                    url:'/workflow/qualityManager/getDetailInfo?isRecification=true&recificationFlag=1&recificationStatus=2'
                });
            }else if(data.value=='5'){
                recificationStatus_1 = '2';
                table.reload("Settlement",{
                    url:'/workflow/qualityManager/getDetailInfo?isRecification=true&recificationFlag=0&recificationStatus=2'
                });
            }else if(data.value=='6'){
				recificationStatus_1 = '0,1,2,3';
                table.reload("Settlement",{
                    url:'/workflow/qualityManager/getDetailInfo?isRecification=true&recificationFlag=0,1,3&recificationStatus=0,1,2,3'
                });
            }
        })
		...
		$('#searchCust').click(function () {
			...
			var searchtext = "";
			var serchObjUrl = "/workflow/qualityManager/getDetailInfo?isRecification=true&recificationStatus="+recificationStatus_1;
			//debugger
			if(serchSelect == "projectId"){
				searchtext = $("#projectId").val();
				if((searchtext==''&&serchSelect=='0') || (searchtext==''&&serchSelect==0)){ //两个都为空查询全部
					serchObjUrl = "/workflow/qualityManager/getDetailInfo?isRecification=true&recificationStatus="+recificationStatus_1;//重载表格
				}else if((searchtext!=""&&serchSelect=='0') || (searchtext!=""&&serchSelect==0)){ //只输入，没有选择
					layer.msg("请选择查询条件");
				}else{
					serchObjUrl = "/workflow/qualityManager/getDetailInfo?isRecification=true&recificationStatus="+recificationStatus_1+"&"+serchSelect+"="+searchtext;//重载表格
				}
		...
		SettlementTable = layui.table.render({
			elem: '#Settlement'
			// , data: []
			, url: '/workflow/qualityManager/getDetailInfo?isRecification=true&recificationStatus=0,1,2,3'//数据接口
				...
				case 'submit'://提交
					if (checkStatus.length == 0 ) {
						...
						if(isTrue){
							...
							$.ajax({
								url:'/workflow/qualityManager/commitAcceptance?ids='+rectificationIds+'&type=rectification',
								type: 'post',
								dataType:'json',
								success:function(res){
									if(res.code===0||res.code==="0"){
					...
					if(checkStatus.length == 0){
						...
						if(flag){
							layer.confirm('确定要重新生成整改通知单吗？', function(index){
								...
								$.ajax({
									type: "post",
									url: '/workflow/qualityManager/generateRectificaNotice?securityContentIds='+securityContentIds,
									dataType: "json",
									success:function (res) {
										if(res.code == "0" || res.code == 0){
											layer.msg(res.msg);
		...
		table.on('tool(SettlementFilter)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
			...
			if(layEvent === 'check'){ //质量整改
				...
				var index = layer.open({
					...
					maxmin: true,
					btn: btnArr,
					content: '/workflow/qualityManager/addRectification',
					success: function () {
					...
					yes: function (index, layero) {
						...
						if(dataa.recificationStatus === '0'||dataa.recificationStatus ==='2'){
							$.ajax({
								url:"/workflow/qualityManager/rectification?recificationFlag=1&securityContentId="+dataa.securityContentId+"&checkliId="+dataa.checkliId,
								dataType:"json",
								data:childData,
								type:"post",
								success:function(res){
					...
					,btn2:function(index,layero){
						var childData  = $(layero).find("iframe")[0].contentWindow.getDate();
						$.ajax({
							url:'/workflow/qualityManager/commitAcceptance?recificationFlag=1&type=rectification&acceptance=true&securityContentId='+dataa.securityContentId+"&checkliId="+dataa.checkliId,
							type: 'post',
							data:childData,
							dataType:'json',
							success:function(res){
