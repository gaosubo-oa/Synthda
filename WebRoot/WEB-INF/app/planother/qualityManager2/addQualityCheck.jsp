...
<script type="text/javascript">
	var securityInfoDate = parent.securityInfoDate;//质量检查
	...
	layui.use(['table','layer','form','element','eleTree','upload','layedit','laydate'], function() {
		...
		detailsInit2 = table.render({
			elem: '#formTable2'
			, page:true
			,url:'/workflow/qualityManager/selectDetailInfo?checkliId='+securityInfoDate.checkliId
					...
					layer.open({
						...
						success: function () {
							...
							$('#drawingImg').on("click", function () {
								$.popWindow("/workflow/qualityManager/selectDrawing?layEvent2=examineTest&securityContentId=" + checkDate.securityContentId);
							})
						...
						yes: function (index, layero) {
							...
							$.ajax({
								url: '/workflow/qualityManager/updateDetailsInfo',
								type: 'post',
								dataType: 'json',
								data: details2,
								success: function (res) {
			...
			if (layEvent === 'initialization') {
				...
				layer.open({
					...
					success: function () {
						...
						$('#drawingImg').on("click", function () {
							$.popWindow("/workflow/qualityManager/selectDrawing?layEvent1=initialization&securityContentId=" + tData.securityContentId);
						})
						form.on('switch(switchTest)', function (data) {
							if (data.elem.checked == true) {
								isShow = true;
				...
				$.popWindow("/workflow/qualityManager/selectDrawing?securityContentId=" + tData.securityContentId);
			}
		})
	})
