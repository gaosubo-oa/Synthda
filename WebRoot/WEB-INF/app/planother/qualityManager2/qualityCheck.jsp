...
<script type="text/html" id="barOperation">
	...
	{{#  } }}
	{{#  if(d.checkStatus == 0){ }}
	<a class="layui-btn layui-btn-xs" lay-event="check" >质量检查</a>
	{{#  } }}
</script>
...
<script type="text/javascript">
	...
	layui.use(['table','layer','form','element','eleTree','upload','layedit','laydate'], function() {
		...
		form.on('select(columnName1)',function (data) {
			if(data.value=='1'){
				table.reload("Settlement",{
					url:'/workflow/qualityManager/getQualityCheck?statuss=0&checkFlag=0'
				});
			}else if(data.value=='2'){
				table.reload("Settlement",{
					url:'/workflow/qualityManager/getQualityCheck?statuss=0&checkFlag=1'
				});
			}else if(data.value=='3'){
				table.reload("Settlement",{
					url:'/workflow/qualityManager/getQualityCheck?statuss=1&checkFlag=1'
				});
			}else if(data.value=='4'){
				table.reload("Settlement",{
					url:'/workflow/qualityManager/getQualityCheck'
				});
			}
		})
		//头工具栏事件
		...
		$('#searchCust').click(function () {
			...
			var searchtext = "";
			var searchtext1 = "";
			var serchObjUrl = "/workflow/qualityManager/getQualityCheck";
			//debugger
			if(serchSelect == "projectId"){
				searchtext = $("#projectId").val();
				if((searchtext==''&&serchSelect=='0') || (searchtext==''&&serchSelect==0)){ //两个都为空查询全部
					serchObjUrl = "/workflow/qualityManager/getQualityCheck";//重载表格
				}else if((searchtext!=""&&serchSelect=='0') || (searchtext!=""&&serchSelect==0)){ //只输入，没有选择
					layer.msg("请选择查询条件");
				}else{
					serchObjUrl = "/workflow/qualityManager/getQualityCheck?"+serchSelect+"="+searchtext;//重载表格
				}
				...
				serchObjUrl = "/workflow/qualityManager/getQualityCheck?";
		...
		SettlementTable = layui.table.render({
			elem: '#Settlement'
			// , data: []
			, url: '/workflow/qualityManager/getQualityCheck'//数据接口
				...
				case 'submit'://提交
					if (checkStatus.length == 0 ) {
						...
						if(isTrue){
							...
							$.ajax({
								url:'/workflow/qualityManager/commitChecklist?checkliIds='+checkliIds+'&status=1&checkFlag=1',
								type: 'post',
								dataType:'json',
								success:function(res){
							...
							for(var i=0;i<checkStatus.length;i++){
								$.ajax({
									type: "post",
									url: '/workflow/qualityManager/selectDetailInfo?checkliId='+checkStatus[i].checkliId,
									dataType: "json",
									success:function (res) {
										if(res.code===0){
											var cntentId="";
											...
											$.ajax({
												type: "post",
												url: '/workflow/qualityManager/generateRectificaNotice?securityContentIds='+cntentId,
												dataType: "json",
												success:function (res) {
		...
		table.on('tool(SettlementFilter)', function(obj){ //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
			...
			if(layEvent === 'check'){ //质量检查
				...
				layer.open({
					...
					maxmin: true,
					btn: btnArr,
					content: '/workflow/qualityManager/addQualityCheck',
					success: function () {
					...
					yes: function (index, layero) {
						...
						if(dataa.checkStatus==="0"){ //未提交
							$.ajax({
								url:'/workflow/qualityManager/commitChecklist?checkliIds='+dataa.checkliId+'&status=1&checkFlag=1',
								type: 'post',
								dataType:'json',
								//data:childData,
								success:function(res){
							...
							$.ajax({
								type: "post",
								url: '/workflow/qualityManager/selectDetailInfo?checkliId='+dataa.checkliId,
								dataType: "json",
								success:function (res) {
									if(res.code===0){
										var cntentId="";
					...
					btn2: function(index, layero){
						$.ajax({
							url:'/workflow/qualityManager/updatechecklist?checkliId='+dataa.checkliId+'&checkFlag=1',
							type: 'post',
							dataType:'json',
							//data:childData,
							success:function(res){
