			...
			<div class="search_module">
				<form class="layui-form clearfix search_form" lay-filter="searchForm">
					<div class="layui-row" style="padding: 5px 0;">
						...
						<div class="layui-form-item query_item layui-col-xs2" style="margin-left: -5%">
						...
						<div class="layui-form-item query_item layui-col-xs2" style="margin-left: -3%">
							<label class="layui-form-label">填报日期:</label>
							<div class="layui-input-block">
								<input class="layui-input" id="beginTime" autocomplete="off"  name="beginTime" style="height: 35px;width: 47%; margin-left: 6px;position: relative">
								<span style="display: block; position: absolute;top: 8px;left: 52%">-</span>
								<input class="layui-input" id="endTime" autocomplete="off"  name="endTime" style="height: 35px;width: 47%;position: absolute;left:57%;top: 0px;">
							</div>
						</div>
						<div class="layui-form-item query_item layui-col-xs2" style="margin-left: 5%">
							<label class="layui-form-label">部门:</label>
							<div class="layui-input-block" style="width: 50%">
								<input class="layui-input" id="department" autocomplete="off"  style="height: 35px;width: 100%; margin-left: 6px;position: relative">
							</div>
						</div>
						<div class="layui-form-item query_item layui-col-xs2" style="margin-left: 5%">
							<label class="layui-form-label">责任人:</label>
							<div class="layui-input-block" style="width: 50%">
								<input class="layui-input" id="person_liable" autocomplete="off"  style="height: 35px;width: 100%; margin-left: 6px;position: relative">
							</div>
						</div>
						...
						<div class="query_button_group query_item layui-col-xs1" style="margin-left: 1%;">
							...
							<button type="button" id="searchBtn" class="layui-btn layui-btn-sm" style="margin-left: 4%">查询</button>
...
<script>
	...
	//选人控件
	$(document).on('click', '#person_liable', function() {
		user_id = 'person_liable';
		if($('#department').attr('deptId')){
			var _deptId = $('#department').attr('deptId').replace(/,$/,'')
			$.popWindow('/plbCommonController/selectUserByDeptIndex?dept_type=0&deptId='+_deptId);
		}else {
	...
	function init() {
		...
		layui.use(['form', 'treeTable', 'xmSelect', 'table', 'soulTable','laydate'], function () {
			var form = layui.form,
			...
			var allYear = '';
			// 获取计划期间年度列表
			$.get('/workflow/workLog/getAllYear', function (res) {
				//debugger
				var nowYearNo = '';
				if (res.data.length > 0) {
					res.data.forEach(function (item) {
						allYear += '<option value="' + item+ '">' + item + '</option>';
						if (!nowYearNo && (nowDateObj.year == item)) {
							nowYearNo = item;
						}
					});
				}
			...
			$('#searchBtn').click(function () {
				...
				searchData.userId = $('#person_liable').attr('user_id')
				var deptIds = $('#department').attr('deptId');
				if(deptIds){
					if(deptIds.indexOf(",")!=-1){
						deptIds = deptIds.substring(0,deptIds.length-1);
					}
					searchData.deptId =deptIds;
				}


				searchData.allFlag = 'allFlag'

				// searchData.page = '1'

				// var serchObjUrl = "/workflow/workLog/getWorkPlan?allFlag=allFlag";
				/*if(searchData.year!=""&&searchData.year!=undefined){
					serchObjUrl+="&year="+searchData.year
				}
				if(searchData.month!=""&&searchData.month!=undefined){
					serchObjUrl+="&month="+searchData.month
				}
				if(searchData.beginTime!=""&&searchData.beginTime!=undefined){
					serchObjUrl+="&beginTime="+searchData.beginTime
				}
				if(searchData.endTime!=""&&searchData.endTime!=undefined){
					serchObjUrl+="&endTime="+searchData.endTime
				}
				if(searchData.userId!=""&&searchData.userId!=undefined){
					serchObjUrl+="&userId="+searchData.userId
				}*/
				table.reload("Settlement",{
					url: '/workflow/workLog/getWorkPlan',
					where:searchData,
					page:true
				});
			...
			SettlementTable = layui.table.render({
				elem: '#Settlement'
				// , data: []
				, url: '/workflow/workLog/getWorkPlan?allFlag=allFlag'//数据接口
				...
				, cols: [[ //表头
					...
					, {field: 'allNum', title: '全部工作数量'}
					, {field: 'successNum', title: '当日全部完成数量'}
					, {field: 'partNum', title: '当日部分完成数量'}
					, {field: 'notStartNum', title: '当日未启动数量'}
					, {field: 'createUserName', title: '填报人'}
			...
			table.on('tool(SettlementFilter)', function(obj){
				...
				if (layEvent === 'detail') { //详情
					var index = layer.open({
						type: 2,
						skin: false,//'layui-layer-molv', //加上边框
						area: ['100%', '100%'], //宽高
						title: '详情',
						maxmin: true,
						//btn: ['确定'],
						content: '/workflow/workLog/workLogAnalysis?workPlanId='+data.workPlanId+"&type=detail",
						success: function () {
	...
	function initChart(searchData) {
		...
		$.get('/workflow/workLog/serchWorkLogByOne?tatusFlag=1&allFlag=allFlag', searchData, function (res) {
			...
			if (res.flag) {
				...
				chartTwo.setOption({
					title: {
						text: (searchData.year || nowDateObj.year) + '年工作日志统计',
						left: 'center',
						textAlign: 'center'
					},
					tooltip: {
