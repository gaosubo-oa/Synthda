...
<%--		<button class="layui-btn layui-btn-sm" lay-event="delete" style="width: 60px">导出</button>--%>
		<button class="layui-btn layui-btn-sm" lay-event="submit" style="margin-left:10px;">提交</button>
	</div>
</script>
		...
		layui.use(['table', 'layer', 'form', 'element', 'eleTree', 'upload', 'laydate'], function () {
			...
			SettlementTable = layui.table.render({
				...
				, cols: [[ //表头
					...
					, {field: 'submitFlag', title: '提交状态', templet: function (d) {
							if (d.submitFlag == "0") {
								return "未提交";
			...
			table.on('tool(SettlementFilter)', function (obj) {
				...
				if (layEvent === 'detail') { //详情
					newAddFun(data.workPlanId)
					/*$.post('/workflow/workLog/insertWorkPlan', function (res) {
						var uri = ''
						if(res.obj&&res.obj.submitFlag==0){
							uri = '/workflow/workLog/addWorkLog?workPlanId='+data.workPlanId;
						}else {
							uri = '/workflow/workLog/addWorkLog?type=detail&workPlanId='+data.workPlanId;
						}
						...
						var index = layer.open({
							...
							title: '填报',
							btnAlign: 'c',
							btn: res.obj&&res.obj.submitFlag==0?['保存','提交','取消']:['确定'],
							maxmin: true,
							content: uri,
							success: function () {
			...
			table.on('toolbar(SettlementFilter)', function (obj) {
				...
				switch (event) {
					...
					case 'submit':  //提交
						...
						if (checkStatus[0].submitFlag == '1') {
							layer.msg("请选择未提交的数据");
							return false;
						}
						layer.confirm('提交后不可修改是否提交？', function (index) {
							$.ajax({
								url: "/workflow/workLog/submitWork?planStatus=1&workPlanId="+checkStatus[0].workPlanId,
								dataType: "json",
								type: 'post',
								success: function (res) {
									if (res.flag) {
			...
			function newAddFun(workPlanId){
				var logIndex = layer.load(1);
				// var workPlanId;
				var param = {}
				if(workPlanId){
					param.workPlanId = workPlanId
				}
				$.post('/workflow/workLog/insertWorkPlan', param,function (res) {
					if (res.code === "0" || res.code === 0) {
						workPlanId = res.obj;

						$.post('/workflow/workLog/getWorkPlanById',{workPlanId:workPlanId}, function (res) {
							var _content = ''
							if(res.obj&&res.obj.submitFlag==0){
								_content = '/workflow/workLog/addWorkLog?workPlanId=' + workPlanId
							}else {
								_content = '/workflow/workLog/addWorkLog?type=detail&workPlanId=' + workPlanId
							}
							...
							var index = layer.open({
								...
								yes: function (index, layero) {
									...
									//layer.confirm('确认后不可编辑，是否继续？', function (indexx) {
									//layer.close(indexx)
									var childData = $(layero).find("iframe")[0].contentWindow.getDate();    //父页面获取子页面的变量

									var dataArr = $(layero).find("iframe")[0].contentWindow.getDate2();
									// 判断必填项

									if(!childData.completionStatusflag){
										layer.msg('完成状态不能为空！', {icon: 0, time: 2000});
										return false;
									}

									/*var requiredFlag = false;
                                    for(var i=0;i<dataArr.length;i++){
                                        if(!dataArr[i].recordBeginDate){
                                            layer.msg('进度开始时间不能为空！', {icon: 0, time: 2000});
                                            requiredFlag = true;
                                            break
                                            return false;
                                        }
                                        if(!dataArr[i].recordDate){
                                            layer.msg('进度日期不能为空！', {icon: 0, time: 2000});
                                            requiredFlag = true;
                                            break
                                            return false;
                                        }
                                        if(!dataArr[i].percentComplete){
									...
									$.ajax({
										url: '/procedureSchedule/insert?isclose=true&workPlanId='+ workPlanId,
									...
									var childData = $(layero).find("iframe")[0].contentWindow.getDate();    //父页面获取子页面的变量
									childData.workPlanId=workPlanId;
									childData.planStatus="1";
									var dataArr = $(layero).find("iframe")[0].contentWindow.getDate2();
                                    ...
                                    if(requiredFlag) return*/
									layer.confirm('提交后不可修改是否提交？', function (index1) {
										$.ajax({
											url: "/workflow/workLog/updateWorkPlan",
											data: childData,
											dataType: 'json',
										...
										$.ajax({
											url: "/workflow/workLog/submitWork?workPlanId="+workPlanId,
											dataType: "json",
											type: 'post',
											success: function (res) {
												if (res.code === "0" || res.code === 0) {
