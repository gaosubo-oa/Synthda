...
<div class="container">
	...
	<div class="wrapper">
		<div class="wrap_left">
			<h2 style="text-align: center;line-height: 35px;">实施策划</h2>
			<div class="left_form">
						...
						<div class="layui-row">
							...
							<div class="layui-col-xs4">
								<div class="layui-form-item" style="/*width: 30%;*/margin: 10px -30px 10px;">
									<label class="layui-form-label" style="width: 35%;">实施策划名称</label>
...
<script>
	...
	layui.use(['treeTable','form','laydate','upload', 'element', 'layer','eleTree','table'], function () {
		...
		function tableInit(projectId) {
			insTb=table.render({
				elem: '#demoTreeTb',
				url: '/planningManage/selectPlanning?projectId=' + projectId,
				toolbar: '#toolbarDemo',
					cols: [[
						{type:'checkbox'}
		...
		table.on('toolbar(demoTreeTb)', function (obj) {
			...
			if (event === 'add') { //新增
				layer.open({
					...
					btn: ['保存','提交', '取消'],
					btnAlign:'c',
					content: '/planningManage/addImplementationPlanning?urlType=add&projectId=' + projectIdd,
					yes: function (index, layero) {
						var childData = $(layero).find("iframe")[0].contentWindow.getData();
						$.ajax({
							url: '/planningManage/insertPlanning',
							dataType: 'json',
							type: 'post',
							data: childData,
				...
				if (checkStatus.length != 1) {
					...
					layer.open({
						...
						btnAlign:'c',
						btn: ['保存','提交','取消'],
						content: '/planningManage/addImplementationPlanning?urlType=edit&planingId=' + securityInfoDate.planingId+'&projectId=' + projectIdd,
						success: function () {
		...
		table.on('tool(demoTreeTb)',function(obj){
			...
			if(layEvent=="details"){
				layer.open({
					...
					btnAlign:'c',
					btn: ['确定'],
					content: '/planningManage/addImplementationPlanning?urlType=details&planingId=' + data.planingId+'&projectId=' + projectIdd,
					yes: function (index, layero) {
						layer.close(index);
					}
				});
