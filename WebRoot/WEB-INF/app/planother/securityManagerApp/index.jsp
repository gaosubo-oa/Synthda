...
<div style="margin-top: 20px;position: relative;">
    <img src="../../img/log2.png" style="position: absolute;left: 21px;top: 2px;width: 25px; "><span
        style="margin-left: 49px;font-size: 20px;">安全检查</span>
...
<script>
    ...
    function initCheck() {
          $.ajax({
              url:'/workflow/secirityManager/getSecurityCheckByOption?checkType=1',
              datatype:'json',
              type:'get',
                  ...
                  if(res.obj==undefined){
                      ...
                      for(var i=0;i<res.obj.otherData.length;i++){
                          htm+='<div   style="border: 1px solid #C0C0C0; border-radius: 10px;  height: 110px; background-color: #FFFFFF; margin-top: 5%;bottom: 0; margin-bottom:auto; left: 0; margin-left:auto; right: 0; margin-right: auto;width: 90%" >\n' +
                              '    <div style="height: 50px; border-bottom: 1px solid #C0C0C0;display: flex;flex-direction: row">\n' +
                              '         <div style="display: flex; width: 30%">\n' +
                              '            <div id="dv'+i+'" style="font-weight: bold; font-size: 17px;color: #000000; top: 0;bottom: 0;left: 0;right: 0;margin: auto">'+res.obj.otherData[i].name+'</div>\n' +
                              '         </div>\n' +
                              '        <div style="display: flex; width: 66%">\n' +
                              ...
                              '        </div>\n' +
                              '    </div>\n' +
                              '    <div id="htm'+i+'" testTypeNo="'+res.obj.otherData[i].testTypeNo+'"  style="display: flex;height: 70px">\n' +
                              '        <div style="width: 30%;position: relative">\n' +
                          ...
                          $(document).on('click','#htm'+i,function(){
                              ...
                              window.location.href="/workflow/secirityManager/securityCheckApp?&testTypeNo="+testTypeNo+"&datatype=1&seciritytype=0";
                          })
                      }
                      ...
                      $("#dvbox1").click(function(){
                          window.location.href="/workflow/secirityManager/securityCheckApp?datatype=0&seciritytype=0"
                      })
                  }
              }
          })
    ...
    function initName() {
        $.ajax({
            url:'/workflow/secirityManager/getSecurityCheckByOption?project=1',
            datatype:'json',
            type:'get',
            success:function(res){
                var htm='';
                if(res.obj==undefined){
                    ...
                    for(var i=0;i<res.obj.otherData.length;i++){
                        ...
                        $(document).on('click','#bx'+i,function(){
                            ...
                            window.location.href="/workflow/secirityManager/securityCheckApp?projectId="+projectId+"&datatype=2&seciritytype=0"
                        })
                    }
