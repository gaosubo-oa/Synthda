...
<div class="head-top">
    <ul class="clearfix">
        <li class="fl head-top-li active" id="general" data-type=""><fmt:message code="email.th.emailReview" /></li>
        <li class="fl head-top-img"><img src="/img/twoth.png" alt=""></li>
        <li class="fl head-top-li" id="filter" data-type="0"><fmt:message code="email.th.filteringWordReview" /></li>
    </ul>
</div>
<div class="navigation" style="margin-top: 46px!important;">
    <img src="/img/commonTheme/${sessionScope.InterfaceModel}/daishen.png" alt="">
    <h2><fmt:message code="email.th.emailReview" /></h2>
</div>
<div class="divTable" style="display: block;">
    ...
    <table border="0" cellpadding="0" cellspacing="0" style="border-collapse: collapse">
        ...
        <tr>
            ...
            <th><fmt:message code="email.th.sender" /></th>
            <th><fmt:message code="email.th.recipients" /></th>
            <th><fmt:message code="notice.th.title" /></th>
            <th><fmt:message code="email.th.senderData" /></th>
...
//        初始值
        ...
        $('#general').on('click',function () {
            $('.navigation').find('h2').html('<fmt:message code="email.th.generalMailApproval" />');
            generalEmail($('#trList'));
        ...
        $('#filter').on('click',function () {
            $('.navigation').find('h2').html('<fmt:message code="email.th.filteringWordApproval" />');
            filterEamil($('#trList'));
            $(this).addClass('active');
...
//            不批准
        ...
        function pizhun(data,hideType,element) {
            $.ajax({
                type: 'post',
                url: '/approveEmail/examEmail',
                dataType: 'json',
                data: data,
                success: function (res) {
                    if (res.flag) {
                        layer.msg('<fmt:message code="user.th.SuccessfulOperation" />！', {icon: 1});
                        if(hideType == '1'){
                        ...
                        layer.msg('<fmt:message code="sms.th.operationFailed" />！', {icon: 2});
                    }
                }
            })
        }
                        ...
                        if(res.flag){
                            ...
                            for(var i=0;i<datas.length;i++){
                                ...
                                str+='<tr data-id="'+datas[i].bodyId+'" >' +
                                    ...
                                    '<td style="word-break: break-all">'+datas[i].toName+'</td>' +
                                    '<td><a href="/approveEmail/details?editType=0&id='+datas[i].bodyId+'" target="_blank">'+datas[i].subject+'</a></td>' +
                                    '<td>'+sendTime+'</td>' +
                                    '<td><a href="javascript:;" style="margin-right: 10px;" class="approval"><fmt:message code="meet.th.Approval" /></a><a href="javascript:;" class="noApproval"><fmt:message code="meet.th.NotApprove" /></a></td>' +
                                    '</tr>'
                            }
                            element.html(str);
    ...
    function filterEamil(element) {
        var ajaxPage={
            ...
            page:function () {
                ...
                $.ajax({
                    type:'get',
                    url:'/approveEmail/selwordExamEmail',
                    dataType:'json',
                    data:me.data,
                    success:function (res) {
                        var datas=res.obj;
