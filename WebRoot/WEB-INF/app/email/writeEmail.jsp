...
<%--    <script src="../js/xoajq/xoajq1.js" type="text/javascript" charset="utf-8"></script>--%>
    ...
    <script src="../js/base/base.js?20210928" type="text/javascript" charset="utf-8"></script>
        ...
        <td width="89%">
            <div class="inPole">
                ...
                <span class="add_img" style="margin-left: 10px;">
							...
							<span style="color: #207BD6;cursor: pointer" id="selectUser" class="Add"><fmt:message code="global.lang.add" /></span>
						</span>
                <span class="add_img">
							<%--<span class="addImg">
								<img src="../img/org_select2.png" class="clearIcon"/>
							</span>--%>
							<span  style="color: #207BD6;cursor: pointer" class="clear"><fmt:message code="notice.th.delete1" /></span>
						</span>
...
<%--    抄送人--%>
    ...
    <tr class="mis" style="display: none;"><td><fmt:message code="email.th.bcc" />：</td>
        <td><textarea id="secritText" name="txt" disabled></textarea>
            ...
            <span class="add_img">
                <a href="javascript:;" class="clearBCC" style="margin-left: 16px;font-size: 14px;"><fmt:message code="notice.th.delete1" /></a>
            </span>
        </td>
    </tr>
<%--    end--%>
    ...
    <tr>
        <td colspan="2">
            <div class="div_btn">
                ...
                <div class="saveBtn" style="width: 140px !important;"><span style="margin-left: 33px;"><fmt:message code="email.th.savedraftbox" /></span></div>
            </div>
        </td>
    </tr>
</table>
<script type="text/javascript">
    ...
    $(function () {
        ...
        // 抄送人员清除
        $(document).on('.clearCC','click',function(){
        ...
        // 密送人员清除
        $(document).on('.clearBCC','click',function(){
        ...
        var flag=$.getQueryString('flag');  //邮箱类型
        ue.ready(function(){
            function dataStyle(id,url){  //回复内容接口
                if(url != undefined){
                    url = url
                }else{
                    url = 'messageEmail'
            ...
            if(TYPE == 0){   //回复
                ...
                if(flag == 2 || flag == 3){
                    queryById(TYPE,data1,'queryByCountID');
                    dataStyle(sId,'messageEmailCount')
                }else{
                    queryById(TYPE,data1,'queryByID');
                    dataStyle(sId,'messageEmail')
                }
                ...
                if(boxType == 'inbox'){
                    ...
                    document.title="<fmt:message code="email.th.ForwardMail" />";
                    if(flag == 2 || flag == 3){
                        queryById(TYPE,data2,'queryByCountID');
                        dataStyle(nId,'messageEmailCount')
                    }else{
                        queryById(TYPE,data2,'queryByID');
                        dataStyle(nId,'messageEmail')
                    }
        ...
        //附件删除
        $('#files_txt').on('click','.claerimg',function(){
            $(this).parents('.dech').html('');
        })
...
//                                    }
    ...
    //	单条数据回写
    function queryById(status,data,url){
        if(url != undefined){
            url = url;
        }else{
            url = 'queryByID'
        ...
        $.ajax({
            ...
            success:function(rsp){
                    ...
                    if(status == 0|| status == 1){
                        for(var i=0;i<arr.length;i++){
                            ...
                            stra+='<div class="dech" deUrl="'+deUrl+'">' +
                                ...
                                '<img class="claerimg" style="margin-left:5px;cursor: pointer;" src="../img/file/icon_deletecha_03.png"/>' +
                ...
                if(status == 0){
                    ...
                    debugger
                    var arrName;
                    arrName=(data2.users.userName + "," + data2.toName).split(',');
                    var arrId;
                    arrId=(data2.users.userId + "," + data2.toId2).split(',');
                    ...
                    $('#senduser').val(arrName.join(','));
                    $('#senduser').attr('user_id',arrId.join(','))
                    // $('#copeNameText').val(arrName.join(','));
                    // $('#copeNameText').attr('user_id',arrId.join(','))
                }
            ...
            success:function (rsp) {
                ...
                if(data2.secretToName != ''){
                    var str='<tr class="mis"><td><fmt:message code="email.th.bcc" />：</td>' +
                        ...
                        '</span>' +
                        '<span class="add_img">' +
                        '<a href="javascript:;" class="clearBCC" style="margin-left: 16px;font-size: 14px;">'+notice_th_delete1+'</a>' +
                        '</span>' +
                        '</td></tr>';
                    $('.append_tr').after(str);
                    $('.a2').text('隐藏密送');
