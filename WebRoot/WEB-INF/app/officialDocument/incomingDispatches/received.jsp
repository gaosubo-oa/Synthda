...
<div style="margin: 0 auto;margin-top: 46px;height: 60px;width: 97%;" class="clearfix">
    ...
    <label class="fl clearfix" style="margin-top: 16px;margin-left: 10px;display: none;">
        ...
        <label class="fl"><input type="text" name="printDate" placeholder="<fmt:message code="doc.th.enterDate"/>"
                                 onclick="laydate({istime: true, format: 'YYYY-MM-DD'})"></label>
    ...
    <label class="fl clearfix" style="margin-top: 16px;margin-left: 10px;">
        ...
        <label class="fl"><input type="text" name="title" placeholder="<fmt:message code="document.th.SerialNumber" />"></label>
    ...
    $.ajax({
        ...
        success: function (res) {
            ...
            pageObj = $.tablePage('#pagediv', sumwidth, [
                ...
                {
                    width: width3,
                    title: '<fmt:message code="doc.th.Scholar2"/>',
                    name: 'createrName',
                ...
                me.init('/document/selOverDocSendOrReceive', [{
                    name: '下发', fn: function (obj) {
            ...
            $(document).on('click', '.editAndDelete0', function () {
                ...
                layer.open({
                    ...
                    yes: function (index) {
                        ...
                        if (transferReceiveOrg != '') {
                            $.post('/documentOrg/select', {
                                transferReceiveOrg: transferReceiveOrg,
                                id: obj.id
                            }, function (json) {
                                if (json) {
                                    $.post('/documentOrg/save', {
                                        transferReceiveOrg: transferReceiveOrg,
                                        id: obj.id
                                    }, function (json) {
                                        if (json.flag) {
                                    ...
                                    $.layerMsg({content: '<fmt:message code="document.th.ItHasBeenIssuedOnce" />', icon: 1})
                                }
                            }, 'json')
                        } else {
                            $.layerMsg({content: '<fmt:message code="document.th.SelectDelivered" />', icon: 1})
                        }
            ...
            $(document).on('click', '.editAndDelete5', function () {
                var obj = pageObj.arrs[$(this).attr('data-i')];
                $.layerConfirm({
                    title: '<fmt:message code="document.th.WithdrawalOfCirculation" />',
                    content: '<fmt:message code="document.th.CirculateTheDocument" />？',
                    icon: 0,
                    offset: "200px"
                }, function () {
                    $.ajax({
