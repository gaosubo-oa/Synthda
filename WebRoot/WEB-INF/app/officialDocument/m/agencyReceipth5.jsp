...
<body>
<header>
    <i><a href="/document/getReceiptMenth5" style="color: #fff">返回</a></i>
    <span>待办收文</span>
...
<script>
                        ...
                        if(res.flag&&res.datas!=0){
                            $.each (res.datas,function(index,item){
                                app+='<div style="box-shadow: 1px 1px 7px rgba(206, 194, 194, 0.85);    border-radius: .1rem;background-color: #fff;margin:.25rem .1rem;padding :.2rem 0">'+
                                    '<a class="list" href="javascript:;" style="color: #333;" flowPrcs="'+ item.flowPrcs +'" flowId="'+item.flowId+'" flowStep="'+item.step+'" tabId="'+item.id+'" tableName="'+item.tableName+'" runId="'+item.runId+'" prcsId="'+item.realPrcsId+'" opFlag="'+item.opFlag+'">'+
    ...
    $(function(){
        $('.content').on('click','a',function () {
            ...
            var flowPrcs=$(this).attr('flowPrcs')
            window.location.href='/document/getSuperiorformh5?flowId='+flowId+'&flowStep='+flowStep+
                '&flowPrcs='+flowPrcs+'&tabId='+tabId+'&tableName='+tableName+'&runId='+runId+'&prcsId='+prcsId+'&isNomalType=false&opFlag='+opFlag+'&lang=zh_CN&backType=2'
        })
    })
</script>
</html>