...
<head>
    ...
    <script src="/js/UserPriv/permission.js?20230425.1"></script>
...
<body>
<div id="hiddenMain" class="clearfix" style="">
    ...
    <div class="mainAdmin clearfix" >
        ...
        <div class="mainRight" style="padding-top: 5px;width: 100%">
            <table border="0" width="100%" cellspacing="0" cellpadding="3" class="tr_td " style="min-width: 756px">
                <tbody><tr style="border: none" id="top">
                    <td class="Big">
                        ...
                        <span class="big3">分支机构角色与权限管理</span>
                        <input type="text" id="privName" style="width: 150px;height: 32px;margin-right: 1%;margin-left: 3%" placeholder="请输入角色名称">
                        <button type="button" class="layui-btn layui-btn-sm" id="search" style="height: 31px;line-height: 25px;margin-top: -4px;">查询</button>
                        <button type="button" class="layui-btn layui-btn-sm" id="clear" style="height: 31px;line-height: 25px;margin-top: -4px;margin-right: 40px">清空</button>
                    </td>
                    <div id="zhu" style="display: none"></div>
                    <td id="fileload" align="right" style="position: relative">
                        <input type="button" value="导入" class="exportsss"  title="导入角色">&nbsp;&nbsp;
                        <input type="button" style="cursor: pointer;" class="fileload">
                        <a href="/userPrivType/outputUserPriv" style="display: inline-block"  class="importTwo">导出</a>
                        <a class="newJuese"><img style="margin-right: 4px;margin-bottom: 4px;" src="/img/mywork/newbuildworjk.png" alt="">新建角色</a>
                    </td>
            ...
            <div style="margin-top: 1%">
                <div class="lefts">
                        ...
                        <span style="display: inline-block;font-size: 22px;color:#2f8ae3;width: 95%;height: 50px;line-height: 50px;border-bottom: 1px solid #2f8ae3;padding-left: 15px">
                            <img style="width: 20px;height: 20px;vertical-align: text-bottom;margin-right: 10px;margin-left: 0;" src="/img/commonTheme/${sessionScope.InterfaceModel}/icon-01.png">角色分类</span>
                    <div class="cont_left">
                        <ul class="all_ul">
                ...
                <div class="rights">
                    <table id="table_style" width="96%" class="tr_td trtd_d" style="margin-left: 30px;">
                        ...
                        <tr><td nowrap="" class="th" align="center">角色排序号</td>
                            <td nowrap="" class="th" align="center">角色名称</td>
                            <td nowrap="" class="th" align="center">角色分类</td>
                            <td nowrap="" class="th" align="center">主角色总用户数(禁止登录用户数)/辅助角色用户数</td>
                            <td nowrap="" class="th" align="center">操作</td>
                        </tr></thead>
                        <tbody>
    ...
    $(function () {
        ...
        $('#clear').on("click",function(){
            if($('#privName').val()!=''){
                ...
                $.ajax({
                    type: 'get',
                    url: '/userPrivType/queryUserPrivByPrivName',
                    dataType: 'json',
                    success: function (date) {
                        ...
                        for(var i=0;i<data.length;i++){
                            ...
                            str+='<tr>' +
                                ...
                                '<td nowrap="" align="center"> <a href="roleManager?'+data[i].userPriv+'"  data-url=""> '+permissions+'</a>&nbsp;&nbsp;'+
                                ...
                                '<a href="character?'+data[i].userPriv+'" data-url="">'+clone+'</a>&nbsp;&nbsp;'+
...
//            点击新建
        $('.newJuese').on("click",function () {
            if (!isImport) {
                $.layerMsg({content: "您没有新建角色权限！", icon: 0});
                return false;
            }

            layer.open({
                type:1,
                title:['新建角色','background-color:#2b7fe0;color:#fff;'],
                area: ['440px', '260px'],
                shadeClose: true, //点击遮罩关闭
                btn: ['确定', '取消'],
                content:'<table class="tr_td" width="400" align="center" style="margin:30px auto;">'+
                    '<tbody><tr>'+
                    '<td nowrap="" class="TableData" style="width: 88px;">角色排序号：</td>'+
                    ...
                    '<td nowrap="" class="TableData">'+
                    // '<input type="text" name="PRIV_NAME" style="margin: 0" class="inp" size="25" maxlength="100" value="">&nbsp;'+
                    '   <select id="zhu" name="privTypeId" class="schoolType noEdit" lay-verify="required" lay-search="" title="角色分类" style="width:200px">\n' +
                ...
                success:function () {
                    ...
                    $.ajax({
                        type: 'post',
                        url: '/userPrivType/selectUserPrivTypeByOrgAdmin',
                        dataType: 'json',
                        success: function (json) {
                            ...
                            if (nowPrivTypeId != "") {
                                $("select[name='privTypeId']").val(nowPrivTypeId);
                            }
                        }
                    })
                },
                yes:function (index) {
                    ...
                    $.ajax({
                        type:'post',
                        url:'/userPriv/addClassifyOrgUserPriv',
                        dataType:'json',
                        data:{
        ...
        $.ajax({
            ...
            success:function(json) {
                ...
                if(data.length === 0){
                    ...
                    return layer.msg('请在分支机构设置中，设置您所属的分支机构的可管理角色分类',{icon:0})
                }
                isImport = true;
                for(var i=0;i<data.length;i++){
                    var er='';
                    if(data[i].privTypeName != undefined){
                        var three='';
                        three +='<li class="three checked" menu_tid='+data[i].privTypeId+' url='+data[i].url+' title="'+data[i].privTypeName+'"><div class="work_sanji"  style="margin-left:18px;"><h1 style="margin-left:50px;"><img style="margin-left: 0;margin-top: 0;vertical-align: middle;margin-right: 10px;float: none;" src="/img/commonTheme/${sessionScope.InterfaceModel}/icon-05.png">'+data[i].privTypeName+'</h1></div></li>' ;
                ...
                $('.one_all').on('click',function () {
                    ...
                    privTypeIds = $(this).attr('menu_tid')
                    nowPrivTypeId = $(this).attr('menu_tid')
                    right(privTypeId)
...
//                     });
//                     $('.one_all')[0].click()
                $('.one_all').eq(2).on("click")
            }
        })
    };

    var isImport = false;// 是否拥有导入权限
    // 初始化数据
    init();
    ...
    function right(privTypeId){
        $.ajax({
            type: 'get',
            url: '/userPrivType/queryAllUserPriv',
            data:{
                privTypeId:privTypeId
            },
            dataType: 'json',
