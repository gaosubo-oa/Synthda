...
<head>
    ...
    <style>
        *{margin: 0;padding: 0;}
        ...
        .TableBlock th{color:#2F5C8F;padding: 10px;width:auto;min-width: 100px}
        .TableBlock td{border: #ccc 1px solid;padding: 6px;width:auto;min-width: 100px}
...
<script>
    ...
    $(function(){
        ...
        $.ajax({
            url: '/newFileContent/signReading',
            type: 'get',
            ...
            success: function (res) {
                ...
                if(res.flag==true){
                    ...
                    if(Number(arrData[1])+Number(arrData[0]) != 0){
                        $('.TableBlock').prepend("<tr><th style='text-align: left;margin-left: 10px;'><fmt:message code="file.th.numberOfPeopleWhoCanView" />："+ (Number(arrData[1])+Number(arrData[0]))+"</th><th style='text-align: left;margin-left: 10px;'><fmt:message code="file.th.theNumberOfPeopleWhoHaveViewed" />："+arrData[1]+"</th><th style='text-align: left;margin-left: 10px;'><fmt:message code="file.th.theNumberOfPeopleWhoHaven'tViewed" />："+((Number(arrData[1])+Number(arrData[0]))-arrData[1])+"<span style='margin-left: 80px'><fmt:message code="file.th.viewingRate" />：</span>"+(arrData[1]/(Number(arrData[1])+Number(arrData[0]))*100).toFixed(2)+"%</th></tr>");
                    }else {
                        $('.TableBlock').prepend("<tr><th style='text-align: left;margin-left: 10px;'><fmt:message code="file.th.numberOfPeopleWhoCanView" />：0</th><th style='text-align: left;margin-left: 10px;'><fmt:message code="file.th.theNumberOfPeopleWhoHaveViewed" />：0</th><th style='text-align: left;margin-left: 10px;'><fmt:message code="file.th.theNumberOfPeopleWhoHaven'tViewed" />：0<span style='margin-left: 80px'><fmt:message code="file.th.viewingRate" />：</span>0%</th></tr>")
                    }
                }
            }
        })
    ...
    function departmentChild(datas,opt_li,level,dept){
        for(var i=0;i<datas.length;i++){
            ...
            var unread = datas[i].unread
            unread = unread.substring(0,unread.length-1)
            var read = datas[i].read
            read = read.substring(0,read.length-1)
            if(dept==datas[i].deptId){
                opt_li+='<tr class=TableData><td class="TableContent">'+String+datas[i].deptName+'</td><td style="">'+read+'</td>'+
                    '<td style="">'+unread+'</td></tr>';
            }else{
                opt_li+='<tr class=TableData><td class="TableContent">'+String+datas[i].deptName+'</td><td style="">'+read+'</td>'+
                    '<td style="">'+unread+'</td></tr>';
            }
