...
<head>
    ...
    <script type="text/javascript" src="/js/common/fileupload.js?20220819.1"></script>
...
<table class="TABLE" border="1" cellspacing="0" cellpadding="0" style="border-collapse: collapse;width: 98%;">
    ...
    <tr>
        ...
        <td>
            <input type="text" id="txt2" value="" class="input_txt" oninput="if (value.length>4)value=value.slice(0,4)"/>
            <img class="td_title2" src="../img/mg2.png" alt="">
        </td>
    </tr>
    <tr>
    ...
    <tr id="attachment">
        ...
        <td class="files" style="position: relative">
            ...
            <form id="uploadimgform" style="float: left;margin: 0 20px 0 0;" target="uploadiframe"  action="/upload?module=file_folder"  method="post" >
...
<%--            </form>--%>
            ...
            <%--&lt;%&ndash;            扫描按钮&ndash;%&gt;--%>
            <%--<div style="margin-top:35px"><a href="javascript:;"  onclick="Scan()"><img style="margin-right:5px;" src="../img/icon_uplod.png">扫描文件</a>--%>
            <%--</div>--%>
...
<script type="text/javascript">
        ...
        $.ajax({
            ...
            success:function (res) {
                if(res.object.length!=0){
                    ...
                    if (data.paraValue!=0){
                        let str = '<span style="color: #000;font-family: Microsoft YaHei;font-size: 14pt;margin-left: 10px; margin-top:10px"><fmt:message code="file.th.newfile" /> </span>'
                        if($.getQueryString('contentId')) {
                            str = '<span style="color: #000;font-family: Microsoft YaHei;font-size: 14pt;margin-left: 10px; margin-top:10px"><fmt:message code="dem.th.Edit" /> </span>'
                        }
                        $('.TABLE').before(str + '<span style="color: red;font-family: Microsoft YaHei;font-weight: bolder;font-size: 12pt;margin-left: 10px;margin-top:10px"> 机密级★ </span>')
                    }
                }
            }
        })
    ...
    <%--}--%>
    <%--}else{--%>
    <%--$.layerMsg({content:'未连接扫描仪',icon:6});--%>
    <%--return false;--%>
    <%--}--%>
    <%--// }else{--%>
    ...
    $(function(){
        ...
        $("#uploadimgform2").click(function (){
            $("#uploadimgform2").attr("action","/upload?module=file_folder")
        })
        function fileBox() {
            file_id='files_txt';
            model = 'email'
        ...
        if ($.getQueryString('contentId')){
            $("title").html("<fmt:message code="dem.th.Edit" />");
                ...
                $.ajax({
                    ...
                    success:function(rsp){
                        ...
                        for(var i = 0; i < arrList.length; i++){
                            var attachName = arrList[i].attachName;
                            var gs = attachName.substring(attachName.lastIndexOf(".") + 1, attachName.length);
                            gs = gs.toLowerCase();
                            if(gs =='ofd'){
                                str += '<div class="dech" name1="'+ arrList[i].attachName +'" deUrl="'+ encodeURI(arrList[i].attUrl) +'">' +
                                            '<a href="/download?'+ encodeURI(arrList[i].attUrl) +'" NAME="'+ arrList[i].attachName +'*" fileSize="' + arrList[i].size + ',">' +
                                                '<img style="width:16px;" src="../img/file/cabinet@.png"/>'+ arrList[i].attachName +
                                            '</a>' +
                                            '<img class="deImgs" style="margin-left:5px;cursor: pointer;" src="../img/file/icon_deletecha_03.png"/>' +
                                            '<a fileExtension="'+ gs +'" onclick="pdurls($(this))" href="javascript:;"  style="padding-left: 5px">' +
                                                '<img src="/img/attachmentIcon/icon_skim.png" style="padding: 0 5px;"><fmt:message code="global.lang.view" />' +
                                            '</a>' +
                                            '<a style="padding-left: 5px" href="/download?'+ encodeURI(arrList[i].attUrl)+'" >' +
                                                '<img src="/img/attachmentIcon/icon_down.png"  style="padding: 0 5px;"><fmt:message code="file.th.download" />' +
                                            '</a>' +
                                            '<a class="operation" href="javascript:;" onclick="editAttachment($(this))" class="editFile" style="margin-left: 10px;" attrurl="' + encodeURI(arrList[i].attUrl) + '">' +
                                                '<img src="/img/attachmentIcon/icon_edit.png" style="margin-right: 5px;" alt=""><fmt:message code="global.lang.edit" />' +
                                            '</a>' +
                                            '<a fileExtension="'+ gs +'" onclick="transfers($(this))" attrurl="' + encodeURI(arrList[i].attUrl) + '" href="javascript:;" style="padding-left: 5px">' +
                                                '<img src="/img/attachmentIcon/tran.png" style="padding: 0 5px;"><fmt:message code="workflow.th.Dump" />' +
                                            '</a>' +
                                            '<input type="hidden" class="inHidden" size="'+ arrList[i].size +'" value="'+ arrList[i].aid +'@'+ arrList[i].ym +'_'+ arrList[i].attachId +',">' +
                                        '</div>';

                            }else if(gs == 'docx'|| gs == 'doc'|| gs == 'xls' || gs == 'xlsx' || gs == 'pptx'){
                                var editBtn = 'display: none;';
                                var prevBtn = '';
                                var Prewtext = '<fmt:message code="global.lang.view" />';
                            } else if(gs == 'png'|| gs == 'jpg' || gs == 'jpeg'|| gs == 'pdf' || gs == 'txt'|| gs == 'html'){
                                var editBtn = 'display: none;';
                                var prevBtn = '';
                                var Prewtext = '<fmt:message code="document.th.ReferTo" />';
                            }else{
                                var editBtn = 'display: none;';
                                var prevBtn = 'display: none;';
                                var Prewtext = '';
                            }
                            str += '<div class="dech" name1="'+ arrList[i].attachName +'" deUrl="'+ encodeURI(arrList[i].attUrl) +'">' +
                                        '<a href="/download?'+ encodeURI(arrList[i].attUrl) +'" NAME="'+ arrList[i].attachName +'*" fileSize="' + arrList[i].size + ',">' +
                                        ...
                                        '</a>' +
                                        '<img class="deImgs" style="margin-left:5px;cursor: pointer;" src="../img/file/icon_deletecha_03.png"/>' +
                                        '<a fileExtension="'+ gs +'" onclick="pdurls($(this))" href="javascript:;" style="padding-left: 5px;'+ prevBtn +'">' +
                                            '<img src="/img/attachmentIcon/icon_skim.png" style="padding: 0 5px;">'+ Prewtext +
                                        '</a>' +
                                        '<a style="padding-left: 5px" href="/download?'+ encodeURI(arrList[i].attUrl) +'" >' +
                                            '<img src="/img/attachmentIcon/icon_down.png"  style="padding: 0 5px;"><fmt:message code="file.th.download" />' +
                                        '</a>' +
                                        '<a class="operation" href="javascript:;" onclick="editAttachment($(this))" class="editFile" style="margin-left: 10px;'+ editBtn +'" attrurl="' + encodeURI(arrList[i].attUrl) + '">' +
                                            '<img src="/img/attachmentIcon/icon_edit.png" style="margin-right: 5px;" alt=""><fmt:message code="global.lang.edit" />' +
                                        '</a>' +
                                        '<a fileExtension="'+ gs +'" onclick="transfers($(this))"  attrurl="' + encodeURI(arrList[i].attUrl) + '" href="javascript:;" style="padding-left: 5px"  >' +
                                            '<img src="/img/attachmentIcon/tran.png"  style="padding: 0 5px;"><fmt:message code="workflow.th.Dump" />' +
            ...
            $("#btn1").on("click",function(){
                ...
                $.ajax({
                    ...
                    success:function(data1){
                        var flag=data1.flag;
                        if (flag==true && data1.msg != "文件已经到最大容量"){
...
//                                        window.opener.location.reload();
                            ...
                            if(data1.msg == '文件已经到最大容量'){
                                layer.msg('<fmt:message code="file.th.theFileHasReachedItsMaximumCapacity" />', {icon: 2});
                            }else{
