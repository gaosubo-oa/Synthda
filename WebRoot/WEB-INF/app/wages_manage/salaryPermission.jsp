...
<div class="header">
    <div class="title">
        ...
        <span style="">薪资权限设置</span>
        <button  type="button" class="one" id="add_btn" style="width: 120px;margin-left:10%" onclick="javascript:window.location.href='/wages/salaryPermissionSet?type=0';">
...
<script>
    $(function () {
        ...
        var pageObj = $.tablePage('#pagediv',sumwidth, [
            ...
            me.data.pageSize = 10;
            me.init('/wagesPriv/selectAllWagesPriv',[
                {name:'编辑'},
                {name:'删除'}
            ])
        })
        ...
        $('#pageTbody').on('click','.editAndDelete0',function(){
            var payPriv = pageObj.arrs[$(this).attr('data-i')].payPriv;
            location.href = '/wages/salaryPermissionSet?type=1&payPriv='+payPriv;
        });
        //ajax 根据orgId 删除数据
        $('#pageTbody').on('click','.editAndDelete1',function(){
            var payPriv=0
            if(pageObj.arrs[$(this).attr('data-i')]!=undefined){
                payPriv=pageObj.arrs[$(this).attr('data-i')].payPriv;
            }
            layer.confirm(qued,{
                btn: [sure,cancel], //按钮
                title:'确定删除？'
