...
<body>
<div>
    <h3 style="margin-left: 60px;margin-top: 50px;margin-bottom: 20px;font-size: 25px">薪资权限设置</h3>
    <div>
...
<script>
    var payPriv = $.GetRequest()['payPriv'] || '';
    var type = $.GetRequest()['type'] || '';
    if(payPriv != '' && payPriv != undefined){
        $.ajax({
            type:'post',
            url:'/wagesPriv/selectAllWagesPriv',
            dataType:'json',
            data:{'payPriv':payPriv},
    ...
    if(type == 0){
        $(document).on('click','.save',function(){
            var WagesPriv = {}
            WagesPriv.userId = $("input[name='userId']").attr('user_id')
            WagesPriv.subordinateUserId = $("textarea[name='subordinateUserId']").attr('user_id')
            WagesPriv.subordinateDeptId = $("textarea[name='subordinateDeptId']").attr('deptid')

            $.ajax({
                url:'/wagesPriv/addWagesPriv',
                data: WagesPriv,
                type: 'post',
                success:function(res){
                    if(res.flag == true){
                        ...
                        // window.history.back()
                        window.open('/wages/salaryPermission','_self');
                    }else{
        ...
        $(document).on('click','.save',function(){
            ...
            wagesAttendancePriv.payPriv = payPriv

            $.ajax({
                url:'/wagesPriv/updateWagesPriv',
