        ...
        <div style="position: relative;line-height: 60px;">
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <select id="dataYear1" name="dataYear1" class="dataYear1" lay-verify="dataYear1" lay-filter="dataYear1">
                    </select>
                </div>
            </div>
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <select id="dataMonth1" name="dataMonth1" class="dataMonth1" lay-verify="dataMonth1" lay-filter="dataMonth1">
                    </select>
                </div>
            ...
            <button type="button" class="layui-btn layui-btn-sm" id="shengcheng" lay-event="shengcheng" style="height: 30px;line-height: 30px;margin-left: 10px">生成薪资数据</button>
            <button type="button" class="layui-btn layui-btn-sm" id="calculate" lay-event="calculate" style="height: 30px;line-height: 30px;margin-left: 10px">重新计算薪资</button>
            <button type="button" class="layui-btn layui-btn-sm" id="notice" lay-event="notice" style="height: 30px;line-height: 30px;">发布薪资确认通知</button>
...
<script>
    ...
    $('#export').on("click",function () {
        var dataYear = $("#dataYear1").val()
        var dataMonth = $("#dataMonth1").val()
        var userName = $("#userName").val()
        var deptId = $('#deptName').attr('deptid');
        if (deptId==undefined){
            deptId=''
        }
        window.location.href='/WagesSalaryData/export?dataYear='+dataYear+'&dataMonth='+dataMonth+'&deptId='+deptId+'&userName='+userName;
    });
    ...
    layui.use(['form', 'table', 'element', 'layedit','eleTree'], function () {
        ...
        for (var i=(y-10);i<(y+11);i++){
            $('#dataYear1').append('<option value='+i+'>'+i+'</option>');
        }
        for (var i=1;i<13;i++){
            $('#dataMonth1').append('<option value='+i+'>'+i+'</option>');
        }
        var month=new Date().getMonth()
        if(month==0){
            $('#dataMonth1').find('option[value=12]').attr("selected", true);
        }else{
            $('#dataMonth1').find('option[value=' + month + ']').attr("selected", true);
        }
        if(month==0){
            $('#dataYear1').find('option[value=' + (y-1) + ']').attr("selected", true);
        }else{
            $('#dataYear1').find('option[value=' + y + ']').attr("selected", true);
        }
        form.render('select');
        form.render()
            ...
            layer.confirm('是否生成薪资数据？', {
                ...
                $.ajax({
                    ...
                    success: function (json) {
                        if(json.flag){
                            layer.closeAll();
                            $.layerMsg({content: '生成成功！', icon: 1}, function () {
                                location.reload();
                            })
                        }
                    }
        ...
        $('#notice').on("click",function () {
            var theYear = $("#dataYear1").val()
            var theMonth = $("#dataMonth1").val()
            layer.confirm('是否发布薪资确认通知吗？', {
                btn: ['确定','取消'], //按钮
                title:"薪资确认通知"
            }, function(){
                $.ajax({
                    type: 'post',
                    url: '/WagesSalaryData/transactionReminder',
                    dataType: 'json',
                    data:{
                        confirm:true,
                        dataYear:theYear,
        ...
        $('#search1').on("click",function () {
            ...
            table.render({
                elem: '#test1'
                ,cellMinWidth:'100'
                , url:'/WagesSalaryData/selectAll?dataYear='+dataYear+'&dataMonth='+dataMonth+'&deptName='+deptName+'&userName='+userName+'&pageSize='+'10'+'&useflag='+true
                ,cols:[[
        ...
        table.render({
            elem: '#test1'
            , url: '/WagesSalaryData/selectAll?pageSize='+'10'+'&useflag='+true+'&dataYear='+y+'&dataMonth='+month
            ,cellMinWidth:'100'
