        ...
        <div style="position: relative;line-height: 60px;">
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <select id="theYear" name="theYear" class="theYear" lay-verify="theYear" lay-filter="theYear">
                    </select>
                </div>
            </div>
            <div class="layui-inline">
                ...
                <div class="layui-input-inline">
                    <select id="theMonth" name="theMonth" class="theMonth" lay-verify="theMonth" lay-filter="theMonth">
                    </select>
                </div>
            </div>
            <div class="layui-inline">
            ...
            <div style="display: flex">
                <div style="width: 120px;margin-left: 20px;margin-top: 10px">
                    <label class="layui-form-label">异常数据</label>
                    <input id="yichang" type="checkbox" class="yichang" name="yichang"  style="height: 40px;
    width: 20px;; display: inline-block;" fuxuan="no">
                </div>
                <div style="width: 120px;margin-left: 20px;margin-top: 10px">
                    <label class="layui-form-label" style="width: 70px">多岗位数据</label>
                    <input id="duogang" type="checkbox" class="duogang" name="duogang"  style="height: 40px;
    width: 20px;; display: inline-block;" fuxuan="no">
                </div>
                <div>
                    ...
                    <button type="button" class="layui-btn layui-btn-sm" id="calculate" lay-event="calculate" style="height: 30px;line-height: 30px;">计算考勤数据</button>
                    <button type="button" class="layui-btn layui-btn-sm" id="notice" lay-event="notice" style="height: 30px;line-height: 30px;margin-left: 10px;">发布考勤确认通知</button>
                    <button type="button" class="layui-btn layui-btn-sm" id="import" lay-event="" style="height: 30px;line-height: 30px;">导入</button>
                    <button type="button" class="layui-btn layui-btn-sm" id="delall" lay-event="" style="height: 30px;line-height: 30px;">删除所有考勤数据</button>
                    <button type="button" class="layui-btn layui-btn-sm" id="export" lay-event="" style="height: 30px;line-height: 30px;">导出</button>
                    <button type="button" class="layui-btn layui-btn-sm" id="export1" lay-event="" style="height: 30px;line-height: 30px;">无考勤数据人员导出</button>
                    <div class="layui-form-item" style="display: inline-block">
                        <label class="layui-form-label">导入开关</label>
                        <div class="layui-input-block">
                            <input type="checkbox" name="close" lay-skin="switch" lay-text="关|开" lay-filter="kai" lay-event="false" id="LOCKED">
                        </div>
                    </div>
                </div>
            </div>
...
<%--        <a id="edit" lay-event="detail1" data-index="i" style="color: blue;cursor:pointer; text-decoration:underline">薪岗排布</a>--%>
...
<script>
    ...
    layui.use(['form', 'table', 'element', 'layedit', 'eleTree', 'upload'], function () {
        ...
        $('#theYear').find('option[value=' + y + ']').attr("selected", true);
        for (var i = 1; i < 13; i++){
            $('#theMonth').append('<option value=' + i + '>' + i + '</option>');
        }
        var month = new Date().getMonth()
        if(month==0){
            $('#theMonth').find('option[value=12]').attr("selected", true);
        }else{
            $('#theMonth').find('option[value=' + month + ']').attr("selected", true);
        }
        if(month==0){
            $('#theYear').find('option[value=' + (y-1) + ']').attr("selected", true);
        }else{
            $('#theYear').find('option[value=' + y + ']').attr("selected", true);
        }
        ...
        $.ajax({
            type: 'post',
            url: '/wagesLock/selectByName?lockName=LOCKED&theYear='+theYear+'&theMonth='+theMonth+'',
            dataType: 'json',
            success: function (json) {
                if(json.object.lockStatus == 0){
                    $('#LOCKED').attr('checked','true')
                    $('#LOCKED').eq(0).next().attr('class','layui-unselect layui-form-switch layui-form-onswitch')
                    document.getElementById("import").style.backgroundColor = "rgb(193,193,193)";
                    document.getElementById("calculate").style.backgroundColor = "rgb(193,193,193)";
                    document.getElementById("notice").style.backgroundColor = "rgb(193,193,193)";
                    document.getElementById("delall").style.backgroundColor = "rgb(193,193,193)";
                    $('#import').attr('disabled','true')
                    $('#calculate').attr('disabled','true')
                    $('#notice').attr('disabled','true')
                    $('#delall').attr('disabled','true')
                    $('#LOCKED').attr('lay-event','true')
                    dvalue = 0
                }else if(json.object.lockStatus == 1){
                    $('#LOCKED').removeAttr('checked')
                    $('#LOCKED').eq(0).next().attr('class','layui-unselect layui-form-switch')
                    document.getElementById("import").style.backgroundColor = "rgb(0,150,136)";
                    document.getElementById("calculate").style.backgroundColor = "rgb(0,150,136)";
                    document.getElementById("notice").style.backgroundColor = "rgb(0,150,136)";
                    document.getElementById("delall").style.backgroundColor = "rgb(0,150,136)";
                    dvalue = 1
                    $('#LOCKED').attr('lay-event','false')
                }
            }
        })
        form.on('switch(kai)',function (data) {
            ...
            $.ajax({
                type: 'post',
                url: '/wagesLock/update?lockName=LOCKED&lockStatus='+checked+'&theYear='+theYear+'&theMonth='+theMonth+'',
                dataType: 'json',
                success: function (json) {
                    var event = $('#LOCKED').attr('lay-event')
                    if(event == 'true'){
                        document.getElementById("import").style.backgroundColor = "rgb(0,150,136)";
                        document.getElementById("calculate").style.backgroundColor = "rgb(0,150,136)";
                        document.getElementById("notice").style.backgroundColor = "rgb(0,150,136)";
        ...
        $('#calculate').on("click",function () {
            var theYear = $("#theYear").val()
            var theMonth = $("#theMonth").val()
            layer.confirm('是否开始计算考勤数据？', {
                btn: ['确定', '取消'], //按钮
                title:"计算考勤数据"
            }, function(){
                $.ajax({
                    type: 'post',
                    url: '/wagesJobDaysalary/calculateDaysalary',
                    dataType: 'json',
                    data: {
                        theYear:theYear,
                        theMonth:theMonth
        ...
        $('#notice').on("click",function () {
            var theYear = $("#theYear").val()
            var theMonth = $("#theMonth").val()
            layer.confirm('是否发布考勤确认通知吗？', {
                btn: ['确定','取消'], //按钮
                title:"考勤确认通知"
            }, function(){
                $.ajax({
                    type: 'post',
                    url: '/AttendanceType/transactionReminder',
                    dataType: 'json',
                    data: {
                        confirm:true,
                        theYear:theYear,
        ...
        $('#delall').on("click",function () {
            var theYear = $("#theYear").val()
            var theMonth = $("#theMonth").val()
            layer.confirm('是否删除本月所有考勤数据？', {
                btn: ['确定','取消'], //按钮
                title:"删除全部考勤数据"
            }, function(){
                $.ajax({
                    type: 'post',
                    url: '/AttendanceType/deleteMonthlyData',
                    dataType: 'json',
                    data: {
                        theYear:theYear,
                        theMonth:theMonth
        ...
        $('#search1').on("click",function () {
            ...
            if($("#yichang").is(":checked")){
                pickUnusual=true
            } else {
                pickUnusual=false
            }
            if($("#duogang").is(":checked")){
                ct = 2
            } else {
                ct = ''
            }
            ...
            if(ct == 2){
                url = '/AttendanceType/selectAllAttendanceDate?theYear='+theYear+'&theMonth='+theMonth+'&deptId='+deptId+'&userName='+userName+'&pageSize='+'10'+'&useFlag='+true+'&pickUnusual='+pickUnusual+'&ct='+ct
            }else{
                url = '/AttendanceType/selectAllAttendanceDate?theYear='+theYear+'&theMonth='+theMonth+'&deptId='+deptId+'&userName='+userName+'&pageSize='+'10'+'&useFlag='+true+'&pickUnusual='+pickUnusual
            }
            var ins1 = table.render({
                ...
                ,done: function(res){
                    ...
                    for(var i = 1; i < trList.length; i++){
                        for(var j = 2; j < trList.eq(i+1).find('td[lay-event]').length; j++){
                            ...
                            if(res.data[i - 1]['attendType'+ (j - 1)] == ''&&checkDate(date)){
                                ...
                                //             trList.eq(i).find('td[lay-event]').eq(j).attr('flag', true)
                                //         }
                                //     }
                                // }
                    ...
                    //         //         var j=i-1
                    //         //         $("tr[data-index="+j+"] td[data-field='attendType"+i+"']").attr('class','select')
                    //         //         // $("td[data-field='attendType"+i+"']").attr('class','select')
                    //         //     }
                    //         // }
                    //     }
                    //
        ...
        var ins1 = table.render({
            elem: '#test1'
            ,id: 'exportTable'
            , url: '/AttendanceType/selectAllAttendanceDate?useFlag='+true+'&theYear='+y+'&theMonth='+month
        ...
        $("#export").on("click",function(){
            ...
            var theMonth = $("#theMonth").val()
            $.ajax({
                url: '/AttendanceType/selectAllAttendanceDate?theYear='+theYear+'&theMonth='+theMonth+'&useFlag=false',
                type: 'post',
                async: false,
                dataType: 'json',
        ...
        $('#export1').on("click",function () {
            var theYear = $("#theYear").val()
            var theMonth = $("#theMonth").val()
            window.location.href='/wagesJobDaysalary/selectNoAttendance?theYear='+theYear+'&theMonth='+theMonth;
        });
            ...
            layer.open({
                ...
                success: function () {
                    ...
                    var uploadInst = upload.render({
                        elem: '#tests' //绑定元素
                        , url: '/AttendanceType/addAttendanceData?theYear='+theYear+'&theMonth='+theMonth+''
                        , accept: 'file'
                        , auto: false
