...
<script>
            ...
            success:function(res){
                ...
                // }
                    for(var i=0;i<data.length;i++) {
                        ...
                        strs += '<li>' + '<p style="display:block; overflow: hidden; white-space: nowrap;text-overflow: ellipsis;width: 330px;">\n' +

                            '                    <img src="/img/file/m/file.png" alt="" style="">\n' +
                            '                    <span onclick="moreInfo(\''+data[i].contentId+'\');" style="font-size: 13px;font-weight: 500;margin-left: 10px;">' + empty(data[i].attachmentName) + '</span>\n' +
                            // '                    <span class="mui-icon mui-icon-arrowright" style="float: right;margin-top: 14px;"></span>\n' +
                            '         </p>       </li>'
                    }
                $('.file').html(strs)
    ...
    function moreInfo(contentId){
        window.location.href="/KnowledgePortalH5/personDetails?contentId="+contentId;
    }

    //个人文件柜所有文件列表
    $.ajax({
        url:'/newFileContent/Ph/serachTopTenFile?serachType=1',
        type:'get',
        dataType:'json',
        beforeSend:function(){
            $(".cabinets1").show()
        ...
        success:function(res){
            ...
            for(var i=0;i<data.length;i++){
                if(data[i].attachmentList[0]==undefined){
                    ...
                    strs+='<a href="/KnowledgePortalH5/personDetails?contentId='+data[i].contentId+'" style="display: block"><p style="overflow: hidden; white-space: nowrap;text-overflow: ellipsis;width: 330px;">\n' +
                        '                    <img src="/img/file/m/file.png" alt="" style="">\n' +
                ...
                success:function(res){
                    ...
                    for(var i=0;i<data.length;i++){
                        if(data[i].sortName==undefined){
                            ...
                            //console.log(data[i].sortName)
                            strs+='<li onclick="getChild(\''+data[i].sortId+'\');">\n' +
                                '                    <img src="/img/file/m/Group.png" alt="" style="">\n' +
                                '                    <span style="font-size: 13px;font-weight: 500;margin-left: 10px;">'+empty(data[i].sortName)+'</span>\n' +
                                '                    <span class="mui-icon mui-icon-arrowright"  style="float: right;margin-top: 14px;"></span>\n' +
                                '                </li>'
            ...
            $.ajax({
                ...
                success:function(res){
                    ...
                    // }
                        for(var i=0;i<data.length;i++) {
                            //console.log(data.file[i].sortId)
                            strs += '<li onclick="moreInfo(\''+data[i].contentId+'\');">' + '<p style="display:block; overflow: hidden; white-space: nowrap;text-overflow: ellipsis;width: 330px;">\n' +
                                '                    <img src="/img/file/m/file.png" alt="" style="">\n' +
                                '                    <span  style="font-size: 13px;font-weight: 500;margin-left: 10px;">' + empty(data[i].attachmentName) + '</span>\n' +
                                // '                    <span class="mui-icon mui-icon-arrowright" style="float: right;margin-top: 14px;"></span>\n' +
                                '        </p>        </li>'
                        }
                    $('.file').html(strs)
                ...
                if(data !=''){
                    for(var i=0;i<data.length;i++){
                        if(data[i].showPriv == false){
                            strs+='<li onclick="getChild(\''+data[i].sortId+'\');" sortParent = "'+data[i].sortParent+'" sortId = "'+data[i].sortId+'">' + '<p style="display:block; overflow: hidden; white-space: nowrap;text-overflow: ellipsis;width: 330px;">\n' +
                                '                    <img src="/img/file/m/Group.png" alt="" style="">\n' +
                                '                    <span style="font-size: 13px;font-weight: 500;margin-left: 10px;">'+empty(data[i].sortName)+'</span>\n' +
                                '                    <span class="mui-icon mui-icon-arrowright" style="float: right;margin-top: 14px;"></span>\n' +
                                '          </p>      </li>'
                        }else{
                            strs+='<li onclick="moreInfo(\''+data[i].contentId+'\');" sortParent = "'+data[i].sortParent+'" sortId = "'+data[i].sortId+'">' + '<p style="display:block; overflow: hidden; white-space: nowrap;text-overflow: ellipsis;width: 330px;">\n' +
                                '                    <img src="/img/file/m/file.png" alt="" style="">\n' +
                ...
                success:function(res){
                    ...
                    for(var i=0;i<data.length;i++){
                        if(data[i].attachmentList[0]==undefined){
                            ...
                            strs+='<a href="/KnowledgePortalH5/personDetails?contentId='+data[i].contentId+'" style="display: block"><p style="display:block; overflow: hidden; white-space: nowrap;text-overflow: ellipsis;width: 330px;">\n' +
                                '                    <img src="/img/file/m/file.png" alt="" style="">\n' +
                                '                    <span style="font-size: 13px;font-weight: 500;margin-left: 10px;">'+empty(data[i].attachmentList[0].attachName)+'</span>\n' +
                                '                </p></a>'
                        }
