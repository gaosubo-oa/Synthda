                ...
                <div class="query layui-row">
                    <div class="layui-col-xs2">
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-block">
                                <input type="text" name="tgName" required lay-verify="required" autocomplete="off"
                                       class="layui-input">
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="layui-col-xs2">
                        <div class="authority_search" style="display: none;">
                            ...
                            <button type="button" class="layui-btn layui-btn-sm sty clear" style="margin-left: 20px;">
                                重置
                            </button>
...
<script>
    ...
    var dictionaryStr = 'CONTROL_LEVEL,PLAN_SYCLE,TG_TYPE,PLAN_PHASE,UNIT,CGCL_TYPE,RENWUJIHUA_TYPE'
    // 获取数据字典数据
    ...
    layui.use(['table', 'layer', 'form', 'element', 'laydate', 'upload'], function () {
        ...
        function tableShow(complete) {
            var cols = [[
                ...
                , {
                    field: 'taskName', title: '子任务名称', minWidth: 300, templet: function (d) {
                        if (d.assistStatus == 1) {
                            return '(协助计划)' + d.taskName
                        } else {
                            return d.taskName
                        }
                    }
                ...
                , {field: 'assistDeptIdName', title: '部门名称', minWidth: 150}
            ]]
            tableInit = table.render({
                elem: '#test'
                , url: '/plcProjectItem/queryAndAssist'
                ...
                , done: function (res) {
                    ...
                    if (res.data == '' || res.data == undefined) {
                        ...
                        var planItemId = res.data[0].planItemId
                        $('#allId').val(planItemId)
                        ...
                        var tName = res.data[0].taskName
                        var deptId = res.data[0].assistDeptId
                        var submitStatus = res.data[0].submitStatus
                        $('#allId').attr('deptId', deptId)
                        $('#allId').attr('submitStatus', submitStatus)
                        //区分是否是协助计划
                        var assistStatus = res.data[0].assistStatus || ''
                        $('#allId').attr('assistStatus', assistStatus)

                        var taskType = res.data[0].taskType
                        if (taskType) {
                            $('#allId').attr('taskType', taskType)
                        } else {
                            $('#allId').attr('taskType', '')
                        }
                        ...
                        rightnum(planItemId, planEndDate, resultStandard, resultDict, openNextTask, itemApprivalStatus, tName, resultFileList, errorFileList, deptId);
        ...
        table.on('row(test)', function (obj) {
            ...
            var tName = obj.data.taskName
            var deptId = obj.data.assistDeptId
            var submitStatus = obj.data.submitStatus
            $('#allId').attr('deptId', deptId)
            $('#allId').attr('submitStatus', submitStatus)
            //区分是否是协助计划
            var assistStatus = obj.data.assistStatus || ''
            $('#allId').attr('assistStatus', assistStatus)
            ...
            rightnum(planItemId, aa, result, resultDict, openNextTask, itemApprivalStatus, tName, resultFileList, errorFileList, deptId)

        });

        //右侧数据
        function rightnum(planItemId, aa, result, resultDict, openNextTask, itemApprivalStatus, tName, resultFileList, errorFileList, deptId) {
            successFileStr = ''
            failFIleStr = ''
            finalSuccessFileStr = ''
            finalFailFIleStr = ''
            planItemId_relation = planItemId
            deptId_relation = deptId
            var datas = {planItemId: planItemId, deptId: deptId}
            $.ajax({
                ...
                success: function (res) {
                    ...
                    strs = '<form class="layui-form content" action="" style="height:100%"><div id="rightHead">\n' +
                        ...
                        '                </div>\n' +
                        '                <p><span style="margin-right: 15px">名&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;称:</span><span id="rightName" planItemId="' + planItemId + '" deptId="' + deptId + '">' + tName + '</span></p>\n' +
                        ...
                        '                <div class="clearfix"><span style="float: left;width: 140px;font-size: 16px; color: #666;">最终成果:</span>' + function () {
                            var resultFileStr = '';
                            if (!!resultFileList && resultFileList.length > 0) {
                                ...
                                resultFileStr = '<div style="float:left;width: calc(100% - (151px));">' + str + '</div>';
                            }
                        ...
                        /* '                <div style="margin: 15px auto;">\n' +
                         ...
                         '                </div>\n' +*/
                        '<iframe id="iframeName" src="/ProjectDaily/taskInfor" style="width: 100%;" frameborder="0"></iframe>' +
                        '            </form>'
                    $('.right').html(strs)
                    $('#iframeName').height($('#rightShow').height() - $('#rightHead').height() - 35)

                    /*获取当前行的数据*/
                    var $str = $('<input type="hidden" id="hiddenDeptId" projectId="' + function () {
                    ...
                    /*if ($('#allId').attr('submitStatus') == 1) {
                        $('.today').css('cursor', 'default')
                        $('.Collaborator').css('cursor', 'default')
                        $('.transfer').css('cursor', 'default')
                        $('#submit').css('cursor', 'default')
                    }else if($('#allId').attr('submitStatus') === '0'){
                        $('.Collaborator').css({
                            'cursor': 'not-allowed',
                            'background': '#C1C1C1'
                        })
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        if (!!data[i].attachmentList && data[i].attachmentList.length > 0) {
                            ...
                            data[i].attachmentList.forEach(function (item) {
                                ...
                                str += '<div class="divShow"><a href="javascript:;" title="' + item.attachName + '" style="display: block; overflow: hidden;text-overflow: ellipsis;color: blue;">' + item.attachName + '</a>' +
                                    ...
                                    + '</div>' +
                                    '</div>'
                                str_1 += '<tr><td nowrap width="90">' + data[i].ctreateTime + '</td><td><div class="divShow" style="width: 92%"><a href="javascript:;" title="' + item.attachName + '" style="overflow: hidden;text-overflow: ellipsis;color: blue;">' + item.attachName + '</a>' +
                                    '<div class="operationDiv">' + function () {
                                        if (fileExtension == 'pdf' || fileExtension == 'png' || fileExtension == 'jpg' || fileExtension == 'txt') { //判断是否需要查阅
                                            return '<a class="operation yulan"  href="javascript:void(0);" data-url="' + encodeURI(attachmentUrl) + '" style="display: block;padding: 5px;"><img src="/img/attachmentIcon/icon_skim.png" style="margin-right: 5px;" alt="">查阅</a>'
                                        } else {
                        ...
                        dayReport += '<div style="border: 1px solid #d9d6d6;margin-bottom: 10px;position: relative;">\n' +
                            function () {
                                if ($('.layui-this').text() == '正进行') {
                                    return '<button type="button" class="layui-btn layui-btn-sm delReport" dailyId="' + data[i].dailyId + '" style="position: absolute;right: 0px;top: 2px;">删除</button>'
                                } else {
                                    return ''
                                }
                            }() +
                    ...
                    // 成果材料附件
                    if (!!object.attachments2 && object.attachments2.length > 0) {
                        var str_1 = '';

                        object.attachments2.forEach(function (item) {
                            var attachName = item.attachName;
                            var fileExtension = attachName.substring(attachName.lastIndexOf(".") + 1, attachName.length);//截取附件文件后缀
                            var attName = encodeURI(attachName).replace(/\+/g, "%2b").replace(/\@/g, "%40").replace(/#/g, "%23").replace(/\&/g, "%26").replace(/\//g, "%2F").replace(/\?/g, "%3F").replace(/\￥/g, "%ef%bf%a5").replace(/$/g, "%24").replace(/\！/g, "%ef%bc%81").replace(/\（/g, "%ef%bc%88").replace(/\）/g, "%ef%bc%89").replace(/\…/g, "%e2%80%a6%e2%80%a6");
                            var fileExtensionName = attName.substring(0, attName.lastIndexOf("."));
                    ...
                    // 异常原因附件
                    if (!!object.attachments3 && object.attachments3.length > 0) {
                        var str_1 = '';

                        object.attachments3.forEach(function (item) {
                            var attachName = item.attachName;
                            var fileExtension = attachName.substring(attachName.lastIndexOf(".") + 1, attachName.length);//截取附件文件后缀
                            var attName = encodeURI(attachName).replace(/\+/g, "%2b").replace(/\@/g, "%40").replace(/#/g, "%23").replace(/\&/g, "%26").replace(/\//g, "%2F").replace(/\?/g, "%3F").replace(/\￥/g, "%ef%bf%a5").replace(/$/g, "%24").replace(/\！/g, "%ef%bc%81").replace(/\（/g, "%ef%bc%88").replace(/\）/g, "%ef%bc%89").replace(/\…/g, "%e2%80%a6%e2%80%a6");
                            var fileExtensionName = attName.substring(0, attName.lastIndexOf("."));
                    ...
                    $('.today').on("click",function () {
                        if ($('#allId').attr('submitStatus') != 1 && $('#allId').attr('submitStatus') !== '0') {
                            return false
                        }
                        ...
                        layer.open({
                            ...
                            content: '<div style="padding: 15px;"  class="progress">\n' +
                                ...
                                '            <div>\n' +
                                '                <span style="margin-right: 30px;margin-left:2px">名&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;称:</span>\n' +
                                ...
                                '                <span>成果标准模板:</span>\n' +
                                '                <span style="margin-left:2px">' + function () {
                                    var resultDictList = ''
                                    if (resultDict) {
                                ...
                                '    <label class="layui-form-label" style="width: 117px;margin-left:-16px;padding:3px 15px">提交的成果资料:</label>\n' +
                                '    <div class="layui-input-block">\n' +
                                ' <div id="fileAll">\n' +
                                '</div>\n' +
                                ...
                                '                <label class="layui-form-label" style="margin: 0px 25px 0px -28px;">异常原因:</label>\n' +
                                '                <div class="layui-input-inline" style="width: 80%">\n' +
                                '                    <input type="text" name="unusualReason"  autocomplete="off" class="layui-input Reason">\n' +
                                '                </div>\n' +
                                '            </div>\n' +
                                ' <div class="layui-form-item"  style="margin-top:15px">\n' +
                                '    <label class="layui-form-label" style="width: 100px;margin-left:-16px;padding:3px 15px">异常支撑资料:</label>\n' +
                                '    <div class="layui-input-block">\n' +
                                ' <div id="filerror">\n' +
                                '</div>\n' +
                                '<a href="javascript:;" class="openFile" style="float: left;position:relative">\n' +
                    ...
                    $('.Collaborator').on("click",function () {
                        if ($('#allId').attr('assistStatus') == 1 || $('.layui-this').text() == '已完成') {
                            return false
                        }
                        layer.open({
                            type: 1,
                            ...
                            yes: function (index) {
                                ...
                                var datasPer = {
                                    ids: planItemId,
                                    assistUser: $('#resUser').attr('user_id'),
                                    deptId: $('#allId').attr('deptId')
                                }
                                var url = '/plcProjectItem/addPeople'
                                $.ajax({
                                    url: url,
                                    dataType: 'json',
                                    type: 'get',
                    ...
                    $('.transfer').on("click",function () {
                        if ($('#allId').attr('submitStatus') != 1 || $('.layui-this').text() == '已完成') {
                            return false
                        }
                        layer.open({
                            type: 1,
                            ...
                            yes: function (index) {
                                ...
                                var datasPer = {
                                    ...
                                    deptId: $('#allId').attr('deptId'),
                                    assistStatus: $('#allId').attr('assistStatus')
                                }
                                var url = '/plcProjectItem/changeDutyUser'
                                $.ajax({
                                    url: url,
                                    dataType: 'json',
                                    type: 'get',
        ...
        function isCheck() {
            var isOk = ''
            var urlSubmit = '/plcProjectItem/checkDutyUser'
            $.ajax({
                url: urlSubmit,
                data: {id: $('#allId').val()},
                dataType: 'json',
        ...
        $(document).on('click', '#submit', function () {
            ...
            if (itemApprivalStatus == 1 || itemApprivalStatus == 2 || itemApprivalStatus == 3 || itemApprivalStatus === '0'|| isSubmit==1) {
                return false
            }*/
            if ($('#allId').attr('submitStatus') != 1 || $('.layui-this').text() == '已完成') {
                return false
        ...
        function goSubmit() {
            layer.open({
                ...
                content: '<div style="padding: 15px;" class="progress">\n' +
                    ...
                    '            <div>\n' +
                    '                <span style="margin-right: 42px;">名&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;称：</span>\n' +
                    '                <span>' + $('#rightName').text() + '</span>\n' +
                    '            </div>\n' +
                    ...
                    '           </div>' +
                    '            <div>\n' +
                    '                <label class="layui-form-label" style="margin: 0px 35px 0px -16px;">异常原因：</label>\n' +
                    '                <div class="layui-input-inline" style="width: 80%">\n' +
                ...
                yes: function (index) {
                    ...
                    var updateData = {
                        ...
                        taskStatus: 5,
                        unusualRes: $('#unusualRes').val(),
                        taskType: $('#allId').attr('taskType')
                    }

                    var updateUrl = '/plcProjectItem/submit';
                    var contentShow = ''
                    ...
                    updateData.planItemId = $('#allId').val();
                    /*事务提醒和短信提醒*/
                    var remind
                    var smsRemind
                    ...
                    // updateData.deptId=$('#hiddenDeptId').data('data').itemBelongDept
                    updateData.deptId = $('#allId').attr('deptId')
                    updateData.planClass = 3
                    // console.log(updateData)

                    //判断是否是协助计划
                    if ($('#allId').attr('assistStatus')) {
                        updateData.assistStatus = 1
                    }
            ...
            /*var url = '/plcProjectItem/checkDutyUser'
            $.get(url, {id: $('#allId').val()}, function (res) {
                if (res.flag && res.code == 1) {
                    layer.open({
                        title: '提交审核',
                        type: 1,
                        ...
                        content: '<div style="padding: 15px;" class="progress">\n' +
                            ...
                            '                    <label class="layui-form-label" style="width: 130px;margin-top: -9px;margin-left: -18px;">累计完成百分比：</label>\n' +
                            '                    <div class="layui-input-inline">\n' +
                            '                        <span id="allpro">' + res.object + '</span>\n' + '%' +
                            '                    </div>\n' +
                            '                </div>\n' +
                            '            </div>\n' +
                            '           <div class="layui-form-item" style="margin-bottom:0px">\n' +
                            ...
                            '                       <img src="../img/mg11.png" alt="">\n' +
                            '                       <span>
            <fmt:message code="email.th.addfile"/></span>\n' +
                            '                       <input type="file" multiple id="upload" data-url="/upload?module=plancheck" name="file">\n' +
                            '                   </a>\n' +
                            '<div class="progress" style="width: 200px;float: left;margin-left: 10px;margin-top: 2px;">\n' +
                            '<div class="bar" style="width: 0%;"></div>\n' +
        ...
        $(document).on('click', '#rightName', function () {
            ...
            $.ajax({
                ...
                dataType: 'json',
                type: 'get',
                data: {planItemId: planItemId, deptId: deptId},
                success: function (res) {
    ...
    //关键任务详情
    function detail(tgId) {
        $.get('/projectTarget/selectByTgId', {tgId: tgId}, function (res) {
            layer.open({
                type: 1,
                title: '关键任务详情',
                area: ['80%', '90%'],
                ...
                success: function () {
                    ...
                    var tableStr = '<table class="layui-table">' +
                        ...
                        '<tr><td class="td_title">中心责任部门负责人：</td><td>' + (data.dutyUserName || '').replace(/,$/, '') + '</td><td class="td_title">中心责任部门：</td><td>' + (data.mainCenterDeptName || '').replace(/,$/, '') + '</td></tr>' +
                        '<tr><td class="td_title">区域责任部门负责人：</td><td>' + (data.mainAreaUserName || '').replace(/,$/, '') + '</td><td class="td_title">区域责任部门：</td><td>' + (data.mainAreaDeptName || '') + '</td></tr>' +
                        '<tr><td class="td_title">总承包部责任部门负责人：</td><td>' + (data.mainProjectUserName || '').replace(/,$/, '') + '</td><td class="td_title">总承包部责任部门：</td><td>' + (data.mainProjectDeptName || '').replace(/,$/, '') + '</td></tr>' +
            ...
            yes: function (index, layero) {
                ...
                $.post('/projectTarget/selectByIds', {ids: ids}, function (res) {
                    if (res.flag) {
                        ...
                        $('#iframeName').contents().find('input[name="tgId"]').val(tgName)
                        $('#iframeName').contents().find('input[name="tgId"]').attr('tgId', tgId)
                        layer.close(index)
                    }
                })
            }
    ...
    function delReport(me) {
        ...
        layer.confirm(
            "确认要删除吗，删除后不能恢复", {title: "删除确认"}, function (res) {
                if (res) {
                    var loadIndex = layer.load();
                    $.post('/ProjectDaily/delProjectDaily', {dailyId: me.attr('dailyId')}, function (res) {
