...
<script>
    ...
    var leftArr=[]
    //获取父页面的关联关键任务id
    if(parent.$('#newOrEditForm input[name="tgId"]').attr('tgId')){
        var tgIds=parent.$('#newOrEditForm input[name="tgId"]').attr('tgId').split(',')
    ...
    layui.use(['eleTree','form','element'], function(){
        ...
        function projectLeft() {
            //获取所选子项目的最高级项目的id
            var projectId=parent.$('.eleTree-node-content-active').parents('.eleTree-node[eletree-floor="0"]').attr('data-ids')
            if (!projectId) {
            ...
            eleTree.render({
                elem: '.ele1',
                url: '/projectTarget/selectByProjectId?projectId='+projectId,
