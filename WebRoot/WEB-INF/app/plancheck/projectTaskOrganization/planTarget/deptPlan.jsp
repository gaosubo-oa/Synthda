		...
		<div class="container">
			...
			<%--	<div class="layui-form-item layui-col-xs2">
				...
				<div class="layui-form-item layui-col-xs3">
					<label class="layui-form-label" style="width: 95px">关键任务类型:</label>
					<div class="layui-input-block" style="margin-left: 130px">
<%--						<div id="tgType" class="xm-select-demo"></div>--%>
							<input type="text" name="tgType" id="tgType_query" placeholder="请选择" readonly style="background-color: #e7e7e7;cursor: pointer;display: inline-block;width: 77%" class="layui-input">
				...
				<div class="layui-col-xs2 query_button_group" style="display: none;">
					<button type="reset" id="resetbtn" class="layui-btn layui-btn-sm query_button">重置</button>
					<button type="button" id="selectBtn" class="layui-btn layui-btn-sm query_button">查询</button>
				</div>
			</form>
...
<%--					{{#  } }}--%>
		...
		<script type="text/javascript">
            ...
            var dictionaryStr = 'PBAG_TYPE,PLAN_SYCLE,CONTROL_LEVEL,TG_TYPE,PLAN_PHASE,CGCL_TYPE,UNIT';
			var tgTypeData=[]
            // 获取数据字典数据
                    ...
                    for (var dict in dictionaryObj) {
                        ...
                        if (res.object[dict]) {
                            res.object[dict].forEach(function (item) {
								...
								if(dict=='TG_TYPE'){
									tgTypeData.push({name:item.dictName,value:item.dictNo})
								}
                            });
                        }
                    }
            ...
            // 初始化页面
            function init() {
                layui.use(['form', 'eleTree', 'laydate', 'table','xmSelect'], function () {
                    var form = layui.form,
                    ...
                    // $('.query_form select[name="controlLevel"]').append(dictionaryObj['CONTROL_LEVEL']['str']);
                    ...
                    eleTree.on("nodeClick(projectLeft)", function (d) {
                        ...
                        $('#leftId').attr('mainCenterDept',currentData.deptId);
                        $('#leftId').attr('mainCenterDeptName',currentData.contractorName);
                        $('#leftId').attr('projOrgId',currentData.projOrgId);
                        if (currentData.orgType=='01' || currentData.orgType=='02' || currentData.orgType=='03' || currentData.orgType=='04') {
							if (currentData.isPermission == 1) {
								$('.tishi').hide();
								$('.table_box').show();
								$('#leftId').val(currentData.projOrgId);
								$('#leftId').attr('dept_name', currentData.contractorName);
								tableShow(currentData.projOrgId);
							} else {
								$('.tishi').show(0, function(){
                    ...
                    table.on('toolbar(demoTreeTb)', function (obj) {
                        ...
                        switch (obj.event) {
                            ...
                            case 'report':
								...
								var projectName = $('#leftId').attr('dept_name');
                                reportPlan(projOrgId,tgId,projectName);
                                break;
                            // 修编
                            case 'revision':
                                if (checkStatus.data.length === 0) {
							...
							//导出
							case 'export':
								// table.exportFile('demoTreeTb', exportData,'xls');
								var page=insTb.config.page.curr
								var limit=insTb.config.page.limit
								window.location.href = '/projectTarget/targetByDept?page='+page+'&limit='+limit+'&projOrgId='+$('#leftId').val()+'&flag=1'+'&out=1'
								break;
                        }
                    });
                                ...
                                layer.open({
		                            ...
		                            success: function () {
		                                ...
		                                var tableStr = '<table class="layui-table">' +
									                            ...
									                            '<tr><td class="td_title">关键任务名称：</td><td colspan="3">'+(data.tgName || '')+'</td></tr>' +
									                            ...
									                            '<tr><td class="td_title">成果提交人：</td><td>'+(data.dutyUserName || '').replace(/,$/, '')+'</td><td class="td_title">责任部门：</td><td>'+(data.mainCenterDeptName || '').replace(/,$/, '')+'</td></tr>' +
									                            ...
									                            '<tr><td class="td_title">成果标准模板：</td><td colspan="3">'+function(){if(!!data.resultDict){
										...
										return resultDictName.replace(/,$/, '');
				                                                ...
				                                                '<tr><td class="td_title">关键任务描述：</td><td>'+(data.tgDesc || '')+'</td><td class="td_title">提前几天提醒：</td><td>'+function () {
					...
					//监听行双击事件
					table.on('rowDouble(demoTreeTb)', function(obj){
                    ...
                    $('#selectBtn').on('click', function () {
                        if (insTb) {
							...
							// data.tgType = tgTypeSelect.getValue('valueStr');
							data.tgType=$('#tgType_query').attr('dictNo') ? $('#tgType_query').attr('dictNo').replace(/,$/,'') : ''
                            var configWhere = insTb.config.where;
                            insTb.reload({
                    ...
                    function tableShow(projOrgId) {
                        insTb = table.render({
                            ...
                            cols: [[ //表头
								...
								, {field: 'examineStatus', title: '状态', width: 100,templet: function (d) {
                                ...
                                , {field: 'tgName', title: '关键任务名称', event: 'tgDetail', width: 300, templet: function(d){
                                        return '<span style="color: blue;cursor: pointer;">'+d.tgName+'</span>'
		                            }}
                                , {
                                ...
                                , {field: 'designQuantity', title: '设计量', width: 150}
                                , {field: 'itemQuantity', title: '工程量', width: 90}
                                , {field: 'itemQuantityNuit', title: '单位', width: 100}*/
                                , {field: 'dutyUserName', title: '成果提交人', width: 130}
                                // , {field: 'assistUserName', title: '协作人', width: 130}
                                , {field: 'mainCenterDeptName', title: '责任部门', width: 200}
                              /*  , {field: 'assistCompanyDeptName', title: '协作部门', width: 200,templet: function (d) {
										return d.assistCompanyDeptName || ''
									}}*/
                    ...
                    function addOrEdit(type, data) {
                        ...
                        if (type === 1) {
                            title = '新增关键任务';
                            url = '/projectTarget/add?deptOrProject=1';
                        } else if (type === 2) {
                            title = '编辑关键任务';
                        ...
                        layer.open({
                            ...
                            content: '<form class="layui-form" id="newOrEditForm" lay-filter="deptTargetForm">' +
                             ...
                             /*   '<div class="layui-row">' +
                              ...
                              /!*  '<div class="newAndEdit layui-col-xs6"> <div class="layui-form-item" >\n' +
                                ...
                                '    <div class="layui-input-block">\n' +
                                ' <select name="planSycle" lay-verify="required" class="jinyong" disabled>\n' +
                                '      </select>' +
                                '    </div>\n' +
                                '  </div></div>' +
                                '<div class="newAndEdit layui-col-xs6">  <div class="layui-form-item">\n' +
                               ...
                               /* ' <select name="tgType" lay-verify="required" class="jinyong">\n' +
									'<option value="">请选择</option>'+
                                '      </select>' +*/
									'<input type="text" name="tgType" id="tgType_add" readonly title="关键任务类型" style="background:#e7e7e7;width:83%;display:inline-block" autocomplete="off" class="layui-input testNull" >\n' +
                            ...
                            success: function () {
                                ...
                                $('#targetBelongDept', $('#newOrEditForm')).val($('#leftId').val());
                                ...
                                if (type === 2) {
									...
									$('#tgType_add').val(dictionaryObj['TG_TYPE']['object'][data.tgType]);
                            ...
                            yes: function (index) {
								...
								obj.planSycle = $('#newOrEditForm select[name="planSycle"]').val()
								obj.tgType = $('#newOrEditForm [name="tgType"]').attr('dictNo') ? $('#newOrEditForm [name="tgType"]').attr('dictNo').replace(/,$/,'') : ''
                    ...
                    function revisionUpdate(tgId, updateData) {
                        $.post('/projectTarget/updatePlus', $.extend({}, {tgId: tgId}, updateData), function (res) {
                            if (res.flag) {
                                layer.msg('修改成功！', {icon: 1, time: 1000});
                                insTb.config.where._ = new Date().getTime();
                                insTb.reload();
                     ...
                     */
                    function reportPlan(projOrgId,tgId,projectName){
                        layer.open({
	                        type: 1,
	                        title: '计划提交',
	                        btn: ['保存', '退出'],
			                               ...
			                               /* '<div class="layui-col-xs4">' +
			                                ...
			                                '<div class="layui-col-xs6">' +
			                                    '<div class="layui-form-item">' +
					                                ...
					                                '<div class="layui-input-block">' +
			                                            '<input type="text" name="dutyUser" readonly id="reportDutyUser" class="layui-input" style="display: inline-block;background-color: #e7e7e7;">' +
	                        ...
	                        success: function(){
								...
								$.get('/plcOrg/queryId', {projOrgId:$('#leftId').attr('projOrgId')}, function (res) {
									if(res.object){
                                ...
                                if (!isLock) {
                                    $.post('/plcPlan/addPlcPlan', obj, function(res){
                                        ...
                                        if (res.flag) {
                                            ...
                                            } else if (res.msg.indexOf('请重新提交并选择符合的职能关键任') > -1) {
						...
						layer.open({
							...
							success: function () {
								...
								var targetModuleTree = eleTree.render({
									...
									url: '/TemplateType/findAllTypeByTplType?tplType=557',
									highlightCurrent: true,
									showLine: true,
									request: {
										name: "typeName", // 显示的内容
							...
							yes: function (index) {
								...
								if (tgIdsArr.length > 0) {
									...
									obj.mainCenterDept =  $('#leftId').attr('mainCenterDept');

									$.post('/projectTarget/initializeDept', obj, function (res) {
										layer.close(loadingIndex);
										if (res.flag) {
											layer.close(index);
			...
			$(document).on('input propertychange', '.number_input', function (event) {
				...
				var value =$(this).val();
				// 判断是否为难度系数
				var eleClass = $(this).hasClass('hardDegree');
				if (isNaN(value)) {
					$(this).val('');
				} else {
					// 难度系数最大值为10
			...
			//前置关键任务的所有关键任务
			function allTarget(year,month,deptId){
				var allTarget=''
				$.ajax({
