...
<script type="text/javascript">
    ...
    function close_window(){
        ...
        if($("#PC_addfriendBOX" , parent.document).length != 0){
            ...
            //判断选择的人是否是本部门的人
            $.get('/user/CheckDeptOtherCoincident',{userIds:selectItemsId.replace(/,$/,'')},function (res) {
                //res为1时可选择，为0时不可选择
                if(res=='1'){
                     if(parent.opener && parent.opener.user_id){
                                  try {
...
//			点击部门显示本部门及子部门人员
        function buildNewUserList(id,text) {
            ...
            $.ajax({
                type:'get',
                url:'/department/getNewChDeptBai?deptId=' + id,
                dataType:'json',
                success: function (res) {
                    var tr = [];
                    var num=0;
                    ...
                    if(res.flag){
                        if(data.users.length > 0){
                            for(var i=0;i<data.users.length;i++){
                                objStr+='<div class="block-right-item '+function () {
                                    if(user_id) {  //判断user_id是否存在
                                        ...
                                        for (var j = 0; j < user_idArr.length; j++) {
                                            if (data.users[i] != undefined&&data.users[i].userId == user_idArr[j]) {
                                                bool = true;
                                                break;
                                            }
                                        }
                                ...
                                }()+'" item_id="'+data.users[i].uid+'" userPrivName="'+function(){if(data.users[i].userPrivName==undefined){return ''}else{return data.users[i].userPrivName}}()+'" item_name="'+data.users[i].userName+'" id="'+data.users[i].userId+'" user_id="'+data.users[i].userId+'" uid="'+data.users[i].uid+'" title="'+data.users[i].userName+'"><span class="name">'+'['+check(data.users[i].deptName)+']&nbsp;'+check(data.users[i].userName)+''+function(){if(data.users[i].userPrivName==undefined){return ''}else{return data.users[i].userPrivName}}()+'<span class="status">'+function () {
...
//			点击各分组
        ...
        function dataList(data) {
            ...
            for(var i=0;i<data.length;i++){
                ...
                }()+'" item_id="'+data[i].uid+'" userPrivName="'+function(){if(data[i].userPrivName==undefined){return ''}else{return data[i].userPrivName}}()+'" item_name="'+data[i].userName+'" id="'+data[i].userId+'" user_id="'+data[i].userId+'" uid="'+data[i].uid+'" title="'+data[i].userName+'"><span class="name">'+'['+check(data[i].deptName)+']&nbsp;'+check(data[i].userName)+'&nbsp;'+function(){if(data[i].userPrivName==undefined){return ''}else{return data[i].userPrivName}}()+function () {
                    //增加显示辅助角色
