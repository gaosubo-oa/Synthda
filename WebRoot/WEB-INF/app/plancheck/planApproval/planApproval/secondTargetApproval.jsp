		...
		<div class="container">
			...
			<div class="layui-tab layui-tab-brief" lay-filter="reportTabs">
				...
				<ul class="layui-tab-title">
					<li class="layui-this" taskApproval="1">未审批计划(<span class="noApproved"></span>)</li>
					<li taskApproval="2">已审批计划</li>
				</ul>
							...
							<form class="layui-form query" style="padding: 5px 0;">
								...
								<div class="layui-row hide_query" style="display: none;padding: 5px 0;">
									...
									<div class="layui-form-item inputs layui-col-xs2">
										...
										<div class="layui-input-block">
											<input type="text" readonly placeholder="选择二级审批人" name="approvalUser" id="query_approvalUser" class="layui-input"
												   style="cursor: pointer;background-color: #e7e7e7;"/>
										</div>
									</div>
								</div>
		...
		<script type="text/html" id="childDemo">
			<div>
				...
				<span class="child_opt_box">
					...
					{{# } }}
					{{# if(authorityObject && authorityObject['33']){ }}
					<a class="layui-btn layui-btn-sm " lay-event="agree" style="float: right;margin-left: 10px;">同意</a>
					{{# } }}
					<a class="layui-btn layui-btn-sm targetButton" lay-event="revisionDetail" style="float: right;">修编详情</a>
					<a class="layui-btn layui-btn-sm" lay-event="delete" style="float: right;margin-left: 10px;">删除</a>
		...
		<script type="text/javascript">
            ...
            function init() {
                layui.use(['element', 'form', 'table', 'treeTable', 'soulTable', 'laydate', 'treeSelect', 'eleTree', 'dropdown'], function () {
					...
					$('#query_approvalUser').on('click',function () {
						user_id = 'query_approvalUser';
						$.popWindow('/common/selectUser?0');
					});
                    ...
                    $('.search_one').on('click',function () {
						...
						if($('#query_approvalUser').attr('user_id')){
							data['approvalUser'] = $('#query_approvalUser').attr('user_id').replace(/,$/, '')
						}else{
							data['approvalUser'] = '';
						}

                        var taskApproval = $('.layui-this').attr('taskApproval')
                        initReportedTable(taskApproval, data);
                    });
					...
					$('#resetBtn').on('click',function () {
						...
						$('#query_approvalUser').attr('user_id','')
					})
                    ...
                    element.on('tab(reportTabs)', function (data) {
                        ...
                        switch (data.index) {
                            case 0:
                                ...
                                initReportedTable(taskApproval);
                                $('.isAgree').show();
                                $('#agreeStatus').val(4);
                                break;
                            case 1:
                    ...
                    initReportedTable($('.layui-this').attr('taskApproval'));

                    function initReportedTable(taskApproval, searchData) {
                        var whereData = {
                            targetOrItem: 1,
                            apprivalStatus: 1,
                            agreeStatus: 2,
                            ...
                            reportedTable = table.render({
                                ...
                                cols: [[ //表头
                                    {type: 'checkbox'},
                                    {
                                        ...
                                        , templet: function (d) {
                                            ...
                                            return '<a projectId=' + d.projectId + ' style="' + style + '">' + d.planName + function () {
                                                var str = ''
                                                if (d.revisionYn == 1) {
                                                    str += '<span style="color: red">(修编审批)</span>'
                                                }
                                                if(d.modify==1){
													str += '<span style="color: red">(修编)</span>'
												}
                                                return str;
                                            }() + '</a>'
                                        }
                                  ...
                                  /*  {
                                    ...
                                    {field: 'approvalUserName', title: '审批人',width:100},
                        ...
                        if (type == 2) {
                            layer.confirm('确定同意审批吗？', function (index) {
                                ...
                                $.ajax({
                                    ...
                                    dataType: 'json',
                                    type: 'post',
                                    data: {planIds: planIds, taskApproval: 2,secondApprovalDate:today},
                                    success: function (res) {
                                        layer.close(index);
                                        if (res.flag) {
											$('.theChildBox').hide();
                            ...
                            layer.open({
                                ...
                                yes: function (index) {
                                    ...
                                    $.post('/plcPlan/updateStatusByIds', {planIds: planIds, taskApproval: 3,secondApprovalDate:today}, function (res) {
                                        if (res.flag) {
                                            layer.close(index);
