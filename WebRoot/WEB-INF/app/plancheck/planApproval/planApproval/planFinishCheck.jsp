                        ...
                        if (type == 2) {
                            layer.confirm('确定同意审批吗？', function (index) {
                                ...
                                $.ajax({
                                    ...
                                    dataType: 'json',
                                    type: 'post',
                                    data: {planIds: planIds, agreeStatus: type},
                                    success: function (res) {
                                        layer.close(index);
                                        if (res.flag) {
                            ...
                            layer.open({
                                ...
                                yes: function (index) {
                                    ...
                                    $.post('/plcPlan/updateStatusByIds', {planIds: planIds, agreeStatus: type}, function (res) {
                                        if (res.flag) {
                                            layer.close(index);
                    ...
                    function editDeptTarget(data, planClass, planId, type) {
                        ...
                        layer.open({
                            ...
                            content: ['<form class="layui-form" id="deptTargetForm" style="padding-right: 20px;" lay-filter="deptTargetForm">',
                                ...
                                '    <label class="layui-form-label"><span class="required_field" field_name="tgNo">*</span>编号</label>\n' +
                                '    <div class="layui-input-block" style="position: relative; padding-right: 100px;">\n' +
                                '      <input type="text" name="tgNo" readonly autocomplete="off" class="layui-input jinyong" style="background:#e7e7e7;">\n' +
                                '<a class="refresh_no" href="javascript:;" style="position: absolute;top: 8px;right: 35px;visibility: hidden;color: #1e9fff;">刷新</a>' +
                                '    </div>\n' +
                                '  </div>' +
                                '  </div>' +
                                '</div>' +
                                    ...
                                    layer.open({
                                        ...
                                        success: function () {
                                            ...
                                            $('.addPreTask').(function () {
                                                var str = '    <tr class="trTask">\n' +
                                                    '      <td>' +
                                                    '  <select naclickme="workTargetName" lay-verify="required">\n' +
                                                    function () {
                                                        var year = $('#targetSet select[name="year"]').val() || ''
                                                        var month = $('#targetSet select[name="month"]').val() || ''
                    ...
                    function editPlanTask(data, planClass, planId, type) {
                        ...
                        layer.open({
                            ...
                            content: ['<form class="layui-form" id="planTaskForm" lay-filter="planTaskForm" style="padding-right: 10px;">',
                                ...
                                '  </div>' +
                                ' <div class="newAndEdit layui-col-xs6"> <div class="layui-form-item">\n' +
                                '    <label class="layui-form-label">子任务类型<span style="color:red;font-size: 25px;vertical-align: middle;">*</span></label>\n' +
                                '    <div class="layui-input-block">\n' +
                                ' <select name="taskType" lay-verify="required" class="jinyong testNull" title="子任务类型">\n' +
                                '      </select>' +
                                '    </div>\n' +
                                '  </div> </div>' +
                                '</div>' +
                                ...
                                '    <label class="layui-form-label">责任部门</label>\n' +
                                '    <div class="layui-input-block">\n' +
                                '  <textarea  type="text" title="责任部门" name="mainCenterDept" id="mainCenterDept" readonly  style="background:#e7e7e7;height: 45px;width: 83%;text-indent:1em;border-radius: 4px;"></textarea>\n' +
                                '<a href="javascript:;" style="color:#1E9FFF;margin-left:10px" chooseNum="1" class="deptAdd">添加</a>\n' +
                                ' <a href="javascript:;" style="color:#1E9FFF;margin-left:5px" class="deptDel">清空</a>\n' +
                                '    </div>\n' +
                                '  </div> </div>' +
