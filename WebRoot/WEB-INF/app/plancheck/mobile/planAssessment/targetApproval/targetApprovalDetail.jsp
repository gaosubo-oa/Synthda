...
<html>
	...
	<body>
		<div id="app">
				...
				<template v-if="planInfo">
					...
					<div class="con_body">
						<template v-if="listData.length > 0">
							<template v-if="planInfo.planClass == 1">
								<tree-list :datas='listData'>
											...
											<div class="list_content">
												<van-row>
													...
													<van-col span="14">
														<p class="list_p p_con">
															<span>任务类型：</span><span
															v-text="dictionaryObj['TG_TYPE'][item.tgType]"></span>
														</p>
													</van-col>
												</van-row>
												<van-row>
			...
			<van-action-sheet v-model="taskDetailModule.sheetShow" title="任务详情" :round="false" :duration="0.2">
				<div class="sheet_content">
					<div class="task_detail" v-if="taskDetailModule.taskDetail">
						<van-row>
							<van-col span="8">关键任务编号:</van-col>
							<van-col span="16" v-text="isShowNull(taskDetailModule.taskDetail.tgNo)"></van-col>
						</van-row>
						<van-row>
							<van-col span="8">关键任务名称:</van-col>
							<van-col span="16" v-text="isShowNull(taskDetailModule.taskDetail.tgName)"></van-col>
						</van-row>
						<van-row>
							<van-col span="8">关注等级:</van-col>
							<van-col span="16" v-text="isShowNull(dictionaryObj['CONTROL_LEVEL'][taskDetailModule.taskDetail.planSycle])"></van-col>
						</van-row>
						<van-row>
						...
						<van-row>
							<van-col span="8">关键任务类型:</van-col>
							<van-col span="16" v-text="isShowNull(dictionaryObj['TG_TYPE'][taskDetailModule.taskDetail.tgType])"></van-col>
						</van-row>
						<van-row>
							<van-col span="8">计划阶段:</van-col>
							<van-col span="16" v-text="isShowNull(dictionaryObj['PLAN_PHASE'][taskDetailModule.taskDetail.planStage])"></van-col>
						</van-row>
						<van-row>
							<van-col span="8">设计量:</van-col>
							<van-col span="16" v-text="isShowNull(taskDetailModule.taskDetail.designQuantity)"></van-col>
						</van-row>
						<van-row>
							<van-col span="8">工程量:</van-col>
							<van-col span="16" v-text="isShowNull(taskDetailModule.taskDetail.itemQuantity)"></van-col>
						</van-row>
						<van-row>
							<van-col span="8">单位:</van-col>
							<van-col span="16" v-text="isShowNull(dictionaryObj['UNIT'][taskDetailModule.taskDetail.itemQuantityNuit])"></van-col>
						</van-row>
						<van-row>
						...
						<van-row>
							...
							<van-col span="16" v-text="isShowNull(taskDetailModule.taskDetail.mainAreaDeptName).replace(/,$/, '')"></van-col>
						</van-row>
						<van-row>
							<van-col span="8">总承包部责任部门:</van-col>
							<van-col span="16" v-text="isShowNull(taskDetailModule.taskDetail.mainProjectDeptName).replace(/,$/, '')"></van-col>
						</van-row>
						<van-row>
							<van-col span="8">难度系数:</van-col>
							<van-col span="16" v-text="isShowNull(taskDetailModule.taskDetail.hardDegree)"></van-col>
						</van-row>
						<van-row>
			...
			<van-action-sheet v-model="backModule.backSheetShow"
				...
				<div class="sheet_content">
					<van-cell-group title="退回原因">
						...
						<van-cell title="选择问题关键任务" is-link @click="backModule.checkBoxShow = true"></van-cell>
		...
		<script>
			...
			Vue.component('tree-list', {
                ...
                methods: {
                    ...
                    getShow: function (item) {
                        let flag = false
                        if (item.tgName || (!!item.children && item.children.length > 0)) {
                            flag = true
                        }
                        return flag
                    },
			...
			let app = new Vue({
                ...
                data: {
                    ...
                    planInfo: null,
	                listData: [],
                    dictionaryStr: 'PLAN_SYCLE,TG_TYPE,CONTROL_LEVEL,PLAN_PHASE,UNIT',
                    dictionaryObj: {
                        PLAN_SYCLE: {},
                        TG_TYPE: {},
                        CONTROL_LEVEL: {},
                ...
                methods: {
	                ...
	                backClick: function () {
                        ...
                        $.post('/plcPlan/updatePlcPlan', data, function (res) {
                        ...
                        $.post('/plcPlan/updateStatusByIds', {planIds: _this.planId, agreeStatus: 3,approvalDate: today}, function (res) {
                            toastIndex.close()
                            if (res.flag) {
                                _this.$notify({
