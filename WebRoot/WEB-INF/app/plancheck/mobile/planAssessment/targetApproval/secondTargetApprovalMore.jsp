					...
					<van-row type="flex" justify="space-around">
						<van-col span="5">
							<van-dropdown-menu active-color="#1989fa" style="margin-top: 8px; overflow: hidden;">
								<van-dropdown-item v-model="searchModule.taskApproval" :options="searchModule.taskApprovalOption" @change="statusChange"/>
							</van-dropdown-menu>
						</van-col>
						<van-col span="14">
							<span class="ant-input-affix-wrapper">
								<input v-model="searchModule.planName" placeholder="任务名称" type="text" class="ant-input">
							</span>
						</van-col>
											...
											<van-row>
												<van-col span="24">
													<p class="list_p" style="display: flex;">
														...
														<span class="p_title" :style="{ color: item.agreeStatus == 3 || item.taskApproval == 3 ? '#f00' : '#000' }"
											...
											<van-row>
												<van-col span="12">
													<p class="list_p p_con">
														<span>一级审批：</span><span v-html="getAgreeStatus(item.agreeStatus, item.apprivalStatus)"
														                        @click.stop="agreeStatusClick(item.agreeStatus, item.returnComments, item.questionName, item.planId)"></span>
													</p>
												</van-col>
												<van-col span="12">
													<p class="list_p p_con">
														<span>二级审批：</span><span v-html="getTaskApproval(item.taskApproval)"
														                        @click.stop="agreeStatusClick(item.taskApproval, item.returnComments, item.questionName, item.planId)"></span>
													</p>
												</van-col>
											</van-row>
											<van-row>
                ...
                methods: {
                    ...
                    getListData: function (callback) {
                        ...
                        let searchData = {
                            ...
                            apprivalStatus: 1,
                            agreeStatus: 2,
                            taskApproval: _this.searchModule.taskApproval,
                            planName: _this.searchModule.planName
                        }

                        $.get('/plcPlan/query2', searchData, function (res) {
                            callback(res)
                        })
                    },
                    onLoad: function () {
                    ...
                    listClick: function (planId) {
                        let url = '/plcPlan/mSecondTargetApprovalDetail?planId=' + planId

                        if (( /(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent) )) {
                            try {
                                window.webkit.messageHandlers.formInfoUrlLink.postMessage({'url': encodeURI(url), 'name': '关键任务二级审批'});
                            } catch (err) {
                                formInfoUrlLink(encodeURI(url), '关键任务二级审批');
                            }
                        } else if (/(Android)/i.test(navigator.userAgent)) {
                            // Android.formInfoUrlLink(encodeURI(url), '关键任务二级审批');
                            // 上面方式跳转页面无法下载文件
