            ...
            let app = new Vue({
                ...
                methods: {
                    ...
                    listClick: function (planId) {
                        let url = '/plcPlan/mTargetApprovalDetail?planId=' + planId

                        if (( /(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent) )) {
                            try {
                                window.webkit.messageHandlers.formInfoUrlLink.postMessage({'url': encodeURI(url), 'name': '关键任务审批'});
                            } catch (err) {
                                formInfoUrlLink(encodeURI(url), '关键任务审批');
                            }
                        } else if (/(Android)/i.test(navigator.userAgent)) {
                            // Android.formInfoUrlLink(encodeURI(url), '关键任务审批');
                            // 上面方式跳转页面无法下载文件
