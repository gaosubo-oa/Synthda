					...
					<van-row type="flex" justify="space-between">
						...
						<van-col span="14">
							<span class="ant-input-affix-wrapper">
								<input v-model="searchModule.tgName" placeholder="关键任务名称" type="text" class="ant-input">
							</span>
						</van-col>
									...
									<div class="card_content">
											...
											<van-row>
												<van-col span="24">
													<p class="list_p" style="display: flex;">
														<span class="p_type" v-text="'[关键任务]'"></span>
														<span class="p_title" v-text="item.tgName"></span>
													</p>
												</van-col>
											</van-row>
										</div>
										<div class="list_content">
											...
											<van-row>
												<van-col span="12">
													<p class="list_p p_con">
														<span>任务类型：</span><span v-text="dictionaryObj['TG_TYPE'][item.tgType] || ''"></span>
													</p>
												</van-col>
												<van-col span="12">
													<p class="list_p p_con">
            ...
            let app = new Vue({
                ...
                methods: {
                    ...
                    listClick: function (scoreId) {
                        let url = '/plcPlan/mTargetAssessment?scoreId=' + scoreId

                        if (( /(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent) )) {
                            try {
                                window.webkit.messageHandlers.formInfoUrlLink.postMessage({'url': encodeURI(url), 'name': '目标考核'});
                            } catch (err) {
                                formInfoUrlLink(encodeURI(url), '目标考核');
                            }
                        } else if (/(Android)/i.test(navigator.userAgent)) {
                            // Android.formInfoUrlLink(encodeURI(url), '目标考核');
                            // 上面方式跳转页面无法下载文件
