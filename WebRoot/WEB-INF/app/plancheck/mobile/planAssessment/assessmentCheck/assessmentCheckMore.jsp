					...
					<van-row type="flex" justify="space-between">
						<van-col span="5">
							<van-dropdown-menu active-color="#1989fa" style="margin-top: 8px; overflow: hidden;">
								<van-dropdown-item v-model="searchModule.groupApprivalStatus" :options="searchModule.checkOption" @change="statusChange"/>
							</van-dropdown-menu>
						</van-col>
						<van-col span="14">
									...
									<div class="card_content">
											...
											<van-row>
												<van-col span="24">
													<p class="list_p" style="display: flex;">
														...
														<span class="p_title" style="flex: 1;" v-text="item.taskName || item.tgName"></span>
													</p>
												</van-col>
											</van-row>
										</div>
										<div class="list_content">
											<van-row>
												...
												<van-col span="12">
													<p class="list_p p_con">
														<span>任务来源：</span><span v-text="getAssistanceStatus(item)"></span>
													</p>
												</van-col>
											</van-row>
											<van-row>
												...
												<van-col span="12">
													<p class="list_p p_con">
														<span>小组审批状态：</span><span v-text="checkStatus(item.groupApprivalStatus)"></span>
													</p>
												</van-col>
											</van-row>
											<van-row>
                ...
                methods: {
                    ...
                    getListData: function (callback) {
                        ...
                        let searchData = {
                            ...
                            apprivalStatus: 1,
                            groupApprivalStatus: _this.searchModule.groupApprivalStatus,
                            tgName: _this.searchModule.tgName
                        }

                        $.get('/PlcAssessScore/findAssessmentData', searchData, function (res) {
                            callback(res)
                        })
                    },
                    onLoad: function () {
                    ...
                    statusChange: function () {
                        this.searchModule.taskName = ''
                        this.search()
                    },
                    listClick: function (socoreId) {
                        let url = '/plcPlan/mAssessmentCheck?scoreId=' + socoreId;

                        if (( /(iPhone|iPad|iPod|iOS)/i.test(navigator.userAgent) )) {
                            try {
                                window.webkit.messageHandlers.formInfoUrlLink.postMessage({'url': encodeURI(url), 'name': '考核审核'});
                            } catch (err) {
                                formInfoUrlLink(encodeURI(url), '考核审核');
                            }
                        } else if (/(Android)/i.test(navigator.userAgent)) {
                            // Android.formInfoUrlLink(encodeURI(url), '考核审核');
                            // 上面方式跳转页面无法下载文件
