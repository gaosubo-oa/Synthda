...
<html>
	...
	<body>
		<div class="container" style="padding-top: 15px;box-sizing: border-box">
			<input type="hidden" id="leftDeptId">
			...
			<div class="content">
				<div style="padding: 0px 20px;" class="clearfix">
					<div class="con_left">
						<!-- 左侧部门 -->
						<div class="eleTree left_dept_tree" style="margin-top: 10px;" lay-filter="leftDeptTree"></div>
					</div>
					<div class="con_right">
						...
						<div class="table_box" style="display: none;">
							<div style="padding: 0px 8px">
								<table id="departmentTime" lay-filter="departmentTime"></table>
							</div>
						</div>
					</div>
				</div>
		...
		<%-- 子任务工具条 --%>
		<script type="text/html" id="sectionBtn">
		...
		<script type="text/html" id="toolbarDemo">
			<div class="layui-btn-container" style="text-align: right">
				{{# if(authorityObject && authorityObject['17']){ }}
				<button class="layui-btn layui-btn-sm save_user" lay-event="save" style="margin: 0">保存</button>
				{{# } }}
			</div>
		</script>
		...
		<script type="text/html" id="checkboxTpl">
			<!-- 这里的 checked 的状态只是演示 -->
			<input type="checkbox" name="sex" value="{{d.userId}}" {{d.assessment ? 'checked' : ''}} {{authorityObject && authorityObject['17'] ? '' : 'disabled'}} lay-skin="switch" lay-text="是|否" lay-filter="assessmentUser" />
		</script>
		...
		<script type="text/javascript">
            ...
            $('.con_left').height($(window).height()-200)
			window.onresize=function (){
				$('.con_left').height($(window).height()-200)
			}

            layui.use(['table', 'eleTree', 'layer', 'form'], function () {
                var table = layui.table,
						layer = layui.layer,
                    eleTree = layui.eleTree,
					form= layui.form;

                var leftListData = null;
                ...
                $.get('/plcOrg/getTreeListByLoginer', function (res) {
                    if (res.flag) {
                        ...
                        leftListData = getListData(res.obj, {});
                    }
                });
				...
				form.on('select(planYear)', function (data) {
					var deptId = $('#leftDeptId').val();
					timeTable(deptId, data.value)
				});

                // 节点点击事件
                eleTree.on("nodeClick(leftDeptTree)", function (d) {
                    ...
                    if (currentData.deptParent == 0) {
                        ...
                        $('.tishi').hide();
                        $('.table_box').show();
                        $('#leftDeptId').val(currentData.deptId);
						timeTable(currentData.deptId, $('.search_module [name="planYear"]').val())
                    }
                });
				/**
				 * 按时间显示表格
				 * @param  id(左侧部门id)
				 */
				function timeTable(deptId, year){
					table.render({
						elem: '#departmentTime',
						url: '/StatisticalReport/getDataByDeptId',
						where: {
							deptId: deptId,
							year: year
						},
						...
						cols: [[
							...
							{field: 'periodYear', title: '年度', align: 'center',templet:function (d) {
									return  d.periodYear+'年'
								}},
							{field: 'count', title: '月度', align: 'center',templet:function (d) {
									return  d.count+'月'
								}},
                            {
                                field: 'comprehensiveScore', title: '综合得分', align: 'center', templet: function (d) {
                                    if (d.comprehensiveScore == '/') {
                                        return d.comprehensiveScore
                                    } else {
                                        return d.comprehensiveScore + '分'
                                    }
                                }
                            },
							{field: 'comprehensiveScore', title: '考核人数', align: 'center',templet:function (d) {
									return  d.num+'人'
								}},
							...
							{title: '明细', align: 'center', toolbar: '#sectionBtn'}
						]],
                ...
                // 监听时间列表工具条
                table.on('tool(departmentTime)', function (obj) {
                    var data = obj.data;
                    var layEvent = obj.event;

                    var leftDeptId = $('#leftDeptId').val();

                    if (layEvent === 'projectDetail') {//时间下------查看明细
                    	var checkedUser = []
						layer.open({
							type: 1,
							title: '明细',
							area: ['100%', '100%'],
							content: '<div style="padding: 8px"><table id="timeDetail" lay-filter="timeDetail"></table></div>',
							success: function(){
								table.render({
									elem: '#timeDetail',
									url: '/StatisticalReport/getDataByDeptIdAndMonth?deptId='+leftDeptId+'&month='+data.count+'&year='+data.periodYear,
									// page: true,
									toolbar: '#toolbarDemo',
									defaultToolbar: [''],
									cols: [[
									...
									parseData: function (res) { //res 即为原始返回的数据
										...
										$('.save_user').on('click', function(){
											...
											$.post('/plcNoAssessUser/save', saveObj, function (res) {
												if (res.flag) {
								...
								form.on('switch(assessmentUser)', function(obj){
									var checked = obj.elem.checked
									if (checked) {
										checkedUser.push(this.value)
									} else {
										var index = checkedUser.indexOf(this.value)
										if (index > -1) {
				...
				table.on('tool(timeDetail)', function (obj) {
					var data = obj.data;
					var layEvent = obj.event;
                    var leftDeptId = $('#leftDeptId').val();
                    if (layEvent === 'personDetail'){//时间下-----个人明细
						layer.open({
							...
							success: function () {
								$.get('/StatisticalReport/personalAssessmentInfo', {adMonth: data.periodMonth,userId:data.userId, year: data.periodYear, deptId: leftDeptId}, function (res) {
									if (res.flag) {
                ...
                // 监听人员列表工具条
                table.on('tool(departmentPerson)', function (obj) {
                    var data = obj.data;
                    var layEvent = obj.event;
             ...
             * @returns {*}
             */
            function getListData(treeData, obj) {
                if (treeData.length > 0) {
                    return treeData.reduce(function (pre, cur) {
	                    var name = cur['contractorName'];
                        if (pre[cur['deptParent']]) {
                            name = pre[cur['deptParent']] + '-' + cur['contractorName'];
                        }

                        pre[cur['deptId']] = name;

                        if (cur['orgList'] && cur['orgList'].length > 0) {
                            return getListData(cur['orgList'], pre);
                        } else {
                            return pre;
                        }
                    }, obj);
