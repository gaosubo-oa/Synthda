	...
	<body>
		<div class="container">
			<div class="search_module">
				<form class="layui-form clearfix search_form">
					...
					<div class="layui-form-item query_item">
						...
						<div class="layui-input-block">
							<input type="text" id="targetBelongDepts" readonly placeholder="请选择" style="cursor: pointer; background-color: #e7e7e7;"
			...
			<div class="content">
				...
				<table id="deptTargetTable" lay-filter="deptTargetTable"></table>
			</div>
		</div>
		...
		<script type="text/javascript">
            ...
            $.get('/getLoginUser', function (res) {
                if (res.flag && res.object) {
                    $('#targetBelongDepts').val(res.object.deptName);
                    $('#targetBelongDepts').attr('deptid', res.object.deptId);
                }
            });
            ...
            var dictionaryStr = 'UNIT,CONTROL_LEVEL,TG_TYPE,PLAN_SYCLE,PLAN_PHASE,CGCL_TYPE,RENWUJIHUA_TYPE';
            // 获取数据字典数据
            ...
            function init() {
                layui.use(['form', 'table', 'xmSelect'], function () {
                    ...
                    // 选择所属部门
                    $('#targetBelongDepts').click(function () {
                        dept_id = 'targetBelongDepts';
                    ...
                    $('#searchBtn').click(function () {
                        ...
                        searchData.dutyUser = ($('#dutyUser').attr('user_id') || '').replace(/,$/, '');
                        // 所属部门
                        searchData.targetBelongDepts = ($('#targetBelongDepts').attr('deptid') || '').replace(/,$/, '');

                        loadPieModule(searchData);

                        deptTargetTable = table.render({
                            elem: '#deptTargetTable',
                            url: '/plcPlan/selectDeptTarReport',
                            ...
                            cols: [[
                                ...
                                // {field: 'tgNo', title: '编码', minWidth: 200, align: 'left'},
                                {
                                    field: 'tgName', title: '关键任务名称', minWidth: 400, align: 'left', templet: function (d) {
                                        return '<a style="width: 100%; color: blue; cursor: pointer; text-decoration: underline;" class="name_link" tgId="' + d.tgId + '">' + d.tgName + '</a>';
                                    }
                                },
                                // {
                                ...
                                //     }
                                // },
                                {field: 'targetBelongDeptName', title: '所属部门', minWidth: 150, align: 'center'},
                                {field: 'dutyUserName', title: '责任人', minWidth: 120, align: 'center'},
                                {
                                    field: 'controlLevel', title: '关注等级', minWidth: 120, align: 'center', templet: function (d) {
                                        if (!!d.controlLevel) {
                    ...
                    $('#resetBtn').click(function () {
                        ...
                        // 所属部门
                        $('#targetBelongDepts').val('');
                        $('#targetBelongDepts').attr('deptid', '');
                    ...
                    var deptTargetTable = null;
                    
                    table.on('toolbar(deptTargetTable)', function (obj) {
                        switch (obj.event) {
                            case 'pieChart':
            ...
            function loadPieModule(searchData) {
                ...
                // 获取数据
                $.get('/plcPlan/getTaskStatusCount?targetOrItem=target', searchData, function (res) {
                    myChart.hideLoading();
                    if (res.flag) {
