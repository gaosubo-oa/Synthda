...
<table class="TABLE" border="1" cellspacing="0" cellpadding="0" id="table" style="border-collapse: collapse;">
	...
	<tr>
		...
		<td class="files">
			...
			<form id="uploadimgform" style="float: left;" target="uploadiframe"  action=""  method="post">
        ...
        $.ajax({
            ...
            success:function(rsp){
				...
				if(data2.attachment.length > 0){   //附件显示
				    for(var i=0;i<arr.length;i++){
                        stra+='<div class="dech" deUrl="'+encodeURI(arr[i].attUrl)+'">' +
                            '<a href="/download?'+encodeURI(arr[i].attUrl)+'" NAME="'+arr[i].attachName+'*" style="text-decoration:none;">' +
				...
				if(status == 0){
                    ...
                    $('#senduser').val(data2.fromName1+',');
                    $('#senduser').attr('user_id',data2.fromId+',');
				}else if(status == 1){
				    if(data2.copyName != ''){
                        str1='<tr class="tian">' +
                            '<td><fmt:message code="main.th.CC" />：</td>'+
                    ...
                    $('#senduser').val(data2.fromName1+','+arrName.join(','));
                    $('#senduser').attr('user_id',data2.fromId+','+arrId.join(','));
				}
