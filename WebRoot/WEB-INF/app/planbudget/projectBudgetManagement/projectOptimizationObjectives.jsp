...
<%--        <script type="text/javascript" src="/js/common/fileupload.js"></script>--%>
	...
	<body>
		<div class="container">
			...
			<div class="wrapper">
				<div class="wrap_left">
					<h2 style="text-align: center;line-height: 35px;">项目优化目标</h2>
					<div class="left_form">
		...
		<script>
            ...
            var colShowObj = {
				...
				optTarAmount:{
					field:'optTarAmount',minWidth:110,title:'优化目标金额',sort:true,hide:false,templet:function(d){
						if(d.plbProjBudgetWithBLOBs&&d.plbProjBudgetWithBLOBs.optTarAmount){
							return d.plbProjBudgetWithBLOBs.optTarAmount;
						}else {
							return '0';
						}
					}
                ...
                // },
                // createUser: {
                //     field: 'createUser', title: '编制人', sort: true, hide: false, templet: function (d) {
            ...
            layui.use(['form', 'laydate', 'table', 'soulTable', 'eleTree'], function () {
                    ...
                    switch (obj.event) {
                        case 'del':
                            ...
                            checkStatus.data.forEach(function (v) {
								if (!(checkStatus.data[0].approvalStatusOpt > 0)) {
                        ...
                        case 'submit':
							...
							// approvalType 后台定义
							$.get('/plbProjBudget/judgeProjectApproval', {projId: projId, approvalType: '07'}, function(res) {
								...
								if (res.data) {
									...
									layer.open({
										...
										success: function () {
											// 数据字典  FLOW_SETTING
											$.get('/plbFlowSetting/getOwnFlowData', {plbDictNo: '13'}, function (res) {
												var flowData = []
										...
										yes: function (index) {
											...
											if (checkStatus.data.length > 0) {
												...
												// 数据字典 RELATION_APPROVAL_PAGE
												var urlParameter = 'projId=' + projId + '&approvalType=05'
                ...
                layTable.on('tool(tableObj)', function (obj) {
                    ...
                    if (layEvent === 'detail') {
                        iframeLayerIndex = layer.open({
                            ...
                            title: '查看详情',
                            area: ['100%', '100%'],
                            content: '/plbProj/projectBudgetDetail?budgetType=3&disabled=1&budgetItemId=' + _budgetItemId
                        });
                    } else if (layEvent === 'edit') {
						if (data.approvalStatusOpt > 0) {
							layer.msg('该项目已提交！', {icon: 0});
							return false;
						}
                ...
                function newOrEdit(type, data) {
                    ...
                    layer.open({
                        ...
                        content: ['<div class="layer_wrap" style="padding: 10px 15px;">',
                            ...
                            '<input type="text" readonly name="budgetNo" style="background-color: #e7e7e7;" autocomplete="off" class="layui-input">' +
                            '</div>' +
                            '</div>' +
                            '</div>',
							'<div class="layui-col-xs3" style="padding: 0 5px">' +
                            ...
                            '<div class="layui-input-block form_block">' +
                            '<input type="text" readonly name="budgetName" style="background-color: #e7e7e7;" autocomplete="off" class="layui-input">' +
                            '</div>' +
                            '</div>' +
                            '</div>',
							'<div class="layui-col-xs3" style="padding: 0 5px">' +
                            ...
                            '<div class="layui-input-block form_block">' +
                            '<input type="text" readonly name="incomeTarNum" style="background-color: #e7e7e7;" autocomplete="off" class="layui-input">' +
                            '</div>' +
                            '</div>' +
                            '</div>',
                            '<div class="layui-col-xs3" style="padding: 0 5px">' +
                            ...
                            '<div class="layui-input-block form_block">' +
                            '<input type="text" readonly name="incomeTarPrice" style="background-color: #e7e7e7;" autocomplete="off" class="layui-input">' +
                            '</div>' +
                            '</div>' +
                            '</div>',
	                        '<div class="layui-col-xs3" style="padding: 0 5px">' +
                            ...
                            '<div class="layui-input-block form_block">' +
                            '<input type="text" readonly name="incomeTarAmount" style="background-color: #e7e7e7;" autocomplete="off" class="layui-input">' +
                            '</div>' +
                            '</div>' +
                            '</div>',
                            '<div class="layui-col-xs3" style="padding: 0 5px">' +
                            ...
                            '<div class="layui-input-block form_block">' +
                            '<input type="text" readonly name="incomeTarExplain" style="background-color: #e7e7e7;" autocomplete="off" class="layui-input">' +
                            '</div>' +
                            '</div>' +
                            '</div>',
                            '</div>',
                            ...
                            '<div class="layui-col-xs3" style="padding: 0 5px">' +
                            '<div class="layui-form-item">' +
                            '<label class="layui-form-label form_label">优化目标数量<span field="optTarNum" class="field_required">*</span></label>' +
                            '<div class="layui-input-block form_block">' +
                            '<input type="text" name="optTarNum" autocomplete="off" minusFlag="1" pointNum="3" pointFlag="1" handleCallback="afterFloatChange" class="layui-input input_floatNum">' +
                            '</div>' +
                            '</div>' +
                            '</div>',
                            '<div class="layui-col-xs3" style="padding: 0 5px">' +
                            '<div class="layui-form-item">' +
                            '<label class="layui-form-label form_label">优化目标单价<span field="optTarPrice" class="field_required">*</span></label>' +
                            '<div class="layui-input-block form_block">' +
                            '<input type="text" name="optTarPrice" autocomplete="off" minusFlag="1" pointNum="3" pointFlag="1" handleCallback="afterFloatChange" class="layui-input input_floatNum">' +
                            '</div>' +
                            '</div>' +
                            '</div>',
	                        '<div class="layui-col-xs3" style="padding: 0 5px">' +
                            '<div class="layui-form-item">' +
                            '<label class="layui-form-label form_label">优化目标金额<span field="optTarAmount" class="field_required">*</span></label>' +
                            '<div class="layui-input-block form_block">' +
                            '<input type="text" readonly name="optTarAmount" autocomplete="off" class="layui-input">' +
                            '</div>' +
                            '</div>' +
                            '</div>',
                            '<div class="layui-col-xs3" style="padding: 0 5px">' +
                            ...
                            '<div class="layui-input-block form_block">' +
                            '<input type="text" name="optTarExplain" autocomplete="off" class="layui-input">' +
                            '</div>' +
                            '</div>' +
                            '</div>',
                            '</div>',
							...
							'<div class="layui-col-xs3" style="padding: 0 5px">' +
							'<div class="layui-form-item">' +
							'<label class="layui-form-label form_label">计划日期<span field="planDate" class="field_required">*</span></label>' +
							'<div class="layui-input-block form_block">' +
                        ...
                        success: function () {
                            ...
                            fileuploadFn('#fileuploadOpt', $('#fileContentOpt'));
	                        ...
	                        if (type == 2) {
								...
								$('#fileContentOpt').html(getFileEleStr(data.attachmentOptList, true));
                        ...
                        yes: function (index) {
							...
							// 优化目标附件
							var attachmentOptId = '';
							var attachmentOptName = '';
							for (var i = 0; i < $('#fileContentOpt .dech').length; i++) {
								attachmentOptId += $('#fileContentOpt .dech').eq(i).find('input').val();
								attachmentOptName += $('#fileContentOpt a').eq(i).attr('name');
							}
							obj.attachmentOptId = attachmentOptId;
							obj.attachmentOptName = attachmentOptName;
				...
				function getSearchObj() {
					var searchObj = {
						...
						approvalStatusOpt: $('select[name="approvalStatusOpt"]', $('.query_module')).val()
					}
			...
			function afterFloatChange (value, ele) {
				...
				// 目标优化金额=收入目标金额-（收入目标数量-优化目标数量）*（收入目标单价-优化目标单价）
				...
				var optTarPrice = 0;
				// 收入目标数量
				var incomeTarNum = checkFloatNum($('[name="incomeTarNum"]', $('#baseForm')).val(), 3);
				// 收入目标单价
				var incomeTarPrice = checkFloatNum($('[name="incomeTarPrice"]', $('#baseForm')).val(), 3);
				// 收入目标金额
				var incomeTarAmount = checkFloatNum($('[name="incomeTarAmount"]', $('#baseForm')).val());

				if (name == 'optTarNum') {
					optTarNum = value;
					optTarPrice = checkFloatNum($('[name="optTarPrice"]', $('#baseForm')).val(), 3);
				} else if (name == 'optTarPrice') {
					optTarPrice = value;
					optTarNum = checkFloatNum($('[name="optTarNum"]', $('#baseForm')).val(), 3);
				}

				var count = BigNumber(incomeTarNum).minus(optTarNum);
				var price = BigNumber(incomeTarPrice).minus(optTarPrice);
				var total =  checkFloatNum(count.multipliedBy(price));
				var money = BigNumber(incomeTarAmount).minus(total).decimalPlaces(2);

				$('[name="optTarAmount"]', $('#baseForm')).val(money);
			}
			...
			function newWorkFlow(flowId, urlParameter, cb) {
				$.ajax({
					...
					data: {
						...
						approvalType: '05', // 预算关联审批页面【数据字典配置 RELATION_APPROVAL_PAGE】(内嵌页面)
