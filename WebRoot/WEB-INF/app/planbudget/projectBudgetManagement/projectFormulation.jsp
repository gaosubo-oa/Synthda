...
<html>
	...
	<body>
		<div class="container">
			...
			<div class="wrapper">
				<div class="wrap_left">
					<h2 style="text-align: center;line-height: 35px;">预算项目编制</h2>
					<div class="left_form">
				...
				<div class="wrap_right">
					<div class="query_module layui-form layui-row" style="position: relative">
						...
						<div class="layui-col-xs2" style="margin-left: 15px;">
							<input type="text" name="budgetName" placeholder="预算名称" autocomplete="off" class="layui-input">
						</div>
						<div class="layui-col-xs2" style="margin-left: 15px;">
...
<%--		        <button class="layui-btn layui-btn-sm layui-btn-normal" lay-event="add">新增</button>--%>
		...
		<script>
            ...
            var colShowObj = {
				...
				incomeTarAmount:{
				     field:'incomeTarAmount',minWidth:110,title:'收入目标金额',sort:true,hide:false,templet:function(d){
				     	if(d.plbProjBudgetWithBLOBs&&d.plbProjBudgetWithBLOBs.incomeTarAmount){
				     		return d.plbProjBudgetWithBLOBs.incomeTarAmount;
						}else {
				     		return '0';
						}
					}
                ...
                layTable.on('toolbar(tableObj)', function (obj) {
                    ...
                    switch (obj.event) {
						case 'revise': // 修编
                            ...
                            $.get('/plbBudgetItem/getBudgetByItemId', {itemId: checkStatus.data[0].budgetItemId}, function (res) {
                                if (res.flag) {
                                    if (res.data.plbProjBudgetWithBLOBs) {
                                        newOrEdit(3, res.data.plbProjBudgetWithBLOBs);
                                    } else {
                        ...
                        case 'submit':
							...
							// approvalType 后台定义
							$.get('/plbProjBudget/judgeProjectApproval', {projId: projId, approvalType: '05'}, function(res) {
								...
								if (res.data) {
									...
									layer.open({
										...
										success: function () {
											// 数据字典  FLOW_SETTING
											$.get('/plbFlowSetting/getOwnFlowData', {plbDictNo: '05'}, function (res) {
												var flowData = []
										...
										yes: function (index) {
											...
											if (checkStatus.data.length > 0) {
												...
												var urlParameter = 'projId=' + projId + '&approvalType=03'
												newWorkFlow(flowData.flowId, urlParameter, function (res) {
													...
													$.get('/plbProjBudget/approval', {
														...
														approvalType: '05' // approvalType 后台定义
													}, function (res) {
														layer.closeAll();
														if (res.flag) {
                ...
                layTable.on('tool(tableObj)', function (obj) {
                    ...
                    if (layEvent === 'detail') {
                        iframeLayerIndex = layer.open({
                            ...
                            title: '查看详情',
                            area: ['100%', '100%'],
                            content: '/plbProj/projectBudgetDetail?budgetType=1&disabled=1&budgetItemId=' + _budgetItemId
                        });
                ...
                function newOrEdit(type, data) {
                    ...
                    layer.open({
                        ...
                        content: ['<div class="layer_wrap" style="padding: 10px 15px;">',
                            ...
                            '<div class="layui-col-xs3" style="padding: 0 5px">' +
                            '<div class="layui-form-item">' +
                            '<label class="layui-form-label form_label">收入目标数量<span field="incomeTarNum" class="field_required">*</span></label>' +
                            '<div class="layui-input-block form_block">' +
                            '<input type="text" name="incomeTarNum" minusFlag="1" pointFlag="1" pointNum="3" handleCallback="afterFloatChange" autocomplete="off" class="layui-input input_floatNum">' +
                            '</div>' +
                            '</div>' +
                            '</div>',
                            '<div class="layui-col-xs3" style="padding: 0 5px">' +
                            '<div class="layui-form-item">' +
                            '<label class="layui-form-label form_label">收入目标单价<span field="incomeTarPrice" class="field_required">*</span></label>' +
                            '<div class="layui-input-block form_block">' +
                            '<input type="text" name="incomeTarPrice" minusFlag="1" pointNum="3" pointFlag="1" handleCallback="afterFloatChange" autocomplete="off" class="layui-input input_floatNum">' +
                            '</div>' +
                            '</div>' +
                            '</div>',
	                        '<div class="layui-col-xs3" style="padding: 0 5px">' +
                            '<div class="layui-form-item">' +
                            '<label class="layui-form-label form_label">收入目标金额<span field="incomeTarAmount" class="field_required">*</span></label>' +
                            '<div class="layui-input-block form_block">' +
                            '<input type="text" readonly name="incomeTarAmount" autocomplete="off" class="layui-input">' +
                            '</div>' +
                            '</div>' +
                            '</div>',
                            '<div class="layui-col-xs3" style="padding: 0 5px">' +
                            ...
                            '<div class="layui-input-block form_block">' +
                            '<input type="text" name="incomeTarExplain" autocomplete="off" class="layui-input">' +
                            '</div>' +
                            '</div>' +
                            '</div>',
                            '</div>',
                        ...
                        success: function () {
	                        ...
	                        if (type ==2 || type == 3) {
								...
								$('[name="planDate"]').val(new Date().format('yyyy-MM-dd'));
            ...
            function afterFloatChange (value, ele) {
            	...
            	var value = BigNumber(value);

            	if (name == 'incomeTarNum') {
					var price = checkFloatNum($('[name="incomeTarPrice"]', $('#baseForm')).val(), 3);

					var money = checkFloatNum(value.multipliedBy(price));

					$('[name="incomeTarAmount"]', $('#baseForm')).val(money);
				} else if (name == 'incomeTarPrice') {
					var count = checkFloatNum($('[name="incomeTarNum"]', $('#baseForm')).val(), 3);

					var money = checkFloatNum(value.multipliedBy(count));
			...
			function newWorkFlow(flowId, urlParameter, cb) {
				$.ajax({
					...
					data: {
						...
						approvalType: '03', // 预算关联审批页面【数据字典配置】(内嵌页面)
