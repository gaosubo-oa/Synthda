...
<div class="container">
    ...
    <div class="wrapper">
        <div class="wrap_left">
            <h2 style="text-align: center;line-height: 35px;">分包预付款</h2>
            <div class="left_form">
...
<script>
    ...
    layui.use(['form', 'laydate', 'table', 'element', 'soulTable', 'eleTree', 'xmSelect', 'layer'], function () {
        ...
        //表格显示顺序
        var colShowObj = {
            mtlPaymentNo: {field: 'mtlPaymentNo', title: '预付款编号', sort: true, hide: false},
            ...
            //     }
            // },
            subcontractName:{field:'contractName',title:'合同名称',sort:false,hide:false},
            advanceMoney: {field: 'advanceMoney', title: '合同预付款', sort: true, hide: false},
            thisPayMon: {field: 'thisPayMon', title: '申请金额', sort: true, hide: false},
            ...
            demo:{field:'demo',title:'用途',sort:true,hide:false},
            remake:{field:'remake',title:'备注',sort:true,hide:false}
        }
            ...
            switch (obj.event) {
                ...
                case 'del':
                    ...
                    layer.confirm('确定删除该条数据吗？', function (index) {
                        $.post('/subcontractAdvancePayment/del', {ids: advanceId}, function (res) {
                            if (res.flag) {
                ...
                case 'submit':
                    ...
                    layer.open({
                        ...
                        success: function () {
                            $.get('/plbFlowSetting/getOwnFlowData', {plbDictNo: '48'}, function (res) {
                                var flowData = []
                        ...
                        yes: function (index) {
                            ...
                            if (checkStatus.data.length > 0) {
                                ...
                                newWorkFlow(flowData.flowId, function (res) {
                                    var submitData = {
                                        advanceId:approvalData.advanceId,
                ...
                case 'dayin':
                    ...
                    if(checkStatus.data[0].auditerStatus != 0){
                        ...
                        $.ajax({
                            url:'/generateDocx/generateDocxByType?runId='+runId+'&type=subAdvancePayment',
                            success:function(res){
                                if(res.flag){
                                    layer.close(index);
        ...
        function newOrEdit(type, data) {
            ...
            if (type == '0') {
                title = '新建分包预付款';
                url = '/subcontractAdvancePayment/insert';
            } else if (type == '1') {
                title = '编辑分包预付款';
                url = '/subcontractAdvancePayment/updateById';
            }else if(type == '4'){
            ...
            layer.open({
                ...
                '                       <input type="text" name="subcontractName" placeholder="查找分包合同" readonly autocomplete="off" class="layui-input chooseManagementBudget" style="padding-right: 25px;cursor: pointer;background: #e7e7e7;">\n' +
                '                       </div>\n' +
                '                   </div>' +
                '               </div>' +
                '               <div class="layui-col-xs4" style="padding: 0 5px;">' +
                ...
                '                       <div class="layui-input-block form_block">\n' +
                '                       <input type="text" name="customerName" readonly autocomplete="off" placeholder="分包商" class="layui-input chooseCustomerName" style="cursor: pointer;background: #e7e7e7;">\n' +
                '                       </div>\n' +
                '                   </div>' +
                '               </div>' +
                '               <div class="layui-col-xs4" style="padding: 0 5px;">' +
                ...
                success: function () {
                    ...
                    if (type == 1 || type == 4) {
                        ...
                        // 获取自动编号
                        getAutoNumber({autoNumberType: 'subAdvancePayment'}, function(res) {
                            $('input[name="mtlPaymentNo"]', $('#baseForm')).val(res.obj);
                            $('input[name="payDate"]', $('#baseForm')).val(res.object.createDate);
                        });
