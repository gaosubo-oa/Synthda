...
<div class="container">
    ...
    <div class="wrapper">
        <div class="wrap_left">
            <h2 style="text-align: center;line-height: 35px;">分包合同</h2>
            <div class="left_form">
...
<script>
    ...
    layui.use(['form', 'laydate', 'table', 'element', 'soulTable', 'eleTree', 'xmSelect'], function () {
        ...
        //表格显示顺序
        var colShowObj = {
            documentNo: {field: 'documentNo',minWidth:100, title: '单据号', sort: true, hide: false},
            ...
            contractPeriod: {field: 'contractPeriod',minWidth:100, title: '合同工期', sort: true, hide: false},
            ...
            switch (obj.event) {
                ...
                case 'export':
                    ...
                    if($('[name="contractName"]').val()){
                        str+='&contractName='+$('[name="contractName"]').val()
                    }
                    if($('[name="contractMoney"]').val()){
                        str+='&contractMoney='+$('[name="contractMoney"]').val()
                    }
                    if($('#leftId').attr('projId')){
                        str+='&projId='+$('#leftId').attr('projId')
                    }
                    window.location.href = '/plbMtlSubcontract/selectAll?export=export'+str
                    break;
                case 'submit': // 提交
                    if (checkStatus.data.length != 1) {
                ...
                case 'entry':
                    ...
                    if (checkStatus.data[0].approvalStatus!=2) {
                        layer.msg('只能更新已审批的数据！', {icon: 0});
                        return false;
                    }
                    newOrEdit(5, checkStatus.data[0])
                    break;
                case 'dayin':
                    if (checkStatus.data.length != 1) {
                                        ...
                                        if(mtlSubpackageData){
                                            for(var i = 0 ; i <_dataa2.length;i++){
                                                if(_dataa2[i].subpackageId == mtlSubpackageData[0].subpackageId){
                                                    $('.layui-table tr[data-index=' + i + '] input[type="radio"]').next(".layui-form-radio").trigger('click');
                                                    //form.render('checkbox');
                                                }
                                            }
                                        }
                ...
                success: function () {
                    ...
                    var cols=[
                        ...
                        },
                        {
                            ...
                            templet: function (d) {
                                return '<input type="text" '+(type==4||type==5 ? 'readonly' : '')+'   name="contractOtherContent" class="layui-input " style="height: 100%;" value="' + (d.contractOtherContent || '')+ '">'
                            }
                        }
                    ]
                    var colsPaymentSettlement=[
                        {type: 'numbers', title: '序号'},
                        {
                            ...
                            templet: function (d) {
                                return '<input type="text" '+(type==4||type==5 ? 'readonly' : '')+' subcontractId="' + (d.subcontractId || '')+ '" subcontractListId="'+(d.subcontractListId||'')+'" projBudgetId="'+(d.projBudgetId||'')+'" name="subcontractContent" class="layui-input " style="height: 100%;" value="' + (d.subcontractContent || '')+ '">'
                            }
                        },
                        {
                            field: 'subcontractUnit', title: '单位',minWidth:100,
                            templet: function (d) {
                                return '<input type="text" '+(type==4||type==5 ? 'readonly' : '')+'  name="subcontractUnit" class="layui-input " style="height: 100%;" value="' + (d.subcontractUnit || '')+ '">'
                            }
                        },
                        {
                            field: 'quantities', title: '工程量',minWidth:140,
                            templet: function (d) {
                                return '<input type="text" '+(type==4||type==5 ? 'readonly' : '')+'   name="quantities" class="layui-input " style="height: 100%;" value="' + (d.quantities || '')+ '">'
                            }
                        },
                        {
                            field: 'comprehensiveUnitPrice', title: '综合单价',minWidth:100,
                            templet: function (d) {
                                return '<input type="text" '+(type==4||type==5 ? 'readonly' : '')+'   name="comprehensiveUnitPrice" class="layui-input " style="height: 100%;" value="' + (d.comprehensiveUnitPrice || '')+ '">'
                            }
                        },
                        {
                            field: 'totalPrice', title: '合价',minWidth:100,
                            templet: function (d) {
                                return '<input type="text" '+(type==4||type==5 ? 'readonly' : '')+'   name="totalPrice" class="layui-input " style="height: 100%;" value="' + (d.totalPrice || '')+ '">'
                            }
                        }
                        /*{
                            field: 'contractMoney', title: '约定付款金额', templet: function (d) {
                                return '<input type="number" '+(type==4||type==5 ? 'readonly' : '')+' mtlSubcontractPayId="' + (d.mtlSubcontractPayId || '') + '" name="contractMoney" class="layui-input contractMoney" style="height: 100%;" value="' + (d.contractMoney || '') + '">'
                            }
                        },
                        {
                            field: 'contractRatio', title: '约定付款比例', templet: function (d) {
                                return '<input type="number" '+(type==4||type==5 ? 'readonly' : '')+' name="contractRatio" class="layui-input contractRatio" style="height: 100%" value="' + (d.contractRatio || '') + '">'
                            }
                        },
                        {
                            field: 'paymentPeriod', title: '约定付款日期', templet: function (d) {
                                return '<input type="text" name="paymentPeriod" '+(type==4||type==5 ? 'disabled' : 'readonly')+' class="layui-input paymentPeriod" style="height: 100%;" value="' + (d.paymentPeriod || '') + '">'
                            }
                        },
                        {
                            field: 'termOfPayment', title: '付款条件', templet: function (d) {
                                return '<input type="text" '+(type==4||type==5 ? 'readonly' : '')+' name="termOfPayment" class="layui-input termOfPayment" style="height: 100%;" value="' + (d.termOfPayment || '') + '">'
                            }
                        },*/
                    ]
                    ...
                    table.render({
                        ...
                        toolbar: type==4||type==5?'':'#plbMtlContrastBar',
                        defaultToolbar: [''],
                        limit: 1000,
                        cols: [[
                            {type: 'numbers', title: '序号'},
