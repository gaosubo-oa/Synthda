...
<head>
    ...
    <style>
        ...
        /*    top: 26px;*/
        /*}*/
        /*.query_module .layui-form .layui-row{*/
        /*    z-index: 99999999;*/
        /*    !*top: 12px;*!*/
        /*}*/
        /*.layui-table-header{*/
...
<div class="container">
    ...
    <div class="wrapper">
        <div class="wrap_left">
            <h2 style="text-align: center;line-height: 35px;" id="rbsTypeName">预算成本报表</h2>
            <div class="left_form">
        ...
        <div class="wrap_right">
            <div style="position: relative;top: 10px">
                <div class="table_box" style="display: none" >
                    <div class="layui-row layui-form" style="position: relative;margin-left: 10px">
...
<%--                            <button type="button" class="layui-btn layui-btn-sm" id="advancedQuery">高级查询</button>--%>
                            <button type="button" class="layui-btn layui-btn-sm" lay-event="export" id="export">导出</button>
                        </div>
                    ...
                    <div style="padding: 10px">
                        ...
                        <table id="tableObj" lay-filter="parse-table-demo" class="layui-table"  style="display: none" >
                            <thead id="tableThe">
                            <tr>
                                <th id="th1" lay-data="{field:'controlType',minWidth:100}" rowspan="2">控制方式</th>
                                <th lay-data="{field:'itemRemark',minWidth:100}" rowspan="2">特征描述</th>
                                <th lay-data="{field:'itemUnit',minWidth:100}" rowspan="2">单位</th>
                                <th class="displayNone" lay-data="{field:'yvsuan',minWidth:100,align:'center'}" colspan="3">收入目标</th>
                                <th class="displayNone" lay-data="{field:'youhua',minWidth:100,align:'center'}" colspan="3">优化目标</th>
                                <th lay-data="{field:'guanli',minWidth:100,align:'center'}" colspan="3">管理目标</th>
                            </tr>
                            <tr>
                                <th class="displayNone" lay-data="{field:'IncomeTarNum',minWidth:100,align:'center'}">数量</th>
                                <th class="displayNone" lay-data="{field:'IncomeTarPrice',minWidth:100,align:'center'}">单价</th>
                                <th class="displayNone" lay-data="{field:'IncomeTarAmount',minWidth:100,align:'center'}">总价</th>
                                <th class="displayNone" lay-data="{field:'OptTarNum',minWidth:100,align:'center'}">优化量</th>
                                <th class="displayNone" lay-data="{field:'OptTarPrice',minWidth:100,align:'center'}">优化单价</th>
                                <th class="displayNone" lay-data="{field:'OptTarAmount',minWidth:100,align:'center'}">优化总价</th>
                                <th lay-data="{field:'ManageTarNum',minWidth:100,align:'center'}">数量</th>
                                <th lay-data="{field:'ManageTarPrice',minWidth:100,align:'center'}">单价</th>
                                <th lay-data="{field:'ManageTarAmount',minWidth:100,align:'center'}">总价</th>
                            </tr>
                            </thead>
                        </table>
                    </div>
...
<script>
    ...
    var _cbsId=getUrlParam('cbsId');
    if(_rbsId!=undefined&&_rbsId!=null&&_rbsId!=""&&_rbsId){
        //去掉收入目标 优化目标列
        $('.displayNone').remove()
        if(1==_rbsId){ //材料
            $("#rbsTypeName").text("材料计划报表");
        }else if(2==_rbsId){ //分包
            $("#rbsTypeName").text("分包计划报表");
        }else if(3==_rbsId){ //设备
            //$("#rbsTypeName").text("");
        }else if(4==_rbsId){ //其他
            $("#rbsTypeName").text("其它费用计划报表");
        }
    }else if(_cbsId!=undefined&&_cbsId!=null&&_cbsId!=""&&_cbsId){
        $("#rbsTypeName").text("间接费报表");
        var colTab='<thead id="tableThe"><tr><th lay-data="{field:\'projectName\',minWidth:100}" rowspan="2">项目名称</th>\n' +
            '<th id="th1" lay-data="{field:\'ManageTarAmount\',minWidth:120}">管理目标金额</th>\n' +
            '<th lay-data="{field:\'currentAmount\',minWidth:120,align:\'center\'}">截止当前额度</th>\n' +
            '<th lay-data="{field:\'realOutMoney\',minWidth:120,align:\'center\'}">累计已发生额度</th>\n'+
            '<th lay-data="{field:\'trnSettleupMoney\',minWidth:120,align:\'center\'}">在途报销金额</th>' +
            '<th lay-data="{field:\'endSsurplusMoney\',minWidth:150,align:\'center\'}">截止当前剩余额度</th>\n' +
            '<th lay-data="{field:\'cumulativeAmount\',minWidth:200,align:\'center\'}">在途付款后累计发生额度</th>\n'+
            '<th lay-data="{field:\'proportion\',minWidth:180,align:\'center\'}">在途付款后占总目标比例</th>' +
            '</tr></thead>\n'
        $('#tableObj').html(colTab);
    }
    ...
    layui.use(['form', 'laydate', 'eleTree', 'xmSelect', 'treeTable', 'table'], function () {
        ...
        var rbsHtm;
        var cbsHtm;
        function tableInit(projId,parms) {
            var loadInde = layer.load(0)
            var urll = '/projectCostAnalysis/costState?projId='+projId+''
            $(".th").remove();
            var ur = '/projectCostAnalysis/getWbsCbsLayerNum?projId='+projId+''
            if(_rbsId!=undefined&&_rbsId!=null&&_rbsId!=""&&_rbsId){
                ur = '/projectCostAnalysis/getWbsCbsLayerNum?projId='+projId+'&rbsId='+_rbsId
            }else if(_cbsId!=undefined&&_cbsId!=null&&_cbsId!=""&&_cbsId){
                $.ajax({
                    url:"/plbCbsType/getCbsIdByName",
                    dataType:"json",
                    data:{cbsName:"间接费"},
                    async:false,
                    success:function(res){
                        if(res.flag){
                            var cbsIdArr = res.obj;
                            var cbsIdStr = "";
                            if(cbsIdArr){
                                for(var i=0;i<cbsIdArr.length;i++){
                                    cbsIdStr+=cbsIdArr[i]+",";
                                }
                                if(cbsIdStr){
                                    ur = '/projectCostAnalysis/getWbsCbsLayerNum?projId='+projId+'&model=otherCost&cbsIdStr='+cbsIdStr
                                    urll = '/projectCostAnalysis/costState?projId='+projId+'&model=otherCost&cbsIdStr='+cbsIdStr
                                }
                            }
                        }
                    }
            ...
            $.ajax({
                ...
                success:function(res){
                    ...
                    if(_rbsId!=undefined&&_rbsId!=null&&_rbsId!=""&&_rbsId){
                        urll = '/projectCostAnalysis/costState?projId='+projId+'&rbsId='+_rbsId
                    }
                    var  _parms = ''
                    if(parms&&parms.cbsId){
                        _parms+='&cbsIdStr='+parms.cbsId
                    }
                    if(parms&&parms.wbsId){
                        _parms+='&wbsId='+parms.wbsId
                    }
                    if(parms&&parms.rbsId){
                        _parms+='&rbsId='+parms.rbsId
                    }
                    if(parms&&parms.controlType){
                        _parms+='&controlType='+parms.controlType
                    }
        ...
        layTable.on('toolbar(parse-table-demo)',function(obj){
            var layEvent=obj.event;
            switch (layEvent) {
                 case 'export':
                     if(_rbsId!=undefined&&_rbsId!=null&&_rbsId!=""&&_rbsId){
                             ...
                             success:function(res){
                                 if(res.flag){
                                     ...
                                     if(cbsIdArr){
                                         ...
                                         if(cbsIdStr){
                                             window.location.href='/projectCostAnalysis/costState?projId='+projectId+'&export=export'+'&model=otherCost&cbsIdStr='+cbsIdStr
                                         }
                                     }
                                 }
                             }
                         ...
                         //导出预算成本分析汇总报表
                         window.location.href='/projectCostAnalysis/costState?projId='+projectId+'&export=export'+'&model=costStatement';
                     }
                     break;
            }
        })
        ...
        $('[name=controlType]').html('<option value="">请选择</option>'+str||'')
        layui.form.render()

        // 查询
        $('#searchBtn').on('click', function () {
            tableInit($('#leftId').attr('projId') || '',getSearchObj());
        });
        // 查询
        $('#export').on('click', function () {
            if(_rbsId!=undefined&&_rbsId!=null&&_rbsId!=""&&_rbsId){
                ...
                window.location.href='/projectCostAnalysis/costState?projId='+projectId+'&export=export'+'&model=costStatement'+_parms;
            }
        });
        ...
        function getSearchObj() {
            var searchObj = {
                ...
                cbsId : cbsSelectTree.getValue('valueStr'),
                wbsId : wbsSelectTree.getValue('valueStr'),
                rbsId : rbsSelectTree.getValue('valueStr'),
                controlType : $('.table_box [name="controlType"]').val(),
            }
