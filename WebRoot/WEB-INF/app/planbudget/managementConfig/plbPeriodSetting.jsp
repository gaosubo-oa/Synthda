    ...
    <div class="layui-row ">
        ...
        <div class="layui-form-item" style="display: none">
            ...
            <div class="layui-input-block">
                <input type="text" id="plbPeriodId" required lay-verify="required" placeholder="" autocomplete="off" class="layui-input">
            </div>
        </div>
...
<script>
    ...
    function getlis() {
        $.ajax({
            url: '/plbPlanPeroidSetting/selectAllYear',
            type: 'get',
            dataType: 'json',
            success: function (res) {
                if (res.code == 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        if (i == 0) {
                            str += '<li class="select" value="' + data[i].periodYear + '"   dictId="' + data[i].plbPeriodId + '">' + data[i].periodYear + '</li>'
                        } else {
                            str += '<li value="' + data[i].periodYear + '"  dictId="' + data[i].plbPeriodId + '" >' + data[i].periodYear + '</li>'
                        }
                    }
    ...
    layui.use(['table', 'layer', 'laydate', 'form', 'element', 'eleTree'], function () {
        ...
        var tableIns = table.render({
            elem: '#questionTable'
            , url: '/plbPlanPeroidSetting/showAllSet'  //数据接口
            , where: {
        ...
        table.on('tool(questionTable-table)', function (obj) { //注：tool 是工具条事件名，test 是 table 原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'del') { //删除
                layer.confirm('确定要删除吗?', function (index) {
                    $.ajax({
                        type: 'post',
                        url: '/plbPlanPeroidSetting/deletePlanPeriodSet',
                        dataType: 'json',
                        data: {plbPeriodId: data.plbPeriodId},
                        success: function (res) {
                            $('#query').trigger('click');
                            if (res.msg) {
        ...
        function add(type, editData) {
            ...
            if (type == 0) {
                title = '新增'
                url = '/plbPlanPeroidSetting/insertPlanPeriodSet'
            } else if (type == 1) {
                title = '编辑'
                url = '/plbPlanPeroidSetting/updatePeriod'
            }
            layer.open({
                type: 1,
                title: title,
                ...
                , yes: function (index) {
                    ...
                    if (type == 1) {
                        params.plbPeriodId = editData.plbPeriodId
                    }
                    $.ajax({
                        type: 'post',
                        url: url,
