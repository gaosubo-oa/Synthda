...
<script type="text/javascript">
    ...
    var dictionaryStr = 'UNIT_TYPE,UNIT_NATURE';
    // 获取数据字典数据
    ...
    layui.use(['table', 'form','laydate', 'soulTable'], function () {
        ...
        // 表格显示顺序
        var colShowObj = {
            unitNo: {field: 'unitNo', title: '单位编号', sort: true, hide: false},
            unitName: {field: 'unitName', title: '单位名称', sort: true, hide: false},
            unitShortname: {field: 'unitShortname', title: '单位简称', sort: true, hide: false},
            unitNature:{field: 'unitNature', title: '行业', sort: true, hide: false,templet:function (d) {
                            return dictionaryObj['UNIT_NATURE']['object'][d.unitNature] || ''
                        }},
            unitType: {field: 'unitType', title: '企业类型', sort: true, hide: false,templet:function (d) {
                            return dictionaryObj['UNIT_TYPE']['object'][d.unitType] || ''
                        }},
            taxRegNum: {field: 'taxRegNum', title: '统一社会信用代码(税号)', sort: true, hide: false},
            accountAddress: {field: 'accountAddress', title: '地址', sort: true, hide: false},
            accountNumber: {field: 'accountNumber', title: '开户行账户', sort: true, hide: false}
        }

        var TableUIObj = new TableUI('plb_internal_unit');
        ...
        function tableShow() {
            ...
            tableList = table.render({
                elem: '#tableList',
                url: '/plbInternalUnit/getDataByCondition',
                page: true,
                height: 'full-80',
                toolbar: '#toolbarDemo',
            ...
            switch (obj.event) {
                ...
                case 'del':
                    ...
                    layer.confirm('确定删除该条数据吗？', function (index) {
                        $.post('/plbInternalUnit/delInternalUnit', {unitIds: unitId}, function (res) {
                            if (res.flag) {
        ...
        function newOrEdit(type, data) {
            ...
            if (type == '0') {
                title = '新建'
                url = '/plbInternalUnit/insertOrUpdate'
            } else if (type == '1') {
                title = '编辑'
                url = '/plbInternalUnit/insertOrUpdate'
            }
            layer.open({
                ...
                '                       <input type="text" name="unitNo"  autocomplete="off" class="layui-input">\n' +
                '                       </div>\n' +
                '                   </div>' +
                '               </div>' +
                '               <div class="layui-col-xs6">' +
                ...
                '                       <div class="layui-input-block">\n' +
                '                       <input type="text" name="unitShortname" autocomplete="off" class="layui-input">\n' +
                '                       </div>\n' +
                '                   </div>' +
                '               </div>' +
                '               <div class="layui-col-xs6">' +
                ...
                '               <div class="layui-col-xs6">' +
                '                   <div class="layui-form-item">\n' +
                '                       <label class="layui-form-label">统一社会信用代码(税号)</label>\n' +
                '                       <div class="layui-input-block">\n' +
                '                       <input type="text" name="taxRegNum" autocomplete="off" class="layui-input">\n' +
                '                       </div>\n' +
                '                   </div>' +
                '               </div>' +
                '               <div class="layui-col-xs6">' +
                ...
                '                       <div class="layui-input-block">\n' +
                '                       <input type="text" name="regCapital" autocomplete="off" class="layui-input">\n' +
                '                       </div>\n' +
                '                   </div>' +
                '               </div>' +
                '               <div class="layui-col-xs6">' +
                ...
                success: function () {
                    ...
                    $('[name="unitNature"]').html(dictionaryObj['UNIT_NATURE']['str'])
                    $('[name="unitType"]').html(dictionaryObj['UNIT_TYPE']['str'])
                    //回显数据
                    if (type == 1) {
