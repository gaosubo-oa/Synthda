...
<script>
    var htmBox=['<div class="layui-collapse _disabled">\n',
        ...
        '    <h2 class="layui-colla-title">材料付款</h2>\n' +
        ...
        '                       <label class="layui-form-label form_label">付款单编号<!--<span field="paymentNo" class="field_required">*</span>--><a title="刷新编号" class="refresh_no_btn"><i class="layui-icon layui-icon-refresh"></i></a></label>\n' +
        '                       <div class="layui-input-block form_block">\n' +
        '                       <input type="text" name="paymentNo" readonly autocomplete="off" style="background: #e7e7e7" class="layui-input">\n' +
        '                       </div>\n' +
        '                   </div>' +
        '               </div>' +
        '               <div class="layui-col-xs4" style="padding: 0 5px;">' +
        ...
        '                       <input type="text" name="contractName" placeholder="查找合同" readonly autocomplete="off" class="layui-input chooseSubcontract testNull"  title="合同名称" style="padding-right: 25px;background:#e7e7e7;cursor: pointer;">\n' +
        '                       </div>\n' +
        '                   </div>' +
        '               </div>' +
        '               <div class="layui-col-xs4" style="padding: 0 5px;">' +
        ...
        '                       <div class="layui-input-block form_block">\n' +
        '                           <textarea type="text" name="remark" style="resize: vertical; margin-top: 0px; margin-bottom: 0px; height: 39px;" autocomplete="off" class="layui-input"></textarea>\n' +
        '                       </div>\n' +
        '                   </div>' +
        '               </div>' +
    ...
    var dictionaryStr = 'CONTROL_MODE,CBS_UNIT,CONTRACT_TYPE,TAX_RATE,INVOICE_TYPE,CONTRACT_CATEGORY';
    ...
    layui.use(['form', 'laydate', 'table', 'element', 'soulTable', 'eleTree', 'xmSelect'], function () {
        ...
        if(!runId){
            ...
            $.ajax({
                url:'/plbMtlPayment/getByRunId?runId='+runId,
                async:false,
            ...
            plbMtlPaymentDetailsListData = [];
            plbMtlPaymentWriteOffListData = [];
            //回显数据
            if (type == 1 || type==4) {
                form.val("formTest", data);
                plbMtlPaymentDetailsListData = data.plbMtlPaymentDetailsList || [];
                plbMtlPaymentWriteOffListData = data.plbMtlPaymentWriteOffList || [];
                // 合同id
                ...
                getAutoNumber({autoNumberType: 'mtlSubpayment'}, function(res) {
                    $('input[name="paymentNo"]', $('#baseForm')).val(res.obj);
            ...
            table.render({
                elem: '#writeOffTable',
                data: plbMtlPaymentWriteOffListData||[],
                    ...
                    layer.open({
                        ...
                        success: function () {
                            table.render({
                                ...
                                cols: [[
                                    {type: 'checkbox'},
                                    {field: 'mtlSettleupNo', title: '结算单编号',},
                                    {field: 'customerName', title: '供应商单位名称',},
                                    {field: 'contractName', title: '合同名称',},
                                    {field: 'subsettleupMoney', title: '累计已结算金额',templet: function (d) {
                                            return d.subsettleupMoney || 0
                                        }},
                                    {field: 'settlementMoney', title: '本次结算金额', templet: function (d) {
                                            return d.settlementMoney || 0
                                        }},
                                    {
                                ...
                                request: {
                                    ...
                                    _dataa=res.data;
                                    if(plbMtlPaymentDetailsListData!=undefined&&plbMtlPaymentDetailsListData.length>0){
                                        for(var i = 0 ; i <_dataa.length;i++){
                                            for(var n = 0 ; n < plbMtlPaymentDetailsListData.length; n++){
                                                if(_dataa[i].mtlSettleupId == plbMtlPaymentDetailsListData[n].mtlSettleupId){
                        ...
                        yes: function (index) {
                            ...
                            $tr.each(function () {
                                var oldDataObj = {
                                    subsettleup: $(this).find('[data-field="subsettleup"] .layui-table-cell').text(),
                                    settleupMoney: $(this).find('[data-field="settleupMoney"] .layui-table-cell').text(),
                                    sumPaymentMoney: $(this).find('[data-field="sumPaymentMoney"] .layui-table-cell').text(),
                                    trnPaymentMoney: $(this).find('[data-field="trnPaymentMoney"] .layui-table-cell').text(),
                                    currPaymentMoney: $(this).find('input[name="currPaymentMoney"]').val(),
                                    subpaymentId: $(this).find('input[name="currPaymentMoney"]').attr('subpaymentId') || '',
                                    subpaymentDetailsId: $(this).find('input[name="currPaymentMoney"]').attr('subpaymentDetailsId') || '',
                                    mtlSettleupId: $(this).find('input[name="currPaymentMoney"]').attr('mtlSettleupId') || '',
                                }
                                plbMtlPaymentDetailsListData.push(oldDataObj);
                            });
                            data.forEach(function (item) {
                                var addRowData={
                                    subsettleup: item.mtlSettleupNo,
                                    settleupMoney: retainDecimal(item.settlementMoney,2) || 0,
                                    sumPaymentMoney: retainDecimal(item.cumulativeMoney,2) || 0,
                                    trnPaymentMoney: retainDecimal(item.trnPaymentMoney,2) || 0,
                                    mtlSettleupId: item.mtlSettleupId,
                                }

                                if(plbMtlPaymentDetailsListData){
                                    var _flag = true
                                    for(var j=0;j<plbMtlPaymentDetailsListData.length;j++){
                                        if(plbMtlPaymentDetailsListData[j].mtlSettleupId==item.mtlSettleupId){
                                            _flag = false
                                        }
                                    }
                                    if(_flag){
                                        plbMtlPaymentDetailsListData.push(addRowData)
                                    }
                            ...
                            table.reload('materialDetailsTable', {
                                data: plbMtlPaymentDetailsListData
                            });
                            layer.close(index)
                        },
                    })
        ...
        table.on('tool(materialDetailsTable)', function (obj) {
            ...
            if (layEvent === 'del') {
                obj.del();
                if (data.subpaymentDetailsId) {
                    $.get('/plbMtlPayment/deleteDetails', {dtype:'writeOff',ids: data.subpaymentDetailsId}, function (res) {
                ...
                $tr.each(function () {
                    ...
                    /*       if ($(this).find('input[name="paymentPre"]').attr('subpaymentListId')) {
                               oldDataObj.subpaymentListId = $(this).find('input[name="paymentPre"]').attr('subpaymentListId');
                           }*/
                ...
                case 'add':
                    ...
                    $tr.each(function () {
                        var oldDataObj = {
                            ...
                            paymentPreOutMoney: $(this).find('[data-field="paymentPreOutMoney"] .layui-table-cell').text(),
        ...
        table.on('tool(writeOffTable)', function (obj) {
            ...
            if (layEvent === 'del') {
                ...
                if (data.subpaymentWriteoffId) {
                    $.get('/plbMtlPayment/deleteDetails', {dtype:'writeOff',ids: data.subpaymentWriteoffId}, function () {
                    });
                }
                //遍历表格获取每行数据进行保存
            ...
            layer.open({
                ...
                success: function () {
                    var contractTable = table.render({
                        ...
                        where:{projId:$('#leftId').attr('projId'),auditerStatusFlag:"auditerStatusFlag",model:"mtlPayment",useFlag:true,paymentId:data.subpaymentId},
                        page:true,
                        cols: [[
                            {type: 'radio'},
                ...
                yes: function (index) {
                    ...
                    if (checkStatus.data.length > 0) {
                        ...
                        plbMtlPaymentWriteOffListData = chooseData.writeOffList;
                        table.reload('writeOffTable', {
                            data: plbMtlPaymentWriteOffListData
                        });
        ...
        $(document).on('input propertychange', '.outMoneyItem', function () {
            ...
            $('[name="conSpareMoney"]').val(retainDecimal(conSpareMoney,3))||0
        });
        ...
        $(document).on('input propertychange', '.applyPayMoney', function () {
            ...
            $('[name="payMoney"]').val(retainDecimal(accAdd(applyPayMoney,a2),2)||0)


            //本次付款后累计付款比例=（累计已付款金额+本次付款申请金额）/累计结算金额
            var a3 = $('[name="payedMoney"]').val()||0 //累计已付款金额
            var a4 = $('[name="payMoney"]').val()||0 //本次付款申请金额
            var a5 = Number($('[name="settleupMoney"]').val())?Number($('[name="settleupMoney"]').val()):1
    ...
    function childFunc(){
        ...
        $tr2.each(function () {
            var materialDetailsObj2 = {
                ...
                paymentPreMoney: retainDecimal($(this).find('[data-field="paymentPreMoney"] .layui-table-cell').text(),2)||0,
                paymentPreOutMoney: retainDecimal($(this).find('[data-field="paymentPreOutMoney"] .layui-table-cell').text(),2)||0,
                paymentPreWriteMoney: retainDecimal($(this).find('[data-field="paymentPreWriteMoney"] .layui-table-cell').text(),2)||0,
                paymentPreCurrWriteMoney: retainDecimal($(this).find('input[name="paymentPreCurrWriteMoney"]').val(),2)||0,
                subpaymentId: $(this).find('input[name="paymentPreCurrWriteMoney"]').attr('subpaymentId') || '',
                subpaymentWriteoffId: $(this).find('input[name="paymentPreCurrWriteMoney"]').attr('subpaymentWriteoffId') || '',
        ...
        obj.plbMtlPaymentWriteOffList = materialDetailsArr2;

        obj.subpaymentId = data.subpaymentId
        var _flag = false;
        $.ajax({
            url: "/plbMtlPayment/updatePlbMtlSubpayment",
