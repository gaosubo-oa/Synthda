...
<%--		<button class="layui-btn layui-btn-sm" lay-event="entry">预付款录入</button>--%>
...
<script>
	...
	var colShowObj = {
		...
		contractNo: {field: 'contractNo', title: '合同编号',minWidth:110, sort: true, hide: false},
		...
		contractType: {
			field: 'contractType', title: '合同类型',minWidth:110, sort: true, hide: false, templet: function (d) {
				return dictionaryObj['CONTRACT_TYPE']['object'][d.contractType] || '';
			}
		},
		signDate: {
			field: 'signDate', title: '合同签订日期',minWidth:140, sort: true, hide: false, templet: function (d) {
				return format(d.signDate);
			}
		},
		contractA: {
			field: 'contractA', title: '甲方', minWidth:50,sort: true, hide: false, templet: function (d) {
				return d.contractAName || '';
			}
		},
		customerId: {
			field: 'customerId', title: '乙方', minWidth:50,sort: true, hide: false, templet: function (d) {
				return d.customerName || '';
			}
		},
		contractMoney: {
			field: 'contractMoney', title: '合同额', minWidth:90,sort: true, hide: false, templet: function (d) {
				return numFormat(d.contractMoney);
			}
		},
		contractPeriod: {
			field: 'contractPeriod', title: '合同有效期',minWidth:120, sort: true, hide: false
		},
		changeDate: {
			field: 'changeDate', title: '合同变更日期',minWidth:140, sort: true, hide: false
		},
		currFlowUserName: {field: 'currFlowUserName',minWidth:110, title: '当前处理人', sort: false, hide: false},
		changeStatus: {
			field: 'changeStatus', title: '合同变更状态',minWidth:140, sort: true, hide: false, templet: function (d) {
				var str = '';
				switch (d.changeStatus) {
					case '0':
						str = '未提交';
						break;
					case '1':
	...
	var dictionaryStr = 'CONTRACT_TYPE,PAYMENT_METHOD,CBS_UNIT,CONTRACT_STATUS,COMPARE_TYPE,TAX_RATE,INVOICE_TYPE,CUSTOMER_NAME,MANAGE_ITEM_TYPE,CONTRACT_CATEGORY';
	...
	function initPage() {
		layui.use(['form', 'laydate', 'table', 'element', 'soulTable', 'eleTree'], function () {
			...
			function tableInit(projId) {
				...
				var option = {
					elem: '#tableObj',
					url: '/mtlContractChange/selectAll',
					toolbar: '#toolbarHead',
					cols: [cols],
			...
			layTable.on('toolbar(tableObj)', function (obj) {
				...
				switch (obj.event) {
					...
					case 'edit':
						...
						if (checkStatus.data[0].changeStatus != 0) {
							layer.msg('该合同已提交，不可编辑！', {icon: 0});
							return false;
						}

						$.get('/mtlContractChange/queryId', {mtlContractId: checkStatus.data[0].mtlContractId}, function (res) {
							if (res.flag) {
					...
					case 'del':
						...
						checkStatus.data.forEach(function (v) {
							...
							if(v.changeStatus&&v.changeStatus!='0'){
								isFlay = true
							}
						});
						...
						layer.confirm('确定删除选中数据吗？', function (index) {
							$.post('/mtlContractChange/delete', {mtlContractIds: mtlContractIds}, function (res) {
								layer.close(index);
								if (res.flag) {
					...
					case 'submit':
						...
						layer.open({
							...
							success: function () {
								$.get('/plbFlowSetting/getOwnFlowData', {plbDictNo: '55'}, function (res) {
									var flowData = []
			...
			layTable.on('tool(tableObj)', function (obj) {
				...
				if (layEvent === 'detail') {
					layer.open({
						...
						btn: ['确定'],
						btnAlign: 'c',
						content: '/mtlContractChange/mtlPurchaseContract?type=4&mtlContractId=' + data.mtlContractId
					});
				}
			});
						...
						layer.open({
							...
							yes: function (index) {
								...
								if (checkStatus.data.length > 0) {
									...
									$trs.each(function () {
										var oldDataObj = {
											changeContractListId: $(this).find('.mtlName').attr('changeContractListId') || '',
											contractListId: $(this).find('.mtlName').attr('contractListId') || '',
											mtlLibId: $(this).find('.mtlName').attr('mtlLibId') || '',
											mtlName: $(this).find('.mtlName').text(),
											mtlStandard: $(this).find('.mtlStandard').text(),
											valuationUnit: $(this).find('.valuationUnit').attr('valuationUnit') || '',
											amount: $(this).find('.amount').text(),
											unitPrice:$(this).find('.unitPrice').text(),
											totalIncludingTax:$(this).find('.totalIncludingTax').text(),
											changeNumber: $(this).find('input[name="changeNumber"]').val(),
											changeMoney: $(this).find('input[name="changeMoney"]').val(),
											changeEndNumber: $(this).find('input[name="changeEndNumber"]').val(),
											changeEndPrice: $(this).find('input[name="changeEndPrice"]').val(),
											changeEndMoney: $(this).find('input[name="changeEndMoney"]').val(),
			...
			layTable.on('tool(contractDetailsTable)', function (obj) {
				...
				if (layEvent === 'del') {
					obj.del();
					if (data.changeContractListId) {
						$.get('/plbMtlContractChangeList/delete', {changeContractListId: data.changeContractListId}, function (res) {
						});
					}
					//遍历表格获取每行数据进行保存
					var $trs = $('#contractDetailsModule').find('.layui-table-main tr');
			...
			layTable.on('toolbar(paymentSettlementTable)', function (obj) {
				switch (obj.event) {
					case 'add':
						...
						$tr.each(function () {
							var oldDataObj = {
								changeMtlContractOutId: $(this).find('input[name="contractMoney"]').attr('changeMtlContractOutId') || '',
			...
			layTable.on('tool(paymentSettlementTable)', function (obj) {
				...
				if (layEvent === 'del') {
					obj.del();
					if (data.changeMtlContractOutId) {
						$.get('/plbMtlContractChangeOut/delete', {changeMtlContractOutId: data.changeMtlContractOutId}, function () {
						});
					}
					//遍历表格获取每行数据进行保存
			...
			function revision() {
				...
				var cols=[
					{type:'radio'},
					{field: 'documentNo', title: '单据号', sort: true, hide: false},
				...
				layer.open({
					...
					success: function () {
						...
						searchObj.customerName = $('.flow_Table [name="customerName"]').val() || '';
			...
			function newOrEdit(type, data) {
				...
				layer.open({
					...
					content: ['<div class="layer_wrap"><div class="layui-collapse">',
						...
						'<input type="text" readonly name="changeDocumentNo" autocomplete="off" style="cursor: pointer;background: #e7e7e7;" class="layui-input">' +
						'</div>' +
						'</div>' +
						'</div>',
						'<div class="layui-col-xs2 layui_col">' +
						...
						'<div class="layui-input-block form_block">' +
						'<input type="text" readonly name="projectName" id="projectName" readonly style="cursor: pointer;background: #e7e7e7;" autocomplete="off" class="layui-input">' +
						'</div>' +
						'</div>' +
						'</div>',
						'<div class="layui-col-xs2 layui_col">' +
						...
						'<div class="layui-input-block form_block">' +
						'<input type="text" readonly name="contractNo" autocomplete="off" style="cursor: pointer;background: #e7e7e7;" class="layui-input">' +
						'</div>' +
						'</div>' +
						'</div>',
						'<div class="layui-col-xs2 layui_col">' +
						...
						'<div class="layui-input-block form_block">' +
						'<input type="text" readonly name="contractName" autocomplete="off" style="cursor: pointer;background: #e7e7e7;" class="layui-input">' +
						'</div>' +
						'</div>' +
						'</div>',
						'<div class="layui-col-xs2 layui_col">' +
						...
						'<div class="layui-input-block form_block">' +
						'<select name="contractType" readonly disabled ></select>' +
						'</div>' +
						'</div>' +
						'</div>',
						'</div>',
						...
						'<div class="layui-input-block form_block">' +
						'<input type="text" name="paymentPre" readonly style="cursor: pointer;background: #e7e7e7;" autocomplete="off" pointFlag="1" class="layui-input input_floatNum">' +
						'</div>' +
						'</div>' +
						'</div>',
						'<div class="layui-col-xs2 layui_col">' +
						...
						'<div class="layui-input-block form_block">' +
						'<select name="taxRate" readonly disabled lay-filter="taxRate">' +
						...
						'<div class="layui-input-block form_block">' +
						'<input type="number" readonly style="background: #e7e7e7;" name="contractMoneyNotax" readonly autocomplete="off" class="layui-input">' +
						'</div>' +
						'</div>' +
						'</div>',
						'<div class="layui-col-xs2 layui_col">' +
						...
						'<div class="layui-input-block form_block">' +
						'<input type="text" readonly name="signDate"  autocomplete="off" class="layui-input" style="cursor: pointer;background: #e7e7e7;" id="signDate">' +
						'</div>' +
						'</div>' +
						'</div>',
						'<div class="layui-col-xs2 layui_col">' +
						...
						'<div class="layui-input-block form_block">' +
						'<input type="text" readonly name="contractPeriod" style="cursor: pointer;background: #e7e7e7;" autocomplete="off" class="layui-input">' +
						'</div>' +
						'</div>' +
						'</div>',
						'<div class="layui-col-xs2 layui_col">' +
						...
						'<div class="layui-input-block form_block">' +
						'<input type="text" readonly name="warrantyPeriod" style="cursor: pointer;background: #e7e7e7;" autocomplete="off" class="layui-input">' +
						'</div>' +
						'</div>' +
						'</div>',
						'<div class="layui-col-xs2 layui_col">' +
						...
						'<div class="layui-input-block form_block">' +
						'<input type="text" readonly name="warrantyCash" style="cursor: pointer;background: #e7e7e7;" pointFlag="1" autocomplete="off" class="layui-input input_floatNum">' +
						'</div>' +
						'</div>' +
						'</div>',
						...
						'<div class="layui-input-block form_block">' +
						'<select name="invoiceType" readonly disabled  lay-filter="invoiceType"></select>' +
						'</div>' +
						'</div>' +
						'</div>',
						...
						'<textarea name="contractContent" placeholder="请输入内容" readonly class="layui-textarea"></textarea>' +
						'</div>' +
						'<div class="layui-tab-item">' +
						'<textarea name="contractScope" placeholder="请输入内容" readonly class="layui-textarea"></textarea>' +
						'</div>' +
						'<div class="layui-tab-item">' +
						'<textarea name="paymentType" placeholder="请输入内容" readonly class="layui-textarea"></textarea>' +
						'</div>' +
						'</div>' +
						'</div>' +
						/* endregion */
					...
					success: function () {
						...
						if (type == 1) {
							// 获取自动编号
							getAutoNumber({autoNumber: 'plbMtlContractChange'}, function (res) {
								$('input[name="changeDocumentNo"]', $('#baseForm')).val(res);
							});
						...
						layTable.render({
							...
							cols: [[
								...
								{
									field: 'mtlName', title: '材料名称', minWidth: 120, templet: function (d) {
										return '<span class="mtlName" changeContractListId="' + (d.changeContractListId || '') + '" contractListId="' + (d.contractListId || '') + '" mtlLibId="' + (d.mtlLibId || '') + '">' + (d.mtlName || '') + '</span>';
										// return '<input type="text" contractListId="' + (d.contractListId || '') + '" mtlLibId="' + (d.mtlLibId || '') + '" readonly name="mtlLibId" class="layui-input mtlLibId" style="height: 100%;" value="' + (d.mtlName || '') + '">'
									}
								},
								{
									field: 'mtlStandard', title: '材料规格',minWidth:100, templet: function (d) {
										return '<span class="mtlStandard">' + (d.mtlStandard || '') + '</span>';
									}
								},
								{
									field: 'valuationUnit', title: '计量单位',minWidth:100, templet: function (d) {
										...
										// }
										return '<span class="valuationUnit" mtlPlanListId="'+(d.mtlPlanListId || '')+'" mtlLibId="'+(d.mtlLibId || '')+'" controlMode="'+(d.controlMode || '')+'" valuationUnit="'+(d.valuationUnit || '')+'">'+(dictionaryObj['CBS_UNIT']['object'][d.valuationUnit] || '')+'</span>';
									}
								},
								{
									field: 'amount', title: '数量', minWidth:50,templet: function (d) {
										return '<span class="amount">' + (d.amount || 0) + '</span>';
								...
								{
									field: 'unitPrice', title: '含税单价(元)',minWidth:120, templet: function (d) {
										return '<span class="unitPrice">' + (d.unitPrice || 0) + '</span>';
								...
								{
									field: 'totalIncludingTax', title: '含税合计(元)', minWidth:120,templet: function (d) {
								...
								{
									field: 'changeNumber', title: '数量变更',minWidth:100, templet: function (d) {
										return '<input type="number" name="changeNumber"   class="layui-input outMoneyItem" autocomplete="off" style="height: 100%;" value="' + (d.changeNumber || "") + '">'
									}
								},
								{
									field: 'changeMoney', title: '变更总价',minWidth:100, templet: function (d) {
										return '<input type="number" name="changeMoney"   class="layui-input outMoneyItem" autocomplete="off" style="height: 100%;" value="' + (d.changeMoney || "") + '">'
									}
								},
								{
									field: 'changeEndNumber', title: '变更后数量', minWidth:120,templet: function (d) {
										return '<input type="number" name="changeEndNumber"  class="layui-input" autocomplete="off" readonly style="height: 100%;cursor: pointer;background: #e7e7e7;" value="' + (d.changeEndNumber || "") + '">'
									}
								},
								{
									field: 'changeEndPrice', title: '变更后单价', minWidth:100,templet: function (d) {
										return '<input type="number" name="changeEndPrice"   class="layui-input" autocomplete="off" readonly style="height: 100%;cursor: pointer;background: #e7e7e7;" value="' + (d.changeEndPrice || "") + '">'
									}
								},
								{
									field: 'changeEndMoney', title: '变更后合合计',minWidth:140, templet: function (d) {
										return '<input type="number" name="changeEndMoney"   class="layui-input" autocomplete="off" readonly style="height: 100%;cursor: pointer;background: #e7e7e7;" value="' + (d.changeEndMoney || "") + '">'
									}
								},
								{
									field: 'memo', title: '备注', minWidth:50,templet: function (d) {
										return '<input type="text" name="memo"   class="layui-input" autocomplete="off" style="height: 100%;" value="' + (d.memo || "") + '">'
									}
								},
							...
							cols: [[
								{type: 'numbers', title: '序号'},
								{
									field: 'contractMoney', title: '约定付款金额(元)', templet: function (d) {
										return '<input type="text" changeMtlContractOutId="' + (d.changeMtlContractOutId || '') + '" mtlContractOutId="' + (d.mtlContractOutId || '') + '" pointFlag="1" name="contractMoney" class="layui-input contractMoney input_floatNum" style="height: 100%;" value="' + (d.contractMoney || '') + '">'
									}
								},
								{
						...
						layTable.render({
							...
							cols: [[
								...
								{field: 'remark', title: '备注'},
								{fixed: 'right', title:'操作', toolbar: '#contTool', width:150}
							]]
						});
					...
					yes: function (index) {
						...
						var baseData = getSaveData(type, false, loadIndex, mtlContractId, projId).dataObj;
			...
			$(document).on('click','.chooseMtlPlanId2',function () {
				...
				layer.open({
					...
					yes: function (index) {
						var checkStatus = layTable.checkStatus('flowTable2');
						if (checkStatus.data.length > 0) {
			...
			function getSearchObj() {
				var searchObj = {
					changeDocumentNo: $('input[name="changeDocumentNo"]', $('.query_module')).val(),
			...
			function getSaveData(saveType, isSubmit, loadIndex, mtlContractId, projId) {
				...
				// 合同类型
				dataObj.contractType = $('[name="contractType"]').val();
				// 税率
				dataObj.taxRate = $('[name="taxRate"]').val();
				// 发票类型
				dataObj.invoiceType = $('[name="invoiceType"]').val();
				...
				//合同变更依据id
				dataObj.manageProjId = $('[name="manageProjName"]').attr('manageProjId')

				// 合同变更附件
				var changeAttachmentId = '';
				var changeAttachmentName = '';
				for (var i = 0; i < $('#fileContent2 .dech').length; i++) {
					changeAttachmentId += $('#fileContent2 .dech').eq(i).find('input').val();
					changeAttachmentName += $('#fileContent2 a').eq(i).attr('name');
				}
				dataObj.changeAttachmentName = changeAttachmentName;
				dataObj.changeAttachmentName = changeAttachmentName;
				...
				$tr1.each(function () {
					var plbMtlContractObj = {
						...
						mtlStandard: $(this).find('.mtlStandard').text(),
						valuationUnit: $(this).find('.valuationUnit').attr('valuationUnit') || '',
						amount: retainDecimal($(this).find('.amount').text(),3),
						unitPrice:retainDecimal($(this).find('.unitPrice').text(),3),
						totalIncludingTax:retainDecimal($(this).find('.totalIncludingTax').text(),2),
						changeNumber: retainDecimal($(this).find('input[name="changeNumber"]').val(),3),
						changeMoney: retainDecimal($(this).find('input[name="changeMoney"]').val(),2),
						changeEndNumber: retainDecimal($(this).find('input[name="changeEndNumber"]').val(),3),
						changeEndPrice: retainDecimal($(this).find('input[name="changeEndPrice"]').val(),3),
						changeEndMoney: retainDecimal($(this).find('input[name="changeEndMoney"]').val(),2),
						memo: $(this).find('input[name="memo"]').val()
					}
				...
				$tr2.each(function () {
					...
					if ($(this).find('input[name="contractMoney"]').attr('changeMtlContractOutId')) {
						plbMtlContractObj.changeMtlContractOutId = $(this).find('input[name="contractMoney"]').attr('changeMtlContractOutId');
					}
			...
			$(document).on('input propertychange', '.outMoneyItem', function () {
				...
				$tr.each(function (index) {
					//数量
					var amount = $(this).find('.amount').text()||0
					//含税合计
					var totalIncludingTax = $(this).find('.totalIncludingTax').text()||0
					//数量变更
					var changeNumber = $(this).find('input[name="changeNumber"]').val()||0
					//变更总价
					var changeMoney = $(this).find('input[name="changeMoney"]').val()||0

					//变更后数量
					var changeEndNumber = accAdd(amount,changeNumber)||0
					$(this).find('input[name="changeEndNumber"]').val(retainDecimal(changeEndNumber,3))

					//变更后合计
					var changeEndMoney = accAdd(totalIncludingTax,changeMoney)||0
					$(this).find('input[name="changeEndMoney"]').val(retainDecimal(changeEndMoney,2))

					//变更后单价
					if(changeEndNumber&&changeEndNumber==0){
						changeEndNumber = 1
					}
					var changeEndPrice= div(changeEndMoney,changeEndNumber)||0
					$(this).find('input[name="changeEndPrice"]').val(retainDecimal(changeEndPrice,3))

					sum=accAdd(sum,changeEndMoney||0)

				});
				$('[name="contractChangeMoney"]').val(retainDecimal(sum,2))
