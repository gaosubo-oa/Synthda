...
<div class="container">
    ...
    <div class="wrapper">
        <div class="wrap_left">
            <h2 style="text-align: center;line-height: 35px;">材料付款</h2>
            <div class="left_form">
        ...
        <div class="wrap_right">
            <div class="query_module layui-form layui-row" style="position: relative">
                <div class="layui-col-xs2">
                    <input type="text" name="paymentNo" placeholder="付款单编号" autocomplete="off" class="layui-input">
                </div>
                ...
                <div class="layui-col-xs2" style="margin-left: 15px;">
                    <input type="text" name="paymentDate" placeholder="付款日期" autocomplete="off" class="layui-input payment_date">
                </div>
                <div class="layui-col-xs2" style="margin-left: 15px;">
...
<script>
    var plbMtlPaymentDetailsListData = [];
    var plbMtlPaymentWriteOffListData = [];
    var _dataa = null;
    ...
    layui.use(['form', 'laydate', 'table', 'element', 'soulTable', 'eleTree', 'xmSelect'], function () {
        ...
        //表格显示顺序
        var colShowObj = {
            paymentNo: {field: 'paymentNo', title: '付款单编号', sort: true, hide: false},
            ...
            applyPayMoney: {field: 'applyPayMoney', title: '本次应付金额', sort: true, hide: false},
            totalPayRatio: {field: 'totalPayRatio', title: '本次付款后累计付款比例', sort: true, hide: false},
            conSpareMoney: {field: 'conSpareMoney', title: '合同余款', sort: true, hide: false},
            payMoney: {field: 'payMoney', title: '本次付款申请金额', sort: true, hide: false},
            payNode: {field: 'payNode', title: '付款节点', sort: true, hide: false},
            paymentDate: {field: 'paymentDate', title: '付款日期', sort: true, hide: false},
        ...
        var TableUIObj = new TableUI('plb_mtl_payment');
            ...
            switch (obj.event) {
                ...
                case 'del':
                    ...
                    layer.confirm('确定删除该条数据吗？', function (index) {
                        $.post('/plbMtlPayment/delPlbMtlSubpayment', {subPaymentIds: subpaymentId}, function (res) {
                            if (res.flag) {
                ...
                case 'submit':
                    ...
                    layer.open({
                        ...
                        success: function () {
                            $.get('/plbFlowSetting/getOwnFlowData', {plbDictNo: '22'}, function (res) {
                                var flowData = []
                ...
                case 'dayin':
                    ...
                    if(checkStatus.data[0].auditerStatus != 0){
                        ...
                        $.ajax({
                            url:'/generateDocx/generateDocxByType?runId='+runId+'&type=mtlPayment',
                            success:function(res){
                                if(res.flag){
                                    layer.close(index);
                ...
                case 'add':
                    ...
                    layer.open({
                        ...
                        success: function () {
                            table.render({
                                elem: '#tableDemoIn',
                                url: '/plbMtlAdvancePayment/select',
                                cols: [[
                                    {type: 'checkbox'},
                                ...
                                /* parseData: function (res) { //res 即为原始返回的数据
                                 ...
                                 request: {
                                    ...
                                    _dataa=res.data;
                                    if(plbMtlPaymentWriteOffListData!=undefined&&plbMtlPaymentWriteOffListData.length>0){
                                        for(var i = 0 ; i <_dataa.length;i++){
                                            for(var n = 0 ; n < plbMtlPaymentWriteOffListData.length; n++){
                                                if(_dataa[i].advanceId == plbMtlPaymentWriteOffListData[n].advanceId){
                        ...
                        yes: function (index) {
                            ...
                            $tr.each(function () {
                                ...
                                plbMtlPaymentWriteOffListData.push(oldDataObj);
                            });
                            data.forEach(function (item) {
                                ...
                                if(plbMtlPaymentWriteOffListData){
                                    var _flag = true
                                    for(var j=0;j<plbMtlPaymentWriteOffListData.length;j++){
                                        if(plbMtlPaymentWriteOffListData[j].advanceId==item.advanceId){
                                            _flag = false
                                        }
                                    }
                                    if(_flag){
                                        plbMtlPaymentWriteOffListData.push(addRowData)
                                    }
        ...
        function tableShow(projId) {
            ...
            tableIns = table.render({
                elem: '#tableDemo',
                url: '/plbMtlPayment/getDataByCondition',
                toolbar: '#toolbarDemo',
                cols: [cols],
            ...
            layer.open({
                ...
                success: function () {
                    ...
                    if (type == 1 || type==4) {
                        ...
                        // 获取自动编号
                        getAutoNumber({autoNumberType: 'mtlpayment'}, function(res) {
                    ...
                    table.render({
                        elem: '#writeOffTable',
                        data: plbMtlPaymentWriteOffListData||[],
                        //toolbar: type!=4?'#toolbarDemoIn2':false,
                        defaultToolbar: [''],
                        limit: 1000,
                        cols: [cols],
                        done:function (obj) {
                            plbMtlPaymentWriteOffListData = obj.data
                            if(type==4){
                                $('.addRow').hide()
                            }
                        }
            ...
            layer.open({
                ...
                success: function () {
                    var contractTable = table.render({
                        ...
                        where:{projId:$('#leftId').attr('projId'),auditerStatusFlag:"auditerStatusFlag",model:"mtlPayment",useFlag:true},
                        page:true,
                        cols: [[
                            {type: 'radio'},
        ...
        $(document).on('input propertychange', '.outMoneyItem', function () {
            ...
            var totalPayRatio = 0
            if(settleupMoney==0){
                totalPayRatio = div(accAdd(payedMoney,payMoney),1)
            }else {
                totalPayRatio = div(accAdd(payedMoney,payMoney),settleupMoney)
            }
