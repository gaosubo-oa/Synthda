...
<head>
    ...
    <script src="/js/base/base.js?202010623.1"></script>
...
<div id="north">
    <div id="navMenu" style="width:auto;">
        ...
        <a href="#" title='<fmt:message code="common.th.selByDepart" />' id="department" block="dept"
           hidefocus="hidefocus" class=" tab_btn active"><span><fmt:message code="userManagement.th.department"/></span></a><%--按部门选择 部门--%>
        ...
        <a href="#" title="从在线人员选择" id="userOnline" class="tab_btn" block="online"
           hidefocus="hidefocus"><span>在线</span></a>
        <%--<a href="#" block="query" class="tab_btn" hidefocus="hidefocus" style="display:none;"><span><fmt:message code="workflow.th.sousuo" /></span></a>&lt;%&ndash;搜索&ndash;%&gt;&lt;%&ndash;&ndash;%&gt;--%>
        <div class="tab_btnBox" style="float: left;">
            ...
            <input type="checkbox" class="checkboxs" title="筛选出姓名与部门名相同的用户">
            <div id="bgm" class="layui-layer-content layui-layer-loading0"></div>
        </div>
        <%--闵行定制化筛选条件--%>
        <div class="tab_btnBoxMin" style="float: left;display: none">
            <label style="float: left;margin-left: 10px;margin-top: 3px;">类型</label>
            <span href="javascript:;">
                <select name="schoolType" class="schoolType" style="width: 100px;">
                </select>
            </span>
            <label style="float: left;margin-left: 10px;margin-top: 3px;">办别</label>
            <span href="javascript:;" class="statePrivateId">
                <select name="statePrivateId1" class="statePrivateId1" style="width: 100px;">
                </select>
            </span>
            <span href="javascript:;">
                <select name="statePrivateId2" class="statePrivateId2" style="width: 100px;">
                    <option value="">请选择</option>
                </select>
            </span>
            <label style="float: left;margin-left: 10px;margin-top: 3px;">角色</label>
            <span href="javascript:;">
                <select name="role" class="role" style="width: 100px;">
                </select>
            </span>
            <button type="button" class="insert" style="margin-left: 10px;">查询</button>
        </div>
    </div>
...
<div>
    ...
    <div class="main-block" id="deptBox" style="display:block;">
        ...
        <div class="right" id="dept_item">
            <div class="block-right" id="dept_item_2">
                ...
                <div id="block-right-add" onclick="addAllUser($('#deptBox'))" class="block-right-add"><fmt:message
    ...
    <div class="main-block" id="priDox">
        ...
        <div class="right" id="priv_item">
            ...
            <div class="pleaseCheck" style="display: block;text-align: center;line-height: 100px;font-size: 12pt;">
                请选择角色
            </div>
    ...
    <div class="main-block" id="groupDox">
        ...
        <div class="right" id="group_item">
            ...
            <div class="pleaseChecks" style="display: block;text-align: center;line-height: 100px;font-size: 12pt;">
                请选择分组
            </div>
...
<script type="text/javascript">
    ...
    var isSelectBranch = "";//流程选人控件 是否允许选择本分支机构之外人员，默认是允许（0-否，1-是）
    if (parent.opener && parent.opener.globalData != undefined && parent.opener.globalData.isSelectBranch) {
        isSelectBranch = parent.opener.globalData.isSelectBranch;
    }

    // 获取当前登录人信息
    $.get('/getLoginUser', function (res) {
        if (res.flag) {
            node1 = res.object.deptId;
            nodeName = res.object.deptName;
        }
    })
    ...
    var jobId = $('.gwpost').val()
    var postId = $('.zwposition').val()
    ...
    $.ajax({
        ...
        success: function (res) {
            var data = res.object;
            if (data == 'MINHANG_EDU') {
                $('.tab_btnBox').css('display', 'none')
                $('.tab_btnBoxMin').css('display', 'block')
    ...
    $.ajax({
        ...
        success: function (res) {
            var data = res.obj;
            var schoolType = '<option value="">请选择</option>';
            if (res.flag) {
                if (data.length > 0) {
    ...
    $.ajax({
        ...
        success: function (res) {
            var data = res.obj;
            var statePrivateId1 = '<option value="">请选择</option>';
            if (res.flag) {
                if (data.length > 0) {
                    for (var i = 0; i < data.length; i++) {
                        statePrivateId1 += '<option value="' + data[i].codeNo + '">' + data[i].codeName + '</option>'
                    }
                }
                $('.statePrivateId1').append(statePrivateId1);
            }
        }
    })
    var stateSelect = $(".statePrivateId").children("select");
    stateSelect.change(function () {
        $('.statePrivateId2').html('')
        var statePrivateId2 = $('.statePrivateId1').val()
        $.ajax({
            type: 'get',
            url: '/code/getCode?parentNo=STATE_PRIVATE_ID2' + '_' + statePrivateId2,
            dataType: 'json',
            success: function (res) {
                var data = res.obj;
                var statePrivateId2 = '<option value="">全部</option>';
                if (res.flag) {
                    if (data.length > 0) {
                        for (var i = 0; i < data.length; i++) {
    ...
    // 角色下拉选
    var userPrivUrl = '/userPriv/getAllPriv';
    if (isSelectBranch != '' && isSelectBranch == '1') {
        userPrivUrl = '/userPriv/getAllPriv?isSelectBranch=' + isSelectBranch;
    }
    $.ajax({
        type: 'get',
        url: userPrivUrl,
        ...
        success: function (res) {
            var data = res.obj;
            var strRole = '<option value="">请选择</option>';
            if (res.flag) {
                if (data.length > 0) {
                    for (var i = 0; i < data.length; i++) {
                        strRole += '<option value="' + data[i].userPriv + '">' + data[i].privName + '</option>'
                    }
                }
                $('.role').append(strRole);
            }
    ...
    $('.insert').click(function () {
        ...
        var statePrivateId = $('.statePrivateId1').val();
        var statePrivateId2 = $('.statePrivateId2').val();
        var role = $('.role').val();
        var text = $('#keyword').val();
        if (paraValue == "") {
            var url = "../user/getBySearchBai"
            var data = {
            ...
            var url = '../user/getBySearchBaiOrg'
            var data = {
                school_type: schoolType,
                state_private_id: statePrivateId,
                state_private_id2: statePrivateId2,
        ...
        if(parent.opener.sanyuanOpen){
            var contentSecrecy = parent.opener.getContentSecrecy()
            data['contentSecrecy'] = contentSecrecy
        }
        $.ajax({
            type: 'get',
            url: url,
    ...
    function autodivheights(text) {
        ...
        if (paraValue == "") {
            var url = "../user/getBySearchBai?search=" + text+'&moduleId='+moduleId
        } else {
            var url = '../user/getBySearchBaiOrg?deptNo=' + searchPara + '&search=' + text
        }
        if(parent.opener.sanyuanOpen){
            var contentSecrecy = parent.opener.getContentSecrecy()
            url += '&contentSecrecy=' + contentSecrecy
        }
        $.ajax({
            type: "get",
            url: url,
    ...
    function close_window() {
        ...
        if (itemnum == 0 || itemnum == '0&type=workflowedit') {
        ...
        if ($("#PC_addfriendBOX", parent.document).length != 0) {
            ...
            if (location.href.indexOf('type=workflowedit') > -1) {
                var userId = selectItemsId.replace(',', '')
                $.get('/workflow/work/selectUserImage', {userId: userId}, function (res) {
                    var data = res.data.attachment
                    if (data != undefined && data != '') {
                        parent.opener.document.getElementById(parent.opener.user_id).setAttribute('signsrc', '/xs?' + data[0].attUrl);
                        parent.opener.document.getElementById(parent.opener.user_id).setAttribute('value', selectItemsName)
                        parent.opener.document.getElementById(parent.opener.user_id).previousSibling.firstChild.setAttribute('src', '/xs?' + data[0].attUrl);
                    } else {
                        parent.opener.document.getElementById(parent.opener.user_id).value = selectItemsName;
                        parent.opener.document.getElementById(parent.opener.user_id).setAttribute('value', selectItemsName)
                        parent.opener.document.getElementById(parent.opener.user_id).setAttribute('style', 'display: block;')
                        parent.opener.document.getElementById(parent.opener.user_id).previousSibling.firstChild.setAttribute('style', 'display: none;')
                        parent.opener.document.getElementById(parent.opener.user_id).setAttribute('orgsignimg', 0)
                    }
                    window.close();
                })
            } else {
                ...
                if (parent.opener) {
                    ...
                    //针对收款人,回显银行账号和开户行
                    if (parent.opener.responsibleOver) {
                        parent.opener.responsibleOver(parent.opener.user_id)
                    }
                    //针对回显人员信息
                    if (parent.opener.personnelInformation) {
                        parent.opener.personnelInformation(parent.opener.user_id)
                    }
                }
                window.close();
            }
...
//            点击角色
        $('#privBtn').click(function () {
            if (!privBtnflag) {
                var index = layer.load();
                privBtnflag = true;
                var url = '/userPriv/getAllPriv';
                if (isSelectBranch != '' && isSelectBranch == '1') {
                    url = '/userPriv/getAllPriv?isSelectBranch=' + isSelectBranch;
                }
                $.ajax({
                    type: 'get',
                    url: url,
...
//                            arr += '<div class="block-right-item active"  userprivname="'+function(){if(userPrivName[i] == undefined){return ''}else{return userPrivName[i]}}()+'"  item_id="' + dataid[i] + '" item_name="' + selectItemsName[i] + '"  id="' + dataid[i] + '"  user_id="' + user_id[i] + '" uid="' + dataid[i] + '" title="' + selectItemsName[i] + '"><span class="name">' + selectItemsName[i] + ' ' + function(){if(userPrivName[i] == undefined){return ''}else{return userPrivName[i]}}() + '<span class="status"></span></span></div>';
                        arr += '<div class="block-right-item active" item_name="' + selectItemsName[i] + '" userprivname="' + thisuserPrivName[i] + '" uid="' + uid + '" id="' + user_id[i] + '"  user_id="' + user_id[i] + '"><span class="name">' + selectItemsName[i] + '<span class="status"></span></span></div>';
                    }
                }
...
//                            arr += '<div class="block-right-item active"  userprivname="'+function(){if(userPrivName[i] == undefined){return ''}else{return userPrivName[i]}}()+'"  item_id="' + dataid[i] + '" item_name="' + selectItemsName[i] + '"  id="' + dataid[i] + '"  user_id="' + user_id[i] + '" uid="' + dataid[i] + '" title="' + selectItemsName[i] + '"><span class="name">' + selectItemsName[i] + ' ' + function(){if(userPrivName[i] == undefined){return ''}else{return userPrivName[i]}}() + '<span class="status"></span></span></div>';
        ...
        initTree = function () {
            ...
            if (paraValue == "") {
                ...
                url = '../getUserOrg'
                if (isSelectBranch != '' && isSelectBranch == '1') {
                    url = '../getUserOrg?isSelectBranch=' + isSelectBranch
                }
            }
            if(parent.opener.sanyuanOpen){
                var contentSecrecy = parent.opener.getContentSecrecy()
                if(url.indexOf('/getUserOrg?isSelectBranch=') > -1){
                    url += '&contentSecrecy=' + contentSecrecy
                }else{
                    url += '?contentSecrecy=' + contentSecrecy
                }
...
//                        buildUserList(node.id,node.text);
                    ...
                    // 这个tree树，点击部门后，默认是没有下级部门树结构，会再查询下级部门，回显结构
                    // 因为这个tree树结构做过优化，一开始会把所有部门树结构加载完毕，所以无需再查询一次
                    // 但是下面这个功能控制展开、收缩树结构，所以在后台进行处理，接受到id时无需返回数据
                ...
                onBeforeExpand: function (node, param) {
                    if (isSelectBranch != '' && isSelectBranch == '1') {
                        var url = "/getChDeptBai?deptId=" + node.id + '&isSelectBranch=' + isSelectBranch;// change the url
                    } else {
                        var url = "/getChDeptBai?deptId=" + node.id;// change the url
                    }
                    if(parent.opener.sanyuanOpen){
                        var contentSecrecy = parent.opener.getContentSecrecy()
                        url += '&contentSecrecy=' + contentSecrecy
                    }
                    $('#deptOrg').tree('options').url = url;
                },
                                ...
                                if (Array.isArray(data[k].child) && data[k].child.length > 0) {
                                    for (var a = 0; a < data[k].child.length; a++) {
                                        ...
                                        if (useres.length > 0) {
                                            ...
                                            for (var b = 0; b < useres.length; b++) {
                                                ...
                                                }() + '" item_id="' + useres[b].uid + '" userPrivName="' + function () {
                                                    if (useres[b].userPrivName == undefined) {
                                                        return ''
                                                    } else {
                                                        return useres[b].userPrivName
                                                    }
                                                }() + '" item_name="' + useres[b].userName + '" id="' + useres[b].userId + '" user_id="' + useres[b].userId + '" uid="' + useres[b].uid + '" title="' + useres[b].userName + '"><span class="name">' + '[' + check(useres[b].deptName) + ']&nbsp;' + check(useres[b].userName) + '&nbsp;' + function () {
                                                    if (useres[b].userPrivName == undefined) {
                                                        return ''
                                                    } else {
                                                        return useres[b].userPrivName
                                    ...
                                    if (data[k].users.length > 0) {
                                        ...
                                        for (var b = 0; b < data[k].users.length; b++) {
                                            ...
                                            }() + '" item_id="' + data[k].users[b].uid + '" userPrivName="' + function () {
                                                if (data[k].users[b].userPrivName == undefined) {
                                                    return ''
                                                } else {
                                                    return data[k].users[b].userPrivName
                                                }
                                            }() + '" item_name="' + data[k].users[b].userName + '" id="' + data[k].users[b].userId + '" user_id="' + data[k].users[b].userId + '" uid="' + data[k].users[b].uid + '" title="' + data[k].users[b].userName + '"><span class="name">' + '[' + check(data[k].users[b].deptName) + ']&nbsp;' + check(data[k].users[b].userName) + '&nbsp;' + function () {
                                                if (data[k].users[b].userPrivName == undefined) {
                                                    return ''
                                                } else {
                                                    return data[k].users[b].userPrivName
                                        ...
                                        if (objects != '') {
                                            for (var i = 0; i < objects.length; i++) {
                                                if (objects.length > 0) {
                                                    ...
                                                    }() + '" item_id="' + objects[i].uid + '" userPrivName="' + function () {
                                                        if (objects[i].userPrivName == undefined) {
                                                            return ''
                                                        } else {
                                                            return objects[i].userPrivName
                                                        }
                                                    }() + '" item_name="' + objects[i].userName + '" id="' + objects[i].userId + '" user_id="' + objects[i].userId + '" uid="' + objects[i].uid + '" title="' + objects[i].userName + '"><span class="name">' + '[' + check(objects[i].deptName) + ']&nbsp;' + check(objects[i].userName) + '' + function () {
                                                        if (objects[i].userPrivName == undefined) {
                                                            return ''
                                                        } else {
                                                            return objects[i].userPrivName
        ...
        function buildUserList(id, text) {
            ...
            var url = "../department/getChDeptBai?deptId=" + id;
            if (isSelectBranch != '' && isSelectBranch == '1') {
                url = "../department/getChDeptBai?deptId=" + id + '&isSelectBranch=' + isSelectBranch;
            }
            if(parent.opener.sanyuanOpen){
                var contentSecrecy = parent.opener.getContentSecrecy()
                url += '&contentSecrecy=' + contentSecrecy
                        ...
                        if (v.userId) {
                            ...
                            if (v.sex == 0) {
                                divObj = $('<div class="block-right-item ' + function () {
                                    ...
                                    /*function () {
                                    //增加显示辅助角色
                                    if(v.userPrivOtherName==undefined || v.userPrivOtherName=='' || v.userPrivOtherName== 'null' ){
                                        return ''
                                    }else{
                                        return '('+v.userPrivOtherName +')'
                                    }
                                }()+*/'<span class="status"> ' + function () {
                                        if (onLine.toString().indexOf(v.userId) > -1) {
                                            return '<span style="color:#ff0000;">在线</span>';
...
//			点击部门显示本部门及子部门人员
        function buildNewUserList(id, text, jobId, postId) {
            ...
            $('#block-right-header').text(text || '');
            id = id ? id : '';
            var url = '/department/getNewChDeptBai?deptId=' + id + '&jobId=' + jobId + '&postId=' + postId + '&moduleId=' + moduleId;
            if(parent.opener.sanyuanOpen){
                var contentSecrecy = parent.opener.getContentSecrecy()
                url += '&contentSecrecy=' + contentSecrecy
            }
                ...
                success: function (res) {
                    ...
                    if (res.flag) {
                        for (var k = 0; k < data.length; k++) {
                            var childarray = [];
                            if (data[k].child != undefined) {
                                tatl = tatl.concat(childarray.concat(data[k].child));
                            }
                                ...
                                userChild += '<div class="block-right-item ' + function () {
                                    if (user_id || dataid) {
                                        ...
                                        if (dataid != null) {
                                            ...
                                            for (var i = 0; i < user_idArr.length; i++) {
                                                console.log(user_idArr[i])
                                                if (data[i] != undefined && data[i].users.length != 0 && data[i].users[n] != undefined && data[i].users[n].userId == user_idArr[i]) {
                                                    bool = true;
                                                    break;
                                                }
                                            }
...
//         }
        ...
        function queryBenbumen() {
            ...
            $("#block-right-add").show();
            $("#block-right-remove").show();
            var url = '/user/selNewNowUsersBai' + '?moduleId=' + moduleId;
            if(parent.opener.sanyuanOpen){
                var contentSecrecy = parent.opener.getContentSecrecy()
                url += '&contentSecrecy=' + contentSecrecy
            }
                        ...
                        for (var i = 0; i < dataU.length; i++) {
                            strU += '<div class="block-right-item ' + function () {
                                    if (user_id) {
                                        ...
                                        var bool = false;
                                        for (var j = 0; j < user_idArr.length; j++) {
                                            if (dataU[i] != undefined && dataU[i].userId == user_idArr[j]) {
                                                bool = true;
                                                break;
                                            }
                                        }
                                ...
                                /*function () {
                                //增加显示辅助角色
                                if(dataU[i].userPrivOtherName==undefined || dataU[i].userPrivOtherName=='' || dataU[i].userPrivOtherName=='null'){
                                    return ''
                                }else{
                                    return '('+dataU[i].userPrivOtherName +')'
                                }
                            }()+*/'<span class="status">' + function () {
                        ...
                        for (var i = 0; i < data.length; i++) {
                            str += '<div class="block-right-item ' + function () {
                                if (choosearr) {
                                    ...
                                    var bool = false;
                                    for (var j = 0; j < user_idArr.length; j++) {
                                        if (data[i] != undefined && data[i].userId == user_idArr[j]) {
                                            bool = true;
                                            break;
                                        }
                                    }
...
//				点击各角色
        $('#allPriv').on('click', '.block-left-item', function () {
            ...
            if (paraValue == "") {
                // var url = '/user/getUserByUserPrivBai?userPriv='+userPr+'&jobId='+$('.gwpost').val()+'&postId='+$('.zwposition').val()
                ...
                if (isSelectBranch != '' && isSelectBranch == '1') {
                    url = '/user/getUserByUserPrivBaiOrg?isSelectBranch=' + isSelectBranch
                }
                var data = {
                    userPriv: userPr1,
                    deptNo: searchPara
            ...
            allPriv(url, data, $(this).attr('title'))
        })

        function allPriv(url, data, title) {
            $(this).addClass('privActive').siblings().removeClass('privActive');
            var index = layer.load();
            if (title != undefined) {
                $('#priv-right-header').html(title);
            }
            $.ajax({
                type: 'get',
                url: url,
                    ...
                    if (res.flag) {
                        if (data.users.length > 0 || data.otherUsers.length > 0) { //判断是否有符合条件的用户
                            ...
                            if (data.users.length > 0) {  //判断集合中是否有数据
                                ...
                                $('#priv_item_2').find('.userItem').html('');
                            }
...
//			公共点击各分组
        ...
        //个人分组的方法
        function allcustom(groupId, jobId, postId) {
            var index = layer.load();
            $(this).addClass('privActive').siblings().removeClass('privActive');
            var datas = {
                groupId: groupId,
        ...
        function dataList(data) {
            ...
            for (var i = 0; i < data.length; i++) {
                str += '<div class="block-right-item ' + function () {
                    ...
                    /*function () {
                    //增加显示辅助角色
                    if(data[i].userPrivOtherName==undefined || data[i].userPrivOtherName==''|| data[i].userPrivOtherName=='null'){
                        return ''
                    }else{
                        return '('+data[i].userPrivOtherName +')'
                    }
                }()+*/'<span class="status">' + function () {
                        if (onLine.toString().indexOf(data[i].userId) > -1) {
                            return '<span style="color:#ff0000;margin-left: 10px;">在线</span>';
    ...
    function addAllUser(element) {
        ...
        element.find('.userItem .active').each(function (i, v) {
            if ($(this).attr('user_id') != '' && $('#selectedDox .userItem #' + $(this).attr('user_id')).length < 1) {
    ...
    function deleteAllUser(element) {
        element.find('.userItem .active').each(function (i, v) {
            if ($(this).attr('user_id') != '' && $('#selectedDox .userItem #' + $(this).attr('user_id')).length > 0) {
