...
<div id="north">
    <div id="navMenu" style="width:auto;">
        ...
        <a href="#" title='<fmt:message code="common.th.selByRole" />' class="tab_btn active" block="priv" hidefocus="hidefocus"><span>机构</span></a>
...
<%--                <div class="block-right-add"><fmt:message code="meet.th.addAll" /></div>&lt;%&ndash;全部添加&ndash;%&gt;--%>
<%--                <div class="block-right-remove"><fmt:message code="meet.th.DeleteAll" /></div>&lt;%&ndash;全部删除&ndash;%&gt;--%>
...
<%--                <div id="block-right-remove" class="block-right-remove"><fmt:message code="meet.th.DeleteAll" /></div>&lt;%&ndash;全部删除&ndash;%&gt;--%>
...
<%--                <div id="block-right-add" class="block-right-add"><fmt:message code="meet.th.addAll" /></div>&lt;%&ndash;全部添加&ndash;%&gt;--%>
<%--                <div id="block-right-remove" class="block-right-remove"><fmt:message code="meet.th.DeleteAll" /></div>&lt;%&ndash;全部删除&ndash;%&gt;--%>
...
//                    buildPrivList(node.id);
        ...
        function buildPriitems(){
            ...
            if(classHierarchicalPriv == 1){
                ...
                var url = '../EduorgMessage/selectAllOrg'
            }
            $.ajax({
                url:url,
                type:'get',
                ...
                success:function(obj){
                    if(obj.flag){
                        ...
                        data.forEach(function(v,i){
                            ...
                            }()+'"orgIds="'+v.orgIds+'" id="'+v.deptName+'" privId="'+v.orgId+'" privName="'+v.deptName+'" userPriv="'+v.orgId+'" title="'+v.deptName+'"><span class="name">'+v.deptName+'<span class="status"> </span></span></div>';
                            tr+=div;
                            if (bool) {
                                dept_item_2 +=div;
                            }
        ...
        $('#priDox #pri_item').on("click",".block-right-item",function(){
            ...
            if(that.attr('class').indexOf('active') > 0){
                ...
                if($('#selectedDox .priItem #'+that.attr('id')).length > 0){
                    $('#selectedDox .priItem #'+that.attr('id')).remove();
                }
...
//                $('#selectedDox .priItem').append(divObj);
    ...
    function autodivheights(text) {
        $.ajax({
            type: "get",
            url: "../EduorgMessage/selectAllOrg?deptName=" + text +"&IsPriv=1&check=1",
            dataType: 'JSON',
