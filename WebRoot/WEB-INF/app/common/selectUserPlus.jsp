...
//                            arr += '<div class="block-right-item active"  userprivname="'+function(){if(userPrivName[i] == undefined){return ''}else{return userPrivName[i]}}()+'"  item_id="' + dataid[i] + '" item_name="' + selectItemsName[i] + '"  id="' + dataid[i] + '"  user_id="' + user_id[i] + '" uid="' + dataid[i] + '" title="' + selectItemsName[i] + '"><span class="name">' + selectItemsName[i] + ' ' + function(){if(userPrivName[i] == undefined){return ''}else{return userPrivName[i]}}() + '<span class="status"></span></span></div>';
                            arr += '<div class="block-right-item active" item_name="' +selectItemsName[i]+ '" id="' + user_id[i] + '"  user_id="' + user_id[i] + '"><span class="name">' + selectItemsName[i] + '<span class="status"></span></span></div>';
                        }
					}
...
//                            arr += '<div class="block-right-item active"  userprivname="'+function(){if(userPrivName[i] == undefined){return ''}else{return userPrivName[i]}}()+'"  item_id="' + dataid[i] + '" item_name="' + selectItemsName[i] + '"  id="' + dataid[i] + '"  user_id="' + user_id[i] + '" uid="' + dataid[i] + '" title="' + selectItemsName[i] + '"><span class="name">' + selectItemsName[i] + ' ' + function(){if(userPrivName[i] == undefined){return ''}else{return userPrivName[i]}}() + '<span class="status"></span></span></div>';
            ...
            initTree = function(){
                $('#deptOrg').tree({
                    url: '../department/getChDeptPlus?OID=${OID}',
                    animate:true,
                    ...
                    onBeforeExpand:function(node,param){
                        $('#deptOrg').tree('options').url = "../department/getChDeptPlus?OID=${OID}&deptId=" + node.id;// change the url
                    }
            ...
            function buildUserList(id,text){
                ...
                $.ajax({
                    type: "get",
                    url: "../department/getChDeptPlus?OID=${OID}&deptId=" + id,
                    dataType: 'JSON',
                    success: function (res) {
                        var tr = [];
						var num=0;
...
//				$('#deptBox .userItem').html(tr);
            ...
            function queryBenbumen() {
                ...
                $.ajax({
                    type:'get',
                    url:'/user/selNowUsersPlus?OID=${OID}',
                    dataType:'json',
                    success:function (res) {
                        if(res.flag){
                            var dataU=res.obj;
