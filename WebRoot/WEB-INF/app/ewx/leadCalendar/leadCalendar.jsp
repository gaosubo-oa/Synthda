...
<head>
    ...
    <style>
        ...
        .mobile{position:absolute;top:170px;left:0;width:100%;right:0;z-index:5;}
        .nav{
            width: 60%;
            height: 42px;
            margin: 18px 0 0 0;
...
<div class="query" style="text-align: center;margin: 10px 0px;">
    <input type="text" style="width: 80%" placeholder="选择查询人员" id="inName"  value="" class="mui-input-clear" id="inName"/>
    <input value="" type="hidden" id="Inreciever"/>
    <span id="Popover" class='mui-icon mui-icon-plus'></span>
</div>
...
<div class="mui-content"  style="display: none" id="queryU">
    <div class="mui-bar mui-bar-nav" id="header">
        ...
        <%--<div style="margin-top: 45px;">--%>
            <%--<span>姓名</span>--%>
            <%--<input type="text" name="username" style="width: 72%;height: 36px;border-radius: 10px" >--%>
...
<script>
    ...
    $(function(){
        ...
        $('#inName,#Popover').on('tap', function() {
            document.activeElement.blur();
            $('#queryU').show().siblings().hide();
            mui.ajax('/calendarPriv/getLeader', {
                dataType: 'json',
                type: 'get',
                success: function (res) {
                    ...
                    for (var i in obj) {
                        ...
                        str+='<div class="mui-input-row mui-radio mui-left">'+
                            '<label style="line-height: 34px">'+obj[i]+'</label>'+
                            '<input style="width: 70px;height: 70px;" class="checkboxU" name="checkboxU" value="'+obj[i]+'" type="radio" ids="'+[i]+'">'+
                            '</div>'
                    }
...
//                                    '<input style="width: 70px;height: 70px;" class="checkboxU" name="checkboxU" value="'+arr[i].userName+'" type="checkbox" ids="'+arr[i].uid+'">'+
//                                    '</div>'
//                            }
//
//                        }
...
//            })
            ...
            mui("#header").on('tap', '#re_mail', function () {

                var rdsObj= document.getElementsByClassName('checkboxU');
                var checkVal = '';
                var k= 0;
                ...
                for(i = 0; i < rdsObj.length; i++){
                    if(rdsObj[i].checked){
                        checkVals1 = rdsObj[i].getAttribute('ids');
                        k++;
                    }
                }
                $('#queryU').hide().siblings().show();
        ...
        function initData(userId_p){
            ...
            $.ajax({
                type: "get",
                url: "/schedule/getLeaderScheduleMonth",
                data:{
                    userId:userId,
                    time:getDay(0),
                    type:"month"
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if(data.flag&&data.obj.length>0){
                                for(var i=0;i<data.obj.length;i++) {
                                    $('#zbtbody').append("<a class='add' href='/ewx/leadCcalendarDetails?calId="+data.obj[i].calId+"&dataType="+$.GetRequest().dataType+"' style='position:relative;' calId='"+data.obj[i].calId+"' calType='"+data.obj[i].calType+"' calLevel='"+data.obj[i].calLevel+"'><img style='width: 40px;height: 40px;position: absolute;top: 13px;left: -5px;' src='../../img/H5/1.10.png' alt=''><ul><li class='ove1'>" + data.obj[i].content + "</li><li class='ove2'>开始时间" + data.obj[i].stim + "</li><li class='ove3'>结束时间" + data.obj[i].etim + "</li></ul></a>");
                                };
                            }
                        },
                    });
                        ...
                        success: function (data) {
                            ...
                            $.each(data.obj, function (index, item) {
                                if(sendDate == item.calData){
                                    for(var i=0;i<item.calMessage.length;i++) {
                                        $('#zbtbody').append("<a class='add' href='/ewx/leadCalendarDetails?calId="+item.calMessage[i].calId+"&dataType="+$.GetRequest().dataType+"' style='position:relative;' calId='"+item.calMessage[i].calId+"' calType='"+item.calMessage[i].calType+"' calLevel='"+item.calMessage[i].calLevel+"'><img style='width: 40px;height: 40px;position: absolute;top: 13px;left: -5px;' src='../../img/H5/1.10.png' alt=''><ul><li class='ove1'>" + item.calMessage[i].content + "</li><li class='ove2'>开始时间" + item.calMessage[i].stim + "</li><li class='ove3'>结束时间" + item.calMessage[i].etim + "</li></ul></a>");
                                    };
                                }
                            });
                        },
