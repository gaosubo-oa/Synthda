...
<head>
    ...
    <style>
        ...
        #CalendarMain{float:left;width:100%;height:50px;}
        ...
        .mobile{position:absolute;top:40px;left:0;width:100%;right:0;z-index:5;}
        p{
            margin: 6px 0;
        }
...
<div class="header1">
    ...
    <span>按日期查看明细</span>
    <i><a class="add" id="add" href="javascript:;"></a></i>
</div>
...
<div class="mobile animated fadeInRight">
    ...
    <div class="downdiv">
        <div class="downdiv-time">
            <p style="text-align: center;line-height:3rem">异常打卡记录</p>
        </div>
        <div class="c_con">
            <%--打卡记录内容部分--%>
            <ul class="c_con_ul">
...
<script>
    ...
    function attendQuery(date){
        ...
        if(date==undefined||date==''){
            date = nowDate.getFullYear()+"-"+(nowDate.getMonth()+1)+"-01";
        }
        $.ajax('/attend/AttendanceStatistics',{
            ...
            success:function(res){
                if(res.flag){
                    ...
                    var str="";
                    for (let i = 3; i <= 6; i++) {
                        var list = datas["count"+i+"List"];
                        for (let j = 0; j < list.length; j++) {
                            str+='<li>'+list[j].str+'</li>';
                        }
                    }
                    $('.c_con_ul').html(str)
                }
