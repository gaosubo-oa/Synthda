...
<script>
    ...
    function repalceHtml(str){
        ...
        if(dds.indexOf('.cont_form{height:auto;}') > -1){
            dds = dds.split('.cont_form{height:auto;}')[0]+dds.split('.cont_form{height:auto;}')[1];
        }
        return dds;
    }
            ...
            success:function(res){
                if(res.flag){
                    ...
                    for(var i=0;i<res.obj.length;i++){
                        ...
                        //     '<span class="fs14">'+res.obj[i].begin+'</span>'+
                        //     '</div>'+
                        ...
                        var str1=repalceHtml(res.obj[i].content)
                        str1=str1.replace(/&nbsp;/ig, "");
                        var avatar = res.obj[i].users.avatar
                        if (avatar==0){
                            avatar = 'boy.png'
                        }else if(avatar==1){
                            avatar = 'girl.png'
                        ...
                        var enclosure = ''
                        if (res.obj[i].attachmentName != '') {
                            enclosure =  '<img style="display: inline-block;position: relative;top: 5px;left: 5px;" src="../img/enclosure.png" alt="">'
                        } else {
                            enclosure = ''
                        }
                        var typeName = ''
                        if (res.obj[i].typeName == '') {
                            ...
                            typeName = res.obj[i].typeName
                        }
                        app +=
                            '<div class="nav-sub" style="display: flex;box-shadow: 1px 1px 7px rgba(206, 194, 194, 0.85);border-radius: .1rem;background-color: #fff;margin: .24rem .1rem;padding: .1rem 0;">' +
                            '<div>' +
                            '<img class="huiqian_logo" style="width: 35px;margin-top: 8px;position: relative;left: 10px;top: 8px;border-radius: 50%;" src="../../img/user/'+ avatar +'">' +
                            '</div>' +
                            '<div style="width: 90%;">' +
                            '<a class="list" href="/ewx/noticeDetail?notifyId=' + res.obj[i].notifyId + '&dataType='+$.GetRequest().dataType+'" style="color: #333" notifyId="' + res.obj[i].notifyId + '">' +
                            '<div class="nav1">' +
                            '<span class="nav2" style="width: 100%;">' + res.obj[i].subject + '</span>' +
                            '<span class="nav2" style="float: right;font-size: 0.29rem;color: #5087d0;width: 25%;">' + typeName + '</span>' +
                            // '<span class="fs14" >' + res.obj[i].begin + '</span>' +
                            '</div>' +

                            '<p style="    position: relative;">' +
                            '<span class="ac2" style="margin-left: 0.4rem;">' + res.obj[i].name + '</span>' +
                            '<span class="fs14" style="float: none;color: #666;margin-left: 5px;">' + res.obj[i].begin + '</span>' +
                            '<span style="">'+enclosure+'</span>' +
                            '</p>' +
                            '<div style="position: relative;">' +
                            '<div class="nav1" style="-webkit-box-orient: vertical;-webkit-line-clamp: 2;margin-bottom: 15px;display: -webkit-box;white-space: normal;font-size: 0.25rem;margin-top: 10px">'+str1+'</div></div>' +
                            '</a>' +
                            '<p class="nav3"><a href="javascript:;" class="ac"></a></p>' +
                            '</div>' +
                                ...
                                app +=
                                    '<div class="nav-sub" style="display: flex;box-shadow: 1px 1px 7px rgba(206, 194, 194, 0.85);border-radius: .1rem;background-color: #fff;margin: .24rem .1rem;padding: .1rem 0;">' +
                                        ...
                                        '<div style="width: 100%;">' +
                                            '<a class="list" href="/ewx/noticeDetail?notifyId=' + res.obj[i].notifyId + '&dataType='+$.GetRequest().dataType+'" style="color: #333" notifyId="' + res.obj[i].notifyId + '">' +
                                            '<div class="nav1">' +
                                            '<span class="nav2" style="width: 70%;">' + res.obj[i].subject + '</span>' +
                                    '<span class="nav2" style="float: right;top: 0.44rem;font-size: 0.29rem;color: #5087d0;width: 25%;">' + typeName + '</span>' +
                                        // '<span class="fs14" >' + res.obj[i].begin + '</span>' +
                                        '</div>' +
