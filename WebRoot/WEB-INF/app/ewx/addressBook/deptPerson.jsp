...
<script>
                    ...
                    for(var i =0;i<data.length;i++){
                        if(data[i].type=="people"){
                            ...
                            if(data[i].avatar != 0 && data[i].avatar != 1 && data[i].avatar != undefined && data[i].avatar != "undefined"  && data[i].avatar != "" ){
                                str += '<div class="sort_list" uid="'+data[i].uid+'" userId="'+data[i].userId+'" type="'+data[i].type+'"  tavatar ="'+data[i].avatar+'"  tsex="'+data[i].sex+'"   tname ="'+data[i].userName+'"  tuserPrivName="'+data[i].userPrivName+'" onclick="childFn($(this))" ">\n' +
                            ...
                            str += '<div class="sort_list"  deptid="'+data[i].deptId+'"  uname= "'+data[i].deptName+'"  type="'+data[i].type+'" onclick="childFn($(this))" ">\n' +
                                '        <div class="num_logo">\n' +
                                '            <img style="    width: 45px;height: 45px;position: absolute;overflow: hidden;border-radius: 45px;" src="../img/address/organ_deptMent@2x.png"  alt="">\n' +
                                '        </div>\n' +
    ...
    function childFn(a) {
        ...
        var newcolor = a.find(".newcolor").css("background-color")
        if(type == "people"){
            var tsex =a.attr("tsex")
            var tname =a.attr("tname")
            var tuserPrivName =a.attr("tuserPrivName")
            var tavatar = a.attr("tavatar")
            if(workflow != ''){
                if(a.find('.layui-icon-ok-circle').length > 0){
                    thisUserId = thisUserId.map(n => n.userId);
                    var Jsonindex = thisUserId.indexOf(userId);
                    thisJson.splice(Jsonindex, 1)
                    a.find('.layui-icon-ok-circle').removeClass('layui-icon-ok-circle').addClass('layui-icon-circle');
                }else{
                    ...
                    a.find('.layui-icon-circle').removeClass('layui-icon-circle').addClass('layui-icon-ok-circle');
                }
            ...
            //部门页面(直到打开人员详情为止)
            var deptidnew =a.attr("deptid")
            var uname =a.attr("uname")
            if(workflow != ''){
                window.location.href = "/ewx/deptPerson?deptId="+deptidnew+"&uname="+uname+"&type=EWC"
            }else{
                window.location.href = "/ewx/deptPerson?deptId="+deptidnew+"&uname="+uname
            }
        }
    }
