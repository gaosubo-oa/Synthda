...
<div class="mbox">
    <div class="item">
        <img src="../img/fu.png" alt="" style="margin: 0 5px 0 20px;"> <span style="font-size: 22px;display: inline-block;vertical-align: middle;">付款单</span>
    </div>
    ...
    <form class="layui-form" action="" style="margin-top: 20px;">
        <div>
            ...
            <div class="layui-inline">
                <label class="layui-form-label" style="width: 70px;">活动类别</label>
                <div class="layui-input-inline" style="width: 180px;">
                    <select name="modules" lay-verify="required" lay-search="" id="activityType">
                        <option value="">请选择</option>
                        <option value="经营支出">经营支出</option>
                        <option value="日常支出">日常支出</option>
                        <option value="银行支出">银行支出</option>
                    </select>
                </div>
            </div>
        ...
        <div>
            <div class="layui-inline" style="margin-top: 10px;">
                ...
                <div class="layui-input-inline" style="display: flex; cursor: pointer;">
                    ...
                    <div style="margin-top: 10px;margin-left: 23px;"> <a class="seled" style="color:#1E9FFF">选择</a>&nbsp;&nbsp; <a id="deles" style="color:#1E9FFF;margin-left: -5px">清空</a></div>
...
<script>
    layui.use(['form', 'layedit', 'laydate','table'], function() {
        ...
        var tableInt = table.render({
            elem: '#test'
            ,url:'/cusReceipt/selectCusPayment'
        ...
        table.on('tool(test)', function(obj){
            ...
            if(obj.event === 'del'){
                layer.confirm('真的删除当前数据吗？', function (index) {
                    $.ajax({
                        type: 'get',
                        url: '/cusReceipt/deleteCusPayment?paymentId='+paymentId,
                        dataType: 'json',
                        success: function (res) {
                            if (res.msg == 'ok') {
        ...
        function create(type,paymentId){
            if(type=='0'){
                var title = '新建'
            }else{
                var title = '编辑'
            ...
            layer.open({
                ...
                content:' <form class="layui-form" id="ajaxforms" lay-filter="formsTest" action="">\n' +
                    ...
                    '                    <div> <input type="text"  id="customerNo" readonly="readonly" class="layui-input contacts" style="width: 190px;"/></div>\n' +
                ...
                success:function(res){
                    ...
                    if(type==1){
                        ...
                        $.ajax({
                            url:'/cusReceipt/selectOneCusPayment',
                            type: 'get',
                            dataType: 'json',
                ...
                yes:function(index){
                    ...
                    if($('select[lay-filter="warehouses"]').val() == ""){
                    ...
                    if(type == 0){
                        var url = '/cusReceipt/insertCusPayment'
                        data.wareHouseId = warehouseId;
                    }else{
                        var url = '/cusReceipt/updateCusPayment '
                        data.paymentId = paymentId;
                        data.wareHouseId = warehouseId;
                    }
                    $.ajax({
