...
(function (global, factory) {
    ...
    var template$1 = "<div> <div> <label> <input type=\"radio\" name=\"color\" value=\"foreColor\" v-model=\"command\">&nbsp; {{$parent.locale[\"fore color\"]}} </label> <label> <input type=\"radio\" name=\"color\" value=\"backColor\" v-model=\"command\">&nbsp; {{$parent.locale[\"background color\"]}} </label> </div> <div> <div v-for=\"color in colors\" :style=\"{'background-color':color}\" class=\"color-card\" @click=\"changeColor(color)\"> </div> <div style=\"clear: both\"></div> </div> </div> ";
        ...
        data: function data(){
            return {
                ...
                colors: [
                    ...
                    '#FFCCCC', '#FFCC99', '#FFFF00', '#FF9900', '#CCFFCC', '#CCFFFF', '#CCFF99','#FFFFFF'
                ]
            }
        },
        methods: {
    ...
    var template$2 = "<div class=\"dashboard-font\" style=\"line-height: 36px\"> <div> <label>{{$parent.locale[\"heading\"]}}:</label> <button v-for=\"h in 6\" type=\"button\" @click=\"setHeading(h)\">H{{h}}</button> </div>   <div> <label> {{$parent.locale[\"line height\"]}}: </label> <button v-for=\"lh in lineHeightList\" type=\"button\" @click=\"setLineHeight(lh)\"> {{lh}} </button> </div> </div>";
    ...
    var template$7 = "<form @submit.prevent=\"insertTable\"> <label> {{$parent.locale[\"row count\"]}} <input type=\"number\" style=\"width: 45px;height: 30px;\" maxlength=\"2\" min=\"2\" max=\"10\" v-model=\"rows\"> </label> <label> {{$parent.locale[\"column count\"]}} <input type=\"number\" style=\"width: 45px;height: 30px;\" maxlength=\"2\" min=\"2\" max=\"10\" v-model=\"cols\"> </label> <label style=\"width: 10%\"><button type=\"submit\">{{$parent.locale.save}}</button></label> </form>";
    ...
    RangeHandler.prototype.execCommand = function execCommand (command, arg) {
        ...
        switch (command) {
            ...
            case Command.BACK_COLOR: {
                ...
                console.log(document.execCommand(Command.BACK_COLOR, false, arg))
                break
            }
