...
// }
//修改
function disFuction(x){
    // x.style.backgroundColor="white";
    var myStatus = $('#remark').val()
    $.ajax({
        type:'get',
        url:'/updateKanBan',
        data: {
        ...
        success:function(obj) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                str_li += '<li style="padding: 0 5px; box-sizing: border-box;white-space: nowrap;" class="chedule_li" onclick="tiaozhuan(this)" data-url="/notice/detail?notifyId=' + data[i].notifyId + '">' +
                    ...
                    '</div>' +
                    '<a><div class="new_title richeng_title" title="' + data[i].subject + '">' + '<span style="color: red;margin-left: 10px;background: yellow;font-size: 13px;">new</span>'+ data[i].subject+'</div>' +
        ...
        success:function(obj) {
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                str_li += '<li style="padding: 0 5px; box-sizing: border-box;white-space: nowrap;" class="chedule_li" onclick="tiaozhuan(this)" data-url="/notice/detail?notifyId=' + data[i].notifyId + '">' +
                    ...
                    '<img src="/img/data_point.png">' +
                    '</div>' +
                    '<a><div class="new_title richeng_title" title="' + data[i].subject + '">' +
                    function(){
                        if(data[i].read == '0'){
                            return '<span style="color: red;margin-left: 10px;background: yellow;font-size: 13px;">new</span>'
                        }else{
                            return ''
                        }
                    }() +
            ...
            if(res.flag && workFlow.length>0){
                ...
                for(var m=0;m<workFlow.length;m++){
                    ...
                    daiba_li+='<li style="padding: 0 5px;box-sizing: border-box;" onclick="windowOpenNew(this)" type1="daiban" data-url="/workflow/work/workform?opFlag='+workFlow[m].opFlag+'&flowId='+workFlow[m].flowRun.flowId+'&flowStep='+workFlow[m].prcsId+'&runId='+workFlow[m].runId+'&prcsId='+workFlow[m].flowPrcs+'"  data-s="2" class="clearfix">' +
                        ...
                        '<div class="n_img" style="margin-right:5px; "><img style="margin-left: 5px;border-radius:50%"  onerror="imgerror(this,1)"  src="'+function () {
                            if (workFlow[m].flowRun.avatar != 0 && workFlow[m].flowRun.avatar != 1 && workFlow[m].flowRun.avatar != '') {
                                return '/img/user/' + workFlow[m].avatar;
                            }
                        ...
                        '<h2 class="n_title" style="width: 100% !important;" title="'+workFlow[m].flowRun.runName+'">'+workFlow[m].flowRun.runName+'<span>&nbsp;</span>' +
                        '</h2></a>'+      '<span style="position: absolute;right: 10px;top: 21px;color: #999;"></span>'+
                        '</div>' +
                        '</div>' +
                        '</li>'
                }
        ...
        success:function(res){
            ...
            if(res.flag && workFlow.length>0){
                ...
                for(var m=0;m<workFlow.length;m++){
                    ...
                    daiba_li+='<li style="padding: 0 5px;box-sizing: border-box;" onclick="windowOpenNew(this)" type1="daiban" data-url="/workflow/work/workform?opFlag='+workFlow[m].opFlag+'&flowId='+workFlow[m].flowRun.flowId+'&flowStep='+workFlow[m].prcsId+'&runId='+workFlow[m].runId+'&prcsId='+workFlow[m].flowPrcs+'"  data-s="2" class="clearfix">' +
                        '<div class="" style="margin: 0;width: 100%;">' +
                        '<div class="n_img" style="margin-right:5px; "><img style="border-radius: 50%;" onerror="imgerror(this,1)"  src="'+function () {
                            if (workFlow[m].avatar != 0 && workFlow[m].avatar != 1 && workFlow[m].avatar != '') {
                                return '/img/user/' + workFlow[m].avatar;
                            }
                            if(workFlow[m].sex == 0){
                                return '/img/user/boy.png';
        ...
        success: function (obj) {
            ...
            for (var i = 0; i < data.length; i++) {
                li += '<li class="chedule_li" onclick="tiaozhuan(this)" data-url="/news/detail?newsId='+data[i].newsId+'">' +
                    ...
                    '</div>' +
                    '<a><div class="new_title richeng_title" title="' + data[i].subject + '">' +
                    '<span style="color: red;margin-left: 10px;background: yellow;font-size: 13px;">new</span>' +
                    data[i].subject +
                    '</div></a>' +
                    '<div class="every_times" style="float: right;padding-right: 10px;">' +function () {
...
$.ajax({
    ...
    success: function (res) {
        ...
        var data = res.data;
        for (var i = 0; i < data.length; i++) {
            li += '<li class="chedule_li" style="white-space: nowrap" onclick="tiaozhuan(this)" data-url="/workflow/work/workformPreView?flowId='+data[i].flowId+'&flowStep=&runId='+data[i].runId+'&prcsId=&parent='+data[i].prcsId+'">' +
                '<div style="margin-left:12px;">' +
                '<img src="/img/data_point.png">' +
                '</div>' +
                ...
                if(data.length < 7){
                    for (var i = 0; i < data.length; i++) {
                        li += '<li class="chedule_li" onclick="parent.getMenuOpen(this)" menu_tid="0124" data-url="calendar">' +
                            '<h2 style="display: none">日程安排</h2><div style="margin-left:12px;"><img src="/img/data_point.png"></div><div class="richeng_title r_title" title="' + data[i].content + '">' + data[i].content + '</div><div class="richeng_time" style="float: right; color: #919191;">' + data[i].stim.replace(/\s/g, '<i style="margin-right: 10px"></i>') + ' -- ' + data[i].etim.split(" ")[1] + '</div></li>'
                    }
                }else{
                    for (var i = 0; i < 6; i++) {
                        li += '<li class="chedule_li" onclick="parent.getMenuOpen(this)" menu_tid="0124" data-url="calendar">' +
            ...
            if(obj.flag){
                ...
                for(var i = 0; i < data.length; i++) {
                    ...
                    li += '<li class="all_li" onclick="parent.getMenuOpen(this)"' +
                        ' data-url="netdisk" data-num="1" data-id="' + data[i].id + '"  menu_tid="3010"><h2 style="display: none">网络硬盘</h2><div class="total_wenjian"  tid="3001" url="netdisk"><img src="/img/main_img/wenjian.png" ></div><h1 title="' + title + '" style="    height: 20px;    line-height: 20px; text-align: center;" >' + text + '</h1></li>'
                    flag = true;
                }
        ...
        success:function(obj){
            ...
            for (var i = 0; i < data.length; i++) {
                ...
                str_li += '<li onclick="tiaozhuan(this)" data-url="/diary/details?id=' + data[i].diaId + '"><div class="d_img">' +
                    ...
                    '</div><div class="d_word"><h1 class="d_name">' + data[i].userName + '</h1><h3 class="d_time" style="height: 25px;line-height: 28px">' +function () {
                        if(data[i].diaDate!=undefined){
                            return data[i].diaDate
                        }else {
                            return ''
                        }
                    }()  + '</h3><a style="color:#000;" class="public_title" target="_blank"><h2 class="d_title">' + data[i].subject + '</h2></a>' +
                    ''+function () {
                        if(data[i].attachmentId==''){
                            return ''
                        }else {
                        ...
                        if(data[i].users != undefined){
                            ...
                            if (data[i].emailList[j].readFlag == 1) {
                                ...
                                read_li += '<li onclick="tiaozhuan(this)" data-url="/email/emailDetail?id=' + data[i].emailList[j].emailId + '"><div class="e_img"><img onerror="imgerror(this,1)" src="'+function () {
                                    ...
                                    '<h2 style="font-size: 8px; color: #1b5e8d;" emil-tid="' + data[i].emailList[j].emailId + '" class="e_title" title="' + data[i].subject + '">' + data[i].subject +
                                   '</h2>' +
                                    '</a>' +
                                    ''+function () {
                                        if(data[i].attachmentId==''){
                            ...
                            all_li += '<li onclick="tiaozhuan(this)" data-url="/email/emailDetail?id=' + data[i].emailList[j].emailId + '"><div class="e_img">' +
                                ...
                                '</div><div class="e_word"><h1 class="e_name" data-userName="'+data[i].users.userName+'" data-userId="'+data[i].users.userId+'">' +data[i].users.userName + '</h1>' +
                                '<h3 class="e_time">' + sendTimews.replace(/\s/g, '<i style="margin-right:10px;"></i>') + '</h3>' +
                                '<a style="color:#1b5e8d;" class="public_title" target="_blank">' +
                                '<h2 style="font-size: 8px; color: #1b5e8d;" emil-tid="' + data[i].emailList[j].emailId + '" class="e_title" title="' + data[i].subject + '">' +
                                function(){
                ...
                for (var i = 0; i < data.length; i++) {
                    if (data[i].users != undefined) {
                        ...
                        li += '<li class="no_read" onclick="tiaozhuan(this)" data-url="/email/emailDetail?id=' + data[i].emailList[0].emailId + '"><div class="e_img"><img onerror="imgerror(this,1)" src="' + function () {
                            ...
                            'class="e_title" title="' + data[i].subject + '">' + '<span style="color: red;margin-left: 10px;background: yellow;font-size: 13px;">new</span>'+ data[i].subject +'</h2>' +
                            '</a>' + function () {
                                if (data[i].attachmentId == '') {
                                    return ''
                                } else {
...
function talkAbout(){
    $.ajax({
        ...
        success:function(obj){
            ...
            var str_li = '';
            for (var i = 0; i < data.length; i++) {
                str_li += '<li style="padding: 0 5px; box-sizing: border-box;white-space: nowrap;" class="chedule_li" menu_tid="3020" onclick="tiaozhuan(this)" data-url="/bbs/theme?boardId=' + data[i].boardId + '&isCheck='+ data[i].needCheck +'">' +
                    '<div style="margin-left:12px;">' +
                    '<img src="/img/data_point.png">' +
                    '</div>' +
                    '<a><div class="new_title taolunqu_title" title="' + data[i].boardName + '">' + data[i].boardName + '</div></a>' +
                    '<div class="submit_times" style="float: right;padding-right: 10px;height: 20px;text-align: right;line-height: 20px;color: #898989;">' +function(){
