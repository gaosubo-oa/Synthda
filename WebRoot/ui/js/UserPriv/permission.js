...
$(function () {
    ...
    $(document).on('click', '.fileload', function () {
        if (!isImport) {
            $.layerMsg({content: "您没有导入权限！", icon: 0});
            return false;
        }
        window.open("/userPrivImport?isOrgScope=1");
    })

    // 导出点击判断是否有权限
    $(document).on('click', '.importTwo', function () {
        if (!isImport) {
            $.layerMsg({content: "您没有导出权限！", icon: 0});
            return false;
        }
    })
...
var htmlloadDate={//界面初始化数据
    init:function (privTypeId) {
        var me = this;
        $.get('/userPrivType/queryAllUserPriv', {privTypeId: privTypeId}, function (json) {
            me.htmlLoad(json);
        }, 'json');
    },
