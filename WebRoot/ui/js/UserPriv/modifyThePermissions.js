...
$(function () {
    var urlbool={//判断从哪里进入
        ...
        checkAjax:function () {
            if(privTypeIds == 'privTypeId'){
                $.get('/userPrivType/selUserPrivType',{'privTypeId':this.urlstr},function (json) {
                    if(json.flag){
    ...
    $(document).on('click','.TableHeader [type="checkbox"]',function () {
        if($(this).is(':checked')){
            $(this).prop('checked',true)
            ...
            $.post('/userPrivType/updateUserPrivType',obj,function (json) {
                if(json.flag) {
                    $.layerMsg({content: menuSSetting_th_editSuccess, icon: 1}, function () {
                        ...
                        location.href='roleAndClassification'
                    })
                }
            },'json')
        }else{
