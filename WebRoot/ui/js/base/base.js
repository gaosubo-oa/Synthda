...
document.write("<script language=javascript src='/js/common/language.js'></script>");
var specialdept='';
var domains = '';  //获取当前域名（）
var datatyoe ={
    'docx': 'word',
...
function myIsNaN(num) {
    try{
        ...
        console.log('判断条件中控件值出现异常，错误是：'+e)
        return false;
    }
}
/*********字符串为数字检测结束*****************************/
...
/**********计算控件计算公式匹配*************/
    ...
    function returnArr(formtitle, formitem){
        ...
        if(formtitle.indexOf('('+formitem+'+') > -1){
            var arr1 = '('+formitem+'+' ; var arr3 = '(' ; var arr4 = '+';
    ...
    function translateTo(formula, v){
        // 增加容错处理，避免传来的计算公式含有空格导致下面判断无法通过，逻辑重复执行
        var opt = formula.replace(/\s+/g, "");
        ...
        if(v.value){
            ...
            if(v.value.indexOf('上午') > -1 || v.value.indexOf('下午') > -1){
                ...
                if((v.value.indexOf('-') > -1 && v.value.indexOf(':') > -1) || reg.test(v.value)){
                    ...
                    if(repalce2.indexOf(',') > -1){
                        // 去除计算值含有千位分隔符的情况
                        repalce2 = repalce2.split(',').join('');
                    }
                }
                repalce2 = arr[2] + repalce2 + arr[3];
            }
...
$(function () {
    ...
    $.extend({
        loadSidebar: function (target, deptId, fn,moduleId) {
            $.ajax({
        ...
        loadrole: function (target, teptId, num, fn,moduleId) {
            $.ajax({
                url: employeeUrl,
                type: 'get',
                data: {
                ...
                success: function (data) {
                    ...
                    if ($(target).children('li').length == 0) {
                        for (var i = 0; i < data.object.length; i++) {
                            str += '<li><span onclick="clickrenwu(' + data.object[i].uid + ',this)" userId="'+data.object[i].userId+'" data-uid="' + data.object[i].uid + '" data-numString="1"  style="padding-left:' + (20 + 20 * parseInt(num)) + 'px;height:35px;line-height:35px;"  class="childdept role"><span></span>' +
                                '<img  src="' + function () {
        ...
        loadrole2: function (target, teptId, num, fn) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if ($(target).children('li').length == 0) {
                        for (var i = 0; i < data.object.length; i++) {
                            str += '<li><span onclick="clickrenwu(\'' + data.object[i].userId + '\',this)" userId="'+data.object[i].userId+'" data-uid="' + data.object[i].uid + '" data-numString="1"  style="padding-left:' + (20 + 20 * parseInt(num)) + 'px;height:35px;line-height:35px;"  class="childdept role"><span></span>' +
                                '<img  src="' + function () {
                                    var avatar = data.object[i].avatar;
                                    if (avatar == undefined || avatar == "") {
                                        avatar = data.object[i].sex;
        ...
        loadrole3: function (target, teptId, num, fn) {
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if ($(target).children('li').length == 0) {
                        for (var i = 0; i < data.obj.length; i++) {
                            str += '<li><span onclick="clickrenwu(' + data.obj[i].userId + ',this)" userId="'+data.object[i].userId+'" data-uid="' + data.obj[i].staffId + '" data-numString="1"  style="padding-left:' + (20 + 20 * parseInt(num)) + 'px;height:35px;line-height:35px;"  class="childdept role"><span></span>' +
                                '<img  src="' + function () {
        ...
        loadroleUser: function (target, teptId, num, fn,moduleId) {
            $.ajax({
                url: employeeUrlExt,
                type: 'get',
                data: {
...
function consult(e){ //附件查阅点击调取
    ...
    if(e.attr('downloadpriv') == 0){
        pdurl(UrlGetRequest('?'+atturl), atturl+'&downloadpriv&time=20230110');
    }else{
