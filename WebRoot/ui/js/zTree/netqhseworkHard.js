...
var index = true;
var parentPath = "/"+getQueryString('diskPath');//获取参数
console.log("parentPath: "+parentPath);
var diskId1 = getQueryString('diskId');
                ...
                $("#fileTree").tree({
                    ...
                    onClick: function (node) {
                        ...
                        $.ajax({
                            ...
                            dataType:'json',
                            data:{
                                 diskId: objNode.id?objNode.id:diskId1
                            },
                            success:function(res){
...
function filechangess(e, type){
    if(objNode == null){
        layer.open({
            ...
            area: ['400px', '200px'],
            btn: ['关闭'],
            content: '<div style="text-align: center; margin-top: 37px; font-size: 21px;"><span>请先选择文件夹</span></div>'
        })
        return;
    }
...
function startUpload(){
    ...
    $('#uploadimgform').attr('action', '/netdisk/uploadFile?timestamp=' + timestamp + '&diskId=' + diskId1);

    if(objNode.attributes != null){
        console.log(objNode.attributes);
        var firstUrl = "";
        if (objNode.attributes.firstUrl == undefined){
            firstUrl = objNode.attributes.fullUrl;
            firstUrl = firstUrl.replace(parentPath,"");
        }else{
            firstUrl =  objNode.attributes.firstUrl;
            firstUrl = firstUrl.substr(firstUrl.lastIndexOf("/"),firstUrl.length);
        }

        console.log("firstUrl: "+firstUrl);
        $('[name="path"]').val(firstUrl);
    }
    var remindVal = 0;
    ...
    $('#uploadimgform').ajaxSubmit({
        ...
        success: function (jsons) {
            ...
            if (json.flag) {
                ...
                $('.attach').hide();
                $('.box').children().remove();
                $.layerMsg({ content: "您没有上传权限，请联系OA管理员！", icon:0 })
            }
...
function elemLoad(node, ordertype, orderby, fn){//搜索当前文件夹下文档(和排序同一个方法)
    ...
    //     return;
    // }
    var diskId = node == null ? diskId1 : node.id;
    var ajaxPage={
        data:{
            diskId: diskId,
        ...
        page: function () {
            ...
            // var diskId1 = $('.tree-node-selected').attr('node-id');

            var me = this;
            console.log(node);
            // if(node != null && node.attributes != undefined && node.attributes.url != undefined && node.attributes.pc != 1){
            //     console.log(node.attributes);
            //     console.log("21213213213");
            //     if (node.attributes.fullUrl != null && node.attributes.fullUrl != '' && node.attributes.fullUrl != undefined){
            //        var fullUrl = node.attributes.fullUrl;
            //         fullUrl = fullUrl.replace(parentPath,"");
            //         me.data.path = fullUrl;
            //     }else {
            //         me.data.path = node.attributes.url;
            //     }
            // }else {
            //     var path1 = $('.tree-node-selected span[class=tree-title]').text();
            //     me.data.path = "/"+path1;
            //
            // }
            //后端写法改变这边根据网络硬盘写法同样改变
            if(node.attributes != undefined && node.attributes.url != undefined && node.attributes.pc != 1){
                me.data.path = node.attributes.url;
            }
            console.log("me.data.path: "+me.data.path);
            ...
            $.ajax({
                type: 'get',
                url: '/netdisk/selectNetdiskBySearch?diskId='+diskId1,
                dataType: 'json',
                data: me.data,
                success: function(json){
                    ...
                    if(json.flag){
                        ...
                        for(var j = 0; j < attachmentArr.length; j++){
                            var attachmentUrl = "/netdisk/xs?path=" + myEncodeURIxS(json.object.list[j].path) + '&diskId=' + diskId1 +
                        ...
                        for(var i = 0;i < json.object.list.length; i++){
                            ...
                            if(GetFileExt(json.object.list[i].neName) == '.docx' || GetFileExt(json.object.list[i].neName) == '.doc' || GetFileExt(json.object.list[i].neName) == '.DOC' || GetFileExt(json.object.list[i].neName) == '.DOCX' || GetFileExt(json.object.list[i].neName) == '.word' || GetFileExt(json.object.list[i].neName) == '.WORD'){
                                ...
                                lunboStr += '<div class="swiper-slide"><img style="height: 500px"  src="/netdisk/xs?path=' + myEncodeURIxS(json.object.list[i].path) + '&diskId=' +diskId1+'" alt=""></div>'
                                iconType = 'pic';
                            ...
                            strData += '<tr class="contentTr" style="margin-right: 33px;">' +
                                ...
                                function(){
                                    if(json.object.showPriv&&(json.object.list[i].type == 'DOCX'||json.object.list[i].type == 'DOC'||json.object.list[i].type == 'XLSX'||json.object.list[i].type == 'XLS'||json.object.list[i].type == 'PPT'||json.object.list[i].type == 'PPTX'||json.object.list[i].type == 'WPS')){
                                        return '<div class="preview" type="'+ json.object.list[i].type.toLowerCase() +'">预览</div><div class="consult" type="'+ json.object.list[i].type.toLowerCase() +'">查阅</div>';
                                    }else{
                                        return '';
                                    }
                                }()+
                                ...
                                function () {
                                    if(json.object.downloadPriv){
                                        return '<div class="downDiv">下载</div>';
                                    } else {
                                        return '';
                                    }
                                }() + function () {
                                    if(GetFileExt(json.object.list[i].neName) == '.jpg' || GetFileExt(json.object.list[i].neName) == '.png' || GetFileExt(json.object.list[i].neName) == '.PNG' || GetFileExt(json.object.list[i].neName) == '.JPG'){
                                       // console.log("liulan --  "+ attachmentUrl);
                                       // console.log("picnums---  "+picNum);
                                        var picNums = picNum;
                                        picNum = picNum+1;
                                        var domain = document.location.origin;
                                        return '<div class="playDiv operationImg" picNum="'+ picNums +'">浏览</div>'+
                                        ...
                                        return '<div class="player" >播放</div>';
                                    }else if(GetFileExt(json.object.list[i].neName) == '.pdf' || GetFileExt(json.object.list[i].neName) == '.PDF'){
                                        if(json.object.downloadPriv){
                                            // 拥有下载权限
                                            return '<div class="seeDiv">在线预览</div>';
                                        }else{
                                            return '<div class="seeDiv" downloadPriv="0" >在线预览</div>';
                                        }
                                    }else{
                                        return '';
                                    }
...
$(function () {
    ...
    $('#file_Tachr').on('mousemove','.floatTd',function () {
        ...
        // $(this).find('.floatDiv').css("z-index",99);
        // $(this).find('.floatDiv').css("position","absolute");
    })
    ...
    $('#file_Tachr').on('click','.downDiv',function () {
        ...
        window.open('/netdisk/download?path=' + myEncodeURI(path)+ '&diskId='+ diskId1);
    })
    ...
    $('#file_Tachr').on('click','.consult',function () {
        ...
        var diskId = $('.tree-node-selected').attr('node-id');
        diskId = (diskId == 'undefined' || diskId == '' || diskId == null) ? diskId1 : diskId;
        ...
        $.popWindow(url,'预览','0','0','1200px','600px');
    ...
    $('#file_Tachr').on('click','.operationImg',function () {
        ...
        // console.log("openimg : "+$(this).next().attr('openimg'));
        var openNmu = $(this).next().attr('openNmu')
        var str3 = '<input type="text" id="getIndex" openNum = "'+openNmu+'" style="display: none" value="'+thisa+'">'
    ...
    $('#file_Tachr').on('click','.seeDiv',function () {
        ...
        if(myBrowser() == "Chrome"||myBrowser() == "IE11"){
            ...
            $.popWindow("/pdfPreview?"+attrUrl+'&diskId='+ diskId1+ urlstr,'预览','0','0','1500px','800px');
        }else{
        ...
        layer.open({
           ...
           <p style="line-height: 8px;height: 12px;">\
            ...
            yes:function (index) {
                ...
                // console.log(objNode+"123")
                var path="";
                if(objNode != null && objNode.attributes!= undefined){
                    if (objNode.attributes.firstUrl != undefined){
                        path = objNode.attributes.firstUrl;
                        path = path.substr(path.lastIndexOf("/"),path.length);
                    }else {
                        path = objNode.attributes.fullUrl;
                        path = path.replace(parentPath,"");
                    }

                }

                console.log("params1[path]:"+path);
                console.log("params1[directoryName]:"+$('[name="newzi"]').val());
                console.log("params1[remind]:"+remindVal);
                console.log("params1[diskId]:"+diskId1);
                console.log("params1[smsDefault]:"+smsDefault);
                ...
                $.post('/netdisk/mkDirectory',{
                    ...
                    if(json.flag){
                        ...
                        $.layerMsg({content:depatement_th_Newfailed,icon:2})
                    }
                },'json');
            },success:function () {
                if (objNode != null) $('.gttext').html('<span><b>&gt;</b>' + objNode.text + '</span>');
            }
        })
    })
    ...
    $('#classA ul li').click(function () {//文件夹里面按钮
        ...
        if($(this).attr('data-id')==1){
            if($('#fileTree .tree-node-selected').parent().parent().hasClass('easyui-tree')){
                $.layerMsg({content: '网络硬盘根目录文件不允许重命名!', icon: 2});
            }else{
                ...
                layer.open({
                  ...
                  <p style="line-height: 65px;height: 65px;">\
                    ...
                    yes:function (index) {
                        ...
                        // console.log($('.tree-node-selected').attr('node-id')+"234")
            ...
            if(objNode != null && objNode.attributes !=undefined){
                console.log(path);
                if (objNode.attributes.fullUrl == undefined){
                    path = objNode.attributes.firstUrl;
                    path = path.substr(path.lastIndexOf("/"),path.length);
                }else{
                    path = objNode.attributes.fullUrl;
                ...
                $.post('/netdisk/deleteFolder',
                    ...
                    ,function (json) {
                        if(json.flag){
                            ...
                            $.layerMsg({content:"没有此文件的管理权限",icon:0})
    ...
    $('#SEARCH').click(function () {
        ...
        layer.open({
            ...
            content:'<p style="line-height: 65px;height: 65px;">\
                  ...
                  <p style="color: #ff0000;font-size: 12px;padding-left: 138px;">'+file3+'</p>',
            area:['500px','300px'],
            btn:[menusetsure,quxiao],
            yes:function (index) {
    ...
    $('.dowmloadOne').click(function () {
        ...
        if($('#file_Tachr input[type="checkbox"]:checked').length==1) {
            ...
            console.log("diskId: "+diskId1);
            window.location.href='/netdisk/download?path=' +myEncodeURI(path)+ '&diskId='+diskId1;
        }else {
            var strarr=[];
            ...
            // window.open('/netdisk/downLoadZipFile?path='+strarr+ '&diskId='+diskId);
             window.location.href='/netdisk/downLoadZipFile?path='+strarr+ '&diskId='+diskId;
        }
    });
    ...
    $('.copyfile').click(function () {
        ...
        if(copynum == 1){
            $.layerMsg({content:'剪切成功',icon:1})
        }else{
            $.layerMsg({content:'复制成功',icon:1})
        }
    })
    ...
    $('.SIX').click(function () {//粘贴
        ...
        if(objNode != null && objNode.attributes !=undefined){
            console.log(toPath);
            if (objNode.attributes.fullUrl == undefined){
                toPath = objNode.attributes.firstUrl;
                toPath = toPath.substr(toPath.lastIndexOf("/"),toPath.length);
            }else{
                toPath = objNode.attributes.fullUrl;
                toPath = toPath.replace(parentPath,"");
            }
