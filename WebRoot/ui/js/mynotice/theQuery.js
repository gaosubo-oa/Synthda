...
function ajaxforms(type) {
    ...
    if(type==2){
        // console.log('/myNotice/notifyList?'+$('#ajaxform').serialize()+'&page=1&pageSize=15&useFlag=true')
        window.location.href='/myNotice/notifyList?'+$('#ajaxform').serialize()+'&page=1&pageSize=15&useFlag=false&specifyTable='+specifyTable;
        return;
    }
...
$(function () {
    ...
    pageObj=$.tablePage('#pagediv',bodyWidth+'px',[
       ...
       border-radius: 3px;">置顶</span>'+
                    ...
                    return '<div style="width: 100%;text-align: left" title="'+name+'">' +
                        '<a href="/myNotice/detail?notifyId='+obj.notifyId+'" ' +
    ...
    $('#pagediv').on('click','.editAndDelete0',function(){
        ...
        $.popWindow("finddetail?notifyId="+notifyId+"&specifyTable="+specifyTable,'查阅情况','0','0','1200px','600px');
    })
