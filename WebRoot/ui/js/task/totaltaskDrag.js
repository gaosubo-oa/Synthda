...
(function(factory) {
    ...
    $.fn.arrangeable1 = function(options) {
        var dragging = false;
        var $clone;
        var dragElement;
        ...
        function dragEndHandler(e) {
            if (dragging) {
                ...
                //主键id
                var taskId = dragElement.getAttribute('taskId')
                //获得当前拖拽元素得typeid
                var parentsFj = $("#"+fId).parents('.content_list_item').attr('typeId')
                //追加在li最后
                // $("#"+fId).parent().append($dragElement)
                //寻找当前拖拽元素得父级
                var parentsF = $("#"+fId).parent()
                var childrenZ = parentsF.children()
                // 获取最外框的顺序
                for(var j=0;j<childrenZ.length;j++){
                    var Liid1 = childrenZ[j].getAttribute('typeid')
                    arr2.push(Liid1)
                }
                var arr3=arr2.toString().substr(1);
                $.ajax({
                    type:'post',
                    url:'/taskClass/updateTask',
                    data:{
                        typeIds:arr3
                    },
                    dataType:'json',
