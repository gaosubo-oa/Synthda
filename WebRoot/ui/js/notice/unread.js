...
//表格初始化
    ...
    $('#button1').on('click',function () {
        layer.confirm(notice_th_IsOneClickRead, {
            btn: [sure,cancel], //按钮  <%--确定 取消--%>
            title:notice_th_OneClickRead
    }, function(){
            var sendTime=''
            $.ajax({
                type: 'post',
                url: '/notice/notifyManage?page=1&pageSize=10&useFlag=true&typeId=0&read=0&sendTime='+sendTime,
                dataType: 'json',
                success: function (res) {
                    var data=res.obj
                    var notifyIds=[]
                    for(i=0;i<data.length;i++){
                        var notifyId=data[i].notifyId
                        notifyIds+=notifyId+','
                    }
                    $.ajax({
                        type: 'post',
                        url: '/notice/readNotify',
                        dataType: 'json',
                        data:{
                            notifyIds:notifyIds,
                            changId:0
                        ...
                        success: function (res) {
                            if(res.flag){
                                layer.msg(notice_th_OneClickReadSuccess,{icon:1,time:2000},function () {
                                    location.reload();
                                })
                            }else{
                                layer.msg(notice_th_OneClickReadFail,{icon:0,time:2000},function () {
                                    location.reload();
                                })
                            }
                        }
...
function detail(nid) {
    layer.open({
        ...
        //     window.print();
        // },
        content:'/notice/detail?notifyId='+nid+'&noticeType=1',
        cancel: function(){
