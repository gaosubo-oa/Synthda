...
function ajaxforms(type) {
    if(!$('#ajaxform select[name="typeId"]').val()){
        layer.msg(notice_th_noticeType+'！',{icon:0});
        return false
    }
    if($('#ajaxform select[name="contentSecrecy"]').val()!=undefined&&$('#ajaxform select[name="contentSecrecy"]').val().indexOf(dem_th_PleaseSecret)!=-1){
        layer.msg(dem_th_PleaseSecret+'！',{icon:0});
        return false
    }
    ...
    if($('[name=subject]').val() == ''){
    ...
    }else if($('#ajaxform select[name="contentSecrecy"]').val()!=undefined&&$('#ajaxform select[name="contentSecrecy"]').val().indexOf(dem_th_PleaseSecret)!=-1){
        layer.msg(dem_th_PleaseSecret+'！',{icon:0});
        return false
    }else{
        ...
        if(index1 == 2 && index3 == 2 &&index2 == 2){
            ...
            if($('[name="topDing"]').prop('checked') && $('[name="topDays"]').val()==''){
                $.layerMsg({content:notice_th_pleaseFillInTheNumberOfDaysForPinning,icon:0});
                return false
            }else if($('[name="topDing"]').prop('checked') && $('[name="topDays"]').val()=='0'){
                layer.confirm(notice_th_pinnedTime, {
                    btn: ['是', '否']
    ...
    function GetDropDownBox(fn) {
        $.ajax({
            ...
            success:function(data){
                var str='<option value="" data-isEdit="'+data.object.isEdit+'">'+notice_th_chosenotifytype+'</option>';
...
//     });
    ...
    if(getRequestObj.type=='new'){
        $.ajax({
            ...
            success:function (res) {
                if(res.object.length!=0){
                    ...
                    if (data.paraValue!=0){
                        $('#symbol').after('<select name="contentSecrecy" style="height: 30px;width: 150px;margin-left: 50px;">' +
                            '<option id="first">'+dem_th_PleaseSecret+'</option>' +
        ...
        var txt= notify_th_AnnounceNoticeOfNotice
        $('.navigation').find('h2').text(txt);
        ...
        ue.ready(function () {
            $.get('getOneById',{
                ...
                if(json.flag){
                    var obj=json.object;
                    contentSecrecy=obj.contentSecrecy;
                    var arr=$('input');
                    (function (fn) {
                        for(var i=0;i<arr.length;i++){
                            for(var key in obj){
                            ...
                            if (data.paraValue!=0){
                                ...
                                $.ajax({
                                    ...
                                    success:function (res) {
                                        var str=''
                                        for (let i = 0; i <res.obj.length ; i++) {
                                            if(contentSecrecy!=undefined&&contentSecrecy.indexOf(res.obj[i].codeNo)!=-1){
