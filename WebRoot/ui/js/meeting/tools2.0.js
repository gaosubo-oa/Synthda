...
getAttachElement:function(attachModel,opts){
	...
	if((priv & 1)==1){//阅读权限
		if(this.isImage(ext)){//如果是图片
			attachElement.attr("pic",attachModel.sid);//如果是图片，则每次渲染的时候，给div加入pic属性，pic值为附件id
			menuData.push({name:"预览",action:function(sid){
				var attachIds = [];
				$("div[pic]").each(function(i,obj){
					attachIds.push(obj.getAttribute("pic"));
				});
				
				var url = contextPath +"/system/core/attachment/picExplore.jsp?id="+sid+"&pics="+attachIds.join(",");
			...
			menuData.push({name:"查看",action:function(sid,fileName){
				var ntkoURL = contextPath +  "/system/core/aip/aip4view.jsp?docId="+sid;
				openFullWindow(encodeURI(ntkoURL),"在线文档阅读");
			} ,extData:[attachModel.sid,attachModel.fileName]});
		}else if(this.isMedia(ext)){
			menuData.push({name:"播放",action:function(sid,fileName){
				openFullWindow(contextPath+"/common/CuPlayer/play.jsp?id="+sid+"&fileName="+encodeURI(fileName),"正在播放"+fileName);
			} ,extData:[attachModel.sid,attachModel.fileName]});
		}
	}
	...
	if((priv & 2)==2){//转储权限
		menuData.push({name:"转存",action:function(sid){
			openWindow(contextPath+"/system/core/base/saveFileTo/indexh5.jsp?attachId="+sid,"附件转存",480,350);
		} ,extData:[attachModel.sid]});
	}
 ...
 * 获取附件空间信息
 */
spaceInfo:function(model){
	var url = contextPath+"/attachmentController/getLimitedSpaceInfo.action";
	var json = tools.requestJsonRs(url,{model:model});
	var datas = json.rtData;
	return datas;
},
string2Unicode:function(str) {
    var res=[];
    for(var i=0;i < str.length;i++)
...
$.fn.addFav=function(title,url){
	$(this).click(function(){
		var json = tools.requestJsonRs(contextPath+"/favoriteRecord/addFavoriteRecord.action",{title:title,url:url});
  ...
  for (var property in json) {
    ...
    if (cntrlCnt == 1) {
      ...
      if (cntrl.tagName.toLowerCase() == "input" && cntrl.type.toLowerCase() == "checkbox") {
        ...
        cntrl.innerHTML = value == null ? "" :value;
