...
//初始化区域图的对象
    ...
    if (property.haveHead) {
        tmp = "<div class='GooFlow_head'>" +
                // "<p class='Toptitle'>" +
                //     "<img src='' alt=''>" +
                //     "<a href='javascript:;' class='atitle' title='" + (property.initLabelText || "newFlow_1") + "'>"+(property.initLabelText || "newFlow_1")+"</a>" +
                // "</p>"
            "<label class='atitle' title='" + (property.initLabelText || "newFlow_1") + "'>" + (property.initLabelText || "newFlow_1") + "</label>" +
            "<span></span>";
    ...
    this.$bgDiv.append("<div class='GooFlow_work' style='width:" + (width) + "px;height:" +function () {
           if(GetRequest().type!=undefined&&GetRequest().type==3){
               return property.height;
           }else {
               return (height)
           }
        }()  + "px;" + (property.haveHead ? "" : "margin-top:3px") + "'></div>");
    this.$workArea = $("<div class='GooFlow_work_inner' style='width:" + width * 2 + "px;height:" + height * 2 + "px'></div>")
...
GooFlow.prototype = {
    ...
    addNode: function (id, json) {
        ...
        if (json.type != "start" && json.type != "end") {
            ...
            this.$nodeDom[id] = $("<div class='GooFlow_item" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><table cellspacing='1' style='width:" + (json.width - 2) + "px;'><tr><td class='ico'><b class='ico_" + json.type + "'></b></td><td class='tdwidth' style='"+ size +"'><div class='divbox'>" + jsonname + "</div></td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
...
//条件设置与通达OA兼容方法
                ...
                for(var i=0;i<arr.length;i++){
                    if(arr[i]!=""){
                        if(arr[i].indexOf('MAIN_USER_SIGN')>-1){
                            ...
                            prcsIn+=arr[i].replace(/CUR_USER_PRIV_OTHER/g,'当前主办人辅助角色')+','
        ...
        this.$nodeDom[id][0].onmouseover=function () {//手动添加鼠标移入事件
            var meTwoa=this;
            // console.log(jsonTwo)
            if(GetRequest().rilwId==undefined) {
                // conditionsSet
                ...
                if(jsonTwo.data!=undefined){
                        ...
                        for (var mxS = 0; mxS < jsonTwoprcsTo.length; mxS++) {
                            for (var ixS = 0; ixS < designdata.length; ixS++) {
                                if (jsonTwoprcsTo[mxS] == designdata[ixS].prcsId) {
                                    jsonTwoprcsTostr += '<span>' + designdata[ixS].prcsName + ',</span>'
                                }
                            }
                        }
                    }
                    ...
                    layer.tips('<p>本步骤名称：'+jsonTwo.name+'</p>' +
                        '<p>'+function () {
                           if(jsonTwoprcsTostr!=''){
                ...
                //<p style="font-weight: bold;color:#111">' + jsonTwo.name + '</p>
                    layer.tips('<div>' +
                        '<p>' +
                        '<a style="color: red;text-decoration:underline " href="javascript:void(0)">' + jsonTwo.data.prcsUserName + '&nbsp;&nbsp;' + function () {
                            ...
                            if (jsonTwo.data.prcsFlag == 1) {
                                return '未接收)(到达时长：'+ jsonTwo.data.arriveTime +'；办理时长：'+ jsonTwo.data.handleTime
                            } else if (jsonTwo.data.prcsFlag == 2) {
                                return '办理中)(到达时长：'+ jsonTwo.data.arriveTime +'；办理时长：'+ jsonTwo.data.handleTime
                            } else if (jsonTwo.data.prcsFlag == 3) {
                                return '已转交)(到达时长：'+ jsonTwo.data.arriveTime +'；办理时长：'+ jsonTwo.data.handleTime
                            } else if (jsonTwo.data.prcsFlag == 4) {
                                return '已办结)(到达时长：'+ jsonTwo.data.arriveTime +'；办理时长：'+ jsonTwo.data.handleTime
                            } else if (jsonTwo.data.prcsFlag == 5) {
                                return '自由流程预设步骤)(到达时长：'+ jsonTwo.data.arriveTime +'；办理时长：'+ jsonTwo.data.handleTime
                            } else if (jsonTwo.data.prcsFlag == 6) {
                                return '已挂起)(到达时长：'+ jsonTwo.data.arriveTime +'；办理时长：'+ jsonTwo.data.handleTime
                            }
                        }() + ')</span>' +
                        '</p>' + function(){
                                if(jsonTwo.data.prcsFlag == 1){
                                    if(jsonTwo.data.timeOutDesc && jsonTwo.data.timeOutFlag) {
                                        return  '<p style="color: '+(jsonTwo.data.timeOutFlag == 1?"red":"green")+'">' + jsonTwo.data.timeOutDesc + '</p>'
                                    }
                                    return ''
                                }else{
                                    return   '<p style="color: #666;">到达于: ' + jsonTwo.data.createTime + '</p>' +
                                                '<p style="color: #666;">接收于: ' + jsonTwo.data.prcsTime + '</p>' +
                                                '<p style="color: #666;">办结于: ' + jsonTwo.data.deliverTime + '</p>'
                                }
                        }()+
                        '</div>', meTwoa, {time: 3000, area: ['300px', '200px'], tips: [2, 'rgba(255,248,220,0.85)']})
                }else {
                    layer.tips(jsonTwo.name, meTwoa, {time: 3000})
                }
            }
    ...
    delNode: function (id,type) {
        ...
        var metwo=this;
        $.post('/flowProcess/delete',{'id':$('#ele_designerId').val()},function (json) {
            if(json.flag) {
                location.reload();
            }
        },'json')
...
///////////以下为有关画线的方法
    ...
    addLine: function (id, json) {
        ...
        for (var k in this.$lineData) {
            if ((json.from == this.$lineData[k].from && json.to == this.$lineData[k].to)
                || (json.from == this.$lineData[k].to && json.to == this.$lineData[k].from))
                return;
        }
        ...
        if (GooFlow.prototype.useSVG == "") {
            ...
            if(newlinebool) {
                ...
                theNewLine(this.$nodeData[this.$lineData[id].from].designerId);
            }
        }
        ++this.$lineCount;
    },
    ...
    delLine: function (id) {
        ...
        var meTwo=this;
        $.get('/flowProcess/deleteTopAndLeft',{'prcsId':this.$lineData[id].from.replace('node_',''),'flowId':flowstr,
        'toPrcsLine':this.$lineData[id].to.replace('node_','')},function (json) {
            // console.log(meTwo)
