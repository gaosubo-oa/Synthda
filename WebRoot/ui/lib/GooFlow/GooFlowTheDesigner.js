...
//初始化区域图的对象
    ...
    if (property.haveHead) {
        tmp = "<div class='GooFlow_head'>" +
            "<p class='Toptitle'>" +
            "<img src='../../img/workflow/flowsetting/titleimg.png' alt=''>" +
            "<a href='javascript:;' class='atitle' title='" + (property.initLabelText || "") + "'>" + (property.initLabelText || "") + "</a>" +
            "</p>"
        // "<label title='" + (property.initLabelText || "newFlow_1") + "'>" + (property.initLabelText || "newFlow_1") + "</label>" +
        // "<span></span>";
    ...
    if (property.haveTool) {
        var heightints = parseInt(document.documentElement.clientHeight || document.body.offsetHeight)
        // height - headHeight - (property.haveHead ? 7 : 10)
        this.$bgDiv.append("<div class='GooFlow_tool'" + (property.haveHead ? "" : " style='margin-top:3px'") + "><div style='height:" + (heightints - $('.GooFlow_head').height() - 7) + "px' class='GooFlow_tool_div'></div></div>");
        this.$tool = this.$bgDiv.find(".GooFlow_tool div");
        //未加代码：加入绘图工具按钮
        this.$tool.append("<a href='javascript:void(0)' class='GooFlow_tool_btndown' " +
            "id='" + this.$id + "_btn_cursor'>" +
            "<b class='ico_cursor'></b><label>选择工具</label>" +
            "</a>" +
            "<a href='javascript:void(0)' class='GooFlow_tool_btn' " +
            "id='" + this.$id + "_btn_direct'><b class='ico_direct'></b><label>节点连线</label>" +
            "</a>" +
            "<a href='javascript:void(0)' class='GooFlow_tool_btn' id='" + this.$id + "_btn_start'>" +
            "<b class='ico_start'></b><label>流程开始节点</label>" +
            "</a>");//<a href='javascript:void(0)' class='GooFlow_tool_btn' id='" + this.$id + "_btn_end'><b class='ico_end'/></a>
        if (property.toolBtns && property.toolBtns.length > 0) {
            tmp = "";
            for (var i = 0; i < property.toolBtns.length; ++i) {
                tmp += "<a href='javascript:void(0)' id='" + this.$id + "_btn_" + property.toolBtns[i] + "' class='GooFlow_tool_btn'><b class='ico_" + property.toolBtns[i] + "'></b>" +
                    "<label>" + function () {
                        if (property.toolBtns[i] == 'chat') {
                            return '流程步骤节点'
                        } else if (property.toolBtns[i] == 'end round') {
                            return '流程结束节点'
                        }
                    }() + "</label>" +
                    "</a>";//加入自定义按钮
        ...
        // this.$tool.append("<span/><a href='javascript:void(0)' class='GooFlow_tool_btn' id='" + this.$id + "_btn_fork'><b class='ico_fork'/></a><a href='javascript:void(0)' class='GooFlow_tool_btn' id='" + this.$id + "_btn_join'><b class='ico_join'/></a><a href='javascript:void(0)' class='GooFlow_tool_btn' id='" + this.$id + "_btn_complex'><b class='ico_complex'/></a><a href='javascript:void(0)' class='GooFlow_tool_btn' id='" + this.$id + "_btn_group'><b class='ico_group'/></a>");
        toolWidth = 31;
        this.$nowType = "cursor";
    ...
    var widthint = parseInt(document.documentElement.clientWidth || document.body.offsetWidth)
    var heightint = parseInt(document.documentElement.clientHeight || document.body.offsetHeight)
    this.$bgDiv.append("<div class='GooFlow_work' style='width:" + (widthint - 250) + "px;height:" + (heightint - $('.GooFlow_head').height() - 7) + "px;" + (property.haveHead ? "" : "margin-top:3px") + "'></div>");
    ...
    if (this.$editable) {
        this.$workArea.on("click", {inthis: this}, function (e) {
            ...
            if (designdata.length != 0) {
                var title = designdata[designdata.length - 1].prcsId + 1;
                title += '.'
            } else {
                var title = '';
            }
            if ($('.GooFlow_tool_btndown').attr('title') == '开始节点') {
                if ($('.item_round').length == 0) {
                    e.data.inthis.addNode("node_" + (function () {
                        if (designdata.length == 0) {
                            return 1;
                        } else {
                            return designdata[designdata.length - 1].prcsId + 1;
                        }
                    })(), {
                        name: title + "步骤" + (function () {
                            if (designdata.length == 0) {
                                return 1;
                            } else {
                                return designdata[designdata.length - 1].prcsId + 1;
        ...
        //划线时用的绑定
        this.$workArea.on('mousemove', {inthis: this}, function (e) {
        ...
        this.$workArea.on('mouseup', {inthis: this}, function (e) {
            if (e.data.inthis.$nowType != "direct") return;
        ...
        this.$lineOper.on("click", {inthis: this}, function (e) {
            ...
            var atPresent = This.$lineData[This.$focus].from
            switch ($(e.target).attr("class")) {
                case "b_x":
                    This.delLine(id, atPresent);
                    this.style.display = "none";
                    break;
                case "b_l1":
                    This.setLineType(id, "lr", true);
                    break;
                case "b_l2":
                    This.setLineType(id, "tb", true);
                    break;
                case "b_l3":
                    This.setLineType(id, "sl", true);
                    break;
            }
        });
...
GooFlow.prototype = {
    ...
    initDraw: function (id, width, height) {
        ...
        if (GooFlow.prototype.useSVG != "") {
            ...
            this.$workArea.prepend("<div class='GooFlow_work_vml' style='position:relative;width:" + width * 2 + "px;height:" + height * 2 + "px'></div>");
        ...
        if (this.$editable)
            $(this.$draw).on("dblclick", tmpClk, {inthis: this}, function (e) {
                ...
                }).data("id", This.$focus).trigger('focus');
    ...
    initGroup: function (width, height) {
        this.$group = $("<div class='GooFlow_work_group' style='width:" + width * 2 + "px;height:" + height * 2 + "px'></div>");//存放背景区域的容器
    ...
    addNode: function (id, json) {
        if (this.onItemAdd != null && !this.onItemAdd(id, "node")) return;
        if (this.$undoStack) {
            this.pushOper("delNode", [id]);
        }
        var mark = json.mark ? " item_mark" : "";
        if (json.type != "start" && json.type != "end") {
            json.width = json.width ? json.width + 24 : 136;
            json.height = json.height ? json.height + 24 : 48;
            if (json.top < 0) json.top = 0;
            if (json.left < 0) json.left = 0;
            var hack = 0;
            if (navigator.userAgent.indexOf("8.0") != -1) hack = 2;
            if (json.name.length > 14) {
                var jsonname = json.name.substr(0, 14) + '...';
            } else {
                var jsonname = json.name;
            }
            if (json.name.length > 7) {
                var size = ''
            } else if (json.name.length < 6) {
                var size = 'font-size: 17px;'
            } else {
                var size = 'font-size: 14px;'
            }
            this.$nodeDom[id] = $("<div class='GooFlow_item" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px'><table cellspacing='1' style='width:" + (json.width - 2) + "px;'><tr><td class='ico' style='position: relative;z-index: 999;'><b style='display: inline-block;width:" + (json.width - 2) + "px;' class='ico_" + json.type + "'></b></td><td class='tdwidth' style='position: relative;z-index: 888;" + size + "'><div class='divbox'>" + jsonname + "</div></td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");

            if (GetRequest().rilwId == undefined) {

                if (json.type == 'complex') this.$nodeDom[id].addClass('item_complex');

                if (json.type == 'end round') this.$nodeDom[id].addClass('item_focusTwo');

            } else {

                if (GetRequest().trueId == undefined) {
                    if (json.data.prcsFlag == 1) {
                        this.$nodeDom[id].addClass('didNotReceive');
                    } else if (json.data.prcsFlag == 2) {
                        this.$nodeDom[id].addClass('inProcess');
                    } else if (json.data.prcsFlag == 4) {
                        this.$nodeDom[id].addClass('processed');
                    } else if (json.data.prcsFlag == 6) {
                        this.$nodeDom[id].addClass('hangUp');
                    } else if (json.data.prcsFlag == 3) {
                        this.$nodeDom[id].addClass('processed');
                    }
                } else {
                    if (json.type == 'start') {
                        this.$nodeDom[id].addClass('startBegin');
                    } else if (json.type == 'end round') {
                        this.$nodeDom[id].addClass('finish');
                    } else if (json.type == 'chat') {
                        this.$nodeDom[id].addClass('ordinary');
                    }
                }
        ...
        if (designdata.length == 0) {
            ...
            numId.prcsId = designdata[designdata.length - 1].prcsId + 1;
        }
        numId.setTop = parseInt(json.top);
        numId.setLeft = parseInt(json.left);
            ...
            if (designdata.length != 0) {
                ...
                for (var i = 0; i < obj.length - 1; i++) {
                    for (var j = 0; j < obj.length - 1 - i; j++) {
                        if (obj[j].prcsId > obj[j + 1].prcsId) {
                            var swap = obj[j].prcsId;
                            obj[j].prcsId = obj[j + 1].prcsId;
                            obj[j + 1].prcsId = swap;
                        }
                    }
                }
                numId.prcsId = obj[obj.length - 1].prcsId + 1;
                numId.prcsName = numId.prcsId + '.步骤' + numId.prcsId;
                if (json.type == "end round") {
                    numId.prcsTo = '0,';
                }
            }
            saveOrUpdate(function () {
                json.designerId = prcsIdTwo;
                location.reload();
            });
        }
...
//条件设置与通达OA兼容方法
        function checkCondition(arrprcsIn) {
            var prcsIn = ''
            if (arrprcsIn != "") {
                var arr = arrprcsIn.split(',');
                for (var i = 0; i < arr.length; i++) {
                    if (arr[i] != "") {
                        if (arr[i].indexOf('MAIN_USER_SIGN') > -1) {
                            prcsIn += arr[i].replace(/MAIN_USER_SIGN/g, '主办人会签意见') + ','
                        } else if (arr[i].indexOf('USER_SIGN') > -1) {
                            prcsIn += arr[i].replace(/USER_SIGN/g, '经办人会签意见') + ','
                        } else if (arr[i].indexOf('ATTACH_NAME') > -1) {
                            prcsIn += arr[i].replace(/ATTACH_NAME/g, '公共附件名称') + ','
                        } else if (arr[i].indexOf('ATTACH_COUNT') > -1) {
                            prcsIn += arr[i].replace(/ATTACH_COUNT/g, '公共附件个数') + ','
                        } else if (arr[i].indexOf('CUR_PRCS_ID') > -1) {
                            prcsIn += arr[i].replace(/CUR_PRCS_ID/g, '当前步骤号') + ','
                        } else if (arr[i].indexOf('CUR_FLOW_PRCS') > -1) {
                            prcsIn += arr[i].replace(/CUR_FLOW_PRCS/g, '当前流程设计步骤号') + ','
                        } else if (arr[i].indexOf('CUR_USER_NAME') > -1) {
                            prcsIn += arr[i].replace(/CUR_USER_NAME/g, '当前主办人姓名') + ','
                        } else if (arr[i].indexOf('CUR_USER_PRIV_OTHER') > -1) {
                            prcsIn += arr[i].replace(/CUR_USER_PRIV_OTHER/g, '当前主办人角色_辅助角色') + ','
                        } else if (arr[i].indexOf('CUR_USER_PRIV') > -1) {
                            prcsIn += arr[i].replace(/CUR_USER_PRIV/g, '当前主办人角色') + ','
                        } else if (arr[i].indexOf('CUR_USER_DEPT_PARENT') > -1) {
                            prcsIn += arr[i].replace(/CUR_USER_DEPT_PARENT/g, '当前主办人上级部门') + ','
                        } else if (arr[i].indexOf('CUR_USER_DEPT') > -1) {
                            prcsIn += arr[i].replace(/CUR_USER_DEPT/g, '当前主办人部门') + ','
                        } else {
                            prcsIn += arr[i] + ',';
                        }
                    }
        ...
        this.$nodeDom[id][0].onmouseover = function (e) {//手动添加鼠标移入事件
            ...
            if (GetRequest().rilwId == undefined) {
                ...
                if (jsonTwo.data != undefined) {
                    var jsonTwoprcsTostr = '';
                    var arrprcsInArrs = '';
                    var arrprcsInArrsTwo = '';
                    if (jsonTwo.data.prcsTo != undefined) {
                        var jsonTwoprcsTo = jsonTwo.data.prcsTo.split(',');
                        for (var mxS = 0; mxS < jsonTwoprcsTo.length; mxS++) {
                            if (jsonTwoprcsTo[mxS] == 0 && jsonTwoprcsTo[mxS] != '') {
                                jsonTwoprcsTostr += '<span>结束，</span>'
                            } else {
                                for (var ixS = 0; ixS < designdata.length; ixS++) {
                                    if (jsonTwoprcsTo[mxS] == designdata[ixS].prcsId) {
                                        jsonTwoprcsTostr += '<span>' + jsonTwoprcsTo[mxS] + '.' + designdata[ixS].prcsName + '，</span>'
                                    }
                                }
                            }
                        }
                    }
                    if (jsonTwo.data.conditionsSet != undefined) {
                        if (jsonTwo.data.conditionsSet.prcsIn != '') {
                            arrprcsInArrs = jsonTwo.data.conditionsSet.prcsIn.replace(/\n/g, ',');
                        }
                        if (jsonTwo.data.conditionsSet.prcsOut != '') {
                            arrprcsInArrsTwo = jsonTwo.data.conditionsSet.prcsOut.replace(/\n/g, ',');
                        }

                    }

                    layer.tips('<p style="color: #333;">本步骤名称：' + jsonTwo.name + '</p>' +
                        '<p style="color: #333;">' + function () {
                            if (jsonTwoprcsTostr != '') {
                                return '下一步骤：' + jsonTwoprcsTostr
                            } else {
                                return ''
                            }
                        }() + '</p>' +
                        '<p style="color: #333;">' + function () {
                            if (arrprcsInArrs != '') {
                                return '转入条件：' + checkCondition(arrprcsInArrs)
                            } else {
                                return ''
                            }
                        }() + '</p>' +
                        '<p style="color: #333;">' + function () {
                            if (jsonTwo.data.conditionsSet.prcsInSet != '') {
                                return '转入条件公式：' + jsonTwo.data.conditionsSet.prcsInSet
                            } else {
                                return ''
                            }
                        }() + '</p>' +
                        '<p style="color: #333;">' + function () {
                            if (arrprcsInArrsTwo != '') {
                                return '转出条件：' + checkCondition(arrprcsInArrsTwo)
                            } else {
                                return ''
                            }
                        }() + '</p>' +
                        '<p style="color: #333;">' + function () {
                            if (jsonTwo.data.conditionsSet.prcsOutSet != '') {
                                return '转出条件公式：' + jsonTwo.data.conditionsSet.prcsOutSet
                            } else {
                                return ''
                            }
                        }() + '</p>', meTwoa, {time: 3000, tips: [2, 'rgba(255,248,220,0.85)']})
                } else {
                    layer.tips('<p style="color: #333;">' + jsonTwo.name + '</p>', meTwoa, {
                        time: 3000,
                        tips: [2, 'rgba(255,248,220,0.85)']
                    })
                }
                ...
                if (GetRequest().trueId == undefined) {
                    ...
                    layer.tips('<div><p style="font-weight: bold;color:#111">' + jsonTwo.name + '</p>' +
                        '<p>' +
                        '<a style="color: red;text-decoration:underline " href="javascript:void(0)">' + jsonTwo.data.prcsUserName + '&nbsp;&nbsp;' + function () {
                            if (jsonTwo.data.opFlag == 0) {
                                return '经办'
                            } else if (jsonTwo.data.opFlag == 1) {
                                return '主办'
                            }
                        }() + '</a><span style="color: #00aa00;margin-left: 5px;"> (' + function () {
                            if (jsonTwo.data.prcsFlag == 1) {
                                return '未接收'
                            } else if (jsonTwo.data.prcsFlag == 2) {
                                return '办理中' + jsonTwo.data.handleTime
                            } else if (jsonTwo.data.prcsFlag == 3) {
                                var handleTimes;
                                if (jsonTwo.data.handleTime == undefined) {
                                    handleTimes = ''
                                } else {
                                    handleTimes = jsonTwo.data.handleTime
                                }
                                return '转交下一步,用时' + handleTimes
                            } else if (jsonTwo.data.prcsFlag == 4) {
                                return '已办结' + jsonTwo.data.handleTime
                            } else if (jsonTwo.data.prcsFlag == 5) {
                                return '自由流程预设步骤'
                            } else if (jsonTwo.data.prcsFlag == 6) {
                                return '已挂起'
                            }
                        }() + ')</span>' +
                        '</p>' +
                        '<p style="color: #666;">开始于: ' + jsonTwo.data.createTime + '</p>' +
                        '<p style="color: #666;">结束于: ' + jsonTwo.data.deliverTime + '</p>' +
                        '</div>', meTwoa, {
                        time: 3000,
                        area: ['300px', '150px'],
                        tips: [2, 'rgba(255,248,220,0.85)']
                    ...
                    layer.tips(jsonTwo.name, meTwoa, {time: 3000})
                }
            }
            // }
        }

        // console.log(this.$nodeDom[id][0])
    ...
    moveNode: function (id, left, top, name) {
        ...
        numIds.setTop = parseInt(top);
        numIds.setLeft = parseInt(left);
    ...
    delNode: function (id, type) {
        if (!this.$nodeData[id]) return;
        if (this.$nodeData[id].data.prcsId == '1' || this.$nodeData[id].data.prcsId == 1) {
            if (this.$nodeCount == 1) {
                layer.open({
                    title: '删除节点',
                    content: '删除流程开始节点会造成流程无法转交，确认是否删除？',
                    icon: 0,
                    btn: ['确定', '取消'],
                    yes: function (index, layero) {
                        ...
                        $.post('/flowProcess/delete', {
                            'id': $('#ele_designerId').val(),
                            prcsId: $('[name="prcsId"]').val(),
                            flowId: flowId
                        }, function (json) {
                            if (json.flag) {
                                location.reload();
    ...
    setTitle: function (text) {
        ...
        if (this.$head) this.$head.find(".atitle").attr("title", text).text(text);
    },
    //载入一组数据
...
///////////以下为有关画线的方法
    ...
    addLine: function (id, json) {
        ...
        if (this.onItemAdd != null && !this.onItemAdd(id, "line")) return;
        if (this.$undoStack) {
        ...
        if (GooFlow.prototype.useSVG == "") {
            ...
            if (newlinebool) {
                if (newLineload != json.from.replace('node_', '')) {
                    newLineend.splice(0, newLineend.length)
                    newLineload = json.from.replace('node_', '');
                    newLineend.push(json.to.replace('node_', ''));
                } else {
                    newLineload = json.from.replace('node_', '');
                    newLineend.push(json.to.replace('node_', ''));
                }

                if (n1.designerId != undefined) {
                    prcsIdTwo = n1.designerId
                }


                // console.log(id)
                // console.log(this.$lineData[id].from)
                // console.log(this.$lineData[id].from)
                // console.log()
                // console.log(newLineend)
                theNewLine(this.$nodeData[this.$lineData[id].from].designerId, this.$nodeData[this.$lineData[id].from]);
            }
        }
        ++this.$lineCount;
    },
    ...
    setLineType: function (id, newType, flag) {
        ...
        if (flag) {
            //调用修改线型接口（步骤id，下一步步骤，线型）
            theUpdateLine(this.$nodeData[from].designerId, to.substring(5), newType);
        }
    },
    ...
    //删除转换线
    delLine: function (id, atPresent) {
        // console.log(this.$lineData[id])
        if (!this.$lineData[id]) return;
        if (this.onItemDel != null && !this.onItemDel1(id, "node")) return;
        if (this.$undoStack) {
            var paras = [id, this.$lineData[id]];
            this.pushOper("addLine", paras);
        }
        var meTwo = this;
        $.get('/flowProcess/deleteTopAndLeft', {
            'prcsId': this.$lineData[id].from.replace('node_', ''), 'flowId': flowstr,
            'toPrcsLine': this.$lineData[id].to.replace('node_', '')
        }, function (json) {
            meTwo.$nodeData[atPresent].data.prcsTo = meTwo.$nodeData[atPresent].data.prcsTo.replace((meTwo.$lineData[id].to.replace('node_', '') + ','), '');
            newLineend.remove(meTwo.$lineData[id].to.replace('node_', ''))
            meTwo.$draw.removeChild(meTwo.$lineDom[id]);
            delete meTwo.$lineData[id];
            delete meTwo.$lineDom[id];
            if (meTwo.$focus == id) meTwo.$focus = "";
            --meTwo.$lineCount;
        }, 'json')
