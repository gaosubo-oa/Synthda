...
layui.define('view', function(exports){
  ...
  ,admin = {
    ...
    ,tabsBodyChange: function(index, options){
      ...
      var hrefsrc = admin.tabsBody(index).find('iframe').attr('src')||'';
      // if(hrefsrc.indexOf('/portals/officePortal') > -1||hrefsrc.indexOf('/document/companyPortal') > -1
      //     ||hrefsrc.indexOf('/common/acceptOA') > -1||hrefsrc.indexOf('/portal/flowIndex') > -1
      //     ||hrefsrc.indexOf('/document/documentIndex') > -1||hrefsrc.indexOf('/document/knowledgeIndex') > -1
      //     ||hrefsrc.indexOf('/document/humanResource') > -1||hrefsrc.indexOf('/document/marketIndex') > -1
      //     ||hrefsrc.indexOf('/document/financialIndex') > -1||hrefsrc.indexOf('/document/reportFormIndex') > -1
      //     ||hrefsrc.indexOf('/document/managementIndex') > -1||hrefsrc.indexOf('/portals/officeIndex') > -1
      //     ||hrefsrc.indexOf('/portals/dazueduIndex') > -1||hrefsrc.indexOf('/cmsPub/portal?siteId=') > -1){
      //   admin.tabsBody(index).find('iframe').attr('src',hrefsrc);
      // }
  ...
  var events = admin.events = {
    ...
    flexible: function(othis){
      ...
      if(isSpread){
        ...
        $('.leftBoxBakG img').css('width', '60px')
      }
      admin.sideFlexible(isSpread ? 'spread' : null);

      //持久化状态
      if ($(iconElem).attr('url')){
        $.ajax({
          type: "post",
    ...
    //弹出主题面板
    ,theme: function(){
      $('#admin-side1').animate({width: "0px"});
      $('#admin-side2').animate({width: "0px"});
      admin.popupRight({
    ...
    ,setTheme: function(othis){
        ...
        $.cookie('color', index, {expires: 7});
        $.get('/users/addAndApplicationUsers', {bpNo:index}, function (json) {
            if (json.flag) {
              if(index == 0||index == 1||index == 2||index == 3||index == 4||index == 5||index == 6||index == 7||index == 8){
                $('style').append('.layui-layout-admin .layui-header{border-bottom: none;}.layui-layout-admin .layui-input-search{border: none;}')
              }else{
                $('style').append('.layui-layout-admin .layui-header{border-bottom: 1px solid #f6f6f6;}.layui-layout-admin .layui-input-search{border: 1px solid #cccccc;}')
              }
              //end
              if(othis.hasClass(THIS)) return;
              othis.addClass(THIS).siblings('.layui-this').removeClass(THIS);
              admin.initTheme(index);
              layer.close(indexthis);
            }else{
                layer.alert('修改失败！', {icon:2},function(indexthis){
                  layer.close(indexthis);
                });
            }
        })
