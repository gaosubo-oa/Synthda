...
layui.define(['laytpl', 'form', 'util'], function (exports) {
    ...
    TreeTable.prototype.initOptions = function (opt) {
        ...
        if ('Array' === isClass(opt.defaultToolbar)) {
            this.options.defaultToolbar = opt.defaultToolbar
        }
    ...
    TreeTable.prototype.init = function () {
        ...
        if (typeof options.initSort === 'object') {
            if (!!options.initSort.field) {
                options.initSort.type = options.initSort.type || 'desc';
                this.sort(options.initSort.field, options.initSort.type);
            }
        }
    };
    ...
    TreeTable.prototype.bindEvents = function () {
        ...
        var $th = components.$tHead.find('th');
        ...
        $th.on('click', function (e) {
            var othis = $(this)
            var elemSort = othis.find('.layui-table-sort')
            var nowType = elemSort.attr('lay-sort')
            var type;

            if (othis.data('type') !== 'normal') {
                return false
            }
    ...
    /** 排序 */
    TreeTable.prototype.sort = function (th, type, pull, formEvent) {
        var that = this;
        var components = this.getComponents();
        var field;
        var filter = components.filter;
        var $tHead = components.$tHead;

        //字段匹配
        if (typeof th === 'string') {
            $tHead.find('th').each(function (i, item) {
                var othis = $(this)
        ...
        try {
            ...
            var elemSort = th.find('.layui-table-sort');
            $tHead.find('th').find('.layui-table-sort').removeAttr('lay-sort'); //清除其它标题排序状态
            elemSort.attr('lay-sort', type || null);
        } catch (e) {
            return console.error('Table modules: Did not match to field');
        }
    ...
    TreeTable.prototype.renderBodyTh = function () {
        ...
        $.each(options.cols, function (i1, item1) {
            ...
            $.each(item1, function (i2, item2) {
                ...
                if (item2.field) html.push(' data-field="' + item2.field + '"');
                ...
                if (!(item2.colGroup) && item2.sort) {
                    html.push('<span class="layui-table-sort layui-inline"><i class="layui-edge layui-table-sort-asc" title="升序"></i><i class="layui-edge layui-table-sort-desc" title="降序"></i></span>')
                }
                html.push('</div></div>');
    ...
    TreeTable.prototype.toggleCol = function (show, field, key) {
        ...
        if (show === undefined) {
            ...
            this.eachCols(function (i, item) {
                ...
                cols.push(' name="' + item.field + '" value="' + item.key + '" title="' + util.escape(item.title || '') + '"');
