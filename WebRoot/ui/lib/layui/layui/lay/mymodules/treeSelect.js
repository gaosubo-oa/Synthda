...
(function ($) {
    ...
    var a = {
          ...
          success: function (d) {
            ...
            if(d.obj != undefined){
              ...
              for(var i=0;i<DATA.length;i++){
                // else{//没有子集直接显示
                var tempObject = {};
                tempObject.name = DATA[i].typeName;
                tempObject.id = DATA[i].typeId;
                tempObject.checked = false;
                if(DATA[i].childs != undefined){//如果有子集
                  tempObject.children = getChildren(DATA[i].childs);
                }
              ...
              function getChildren(childs){
                ...
                if(childs != undefined){//如果有子集
                  for(var h=0; h<childs.length; h++){
                    var tempChild = {};
                    tempChild.name = childs[h].typeName;
      ...
      input: function(){
        ...
        var selectHtml = '<div class="'+ TREE_SELECT_CLASS +' layui-unselect layui-form-select" id="'+ TREE_SELECT_ID +'">' +
            ...
            ' <input type="text" id="'+ TREE_INPUT_ID +'" placeholder="'+ placeholder +'" value="" '+ readonly +' unselect="unselect" class="layui-input layui-unselect">' +
   ...
   * @param id 选中的id
   */
  TreeSelect.prototype.checkNode = function(filter,treeId, id){
    ...
    var o = obj.filter(filter),
        ...
        name = node.name;
    $('#'+treeId).parent().prev().find('input').val(name)
    treeInput.val(name);
    o.find('a[treenode_a]').removeClass('curSelectedNode');
    treeObj.selectNode(node);
  };

  TreeSelect.prototype.checkNodes = function(filter, id,treeId,paramName){
    ...
    var node = treeObj.getNodeByParam(paramName, id, null);
  ...
  TreeSelect.prototype.zTree = function (filter) {
    ...
    TreeSelect.prototype.setChkDisabled = function (filter,id) {
        var o = obj.filter(filter),
            treeInput = o.find('.layui-select-title input'),
            treeObj = obj.treeObj(filter);
           console.log(treeObj)
