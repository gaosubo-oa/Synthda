...
;layui.define(["layer", "form"], function (t) {
    ...
    var e = layui.$,
        i = layui.layer,
        a = layui.form,
        l = (layui.hint(), layui.device()), n = "layedit", o = "layui-show", r = "layui-disabled", c = function () {
            var t = this;
            t.index = 0, t.config = {
    ...
    var s = function (t, i, a) {
            ...
            var l = this.document, n = l.createElement(t);
                        ...
                        o.render({
                            url: r.url, method: r.type, elem: e(n).find("input")[0], done: function (e) {
                                if(e.code){
                                    ...
                                    true == e.flag ? (e.data = e.obj[0] || {}, v.call(t, "img", {
                                        src: '/xs?'+e.obj[0].attUrl,
                                        alt: e.obj[0].attachName
                                    }, a)) : i.msg(e.msg || "上传失败")
                                }
                            }
                        })
            ...
            var l = this,
                n = i.open({
                    ...
                    content: [
                        ...
                        '<input name="url" autofocus="true" autocomplete="off" value="' + t.href + '" class="layui-input" style="display: initial; width: 60%;">',
                        '<button name="selectURL" class="layui-btn layui-btn-sm layui-btn-normal" style="margin: 10px;" >选择URL</button>',
                        '<button id="layEditUrlUploadFile" class="layui-btn layui-btn-sm layui-btn-normal" style="margin: 10px;" >上传附件</button>',
                        "</div>",
                        "</li>",
                        '<li class="layui-form-item">',
                        ...
                        '<form style="display: none;" id="layEditUrlUploadForm">',
                        '<input name="file" type="file" id="layEditUrlFile" />',
                        '</form>',
                        "</li>",
                        "</ul>"
                    ].join(""),
                    success: function (t, n) {
                        ...
                        //按钮点击事件
                        $('button[name="selectURL"]').click(function () {
                            var nameType = $('#demo2').attr('nameType');
                            if (nameType == '制度') {
                                institutionURL();
                            } else {
                                i.msg('没有要选择的URL');
                        ...
                        // 按钮上传附件
                        $('#layEditUrlUploadFile').click(function () {
                            $('#layEditUrlFile').trigger('click');
                        });
                        $('#layEditUrlFile').change(function () {
                            var form = new FormData(document.getElementById("layEditUrlUploadForm"));
                            $.ajax({
                                ...
                                success: function (res) {
                                    try {
                                        if (res.obj && res.obj.length > 0) {
                                            var attUrl = res.obj[0].attUrl.split('&ATTACHMENT_NAME=')[0] + "&ATTACHMENT_NAME="
                                            $('input[name="url"]').val('/download?' + encodeURI(attUrl));
                                            $('#layEditUrlFile').val('');
                                        }
                                    } catch (e) {
                                        console.log(e);
