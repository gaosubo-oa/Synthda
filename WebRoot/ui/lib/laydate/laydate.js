...
//检测日期是否合法
    Dates.check = function(){
        ...
        if(value.indexOf('上午') > -1||value.indexOf('下午') > -1){
            value = value.split(' ')[0];
        }
        if(value.replace(/\s/g, '') !== ''){
            if(!exp.test(value)){
                ...
                if(isvoid.auto){
                    ...
                    if( Dates.elem[as.elemv].indexOf('上午') == -1&&Dates.elem[as.elemv].indexOf('下午') == -1){
...
//生成日期
    ...
    Dates.viewDate = function(Y, M, D){
        ...
        dateData = [De.getFullYear(), De.getMonth()+1, De.getDate()];
        ...
        //增加自定义的上、下午功能
        if(Dates.elem[as.elemv]&&!$('#laydate_customize').is(':hidden')){
            if(Dates.elem[as.elemv].indexOf('上午') > -1
                ||Dates.elem[as.elemv].indexOf('下午') > -1){
                $('#laydate_customize select').val(Dates.elem[as.elemv].split(' ')[1]);
            }else{
                $('#laydate_customize select').val('');
            }
...
//是否显示零件
    Dates.iswrite = function(){
        ...
        if(Dates.options.istime){
            $('#laydate_customize').hide();
        }else{
            if($(Dates.elem).hasClass('form_item')&&selectCalendar){
                $('#laydate_customize').show();
            }else{
                $('#laydate_customize').hide();
            }
        }
...
//渲染控件骨架
    Dates.view = function(elem, options){
        ...
        if(!Dates.box){
            ...
            if(Dates.options.customize != undefined &&!Dates.options.customize){
                var str = '<ul id="laydate_customize" style="background-color: #fff;border: 1px solid #ccc;"><li class="laydate_sj" style="width: 40px;">上下午</li><select style="width: 60px;height: 20px;margin: 0 7px;"><option value=""></option><option value="上午">上午</option><option value="下午">下午</option></select></ul>';
            }else{
                var str = '';
            }
...
//返回最终日期
    Dates.creation = function(ymd, hide){
        ...
        var getDates = Dates.parse(dateData,[hms[0].value, hms[1].value, hms[2].value]);
        if($('#laydate_customize').length == 1&&!$('#laydate_customize').is(':hidden')&&$('#laydate_customize select').val() != ''){
            getDates += ' '+$('#laydate_customize select').val();
        }
    ...
    Dates.events = function(){
        ...
        Dates.each(S('#laydate_table td'), function(i, elem){
            Dates.on(elem, 'click', function(ev){
                if(!Dates.hasClass(this, as[1])){
                    Dates.stopmp(ev);
                    Dates.removeClass($('td.laydate_click')[0],'laydate_click')
                    Dates.addClass(this,'laydate_click')
                    dateData = [this.getAttribute('y')|0, this.getAttribute('m')|0, this.getAttribute('d')|0]
        ...
        Dates.on(as.otoday, 'click', function(){
            var now = new Date();
            dateData = [now.getFullYear(), now.getMonth() + 1, now.getDate()];
        ...
        Dates.on(as.ok, 'click', function(){
            if(Dates.valid){
                // Dates.creation([Dates.ymd[0], Dates.ymd[1]+1, Dates.ymd[2]]);
                // dateData = [Dates.ymd[0], Dates.ymd[1]+1, Dates.ymd[2]]
                Dates.creation(dateData);
            }
        });
