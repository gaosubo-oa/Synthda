 ...
 */
document.write("<script src='/js/xoajq/xoajq1.js'><\/script>");
...
var templates = [
    ...
    $.ajax({
        type:'post',
        url:'/ueTemplate/getUeTemplateList',
        dataType:'json',
        success:function (res) {
            if(res.flag){
                ...
                for(i;i<data.length;i++){
                    plant(data[i].title,data[i].html)
                }
    ...
    function plant(n,h) {
        ...
        if (i == lg - 1) {
            ...
            var initPre = function () {
                ...
                for ( var i = 0, tmp; tmp = tmps[i++]; ) {
                    // str += '<div class="preitem" onclick="pre(' + i + ')"><img src="' + "images/" + tmp.pre + '" ' + (tmp.title ? "alt=" + tmp.title + " title=" + tmp.title + "" : "") + '></div>';
                    str += '<div class="preitem" onclick="pre(' + i + ')"><p value= ' +tmp.title+'>'+tmp.title+'</p></div>';
                }
                preitem.innerHTML = str;
            };
            var pre = function ( n ) {
