...
public class OfficePluginManager {
    ...
    private void startOfficeManager(){
        ...
        if (killOffice) {
            L.w("检测到有正在运行的office进程，已自动结束该进程");
        }
        try {
            ...
            L.e("启动office组件失败，请检查office组件是否可用");
            e.printStackTrace();
        }
    }
    ...
    private boolean killProcess() {
        ...
        try {
            if (props.getProperty("os.name").toLowerCase().contains("windows")) {
                ...
                String s = baos.toString();
                if (s.contains("soffice.bin")) {
                    String[] cmd ={"/bin/sh","-c","kill -9 $(ps -ef|grep soffice.bin |grep -v grep |awk '{print $2}')"};
